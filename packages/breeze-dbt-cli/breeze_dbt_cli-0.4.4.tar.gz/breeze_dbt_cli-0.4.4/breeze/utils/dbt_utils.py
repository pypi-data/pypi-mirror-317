...
def find_model_in_manifest(manifest: dict, model_name: str) -> dict:
    ...
    for node_id, node in manifest["nodes"].items():
        if (node["resource_type"] in ["model", "seed", "snapshot"] and node["name"] == model_name):
            model_unique_id = node_id
            break

    if not model_unique_id:
        raise Exception(f"'{model_name}' not found in manifest.")
...
def get_entity_paths_from_dbt_project(entity) -> list:
    ...
    model_paths = dbt_project.get(entity + "-paths")
