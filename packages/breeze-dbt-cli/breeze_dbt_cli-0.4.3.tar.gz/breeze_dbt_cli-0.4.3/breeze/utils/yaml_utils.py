...
def find_yaml_path(entity_name: str, entity_type: str) -> Optional[str]:
    ...
    search_dir = {
        "model": "models",
        "seed": "seeds",
        "source": "models",  # Sources are typically under the `models` directory
    }.get(entity_type)

    if not search_dir:
        raise Exception(f"Unknown entity type '{entity_type}'. Supported types: model, seed, source.")
                ...
                if entity_type == "model" and yml_data and "models" in yml_data:
                    for model in yml_data["models"]:
                        if model.get("name") == entity_name:
                            return yml_path
                # Check for models or sources in the YAML data
                if entity_type == "seed" and yml_data and "seeds" in yml_data:
                    for model in yml_data["seeds"]:
                        if model.get("name") == entity_name:
                            return yml_path
