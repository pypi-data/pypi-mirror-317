...
from whitebox_plugin_adsb_gps.whitebox_plugin_adsb_gps import should_emit


async def deactivate_websocket_loop_after_delay(plugin, delay):
    await asyncio.sleep(delay)
    plugin.is_active = False


class TestWhiteboxPluginASDBGPS(TestCase):
    def setUp(self) -> None:
    ...
    def test_plugin_initialization(self):
        self.assertEqual(self.plugin.name, "ADS-B GPS")
        self.assertEqual(self.plugin.HOST, "stratux")
        self.assertEqual(self.plugin.PORT, "80")
    ...
    def test_plugin_template(self):
        expected_template = "whitebox_plugin_adsb_gps/whitebox_plugin_adsb_gps.html"
        self.assertEqual(self.plugin.plugin_template, expected_template)

    def test_plugin_js(self):
        expected_js = ["/static/whitebox_plugin_adsb_gps/whitebox_plugin_adsb_gps.js"]
    ...
    async def test_gather_situation(self, mock_connect):
        ...
        deactivate_task = asyncio.create_task(
            deactivate_websocket_loop_after_delay(self.plugin, 0.1)
        )
