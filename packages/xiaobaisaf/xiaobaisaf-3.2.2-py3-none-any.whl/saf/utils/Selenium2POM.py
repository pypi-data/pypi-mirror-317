...
class PageListener(object):
    def __init__(
        ...
        if not path.exists(self.PROJECT_PATH):
            os.makedirs(self.PROJECT_PATH)
        new_title = "".join(re.findall(r"\w+", self.driver.title))
        filename = "_".join(lazy_pinyin(new_title)).title()
        if len(filename) > 10:
            filename = ''.join([x[0] for x in filename.split('_')  if len(x) > 0])
        if not filename:
            if 'FILE_COUNT' not in os.environ.keys():
                os.environ['FILE_COUNT'] = '0'
            else:
                os.environ['FILE_COUNT'] = str(int(os.environ['FILE_COUNT']) + 1)
            filename = f'FILE_{os.environ["FILE_COUNT"]}'
        if filename in self.PY_FILE_NAME_LIST:
            filename = f'{filename}_{self.PY_FILE_NAME_LIST.count(filename)}'
        self.PY_FILE_NAME_LIST.append(filename)
        new_filename = '.'.join([filename, 'py'])
    ...
    def identify_inputs_and_buttons(self, url='', html='',
        ...
        for input_tag in find_all_input:
            if input_tag not in soup.find_all(
                ...
                input_name = input_tag.get("name") or input_tag.get("id") or input_tag.text.strip() or input_tag.name
                input_xpath = self.get_xpath(input_tag)
                if input_name in input_list:
                    input_name = f"{input_name}_{input_list.count(input_name)}"
                if input_name:
                    input_list.append(
        ...
        for button_tag in find_all_button:
            button_name = (
                button_tag.get("name") or button_tag.get("id") or button_tag.text.strip() or button_tag.name
            )
            button_xpath = self.get_xpath(button_tag)
            if button_name in button_list:
                button_name = f"{button_name}_{button_list.count(button_name)}"
            if button_name:
                button_list.append(
        ...
        # title字符数量超过10缩减到10字符以内，取每个单词的首字母
        if len(title) > 10:
            title = ''.join([x[0] for x in title.split('_') if len(x) > 0])
        if not title:
        ...
        return self.converter(
            page_name=title[:10], url=url, input_list=input_list, button_list=button_list
        )
    ...
    def converter(self, page_name: str, url: str, input_list: list, button_list: list):
        ...
        for input_item in input_list:
            function_name = "_".join(lazy_pinyin(input_item["name"] or input_item["id"]))
            ...
            if not function_name:
                if 'FUNC_COUNT' not in os.environ.keys():
                    os.environ['FUNC_COUNT'] = '0'
                else:
                    os.environ['FUNC_COUNT'] = str(int(os.environ['FUNC_COUNT']) + 1)
                function_name = f'FUNC_{os.environ["FUNC_COUNT"]}'
        ...
        for button_item in button_list:
            function_name = "_".join(lazy_pinyin(button_item["name"] or button_item["id"]))
            function_name = "".join(re.findall("[a-zA-Z_]+", function_name))
            if len(function_name) > 10:
                function_name = ''.join([x[0] for x in function_name.split('_') if len(x) > 0])
            if not function_name:
