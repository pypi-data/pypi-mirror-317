...
class PageListener(object):
    def __init__(
        ...
        if self.PY_FILE_NAME_LIST.count(filename) > 1:
            filename = f'{filename}_{self.PY_FILE_NAME_LIST.count(filename) - 1}'
    ...
    def get_func_name(self, element):
        try:
            func_name = element.get("name") or element.get("id") or element.get("class")[0] or element.text or element.name
        except Exception as e:
    ...
    def converter(self, class_name: str, url: str, input_list: list, button_list: list):
        ...
        for input_item in input_list:
            function_name = self.get_func_name(input_item)
        ...
        for button_item in button_list:
            function_name = self.get_func_name(button_item)
