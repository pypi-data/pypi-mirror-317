...
def _crop_cam(K: np.ndarray, crop_row: int, crop_col: int) -> np.ndarray:
    """Adjusts intrinsics matrix principle point coordinates corresponding to image cropping.

    Parameters:
        K: 3x3 intrinsics matrix of the form:
            |fx s  cx|
            |0  fy cy|
            |0  0  1 |
...
def _scale_and_crop(K, h=None, w=None, max_h=None, max_w=None, scale=None):
    K = K.copy()
