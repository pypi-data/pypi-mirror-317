...
from perin_parser.thirdparty import torchtext

from perin_parser.data.parser.from_mrp.amr_parser import AMRParser
from perin_parser.data.parser.from_mrp.drg_parser import DRGParser
from perin_parser.data.parser.from_mrp.eds_parser import EDSParser
from perin_parser.data.parser.from_mrp.ptg_parser import PTGParser
from perin_parser.data.parser.from_mrp.ucca_parser import UCCAParser
from perin_parser.data.parser.from_mrp.evaluation_parser import EvaluationParser
from perin_parser.data.parser.from_mrp.request_parser import RequestParser
from perin_parser.data.field.edge_field import EdgeField
from perin_parser.data.field.edge_label_field import EdgeLabelField
from perin_parser.data.field.edge_permutation_field import EdgePermutationField
from perin_parser.data.field.field import Field
from perin_parser.data.field.nested_field import NestedField
from perin_parser.data.field.basic_field import BasicField
from perin_parser.data.field.bert_field import BertField
from perin_parser.data.field.relative_label_field import RelativeLabelField
from perin_parser.data.field.property_field import PropertyField
from perin_parser.data.field.anchor_field import AnchorField
from perin_parser.utility.label_processor import LabelProcessor
...
class Dataset:
    def __init__(self, args):
        ...
        self.every_word_input_field = Field(lower=True, init_token=self.sos, eos_token=self.eos, batch_first=True,
                                            include_lengths=True)
        self.every_lemma_field = Field(lower=True, init_token=self.sos, eos_token=self.eos, batch_first=True)

        char_form_nesting = torchtext.data.Field(tokenize=char_tokenize, init_token=self.sos, eos_token=self.eos,
                                                 batch_first=True)
        self.char_form_field = NestedField(char_form_nesting, include_lengths=True)

        char_lemma_nesting = torchtext.data.Field(tokenize=char_tokenize, init_token=self.sos, eos_token=self.eos,
                                                  batch_first=True)
        ...
        self.label_field = Field(preprocessing=lambda nodes: [n["label"].lower() for n in nodes], include_lengths=True,
                                 batch_first=True, )
    ...
    def state_dict(self):
        return {
            ...
            "vocabs": {key: pickle.dumps(value.vocab) for key, value in self.__dict__.items() if
                       hasattr(value, "vocab")}
        }

    def load_sentences(self, sentences, args, language: str, tokenizer):
    ...
    def load_dataset(self, args, gpu, n_gpus, framework: str, language: str, tokenizer, load_all=True):
        dataset = {
        ...
        self.val = dataset(
            ...
            precomputed_dataset=self.train.data if load_all else None,
            tokenizer=tokenizer,
        )
        ...
        print(f"{self.train_size} sentences in the train split")
        ...
        self.every_word_input_field.build_vocab(self.val, self.test, min_freq=1,
        ...
        self.every_lemma_field.build_vocab(self.val, self.test, min_freq=1,
                                           specials=[self.pad, self.unk, self.sos, self.eos])
    ...
    def relative_output_tensor_to_str(self, relative_label, anchors, tokens, lemmas, concat_rules: bool,
                                      num_lemmas=False, ignore_nonalnum=False):
        relative_label_str = self.relative_label_field.vocab.itos[relative_label]

        def safe_item(itos, index):
            index = index.item()
            index = max(0, min(index, len(itos) - 1))
            return itos[index]

        input_strings = [safe_item(self.every_word_input_field.vocab.itos, i) for i in tokens[anchors]]
        lemma_strings = [safe_item(self.every_lemma_field.vocab.itos, i) for i in lemmas[anchors]]

        absolute_label = self.processor.apply_label_rule(input_strings, lemma_strings, relative_label_str, concat_rules,
                                                         num_lemmas, ignore_nonalnum)

        if absolute_label is None:
            absolute_label = ' '.join(lemma_strings)
    ...
    def create_edge_freqs(self, args):
        edge_counter = [
            self.edge_label_field.vocab.freqs[self.edge_label_field.vocab.itos[i]] for i in
            range(len(self.edge_label_field.vocab))
        ]
        ...
        edge_attribute_counter = [
            self.edge_attribute_field.vocab.freqs[self.edge_attribute_field.vocab.itos[i]] for i in
            range(len(self.edge_attribute_field.vocab))
        ]
    ...
    def create_property_freqs(self, args):
        property_counter = {
            key: [vocab.freqs[vocab.itos[i]] for i in range(len(vocab))] for key, vocab in
            self.property_field.vocabs.items()
        }
