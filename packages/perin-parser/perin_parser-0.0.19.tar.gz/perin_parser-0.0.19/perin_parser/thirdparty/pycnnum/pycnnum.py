...
def cn2num(
    ...
    def split_arabic_chinese(chinese_string: str):
        i = 0
        for i, c in enumerate(chinese_string):
            if c not in string.digits + '.+-':
                break
        return chinese_string[:i], chinese_string[i:]

    arabic, chinese = split_arabic_chinese(chinese_string)
    if arabic:
        arabic = float(arabic)
        chinese = '一' + chinese
    system = NumberingSystem(numbering_type)
    int_part, dec_part = string2symbols(chinese, system)
    ...
    if dec_part:
        base = float(f"{int_value}.{dec_str}")
        if isinstance(dec_part[-1], ChineseNumberUnit):
            base *= 10 ** dec_part[-1].power
        if arabic:
            base *= arabic
        return base
    if arabic:
