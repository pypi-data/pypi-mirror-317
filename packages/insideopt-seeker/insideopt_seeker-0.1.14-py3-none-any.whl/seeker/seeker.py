...
def __bootstrap__():
   ...
   if os_name == 'windows':
      pyd_file = os.path.join(os.path.dirname(__file__),f"seeker.1.2.0.cpython-{py_version}-{os_name}.pyd")
      spec = importlib.util.spec_from_file_location("seeker", pyd_file)      
   else:
      so_file = os.path.join(os.path.dirname(__file__),f"seeker.1.2.0.cpython-{py_version}-{os_name}.so")
