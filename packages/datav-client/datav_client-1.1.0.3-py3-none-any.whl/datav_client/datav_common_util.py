...
def datav_init_start(userinfo: dict, tokenid, login_id, service_url, yth_ip, customParams={}):
    ...
    response = requests.get(url, verify=False, headers={"loginid": login_id,"user_info":json.dumps(userinfo)})
            ...
            if datav_configs:
                for temp_datav_configs in datav_configs:
                    ...
                    try:
                        ...
                        response = requests.post(url, json=param, verify=False, headers={"loginid": login_id,"user_info":json.dumps(userinfo)})
            ...
            if str(res_json.get("code")) == "200":
                ...
                for tepm_style_json in style_json:
                    ...
                    if data_list and len(data_list) > 0:
                        ...
                        response = session.get(url, verify=False, headers={"loginid": login_id,"user_info":json.dumps(userinfo)})
                        table_exists = False
