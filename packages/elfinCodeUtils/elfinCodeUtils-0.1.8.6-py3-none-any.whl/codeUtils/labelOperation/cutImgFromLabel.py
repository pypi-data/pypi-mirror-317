...
class CutImgFromLabel:
    ...
    def __init__(
        ...
        self.target_size = target_size if target_size is not None else 640
    ...
    def padding_cut_bbox(self, box: list[int], img_shape: tuple, target_size: int):
        ...
        w = target_size if w < target_thresh else w * 1.2
        h = target_size if h < target_thresh else h * 1.2
        # 重置bbox的高宽
        max_size = max(w, h)
    ...
    def __call__(self, img_dir_name: str = None, lbl_dir_name: str = None):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            task_bar = tqdm(
                as_completed(all_tasks), total=len(all_tasks), 
                desc=colorstr("bright_yellow", "bold", "Cut")
            )
            for ti, _ in enumerate(task_bar):
                task_bar.set_postfix({"task": ti})
