...
def test_bijection():
    """Ensure that the convertion of A to B to A with B higher is identity."""
    for p_in, p_out in AudioConvertor.all_layouts():
        if len(Layout(p_in).channels) <= len(Layout(p_out).channels):
            ...
            frame = FrameAudio(0, 1, p_in, torch.randn((len(Layout(p_in).channels), 1024)))
