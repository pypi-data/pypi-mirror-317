...
class LinearAudioConvertor:
    ...
    def __init__(self, in_layout: Layout, out_layout: Layout, matrix: np.ndarray[np.float64]):
        ...
        assert matrix.shape == (len(out_layout.channels), len(in_layout.channels))
...
class AudioConvertor(LinearAudioConvertor):
    ...
    def __init__(
        self,
        in_layout: typing.Union[Layout, str, numbers.Integral],
        out_layout: typing.Union[Layout, str, numbers.Integral],
    ):
        """Initialise and create the class.
        ...
        try:
            ...
            raise NotImplementedError(
                f"impossible convertion from {in_layout.name} to {out_layout.name}"
            ) from err
    ...
    def all_layouts() -> dict[tuple[str, str], list[list[float]]]:
        """To each layouts, associate the convertion matrix."""
        downmixing = {
            ("stereo", "mono"): (  # 'fl', 'fr'
