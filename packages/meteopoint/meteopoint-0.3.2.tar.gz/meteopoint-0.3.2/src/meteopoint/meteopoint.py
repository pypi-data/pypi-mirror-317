...
def get_api_keys():
    """Get API keys from environment, returns None if not set"""
    return {
        "openweather": os.getenv("OPENWEATHER_API_KEY"),
        "google_maps": os.getenv("GOOGLE_MAPS_API_KEY"),
        "google_maps_secret": os.getenv("GOOGLE_MAPS_SIGNING_SECRET")
    }
...
def main(
    ...
    """
    # Skip everything if running setup command or any other subcommand
    ...
    # Check for API key before proceeding with any other command
    if not api_keys["openweather"]:
        console.print("[red]Error: OpenWeather API key is not set[/red]")
        console.print("[yellow]Run 'meteopoint setup' to configure your API keys[/yellow]")
        sys.exit(1)

    if not city:
        # If no city, use default city
        city = cfg.get_default_city()
        if not city:
            console.print("[red]Error: No city specified and no default city set[/red]")
            return

    # Resolve city alias if it exists
    resolved_city = cfg.get_city(city)
    if resolved_city:
        city = resolved_city

    # Get output format
    output_format = format or cfg.get_format()
    
    # Get temperature unit
    use_fahrenheit = fahrenheit or cfg.get_temperature_unit() == "fahrenheit"

    try:
        # Fetch weather data
        weather_data = get_weather_data(city, api_keys)
        ...
        aqi_data = get_air_quality(lat, lon, api_keys)
        forecast_data = get_forecast(lat, lon, api_keys)
...
def save(
    ...
    try:
        ...
        weather_data = get_weather_data(city, api_keys)
        saved_alias = cfg.add_city(city, alias)
...
def list():
    ...
    default_city = cfg.get_default_city()
    for city in cities:
...
def config_cmd(
    ...
    if unit:
        if unit not in ["celsius", "fahrenheit"]:
            ...
            return
        cfg.set_temperature_unit(unit)
        console.print(f"[green]✓ Set temperature unit to {unit}[/green]")

    if default_city:
        try:
            # Verify city exists
            weather_data = get_weather_data(default_city, api_keys)
            cfg.set_default_city(default_city)
...
def get_weather_data(city: str, api_keys: dict):
...
def get_air_quality(lat: float, lon: float, api_keys: dict):
    """Fetch air quality data using Google Maps Air Quality API with OpenWeather fallback"""
    # Try Google Maps Air Quality API first if available
    if api_keys["google_maps"]:
        ...
        try:
            ...
            pass  # Fall through to OpenWeather
...
def get_forecast(lat: float, lon: float, api_keys: dict):
