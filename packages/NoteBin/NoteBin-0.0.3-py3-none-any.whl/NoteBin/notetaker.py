...
def fetch_paper_info(title):
    ...
    Returns:
        dict: A dictionary containing the paper's title, year, citations, abstract,
              first author, author's affiliation, corresponding author, and contact.
              Returns None if no information is found.
    """
    url = f"https://api.semanticscholar.org/graph/v1/paper/search?query={title}&fields=title,year,citationCount,abstract,authors"
        ...
        if match:
            ...
            if not has_info:
                ...
                if paper_info:
                    ...
                    table = (
                        f"| Year | Citations | Abstract | First Author | Affiliation | Corresponding Author | Contact Info |\n"
                        f"|------|-----------|----------|--------------|-------------|----------------------|--------------|\n"
                        f"| {paper_info['Year']} | {paper_info['Citations']} | {paper_info['Abstract']} | {paper_info['First Author']} | {paper_info['First Author Affiliation']} | {paper_info['Corresponding Author']} | {paper_info['Contact Info']} |\n"
                    )
