    ...
    try:
        ...
        for g in group_list:
            ...
            group_list_result.append(
                GroupResult(
                    group_id=g.group_id, group_name=g.group_name or "", ava_url=ava_url
                )
            )
    except (ValueError, KeyError):
        return Result.warning_("指定Bot未连接...")
    except Exception as e:
        logger.error(f"WebUi {router.prefix}/get_group_list 调用错误 {type(e)}:{e}")
...
async def _(group: UpdateGroup) -> Result[str]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/update_group 调用错误 {type(e)}:{e}")
...
async def _(bot_id: str) -> Result[list[Friend]]:
    ...
    try:
        ...
        for f in friend_list:
            ...
            result_list.append(
                Friend(user_id=f.user_id, nickname=f.name or "", ava_url=ava_url)
            )
        return Result.ok(
            result_list,
            "拿到了新鲜出炉的数据!",
        ...
        logger.error(f"WebUi {router.prefix}/get_friend_list 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[dict[str, int]]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_request_count 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[ReqResult]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_request_list 调用错误 {type(e)}:{e}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: HandleRequest) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/refuse_request 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: HandleRequest) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/approve_request 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: LeaveGroup) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/leave_group 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: DeleteFriend) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/delete_friend 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _(bot_id: str, user_id: str) -> Result[UserDetail]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_friend_detail 调用错误 {type(e)}:{e}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(group_id: str) -> Result[GroupDetail]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_group_detail 调用错误 {type(e)}:{e}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: SendMessageParam) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/send_message 调用错误 {type(e)}:{e}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
