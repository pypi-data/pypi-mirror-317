...
async def post_kernel_start(
    ...
    if kernel.name not in list_available_kernels():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Kernel: '{}' not available on this machine.".format(
                kernel.name),
        )
    ...
    print("Requested Kernel Startup for kernel: {}".format(kernel.name))
    ...
    toc = time.time()
    print("Kernel startup complete. (elapsed time: {:03}s)".format(toc - tic))
...
async def post_kernel_execute(
    ...
    if payload.kernel_name not in app_state.kernel_clients:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Kernel: '{}' not currently running.".format(
                payload.kernel_name),
        )
    ...
    rprint("Is kernel alive? {}".format(await kernel_client.is_alive()))
    msg_id = kernel_client.execute(payload.source_code + "\n")
    rprint("Sent message with id: {}".format(msg_id))

    # Consume Messages while the kernel is executing code
    executing = True
    while executing:
        rprint("Waiting for kernel client to receive iopub msg...")
        try:
            msg = Message(**await kernel_client.get_iopub_msg(timeout=1.0))
        except Exception:
            rprint("No iopub_msg received.")

            try:
                kernel_logs = await kernel_client.get_shell_msg(timeout=1)
                rprint("Kernel Logs: '{}'".format(kernel_logs))
                return dict(logs=kernel_logs)

            except Exception:
                rprint("No shell message either!!!")
