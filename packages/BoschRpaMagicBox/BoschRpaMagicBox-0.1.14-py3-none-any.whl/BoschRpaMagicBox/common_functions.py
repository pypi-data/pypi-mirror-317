    ...
    def auto_get_browser_screenshots(self, save_folder_path, time_interval=15, browser_screenshot_tag='', remote_save=False, username='', password='', server_name='',
        ...
        while not self.stop_screenshot_thread:
            ...
            try:
                ...
                sleep(1)
                if remote_save:
                    is_folder_exist=smb_check_folder_exist(username, password, server_name, share_name, remote_folder_path)
                    if not is_folder_exist:
                        smb_create_folder(username, password, server_name, share_name, remote_folder_path)

                    remote_file_path = remote_folder_path + os.sep + screenshot_file_path.split(os.sep)[-1]
