...
def streamdiff(params_pars, target, metric, channel):
    '''
    ...
    retr = strm[strm[metric] > target].groupby(channel).count()[[metric]].merge(strm[strm[metric] < target].groupby(channel).count()[[metric]], right_index=True, left_index=True, how = 'outer', suffixes = ['_more '+target, '_less '+target]).fillna(0)
    retr['percent >100'] = retr[metric+'_more '+target]*100/(retr[metric+'_more '+target] + retr[metric+'_less '+target])
    return(retr.sort_values('percent >100', ascending = 0))    