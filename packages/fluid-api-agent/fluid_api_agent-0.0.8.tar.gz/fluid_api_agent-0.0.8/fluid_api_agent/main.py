...
async def process_task_with_agent(
    ...
    try:
        ...
        # Create an event loop for running synchronous agent operations
        loop = asyncio.get_event_loop()
        
        # Run synchronous agent operations in a thread pool
        agent = await loop.run_in_executor(
            None, 
            lambda: initialize_agent(documentation, verbose=verbose)
        )
        ...
        # Parse and validate the agent's output in the thread pool
        api_request = await loop.run_in_executor(
            None,
            lambda: validate_agent_output(
                parse_agent_response(response, verbose), 
                verbose
            )
        )

        # Execute the API call (already async)
