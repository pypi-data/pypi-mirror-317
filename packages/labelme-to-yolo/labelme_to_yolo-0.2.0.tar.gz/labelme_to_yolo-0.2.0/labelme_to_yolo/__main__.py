...
from labelme_to_yolo.datatypes import (
...
def write_shapes(
    ...
    TOTAL_PERCENTAGE = 100  # noqa N806
    shapes = np.array(shapes_processed.shapes)

    total_percentage = train_percentage + val_percentage + test_percentage
    if total_percentage != TOTAL_PERCENTAGE:
        raise ValueError(
            "The sum of train_percentage, val_percentage, and test_percentage must be equal to 100."  # noqa E501
        )

    index_to_split_train = int(len(shapes) * (train_percentage / TOTAL_PERCENTAGE))
    index_to_split_val = int(
        len(shapes) * ((train_percentage + val_percentage) / TOTAL_PERCENTAGE)
    )
    index_to_split_test = int(
        len(shapes)
        * ((train_percentage + val_percentage + test_percentage) / TOTAL_PERCENTAGE)
    )
...
def process_write_shapes(
    ...
    for shape in shapes_processed.shapes:
        ...
        if source_image_path is None:
            logging.error(
                f"Warning: source_image_path is None for shape {shape.file_name}"
            )
            continue
...
def get_image_path(source_path: str, file_name: str) -> str | None:
...
@click.option("--output-path", required=True, help="Path of the yolo format files")
def main(source_path: str, output_path: str):
    ...
    write_yolo_yml(yolo_yml, output_path)
