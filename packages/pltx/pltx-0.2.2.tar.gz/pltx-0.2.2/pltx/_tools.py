...
def plot_lines(*data_lists: list, x: list = None, labels: list[str] = None, title='Linear Figure', 
    ...
    *data_lists: 一个或多个数据列表，每个列表包含一组要绘制的y轴数据。
    ...
    markers: 用于绘制折线的标记样式列表。默认为["o-", "x-", "s-"]，循环使用。
    """
...
def plot_image_boxes(image, bboxes: list = None, bbox_data_type: str = 'left_top_width_height'):
    """
    在给定的图像上绘制目标检测框。
    ...
    if bboxes:
        for bbox in bboxes:
            # 从bbox列表中提取边界框的坐标和尺寸
            x, y, w, h = bbox
            if bbox_data_type == 'left_top_right_bottom':  # xyxy
                x, y, x2, y2 = bbox
                w = x2 - x
                h = y2 - y
            elif bbox_data_type == 'center_width_height':  # xywh
                cx, cy, w, h = bbox
                x = cx - w / 2
                y = cy - h / 2
