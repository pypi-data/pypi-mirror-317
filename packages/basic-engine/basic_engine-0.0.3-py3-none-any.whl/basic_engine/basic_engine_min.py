...
As='ExSlimET'
Ar='ExSlimT'
Aq='solar_setback_min_height'
Ap='solar_setback_ratio'
...
AU={'bounding_box_config':0,'diagonalize_config':0,'cutting_policy_config':1,'gridsize':.3,'mass_cut_length':2.8,An:F,'tolerance':.0003,'use_sub_core':F}
AV={'angle_min':0,'angle_max':90,'tolerance_angle':.1,'diangonals_simplifying_angle':15.,'min_mass_area':14,'simple_deadspace_len':1.8,'bcr_margin':1.03,'far_margin':.11,'far_margin_for_commercial':.1,'use_mass_intersection':E,'pack_after_cut':F,'use_postprocess_emboss_cut':E,'use_deadspace_cut_with_exterior':E,'use_same_grid':E,'build_simple_check_parking':E,'use_real_parking':F,'is_alt_ver':E,'check_core_center_intersecting_mass':E,'add_core_center_as_candidates':E,'min_mass_width':2.,'mass_cut_depth_divider':1.6,'emboss_cut_policy':1,'use_emboss_cut_length_sorting':E,'longest_length_baseline':2.6,'shortest_length_baseline':1.2,'mass_aspect_ratio_baseline':6.,'mass_bb_shortest_length_baseline':4.2,'core_size_list':C,'core_type_list':C,i:C}
...
class AW:
	...
	def set_core_config(G,archi_line,mass,building_purpose,use_small_core,is_commercial_area,commercial_type,res,engine_type,use_sub_core=F):
		...
		G.has_elevator=J;G.core_type_list=L;G.core_size_list=O;G.sub_core_size_list=T;G.is_escape_core=I;G.is_sub_core_necessary=Q;G.is_specific_escape_sub_core=R;G.is_specific_escape_emgcy_elev_sub_core=S;G.commercial_type=commercial_type
class A7(c):
...
def b(polygon,buffer_distance,join_style=N.mitre,use_intersection=E,choose_biggest_polygon=E,choose_biggest_polygon_before_dilation=F):
	...
	return H
def e(polygon,buffer_distance,join_style=N.mitre,use_simplification=E,simplify_tolerance=.001,choose_biggest_polygon=E):
...
def AX(polygon,deadspace_len,tolerance,core=G(),regulation=G(),building_purpose=C,return_only_splits=F):
	S=building_purpose;Q=regulation;P=deadspace_len;J=polygon;G=tolerance;d=T(J.boundary);V=J.buffer(-G);K=[]
	for W in d:
...
def Aa(mass,has_piloti,regulation,min_mass_area):
	C=mass;D=[]
	for B in K(A(C)):
		if B==0 and has_piloti or B==0 and C[B][0].is_empty:D.append([wkt.loads(BG)])
		elif C[B][0].area>=min_mass_area:C[B]=[C[B][0].intersection(regulation[B])];D.append(C[B])
		else:break
	return D
def BN(mass,core,engine_type,regulation,simple_deadspace_len):
	D=mass
	for F in K(2,A(D)):
		...
		C=b(polygon=C,buffer_distance=simple_deadspace_len/2,use_intersection=E,choose_biggest_polygon=E);D[F]=[C]
	return D
def AA(polygon):
	B=n(polygon);A=[]
	for C in B:
	...
	return A
def AB(polygon,width,policy,time_counter_dict,is_postprocess,tolerance_angle,mass_cut_depth_divider=k,use_emboss_cut_length_sorting=F,longest_length_baseline=C,shortest_length_baseline=C):
	X=shortest_length_baseline;W=longest_length_baseline;V=policy;R=is_postprocess;N=width;M=tolerance_angle;K=polygon;Y=O.process_time()
	if K.is_empty:O.process_time()-Y;return K
	Z=N/mass_cut_depth_divider;J=AA(K);J=J[:-1];F=H(K.exterior.coords);F=F[:-1];L=[]
	if R:L=H(n(K))
	...
	while E:
		if J[I%A(F)]==0 and J[(I+1)%A(F)]==1 and J[(I+2)%A(F)]==0:
			...
			if use_emboss_cut_length_sorting:e,f=g([T,U]);h=b and(e<=X and f<=W);i=c and(e<=X or f<=W)
...
def BO(mass,core,tolerance,tolerance_angle,regulation,postprocess_emboss_cut_length,time_counter_dict,mass_cut_depth_divider):
	F=regulation;D=tolerance_angle;G=[]
	for(H,I)in X(mass):C=Q(I[0],tol_angle=D,tol_length=B.TOLERANCE,container_geometry=F[H]);A=R([C,core]);A=e(polygon=A,buffer_distance=B.TOLERANCE_MARGIN,use_simplification=E,choose_biggest_polygon=E);A=Q(A,tol_angle=D,tol_length=tolerance,container_geometry=F[H]);A=AB(A,postprocess_emboss_cut_length,0,time_counter_dict,E,D,mass_cut_depth_divider=mass_cut_depth_divider);C=A.difference(core);C=b(polygon=C,buffer_distance=B.TOLERANCE_SLIVER,use_intersection=E,choose_biggest_polygon=E);G.append([C])
	return G
def BP(mass,tolerance_angle,tolerance,core,engine_type,simple_deadspace_len,regulation):
	C=[]
	for(D,E)in X(mass):
		A=Q(E[0],tol_angle=tolerance_angle,tol_length=tolerance,container_geometry=regulation[D])
		...
		A=b(A,simple_deadspace_len/2);C.append([A])
	return C
...
def Ba(polygon,mass_angle,gridsize,tolerance,time_counter_dict,regulation_bounds):
	m=regulation_bounds;l=mass_angle;Z=polygon;H=tolerance;F=gridsize;z=O.process_time();Z=b(polygon=Z,buffer_distance=B.TOLERANCE_SLIVER,use_intersection=E,choose_biggest_polygon=E);n=shapely.affinity.rotate(Z,-l,(0,0),use_radians=E);R,S,a,c=n.bounds
...
def Ab(polygon,depth,width,tolerance,time_counter_dict):
	X=depth;S=tolerance;N=polygon;Y=O.process_time()
...
def Bb(regulation_polygon,grid_polygon,angle_min,angle_max):
	N=grid_polygon
	...
	for F in K(A(B)):
		if L[F]==1 and L[(F+1)%A(B)]==0 and L[(F+2)%A(B)]==1:
			...
			if regulation_polygon.contains(W):
				...
				if S>=angle_min and T<=angle_max:I[(F+1)%A(B)]=C
	I=[A for A in I if A];I.append(I[0]);Z=G(I);return Z
def Bc(diagonalized_polygon,diangonals_simplifying_angle,regulation_polygon):
	E=diagonalized_polygon
	if E.is_empty:return E
	B=D.array(E.exterior.coords)[:-1];L=n(E)[:-1];F=y.deepcopy(B)
	for H in K(A(B)):
		I=diangonals_simplifying_angle/180*D.pi
...
def Ae(mass_polygon,obb,target_area,core=G()):
	...
	return B
def A1(regulation_polygon,floor_polygon,obb,bcr,cut_policy,bcr_margin,is_alt_ver,mass_angle=C,is_premium=F,core=G()):
	J=regulation_polygon;I=core;H=mass_angle;G=cut_policy;F=obb;A=floor_polygon
	if H is not C:F=A9(A,(D.cos(H),D.sin(H)))
	B=J.area*bcr*bcr_margin
	if is_alt_ver:B=J.area
	E=A
	...
	return E
def Bf(regulation_polygon,obb,floors,max_far_with_margin,smallest_core_area,cut_policy,min_mass_area,has_piloti,building_purpose,is_alt_ver,elev_area_for_advantage,use_small_core):
...
def Ag(buffered_top_floor,core_corner,core_size,core_type,allow_outside=F):
	...
	return F,G
def Bi(floors,mass_angle,tolerance,simple_deadspace_len,gridsize,refined_site,road_edge,is_commercial_area,use_small_core,mass_config,regulation):
	AI='Polygon';p=road_edge;o=simple_deadspace_len;k=tolerance;j=floors;d=mass_angle;Y=refined_site;I=mass_config
	if not I.is_escape_core:
		q=[];r=[];q.append(I.core_size_list[0]);r.append(I.core_type_list[0])
		for O in K(A(I.core_size_list)):
			if O>1:
				if I.core_size_list[O][0]<I.core_size_list[0][0]or I.core_size_list[O][1]<I.core_size_list[0][1]:q.append(I.core_size_list[O]);r.append(I.core_type_list[O])
		I.core_size_list=q;I.core_type_list=r
	...
	for V in Am(S):
		...
		Q=w
		if I.add_core_center_as_candidates and not I.is_sub_core_necessary:
			AL=l(l(A)for A in I.core_size_list);AM=T(V.exterior)
			for x in AM:
				if x.length>B.CORE_SEGMENT_GAP_LENGTH+AL+B.CORE_SEGMENT_GAP_LENGTH:Q.append(D.array(x.centroid.coords[0]))
		z=[];A0=S[e-1];t=A0.difference(V);t=b(polygon=t,buffer_distance=gridsize/2,use_intersection=E,choose_biggest_polygon=F);A1=[]
		for(AN,A3)in X(Q):
		...
		for(u,AA)in m(z,A1):
			...
			for(AS,v)in m(I.core_size_list,I.core_type_list):
				...
				if I.is_sub_core_necessary:
					...
					for AY in AX:
						AZ=I.sub_core_size_list[v-1];Aa,Ab=Ag(A8,AY,AZ,v,allow_outside=E);AB=[];AC=[]
		...
		for(R,AE)in m(A5,A6):
			...
			for Ad in S[:A(Z)]:
				...
				if J.area<I.min_mass_area:U=F;break
				Ai,Aj,Ak,Al=J.bounds
				if Ak-Ai<I.min_mass_width or Al-Aj<I.min_mass_width:U=F;break
				if I.has_elevator and not Ah(R,J,k,I.check_core_center_intersecting_mass):U=F;break
			if not U:continue
	...
	return AG
def Ah(core,mass,tolerance,check_core_center_intersecting_mass):
	B=tolerance;A=mass;D=T(core.boundary);C=g(D,key=lambda x:x.length,reverse=E)
	...
	return E
def A2(floor,core,simple_deadspace_len,use_simplify=E):
	D=simple_deadspace_len;A=floor.difference(core);C=A.buffer(-D/2,join_style=N.mitre);C=U(C);A=C.buffer(D/2,join_style=N.mitre).intersection(A);A=U(A)
	if use_simplify:A=Q(A,tol_length=B.TOLERANCE)
	return A
def u(archi_line,each_floor_regulation,preset,mass_cut_length,gridsize,tolerance,tolerance_angle,diangonals_simplifying_angle,angle_min,angle_max,pack_after_cut,time_counter_dict,preset_mass_dict,mass_angle=C,mass_cut_depth_divider=k,fill_gap=E,regulation_bounds=C,emboss_cut_policy=0,use_emboss_cut_length_sorting=F,longest_length_baseline=C,shortest_length_baseline=C):
	R=mass_cut_depth_divider;P=preset_mass_dict;N=tolerance_angle;M=mass_cut_length;L=preset;J=time_counter_dict;I=tolerance;H=gridsize;E=each_floor_regulation;B=mass_angle;K:0;B:0
	if B is not C:K=A9(E,(D.cos(B),D.sin(B)))
	elif L.bounding_box_config==0:K,B=BY(E)
	elif L.bounding_box_config==1:K,B=BZ(E)
	...
	if T is not C:V,G=T;A=wkt.loads(V)
		...
		if fill_gap:
			U=Q(e(A,H))
			if E.contains(U):A=U
		A=Ab(A,H*10000,M,I,J);G.append(A);A=AB(A,M,emboss_cut_policy,J,F,N,mass_cut_depth_divider=R,use_emboss_cut_length_sorting=use_emboss_cut_length_sorting,longest_length_baseline=longest_length_baseline,shortest_length_baseline=shortest_length_baseline);G.append(A);A=Ab(A,H+I,H*10000,I,J);A=AB(A,H+I,1,J,F,N,mass_cut_depth_divider=R);G.append(A);P[S]=A.wkt,G
	W=O.process_time();O.process_time()-W;BS(A)
	if L.diagonalize_config==1:A=Bb(E,A,angle_min,angle_max);A=Bc(A,diangonals_simplifying_angle,E)
	A=A.buffer(0);G.append(A);return A,G,K,B
def Bj(archi_line,regulation,preset,mass_cut_length,gridsize,tolerance,time_counter_dict,preset_mass_dict,bcr,mass_config):
	T=regulation;Q=bcr;P=preset_mass_dict;O=time_counter_dict;N=tolerance;M=gridsize;L=mass_cut_length;I=preset;F=archi_line;A=mass_config
	if not A.is_alt_ver:G,V,J,K=u(F,T[0],I,L,M,N,A.tolerance_angle,A.diangonals_simplifying_angle,A.angle_min,A.angle_max,A.pack_after_cut,O,P,mass_cut_depth_divider=A.mass_cut_depth_divider,emboss_cut_policy=A.emboss_cut_policy,use_emboss_cut_length_sorting=A.use_emboss_cut_length_sorting,longest_length_baseline=A.longest_length_baseline,shortest_length_baseline=A.shortest_length_baseline);G=A1(F,G,J,Q,I.cutting_policy_config,A.bcr_margin,A.is_alt_ver)
	else:
		D=T[:];R=0
		while E:
			G,V,J,K=u(F,D[0],I,L,M,N,A.tolerance_angle,A.diangonals_simplifying_angle,A.angle_min,A.angle_max,A.pack_after_cut,O,P,mass_cut_depth_divider=A.mass_cut_depth_divider,emboss_cut_policy=A.emboss_cut_policy,use_emboss_cut_length_sorting=A.use_emboss_cut_length_sorting,longest_length_baseline=A.longest_length_baseline,shortest_length_baseline=A.shortest_length_baseline)
			if G.area<=F.area*Q*A.bcr_margin or R>d(1/B.SIMPLE_REDUCE_RATIO_INTERVAL):break
			else:R+=1;Y=shapely.affinity.scale(F,1-R*B.SIMPLE_REDUCE_RATIO_INTERVAL);D[0]=A1(Y,D[0],J,Q,I.cutting_policy_config,A.bcr_margin,A.is_alt_ver);D[0]=U(D[0])
		D[1:]=[A.intersection(D[0])for A in D[1:]];D[1:]=[U(A)for A in D[1:]];D=H(AE(lambda x:not x.is_empty,D));W=[G];S=C
		if A.use_same_grid:S=BR(D,K)
		for Z in D[1:]:
			try:a,X,X,X=u(F,Z,I,L,M,N,A.tolerance_angle,A.diangonals_simplifying_angle,A.angle_min,A.angle_max,A.pack_after_cut,O,P,K,mass_cut_depth_divider=A.mass_cut_depth_divider,regulation_bounds=S,emboss_cut_policy=A.emboss_cut_policy,use_emboss_cut_length_sorting=A.use_emboss_cut_length_sorting,longest_length_baseline=A.longest_length_baseline,shortest_length_baseline=A.shortest_length_baseline);W.append(a)
			except c:pass
	return W,G,V,J,K,D,S
def Bk(archi_line,bounding_box,mass,max_far_with_margin,smallest_core_area,preset,has_piloti,building_purpose,regulation_cut,mass_angle,mass_cut_length,gridsize,tolerance,time_counter_dict,preset_mass_dict,regulation_bounds,mass_config,elev_area_for_advantage,use_small_core):
	P=mass_angle;O=bounding_box;N=archi_line;K=regulation_cut;J=preset;I=smallest_core_area;D=mass_config;C=mass;Q=[]
	for T in C:
		R=b(T,D.simple_deadspace_len)
		if R.is_empty:break
		else:Q.append(R)
	C=Q;C,V,G=Bf(N,O,C,max_far_with_margin,I,J.cutting_policy_config,D.min_mass_area,has_piloti,building_purpose,D.is_alt_ver,elev_area_for_advantage,use_small_core)
	if D.is_alt_ver and not V:
		...
		try:
			while E:
				...
				X=shapely.affinity.scale(W,1-F*B.SIMPLE_REDUCE_RATIO_INTERVAL);H=A1(X,K[A(C)-1],O,0,J.cutting_policy_config,0,D.is_alt_ver,P);H=U(H);C[-1],S,S,S=u(N,H,J,mass_cut_length,gridsize,tolerance,D.tolerance_angle,D.diangonals_simplifying_angle,D.angle_min,D.angle_max,D.pack_after_cut,time_counter_dict,preset_mass_dict,P,mass_cut_depth_divider=D.mass_cut_depth_divider,regulation_bounds=regulation_bounds)
				if C[-1].area>G:L=F+1
				else:M=F-1
			K[A(C)-1]=H
		except c:pass
		...
		elif C[-1].area<D.min_mass_area+I:C=C[:-1]
	return C
def Bl(mass,mass_angle,tolerance,engine_type,gridsize,refined_site,road_edge,is_commercial_area,use_small_core,has_piloti,archi_line,regulation,max_far_with_margin,bcr,mass_config):
	Q=use_small_core;P=is_commercial_area;I=mass_config;H=mass;g=Bi(H,mass_angle,tolerance,I.simple_deadspace_len if engine_type==B.ENGINE_TYPE_LIGHT else B.TOLERANCE,gridsize,refined_site,s([A['edge_geom']for A in road_edge['edges']]),P,Q,I,regulation);Y=[]
	for h in g:
		L,H,Z,a,c=h
...
def Ai(mass_for_parklot_check,law_parklot_count,estimated_parklot_count,parklot_datas,underground_parking_boundaries,regulation_cut,bounding_box,mass_generation_preset,mass_config,mass_angle,core,hall_geom,stair_geom,elev_geom,archi_line,time_counter_dict,preset_mass_dict,regulation_bounds,engine_type,building_purpose,parking_commercial,commercial_type,first_floor_reduce_area,has_piloti,packed_unit_space_area_test_set_index,packed_unit_space_equal_division,packed_unit_space_sequantial,res,env_plan):
	...
	while M>X or G.is_sub_core_necessary and m==1 or Q in(x,w)and A([A for B in H.packed_unit_space for A in B if not A.is_empty])>Z:
		if A(C)<=3:break
		n=C[-1][:];W+=1;o=shapely.affinity.scale(e,1-W*B.SIMPLE_REDUCE_RATIO_INTERVAL);J[A(C)-1]=A1(o,J[A(C)-1],bounding_box,0,P.cutting_policy_config,0,is_alt_ver=E,mass_angle=c,core=I);J[A(C)-1]=U(J[A(C)-1]);K,g,g,g=u(archi_line,J[A(C)-1],P,P.mass_cut_length,P.gridsize,P.tolerance,G.tolerance_angle,G.diangonals_simplifying_angle,G.angle_min,G.angle_max,G.pack_after_cut,time_counter_dict,preset_mass_dict,c,mass_cut_depth_divider=G.mass_cut_depth_divider,regulation_bounds=regulation_bounds);K=A2(K,I,G.simple_deadspace_len if engine_type==B.ENGINE_TYPE_LIGHT else B.TOLERANCE);C[-1]=[K];a=[I]
		if L(I,R):a=I.geoms
		h=E
		for b in a:
		...
		if G.has_elevator:
			for b in a:
				if not Ah(b,K,P.tolerance,G.check_core_center_intersecting_mass):i=F;break
		if K.area<G.min_mass_area or W>=d(1/B.SIMPLE_REDUCE_RATIO_INTERVAL)or l.disjoint(K)or not h or G.has_elevator and not i or B.BUILDING_PURPOSE_MAP[Q]>=3 and(M-X)*B.PARKING_AREA_DIVISOR>f(A.area for A in n):
			C=C[:-1];e=J[A(C)-1];W=0
...
def Bm(use_mech_parking,use_under_parking,core_list,hall_geom,stair_geom,elev_geom,parking_result_dict,refined_site,archi_line,road_edge,mass,building_purpose,is_flag_lot,max_far,core,regulation_cut,bounding_box,mass_generation_preset,time_counter_dict,preset_mass_dict,mass_angle,engine_type,mass_after_pack,first_floor_reduce_area,estimated_parklot_count,regulation_bounds,mass_config,commercial_type,sub_core_related_geoms,res):
	...
	if M.build_simple_check_parking:
...
def Bn(mass,has_piloti,min_mass_area,regulation,use_mass_intersection,engine_type,simple_deadspace_len,core,use_postprocess_emboss_cut,tolerance_angle,tolerance,postprocess_emboss_cut_length,time_counter_dict,mass_cut_depth_divider,use_deadspace_cut_with_exterior,first_floor_reduce_area,use_real_parking,mass_aspect_ratio_baseline,mass_bb_shortest_length_baseline,is_mech_park_weight_needed,env_plan):
	L=env_plan;K=tolerance;J=tolerance_angle;I=min_mass_area;G=simple_deadspace_len;F=engine_type;E=has_piloti;D=regulation;C=core;A=mass;A=Aa(A,E,D,I)
	if use_mass_intersection:A=BN(A,C,F,D,G)
	if use_postprocess_emboss_cut:A=BO(A,C,K,J,D,postprocess_emboss_cut_length,time_counter_dict,mass_cut_depth_divider)
	...
	if use_deadspace_cut_with_exterior:
		...
		A=BP(A,J,K,C,F,N,D)
	A=Aa(A,E,D,I);H=Ak(C,A,first_floor_reduce_area,E,use_real_parking,F==B.ENGINE_TYPE_BASIC)
	...
	A=Bp(A,C,mass_aspect_ratio_baseline,mass_bb_shortest_length_baseline);return A,H
...
def Ak(core,mass,first_floor_reduce_area,has_piloti,use_real_parking,is_engine_type_basic):
	B=mass
	if A(B)==0:raise c(BH)
	else:C=Bo(core,B,first_floor_reduce_area,has_piloti,use_real_parking,is_engine_type_basic);C-=core.area*A(B);D=C
	return D
def Bo(core,mass,first_floor_reduce_area,has_piloti,use_real_parking,is_engine_type_basic):
	D=mass
	...
	return E
def Bp(mass,core,mass_aspect_ratio_baseline,mass_bb_shortest_length_baseline):
	C=D.array(T(core.exterior)[0].coords);H=C[1]-C[0];B=[]
	for(I,A)in X(mass):
		if I==0:B.append(A);continue
		J,K=BJ(A[0],H,return_bb=E);L=g([A.length for A in T(K.exterior)])[0];F=E;G=E;F=L>=mass_bb_shortest_length_baseline;G=J<mass_aspect_ratio_baseline
		if not A[0].is_empty and(G or F):B.append([Q(A[0])])
	return B
def Bq(original_mass,regulation,tolerance,tolerance_angle,simple_deadspace_len):
	J=simple_deadspace_len;I=regulation;H=tolerance;L=[]
	for D in original_mass:D=U(D);D=b(polygon=D,buffer_distance=H,use_intersection=E,choose_biggest_polygon=E);L.append(D)
	C=L;M=[]
	for(O,P)in X(C):R=Q(P,tol_angle=tolerance_angle,tol_length=B.TOLERANCE,container_geometry=I[O]);M.append(R)
	C=M
	for F in K(1,A(C)):
		...
		while A(AX(polygon=G,deadspace_len=J,tolerance=H,return_only_splits=E))>1:G=AX(polygon=G,deadspace_len=J,tolerance=H)
		C[F]=Q(G)
	return C
...
def Br(mass_generation_preset,refined_site,archi_line,regulation,building_purpose,time_counter_dict,postprocess_emboss_cut_length,estimated_parklot_count,first_floor_reduce_area,preset_mass_dict,parking_result_dict,road_edge,use_small_core,is_commercial_area,is_flag_lot,use_mech_parking,use_under_parking,custom_config,engine_type,mass_config,res,commercial_type,visualize_inside_get_mass):
	...
	try:
		AA=H[Y][p];AB=AA+C.far_margin_for_commercial;S=[];J=[Q(A,tol_length=B.TOLERANCE)for A in J];AK=A(J);AL=O.process_time();D,q,S,r,d,s,t=Bj(K,J,G,G.mass_cut_length,G.gridsize,G.tolerance,X,f,H[Y][j],C);O.process_time()-AL;S.append(q)
		if not C.is_alt_ver:
			D=[q]
			for AM in J[1:]:D.append(AM.intersection(q))
		C.set_core_config(K,D,P,R,b,o,H,I,use_sub_core=G.use_sub_core);AC=W(0,0,C.core_size_list[0][0],C.core_size_list[0][1]);u=A0([AC],C.core_type_list[0],[AC],R,b,is_escape_core=C.is_escape_core,is_center_core_placed=F,is_using_adjusted_core=F,is_last_gen_core_called=E)[2][0].area;AD=C.core_size_list[0][0]*C.core_size_list[0][1]-u;v=y.deepcopy(D)
		for w in Am(D):
			w=U(w)
			if w.area<C.min_mass_area+AD:v=v[:-1]
			else:break
		D=v;S.append(D);AN=O.process_time();D=Bk(K,r,D,AB,AD,G,H[V][Z][0],P,s,d,G.mass_cut_length,G.gridsize,G.tolerance,X,f,t,C,u,R);S.append(D);C.set_core_config(K,D,P,R,b,o,H,I,use_sub_core=G.use_sub_core);O.process_time()-AN;D=Bq(D,J,G.tolerance,C.tolerance_angle,C.simple_deadspace_len);AO=O.process_time();k=Bl(D,d,G.tolerance,I,G.gridsize,A8,a,b,R,H[V][Z][0],K,J,AB,H[Y][j],C)
		if I==B.ENGINE_TYPE_BASIC or C.use_real_parking and A9:k=k[:1]
		...
		for(AP,AQ)in k:
			try:
				...
				if G.use_sub_core and not C.is_sub_core_necessary:raise A7('코어 2개소가 배치되지 않아도 되는 설계안에서 코어 2개소가 배치되었습니다.')
				O.process_time()-Ae;D=x;S.append([A[0]for A in D]);Ar=F;D,AI=Bn(D,H[V][Z][0],C.min_mass_area,J,C.use_mass_intersection,I,C.simple_deadspace_len,e,C.use_postprocess_emboss_cut,C.tolerance_angle,G.tolerance,postprocess_emboss_cut_length,X,C.mass_cut_depth_divider,C.use_deadspace_cut_with_exterior,L,C.use_real_parking,C.mass_aspect_ratio_baseline,C.mass_bb_shortest_length_baseline,Ar,m);As=C.is_escape_core;At,Au=D,D;D,T,m,M=Ai(D,T[1],T[0],T,[],s,r,G,C,d,shapely.ops.unary_union([e,z]),shapely.ops.unary_union([A1,A4]),shapely.ops.unary_union([A2,A5]),shapely.ops.unary_union([A3,A6]),K,X,f,t,I,P,H[Y][AH],o,L,H[V][Z][0],T[4],At,Au,H,m);AI=Ak(e,D,L,H[V][Z][0],C.use_real_parking,I==B.ENGINE_TYPE_BASIC);N=Aj(I,H,AI,e,A1,A2,A3,z,A4,A5,A6,AE,AF,Ap,Aq,D,d,S,T,L,Af,Ag,a,AW,Ac,AV,Ah,An,AJ,u,m,As,R,H[V][i][0],b,H[V][AG][0],C.is_specific_escape_sub_core,C.is_specific_escape_emgcy_elev_sub_core,J,Ao,visualize_inside_get_mass);h.append(N)
	...
	return N
def Bs(site_polygon,solar_setback_flag_polygon,openspace_buffer_len,estimated_parklot_count,floor_height,solar_setback_min_height,solar_setback_ratio_from_user_input,mass_generation_preset,mass_config,engine_input):
	H=engine_input;G=solar_setback_ratio_from_user_input;D=mass_config;C=mass_generation_preset;B=estimated_parklot_count;A=site_polygon;L=A;M=A;I=H[Y][q];E=[A.buffer(-openspace_buffer_len,join_style=N.mitre)]*I
	for(J,O)in X(E):
		K=floor_height*(J+1)
		if K>solar_setback_min_height:E[J]=O.intersection(shapely.affinity.translate(solar_setback_flag_polygon,0,-G*K))
