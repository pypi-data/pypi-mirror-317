...
class KeyValueCache:
    ...
    def __init__(
            ...
            enable_text_similarity_lookup: bool = False,
        ...
        if self.db_params is not None:
            ...
            self.cache_db = Datastore(db_params=db_params, logger=self.logger).get_data_store()
        ...
        if self.enable_text_similarity_lookup:
            ...
            # from poc.nlp.rawtext.TextDiff import TextDiff
    ...
    def test_cache_db(self):
        ...
        for i, (obj, val) in enumerate(data):
            ...
            assert str(res) == str(val), \
                'Test DB cache #' + str(i) + ' for object "' + str(obj) + '", got ' + str(res) \
                + ' type "' + str(type(res)) + '" not ' + str(val) + ' type "' + str(type(val)) + '"'
...
if __name__ == '__main__':
    ...
    KeyValueCacheUnitTest(logger=lgr).test()
