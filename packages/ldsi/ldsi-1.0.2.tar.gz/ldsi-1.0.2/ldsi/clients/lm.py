...
from ldsi.clients.openai import OpenAIProvider
from ldsi.clients.provider import Provider, TrainingJob
from ldsi.clients.utils_finetune import DataFormat, infer_data_format, validate_data_format
from ldsi.utils.callback import BaseCallback, with_callbacks
...
class LM(BaseLM):
    """
    A language model supporting chat or text completion requests for use with LDSI modules.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Create a new language model instance for use with LDSI modules and programs.
        ...
        if "o1-" in model:
            ...
            ), "OpenAI's o1-* models require passing temperature=1.0 and max_tokens >= 5000 to `ldsi.LM(...)`"
    ...
    def finetune(
        ...
        from ldsi import settings as settings

        err = "Fine-tuning is an experimental feature."
        err += " Set `ldsi.settings.experimental` to `True` to use it."
    ...
    def infer_adapter(self) -> Adapter:
        ...
        if ldsi.settings.adapter:
            return ldsi.settings.adapter

        model_type_to_adapter = {
            "chat": ldsi.ChatAdapter(),
        }
        model_type = self.model_type
...
def request_cache(maxsize: Optional[int] = None):
    ...
    good performance when optimizing and evaluating LDSI LMs (disk caching alone is too slow).
    ...
    def decorator(func):
        @cached(
            ...
            # Use a lock to ensure thread safety for the cache when LDSI LMs are queried
...
def litellm_completion(request: Dict[str, Any], num_retries: int, cache={"no-cache": True, "no-store": True}):
    ...
    stream = ldsi.settings.send_stream
    if stream is None:
