...
import ldsi
from ldsi.dsp.utils import dotdict
...
try:
    ...
    raise ImportError(
        "The snowflake-snowpark-python library is required to use SnowflakeRM. Install it with ldsi-ai[snowflake]",
    )


class SnowflakeRM(ldsi.Retrieve):
    ...
    def __init__(
        ...
        if self.auto_filter:
            ...
            self.filter_lm = ldsi.Snowflake(session=snowflake_session, model="mixtral-8x7b")
    ...
    def forward(
        ...
        Returns:
            ldsi.Prediction: An object containing the retrieved passages.
        """
    ...
    def _fetch_cortex_service(self, snowpark_session, snowflake_database, snowflake_schema, cortex_search_service_name):
        ...
        snowpark_session.query_tag = {"origin": "sf_sit", "name": "ldsi", "version": {"major": 1, "minor": 0}}
    ...
    def _cortex_search_with_auto_filter(self, cortex_search_service, query, columns, k):
        ...
        with ldsi.settings.context(lm=self.filter_lm):
            if self.auto_filter:
...
class GenerateFilter(ldsi.Signature):
    """
    ...
    query = ldsi.InputField(desc="user query")
    attributes = ldsi.InputField(desc="attributes to filter on")
    sample_values = ldsi.InputField(desc="examples of values per attribute")
    answer: JSONFilter = ldsi.OutputField(
        desc="filter query in valid JSON format. ONLY output the filter query in JSON, no reasoning"
    )


class SmartSearch(ldsi.Module):
    def __init__(self):
        super().__init__()
        self.filter_gen = ldsi.ChainOfThought(GenerateFilter)
