...
def get_async_max_workers():
    ...
    return ldsi.settings.async_max_workers
...
def asyncify(program: Module) -> Callable[[Any, Any], Awaitable[Any]]:
    """
    Wraps a LDSI program so that it can be called asynchronously. This is useful for running a
    program in parallel with another task (e.g., another LDSI program).

    This implementation propagates the current thread's configuration context to the worker thread.

    Args:
        program: The LDSI program to be wrapped for asynchronous execution.
    ...
    async def async_program(*args, **kwargs) -> Any:
        ...
        from ldsi.dsp.utils.settings import thread_local_overrides
