...
from ldsi.dsp.utils.utils import dotdict
...
class Settings:
    """
    A singleton class for LDSI configuration settings.
    ...
    - 'context' sets thread-local overrides. These overrides propagate to threads spawned
      ...
      1. Only one unique thread (which can be any thread!) can call ldsi.configure.
      2. It affects a global state, visible to all. As a result, user threads work, but they shouldn't be
         mixed with concurrent changes to ldsi.configure from the "main" thread.
         (TODO: In the future, add warnings: if there are near-in-time user-thread reads followed by .configure calls.)
      3. Any thread can use ldsi.context. It propagates to child threads created with LDSI primitives: Parallel, asyncify, etc.
    """
    ...
    def configure(self, **kwargs):
        ...
        with self.lock:
            ...
            if config_owner_thread_id in [None, current_thread_id]:
                ...
                raise RuntimeError("ldsi.settings can only be changed by the thread that initially configured it.")
