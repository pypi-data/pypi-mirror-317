...
import ldsi
from ldsi.propose.utils import strip_prefix
...
class ObservationSummarizer(ldsi.Signature):
    ...
    summary = ldsi.OutputField(desc="Two to Three sentence summary of only the most significant highlights of my observations")
...
class DatasetDescriptorWithPriorObservations(ldsi.Signature):
    ...
    observations = ldsi.OutputField(desc="Somethings that holds true for most or all of the data you observed or COMPLETE if you have nothing to add")
...
def create_dataset_summary(trainset, view_data_batch_size, prompt_model, log_file=None, verbose=False):
    ...
    prompt_model = prompt_model if prompt_model else ldsi.settings.lm
    with ldsi.settings.context(lm=prompt_model):
        observation = ldsi.Predict(DatasetDescriptor, n=1, temperature=1.0)(examples=order_input_keys_in_string(trainset[0:upper_lim].__repr__()))
    ...
    try:
        ...
        for b in range(view_data_batch_size, len(trainset), view_data_batch_size):
            ...
            with ldsi.settings.context(lm=prompt_model):
                output = ldsi.Predict(DatasetDescriptorWithPriorObservations, n=1, temperature=1.0)(prior_observations=observations, examples=order_input_keys_in_string(trainset[b:upper_lim].__repr__()))
