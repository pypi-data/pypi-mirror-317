...
class DatabricksRM(ldsi.Retrieve):
    """
    ...
    Examples:
        ...
        and configure a DatabricksRM LDSI retriever module to query the index.
    ...
    def forward(
        ...
        query_type: str = "ANN",
        filters_json: Optional[str] = None,
    ) -> Union[ldsi.Prediction, List[Dict[str, Any]]]:
        """
        ...
        Returns:
            ...
            or a ``ldsi.Prediction`` object when ``use_with_databricks_agent_framework`` is
            ``False``.
        """
        if query_type in ["vector", "text"]:
            # Older versions of LDSI used a `query_type` argument to disambiguate between text
            ...
            # backwards compatibility with older versions of LDSI, we map the old `query_type`
