...
def get_dependency_versions():
    ...
    return {
        ...
        "ldsi": importlib_metadata.version("ldsi"),
        "cloudpickle": cloudpickle_version,
    }


def load(path):
    """Load saved LDSI model.

    This method is used to load a saved LDSI model with `save_program=True`, i.e., the model is saved with cloudpickle.

    Args:
        path (str): Path to the saved model.

    Returns:
        The loaded model, a `ldsi.Module` instance.
    """
    path = Path(path)
    if not path.exists():
