...
from ldsi.predict.avatar.signatures import Actor
from ldsi.signatures.signature import ensure_signature
from ldsi.predict.avatar.models import Action, ActionOutput, Tool
...
class Avatar(ldsi.Module):
    def __init__(
        ...
        self.actor = ldsi.TypedPredictor(self.actor_signature)

        self.actor_clone = deepcopy(self.actor)


    def _get_field(self, field_info: FieldInfo):
        if field_info.json_schema_extra['__ldsi_field_type'] == 'input':
            return ldsi.InputField(
        ...
        elif field_info.json_schema_extra['__ldsi_field_type'] == 'output':
            return ldsi.OutputField(
                prefix=field_info.json_schema_extra['prefix'],
                desc=field_info.json_schema_extra['desc'],
                format=field_info.json_schema_extra['format'] if 'format' in field_info.json_schema_extra else None,
            )
        else:
            raise ValueError(f"Unknown field type: {field_info.json_schema_extra['__ldsi_field_type']}")
