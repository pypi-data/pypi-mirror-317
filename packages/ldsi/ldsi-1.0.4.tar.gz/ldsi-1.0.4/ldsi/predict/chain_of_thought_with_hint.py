...
class ChainOfThoughtWithHint(Module):
    def __init__(self, signature, rationale_type=None, **config):
        self.signature = ldsi.ensure_signature(signature)
        self.module = ldsi.ChainOfThought(signature, rationale_type=rationale_type, **config)
    ...
    def forward(self, **kwargs):
        if 'hint' in kwargs and kwargs['hint']:
            ...
            this_trace = ldsi.settings.trace[-1]
            ldsi.settings.trace[-1] = (this_trace[0], original_kwargs, this_trace[2])
            return pred
