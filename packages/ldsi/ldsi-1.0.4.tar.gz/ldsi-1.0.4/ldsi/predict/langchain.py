...
# import ldsi
# from ldsi.predict.parameter import Parameter
# from ldsi.predict.predict import Predict
# from ldsi.primitives.prediction import Prediction
# from ldsi.signatures.field import OldInputField, OldOutputField
# from ldsi.signatures.signature import infer_prefix
...
#         self.demos = [ldsi.Example(**x) for x in self.demos]
...
#         with ldsi.context(lm=gpt4T): parts = ldsi.Predict(Template2Signature)(template=template)
...
#             v.finalize(k, infer_prefix(k))  # TODO: Generate from the template at ldsi.Predict(Template2Signature)
...
#         ldsi.settings.langchain_history.append((prompt, pred))
...
# class LangChainModule(ldsi.Module):
...
#         return ldsi.Prediction({k: output for k in output_keys})
