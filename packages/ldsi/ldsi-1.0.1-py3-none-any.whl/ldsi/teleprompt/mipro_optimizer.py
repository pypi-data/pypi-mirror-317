...
# from ldsi.signatures import Signature
# from ldsi.signatures.signature import signature_to_template
# from ldsi.teleprompt import BootstrapFewShot
...
# ``` python
# from ldsi.teleprompt import MIPRO
...
# * prompt_model: The model used for prompt generation. When unspecified, defaults to the model set in settings (i.e., ldsi.settings.configure(lm=task_model)).
# * task_model: The model used for running your task. When unspecified, defaults to the model set in settings (i.e., ldsi.settings.configure(lm=task_model)).
...
#     basic_instruction = ldsi.InputField(desc="The initial instructions before optimization")
#     proposed_instruction = ldsi.OutputField(desc="The improved instructions for the language model")
#     proposed_prefix_for_output_field = ldsi.OutputField(
...
#     basic_instruction = ldsi.InputField(desc="The initial instructions before optimization")
#     observations = ldsi.InputField(desc="Observations about the dataset and task")
#     proposed_instruction = ldsi.OutputField(desc="The improved instructions for the language model")
#     proposed_prefix_for_output_field = ldsi.OutputField(
#         desc="The string at the end of the prompt, which will help the model start solving the task",
#     )
...
# class BasicGenerateInstructionWithExamples(ldsi.Signature):
...
#     # attempted_instructions = ldsi.InputField(format=str, desc="Previously attempted task instructions, along with their resulting validation score, and an example of the instruction in use on a sample from our dataset.")
#     basic_instruction = ldsi.InputField(desc="The initial instructions before optimization")
#     # examples = ldsi.InputField(desc="Example(s) of the task")
#     examples = ldsi.InputField(desc="Example(s) of the task")
#     proposed_instruction = ldsi.OutputField(desc="The improved instructions for the language model")
#     proposed_prefix_for_output_field = ldsi.OutputField(
#         desc="The string at the end of the prompt, which will help the model start solving the task",
...
# class BasicGenerateInstructionWithExamplesAndDataObservations(ldsi.Signature):
...
# class ObservationSummarizer(ldsi.Signature):
...
#     observations = ldsi.InputField(desc="Observations I have made about my dataset")
#     summary = ldsi.OutputField(
#         desc="Two to Three sentence summary of only the most significant highlights of my observations",
#     )


# class DatasetDescriptor(ldsi.Signature):
#     (
...
#     examples = ldsi.InputField(desc="Sample data points from the dataset")
#     observations = ldsi.OutputField(desc="Somethings that holds true for most or all of the data you observed")


# class DatasetDescriptorWithPriorObservations(ldsi.Signature):
#     (
#         """Given several examples from a dataset please write observations about trends that hold for most or all of the samples. """
...
#     examples = ldsi.InputField(desc="Sample data points from the dataset")
#     prior_observations = ldsi.InputField(desc="Some prior observations I made about the data")
#     observations = ldsi.OutputField(
...
#         self.prompt_model = prompt_model if prompt_model is not None else ldsi.settings.lm
#         self.task_model = task_model if task_model is not None else ldsi.settings.lm
...
#         observation = ldsi.Predict(DatasetDescriptor, n=1, temperature=1.0)(examples=(trainset[0:upper_lim].__repr__()))
...
#             output = ldsi.Predict(DatasetDescriptorWithPriorObservations, n=1, temperature=1.0)(
...
#         summary = ldsi.Predict(ObservationSummarizer, n=1, temperature=1.0)(observations=observations)
...
#             with ldsi.settings.context(lm=self.prompt_model):
...
#                 # Neither
#                 else:
#                     instruct = ldsi.Predict(BasicGenerateInstruction, n=N - 1, temperature=self.init_temperature)(
#                         basic_instruction=basic_instruction,
#                     )
...
#     def compile(
#         self,
#         student: ldsi.Program,
#         *,
#         trainset: list[ldsi.Example],
#         num_trials: int,
