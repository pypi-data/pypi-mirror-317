...
class UnderstandTask(ldsi.Signature):
    ...
    task_description = ldsi.InputField(
        prefix="Task Description:",
        desc="Description of the task.",
    )
    explanation = ldsi.OutputField(
        prefix="Task Description:",
        desc="Explanation of the task.",
    )

class ExplainTask(ldsi.Signature):
...
class UpdateTaskDescriptionBasedOnFeedback(ldsi.Signature):
    ...
    feedback = ldsi.InputField(
        prefix="Feedback:",
        desc="Feedback on the task description.",
    )
    updated_task_description = ldsi.OutputField(
        prefix="Task Description:",
        desc="Updated description of the task.",
    )

class GetFeedbackOnGeneration(ldsi.Signature):
    ...
    synthetic_data = ldsi.InputField(
    ...
    feedback = ldsi.OutputField(
        prefix="Feedback:",
        desc="Feedback on the synthetic data.",
    )

class GenerateFieldDescription(ldsi.Signature):
    ...
    field_name = ldsi.InputField(
        prefix="Field Name:",
        desc="Name of the field to generate synthetic data for.",
    )
    field_description = ldsi.OutputField(
        prefix="Field Description:",
        desc="Description of the field.",
    )

class GenerateInputFieldsData(ldsi.Signature):
...
A knowledge seed is the index of the knowledge base you have, each index represents a different knowledge base."""
    ...
    knowledge_seed = ldsi.InputField(
...
class GenerateOutputFieldsData(ldsi.Signature):
    pass