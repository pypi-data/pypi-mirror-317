...
from ldsi.adapters.base import Adapter
from ldsi.adapters.image_utils import Image
from ldsi.adapters.utils import find_enum_member, format_field_value, serialize_for_json
from ldsi.signatures.signature import SignatureMeta
from ldsi.signatures.utils import get_ldsi_field_type
...
def _format_field_value(field_info: FieldInfo, value: Any) -> str:
    """
    Formats the value of the specified field according to the field's LDSI type (input or output),
    annotation (e.g. str, int, etc.), and the type of the value itself.

    Args:
      field_info: Information about the field, including its LDSI field type and annotation.
...
def format_fields(role: str, fields_with_values: Dict[FieldInfoWithName, Any]) -> str:
    """
    Formats the values of the specified fields according to the field's LDSI type (input or output),
...
def format_turn(signature: SignatureMeta, values: Dict[str, Any], role, incomplete=False) -> Dict[str, str]:
    ...
    so that it can instruct an LLM to generate responses conforming to the specified LDSI signature.

    Args:
        signature: The LDSI signature to which future LLM responses should conform.
        values: A dictionary mapping field names (from the LDSI signature) to corresponding values
...
def prepare_instructions(signature: SignatureMeta):
    ...
    def field_metadata(field_name, field_info):
        ...
        if get_ldsi_field_type(field_info) == "input" or type_ is str:
            desc = ""
        elif type_ is bool:
...
def _get_structured_outputs_response_format(signature: SignatureMeta) -> pydantic.BaseModel:
    ...
    an LM request, based on the output fields of the specified LDSI signature.

    Args:
        signature: The LDSI signature for which to obtain the `response_format` request parameter.
    Returns:
    ...
    def filter_json_schema_extra(field_name: str, field_info: FieldInfo) -> FieldInfo:
        """
        Recursively filter the `json_schema_extra` of a FieldInfo to exclude LDSI internal attributes
        (e.g. `__ldsi_field_type`) and remove descriptions that are placeholders for the field name.
        """
        ...
        if field_copy.json_schema_extra:
            field_copy.json_schema_extra = {
                ...
                if key not in ("desc", "__ldsi_field_type")
            }
    ...
    return create_model("LDSIProgramOutputs", **output_pydantic_fields)
