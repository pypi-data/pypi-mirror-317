...
import ldsi
from ldsi.propose.dataset_summary_generator import create_dataset_summary
from ldsi.propose.utils import create_example_string, create_predictor_level_history_string, strip_prefix, get_ldsi_source_code
from ldsi.teleprompt.utils import get_signature, get_prompt_model

from ldsi.propose.propose_base import Proposer
...
class DescribeProgram(ldsi.Signature):
    (
...
class DescribeModule(ldsi.Signature):
    (
...
def generate_instruction_class(
    ...
    class GenerateSingleModuleInstruction(ldsi.Signature):
        (
            """Use the information below to learn about a task that we are trying to solve using calls to an LM, then generate a new instruction that will be used to prompt a Language Model to better solve the task."""
        )
        if use_dataset_summary:
            dataset_description = ldsi.InputField(
...
class GenerateModuleInstruction(ldsi.Module):
    def __init__(
        ...
        self.describe_program = ldsi.Predict(DescribeProgram)
        self.describe_module = ldsi.Predict(DescribeModule)
    ...
    def forward(
        ...
        if self.program_aware:
            try:
                ...
                for field_name, field in get_signature(program.predictors()[pred_i]).fields.items():
                    # Access the '__ldsi_field_type' from the extra metadata
                    ldsi_field_type = field.json_schema_extra.get('__ldsi_field_type')
                    
                    # Based on the '__ldsi_field_type', append to the respective list
                    if ldsi_field_type == "input":
        ...
        return ldsi.Prediction(proposed_instruction=proposed_instruction)
...
class GroundedProposer(Proposer):
    def __init__(
        ...
        if self.program_aware:
            try:
                self.program_code_string = get_ldsi_source_code(program)
