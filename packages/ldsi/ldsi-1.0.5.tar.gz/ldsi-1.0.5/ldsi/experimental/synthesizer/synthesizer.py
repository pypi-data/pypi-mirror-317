...
import ldsi
from ldsi.experimental.synthesizer.config import SynthesizerArguments
from ldsi.experimental.synthesizer.instruction_suffixes import (
    INPUT_GENERATION_TASK_WITH_EXAMPLES_SUFFIX,
    INPUT_GENERATION_TASK_WITH_FEEDBACK_SUFFIX,
)
from ldsi.experimental.synthesizer.signatures import (
    ExplainTask,
...
from ldsi.experimental.synthesizer.utils import format_examples


class Synthesizer:
    def __init__(self, config: SynthesizerArguments):
        self.config = config
        self.input_lm = config.input_lm_model or ldsi.settings.lm
        self.output_lm = config.output_lm_model or ldsi.settings.lm

        self.explain_task = ldsi.Predict(ExplainTask)
        self.understand_task = ldsi.Predict(UnderstandTask)
        self.get_feedback_on_generation = ldsi.Predict(GetFeedbackOnGeneration)
        self.generate_field_description = ldsi.Predict(GenerateFieldDescription)
        self.update_task_description = ldsi.Predict(UpdateTaskDescriptionBasedOnFeedback)

        self.generate_input_data = GenerateInputFieldsData
        self.generate_output_data = GenerateOutputFieldsData

    def _gather_feedback(self, examples: ldsi.Example) -> str:
    ...
    def _prepare_synthetic_data_predictors(
        ...
        return ldsi.ChainOfThought(self.generate_input_data), ldsi.Predict(self.generate_output_data)

    def _get_dataset_metadata(self, ground_source: Union[List[ldsi.Example], ldsi.Signature]):
        if isinstance(ground_source, ldsi.SignatureMeta):
        ...
        elif isinstance(ground_source, list) and isinstance(ground_source[0], ldsi.Example):
    ...
    def generate(
        self,
        ground_source: Union[List[ldsi.Example], ldsi.Signature],
        num_data: int,
        batch_size: int = 1,
    ):
        ...
        for idx in trange(0, num_data, batch_size, desc="Generating Synthetic Data"):
            ...
            with ldsi.context(lm=self.input_lm):
            ...
            for kwargs in input_kwargs:
                ...
                with ldsi.context(lm=self.output_lm, temperature=iter_temperature):
                ...
                data.append(ldsi.Example(**kwargs, **output_kwargs).with_inputs(*input_keys))
    ...
    def export(self, data: List[ldsi.Example], path: str, mode: str = None, **kwargs):
