...
from ldsi.adapters.chat_adapter import FieldInfoWithName, field_header_pattern, format_fields
from ldsi.clients.lm import LM
...
class DummyLM(LM):
    ...
    ldsi.settings.configure(lm=lm)
    predictor("What color is the sky?, demos=ldsi.Example(input="What color is the sky?", output="blue"))
    # Output:
    # [[## answer ##]]
    # blue
    ```
...
def dummy_rm(passages=()) -> callable:
    ...
    def inner(query: str, *, k: int, **kwargs):
        ...
        # return ldsi.Prediction(passages=[passages[i] for i in largest_idx])
