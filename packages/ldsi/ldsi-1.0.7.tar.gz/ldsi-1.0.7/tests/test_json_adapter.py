...
def test_json_adapter_passes_structured_output_when_supported_by_model():
    ...
    class TestSignature(ldsi.Signature):
        input1: str = ldsi.InputField()
        output1: str = ldsi.OutputField()  # Description intentionally left blank
        output2: bool = ldsi.OutputField(desc="Boolean output field")
        output3: OutputField3 = ldsi.OutputField(desc="Nested output field")
        output4_unannotated = ldsi.OutputField(desc="Unannotated output field")

    program = ldsi.Predict(TestSignature)

    # Configure LDSI to use an OpenAI LM that supports structured outputs
    ldsi.configure(lm=ldsi.LM(model="openai/gpt4o"), adapter=ldsi.JSONAdapter())
    ...
    def clean_schema_extra(field_name, field_info):
        ...
        if "json_schema_extra" in attrs:
            attrs["json_schema_extra"] = {
                ...
                if k != "__ldsi_field_type" and not (k == "desc" and v == f"${{{field_name}}}")
            }
        return attrs
    ...
    # Configure LDSI to use a model from a fake provider that doesn't support structured outputs
    ldsi.configure(lm=ldsi.LM(model="fakeprovider/fakemodel"), adapter=ldsi.JSONAdapter())
    with mock.patch("litellm.completion") as mock_completion:
        program(input1="Test input")
...
def test_json_adapter_falls_back_when_structured_outputs_fails():
    class TestSignature(ldsi.Signature):
        input1: str = ldsi.InputField()
        output1: str = ldsi.OutputField(desc="String output field")
