...
class Predict(Module, Parameter):
    ...
    def load_state(self, state):
        ...
        if "extended_signature" in state: # legacy, up to and including 2.5, for CoT.
            raise NotImplementedError("Loading extended_signature is no longer supported in LDSI 2.6+")
    ...
    def forward(self, **kwargs):
        ...
        lm = kwargs.pop("lm", self.lm) or ldsi.settings.lm
        assert isinstance(lm, ldsi.LM), "No LM is loaded."
        ...
        if kwargs.pop("_trace", True) and ldsi.settings.trace is not None:
