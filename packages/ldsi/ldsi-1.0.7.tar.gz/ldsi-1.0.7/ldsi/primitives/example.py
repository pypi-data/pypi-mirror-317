class Example:
    ...
    def __len__(self):
        return len([k for k in self._store if not k.startswith("ldsi_")])

    def __repr__(self):
        # return f"Example({self._store})" + f" (input_keys={self._input_keys}, demos={self._demos})"
        d = {k: v for k, v in self._store.items() if not k.startswith("ldsi_")}
    ...
    def keys(self, include_ldsi=False):
        return [k for k in self._store.keys() if not k.startswith("ldsi_") or include_ldsi]

    def values(self, include_ldsi=False):
        return [v for k, v in self._store.items() if not k.startswith("ldsi_") or include_ldsi]

    def items(self, include_ldsi=False):
        return [(k, v) for k, v in self._store.items() if not k.startswith("ldsi_") or include_ldsi]
