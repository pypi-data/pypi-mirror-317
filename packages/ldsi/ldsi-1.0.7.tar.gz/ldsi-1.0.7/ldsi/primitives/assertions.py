...
# class LDSIAssertionError(AssertionError):
#     """Custom exception raised when a LDSI `Assert` fails."""
...
# class LDSISuggestionError(AssertionError):
#     """Custom exception raised when a LDSI `Suggest` fails."""
...
#             if self.result:
#                 return True
#             elif ldsi.settings.bypass_assert:
#                 logger.error(f"AssertionError: {self.msg}")
#                 return True
#             else:
#                 logger.error(f"AssertionError: {self.msg}")
#                 raise LDSIAssertionError(
#                     id=self.id,
#                     msg=self.msg,
#                     target_module=self.target_module,
#                     state=ldsi.settings.trace,
...
#             if self.result:
#                 return True
#             elif ldsi.settings.bypass_suggest:
#                 logger.info(f"SuggestionFailed: {self.msg}")
#                 return True
#             else:
#                 logger.info(f"SuggestionFailed: {self.msg}")
#                 raise LDSISuggestionError(
#                     id=self.id,
#                     msg=self.msg,
#                     target_module=self.target_module,
#                     state=ldsi.settings.trace,
...
#         with ldsi.settings.context(bypass_assert=True, bypass_suggest=True):
...
#     def wrapper(*args, **kwargs):
#         with ldsi.settings.context(bypass_suggest=True, bypass_assert=False):
#             return func(*args, **kwargs)
...
#     def wrapper(*args, **kwargs):
#         with ldsi.settings.context(bypass_assert=True):
#             return func(*args, **kwargs)
...
#         with ldsi.settings.lock:
#             ldsi.settings.backtrack_to = None
#             ldsi.settings.suggest_failures = 0
#             ldsi.settings.assert_failures = 0

#             # Predictor -> List[feedback_msg]
#             ldsi.settings.predictor_feedbacks = {}

#             current_error = None
#             for i in range(max_backtracks + 1):
#                 if i > 0 and ldsi.settings.backtrack_to is not None:
#                     # generate values for new fields
#                     feedback_msg = _build_error_msg(
#                         ldsi.settings.predictor_feedbacks[ldsi.settings.backtrack_to],
#                     )

#                     ldsi.settings.backtrack_to_args = {
...
#                 if i == max_backtracks:
#                     if isinstance(current_error, LDSIAssertionError):
#                         raise current_error
#                     ldsi.settings.trace.clear()
...
#                         break
#                     except (LDSISuggestionError, LDSIAssertionError) as e:
...
#                         if isinstance(e, LDSISuggestionError) and e.is_metric:
#                             ldsi.settings.suggest_failures += 1
#                         elif isinstance(e, LDSIAssertionError) and e.is_metric:
#                             ldsi.settings.assert_failures += 1

#                         if ldsi.settings.trace:
#                             if error_target_module:
#                                 for i in range(len(ldsi.settings.trace) - 1, -1, -1):
#                                     trace_element = ldsi.settings.trace[i]
#                                     mod = trace_element[0]
#                                     if mod == error_target_module:
#                                         error_state = e.state[i]
#                                         ldsi.settings.backtrack_to = mod
#                                         break
#                             else:
#                                 ldsi.settings.backtrack_to = ldsi.settings.trace[-1][0]

#                             if ldsi.settings.backtrack_to is None:
#                                 logger.error("Module not found in trace. If passing a LDSI Signature, please specify the intended module for the assertion (e.g., use `target_module = self.my_module(my_signature)` instead of `target_module =  my_signature`).")

#                             # save unique feedback message for predictor
#                             if error_msg not in ldsi.settings.predictor_feedbacks.setdefault(
#                                 ldsi.settings.backtrack_to,
#                                 [],
#                             ):
#                                 ldsi.settings.predictor_feedbacks[ldsi.settings.backtrack_to].append(error_msg)
...
#             ldsi.settings.configure(bypass_assert=args_to_vals["bypass_assert"])
...
#     if all(
#         map(lambda p: isinstance(p[1], ldsi.retry.Retry), module.named_predictors()),
#     ):
#         pass  # we already applied the Retry mapping outside
#     elif all(
#         map(lambda p: not isinstance(p[1], ldsi.retry.Retry), module.named_predictors()),
#     ):
#         module.map_named_predictors(ldsi.retry.Retry)
#     else:
