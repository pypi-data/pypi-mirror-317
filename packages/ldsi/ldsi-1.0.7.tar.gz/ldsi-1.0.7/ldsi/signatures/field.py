import pydantic
# The following arguments can be used in LDSI InputField and OutputField in addition
# to the standard pydantic.Field arguments. We just hope pydanitc doesn't add these,
# as it would give a name clash.
LDSI_FIELD_ARG_NAMES = ["desc", "prefix", "format", "parser", "__ldsi_field_type"]
...
def move_kwargs(**kwargs):
    ...
    for k, v in kwargs.items():
        if k in LDSI_FIELD_ARG_NAMES:
            json_schema_extra[k] = v
        else:
            pydantic_kwargs[k] = v
    # Also copy over the pydantic "description" if no ldsi "desc" is given.
...
def InputField(**kwargs):
    return pydantic.Field(**move_kwargs(**kwargs, __ldsi_field_type="input"))


def OutputField(**kwargs):
    return pydantic.Field(**move_kwargs(**kwargs, __ldsi_field_type="output"))


def new_to_old_field(field):
    return (OldInputField if field.json_schema_extra["__ldsi_field_type"] == "input" else OldOutputField)(
