...
class Embedder:
    ...
    Examples:
        ...
        embedder = ldsi.Embedder("openai/text-embedding-3-small", batch_size=100)
        ...
        embedder = ldsi.Embedder(my_embedder)
    ...
    def __call__(self, inputs, batch_size=None, caching=None, **kwargs):
        ...
        for batch_inputs in chunk(inputs, batch_size):
            if isinstance(self.model, str):
                ...
                raise ValueError(
                    f"`model` in `ldsi.Embedder` must be a string or a callable, but got {type(self.model)}."
                )
