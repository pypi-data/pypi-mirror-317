...
from ldsi.clients.lm import LM  # TODO: Remove after the old LM class is removed
from ldsi.clients.utils_finetune import infer_data_format
from ldsi.evaluate.evaluate import Evaluate
from ldsi.predict.predict import Predict
from ldsi.primitives.example import Example
from ldsi.primitives.program import Program
...
class BootstrapFinetune(FinetuneTeleprompter):
    ...
    def __init__(
        ...
        assert ldsi.settings.experimental, err
    ...
    def compile(self, student: Program, trainset: List[Example], teacher: Optional[Program] = None) -> Program:
        ...
        # that the default LDSI logger logs info level messages in notebook
        # environments.
...
def bootstrap_trace_data(
    ...
    def wrapped_program(**kwargs):
        with ldsi.context(trace=[]):
            return program(**kwargs), ldsi.settings.trace.copy()
...
# Similar methods are implemented separately and used by other LDSI
