...
# import ldsi
# from ldsi.dsp.modules.sentence_vectorizer import BaseSentenceVectorizer, FastEmbedVectorizer
...
#     raise ImportError(
#         "The 'qdrant' extra is required to use QdrantRM. Install it with `pip install ldsi-ai[qdrant]`",
#     ) from e


# class QdrantRM(ldsi.Retrieve):
...
#         llm = ldsi.OpenAI(model="gpt-3.5-turbo")
#         qdrant_client = QdrantClient()
#         retriever_model = QdrantRM("my_collection_name", qdrant_client=qdrant_client)
#         ldsi.settings.configure(lm=llm, rm=retriever_model)
#         ```
...
#     def forward(self, query_or_queries: Union[str, list[str]], k: Optional[int] = None, filter: Optional[models.Filter]=None) -> ldsi.Prediction:
