...
# import ldsi
# from ldsi import Predict
# from ldsi.signatures.field import InputField, OutputField
# from ldsi.signatures.signature import ensure_signature, make_signature, signature_to_template
...
#     return [
#         k for k, v in template.kwargs.items() if isinstance(v, ldsi.signatures.OldInputField)
#     ]
...
#     # template_vars = list(template.kwargs.keys())
#     return [
#         k for k, v in template.kwargs.items() if isinstance(v, ldsi.signatures.OldOutputField)
#     ]


# class LDSIPromptTemplate(BasePromptTemplate):
#     """A prompt template for LDSI.

#     Takes in a predict module from LDSI (whether unoptimized or optimized),
...
# class Template2Signature(ldsi.Signature):
...
#     template = ldsi.InputField(format=lambda x: f"```\n\n{x.strip()}\n\n```\n\nLet's now prepare three modular pieces.")
#     essential_instructions = ldsi.OutputField()
#     input_keys = ldsi.OutputField(desc='comma-separated list of valid variable names')
#     output_key = ldsi.OutputField(desc='a valid variable name')


# def build_signature(prompt: PromptTemplate) -> ldsi.Signature:
#     """Attempt to build signature from prompt."""
#     # TODO: allow plugging in any llamaindex LLM 
#     gpt4T = ldsi.OpenAI(model='gpt-4-1106-preview', max_tokens=4000, model_type='chat')

#     with ldsi.context(lm=gpt4T): 
#         parts = ldsi.Predict(Template2Signature)(template=prompt.template)

#     inputs = {k.strip(): InputField() for k in parts.input_keys.split(',')}
#     outputs = {k.strip(): OutputField() for k in parts.output_key.split(',')}

#     # dynamically create a pydantic model that subclasses ldsi.Signature
#     fields = {
...
#         LLM is a TODO - currently use LDSI LLM classes.
...
# class LlamaIndexModule(ldsi.Module):
#     """A module for LlamaIndex.

#     Wraps a QueryPipeline and exposes it as a ldsi module for optimization.
...
#             if isinstance(module, LDSIComponent):
...
#         return ldsi.Prediction(**output_dict)
