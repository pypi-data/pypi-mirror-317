...
class DescriptionSignature(ldsi.Signature):
    field_name = ldsi.InputField(desc="name of a field")
    example = ldsi.InputField(desc="an example value for the field")
    description = ldsi.OutputField(desc="a short text only description of what the field contains")


class SyntheticDataGenerator:
    def __init__(self, schema_class: Optional[BaseModel] = None, examples: Optional[List[ldsi.Example]] = None):
        self.schema_class = schema_class
        self.examples = examples

    def generate(self, sample_size: int) -> List[ldsi.Example]:
        ...
        Returns:
            List[ldsi.Example]: list of synthetic examples generated
        """
    ...
    def _define_or_infer_fields(self):
        ...
        if self.schema_class:
            ...
            descriptor = ldsi.Predict(DescriptionSignature)
    ...
    def _generate_additional_examples(self, additional_samples_needed: int) -> List[ldsi.Example]:
        ...
        Returns:
            List[ldsi.Example]: list of synthetic examples
        """
        ...
        signature_class = type(class_name, (ldsi.Signature,), fields)
        generator = ldsi.Predict(signature_class, n=additional_samples_needed)
        response = generator(sindex=str(random.randint(1, additional_samples_needed)))

        return [ldsi.Example({field_name: getattr(completion, field_name) for field_name in properties.keys()})
    ...
    def _prepare_fields(self, properties) -> dict:
        ...
        return {
            ...
            'sindex': ldsi.InputField(desc="a random string"),
            **{field_name: ldsi.OutputField(desc=properties[field_name].get('description', 'No description'))
