...
import ldsi
from ldsi.datasets.dataset import Dataset
...
class DataLoader(Dataset):
    ...
    def from_huggingface(
        ...
        **kwargs,
    ) -> Union[Mapping[str, List[ldsi.Example]], List[ldsi.Example]]:
        ...
        try:
            ...
            for split_name in dataset.keys():
                if fields:
                    returned_split[split_name] = [
                        ldsi.Example({field: row[field] for field in fields}).with_inputs(*input_keys)
                        for row in dataset[split_name]
                    ]
                else:
                    returned_split[split_name] = [
                        ldsi.Example({field: row[field] for field in row.keys()}).with_inputs(*input_keys)
                        for row in dataset[split_name]
                    ]
            ...
            if fields:
                return [
                    ldsi.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for row in dataset
                ]
            else:
                return [
                    ldsi.Example({field: row[field] for field in row.keys()}).with_inputs(*input_keys)
    ...
    def from_csv(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[ldsi.Example]:
        dataset = load_dataset("csv", data_files=file_path)["train"]

        if not fields:
            fields = list(dataset.features)

        return [ldsi.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for row in dataset]
    ...
    def from_pandas(
        ...
        return [
            ldsi.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for _, row in df.iterrows()
        ]

    def from_json(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[ldsi.Example]:
    ...
    def from_parquet(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[ldsi.Example]:
    ...
    def from_rm(self, num_samples: int, fields: List[str], input_keys: List[str]) -> List[ldsi.Example]:
        try:
            rm = ldsi.settings.rm
            try:
                return [
            ...
            raise ValueError(
                "Retrieval module not found. Please set a retrieval module using `ldsi.settings.configure`."
            )

    def sample(
        self,
        dataset: List[ldsi.Example],
        n: int,
        *args,
        **kwargs,
    ) -> List[ldsi.Example]:
        if not isinstance(dataset, list):
            raise ValueError(
                f"Invalid dataset provided of type {type(dataset)}. Please provide a list of `ldsi.Example`s."
            )
