...
try:
    ...
    raise ImportError(
        "The 'pgvector' extra is required to use PgVectorRM. Install it with `pip install ldsi-ai[pgvector]`. Also, try `pip install pgvector psycopg2`.",
    )
try:
    import openai
except ImportError:
...
class PgVectorRM(ldsi.Retrieve):
    """
    ...
    Returns a list of ldsi.Example objects
        ...
        with self.conn as conn:
            with conn.cursor() as cur:
                ...
                for row in rows:
                    ...
                    retrieved_docs.append(ldsi.Example(**data))
        # Return Prediction
