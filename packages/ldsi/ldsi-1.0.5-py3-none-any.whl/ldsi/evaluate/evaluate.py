...
import ldsi
from ldsi.utils.parallelizer import ParallelExecutor
    ...
    def __call__(
        ...
        def process_item(example):
            ...
            if hasattr(program, "_assert_failures"):
                program._assert_failures += ldsi.settings.get("assert_failures")
            if hasattr(program, "_suggest_failures"):
                program._suggest_failures += ldsi.settings.get("suggest_failures")
        ...
        results = [((ldsi.Prediction(), self.failure_score) if r is None else r) for r in results]
