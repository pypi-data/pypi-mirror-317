...
from ldsi.teleprompt.bootstrap_finetune import BootstrapFinetune, prepare_student, set_missing_predictor_lms
...
class BetterTogether(Teleprompter):
    ...
    def __init__(self,
        ...
        if not ldsi.settings.experimental:
            raise ValueError("This is an experimental optimizer. Set `ldsi.settings.experimental` to `True` to use it.")
    ...
    def _compile_prompt_optimizer(self, student, trainset, valset_ratio) -> Program:
        ...
        # TODO: We should ensure that the prompt optimizers in LDSI respect the
