...
import ldsi
from ldsi.adapters.json_adapter import get_annotation_name
from ldsi.primitives.program import Module
from ldsi.signatures.signature import ensure_signature
from ldsi.utils.callback import with_callbacks
...
class ReAct(Module):
    def __init__(self, signature, tools: list[Callable], max_iters=5):
        """
        `tools` is either a list of functions, callable classes, or `ldsi.Tool` instances.
        """
        ...
        react_signature = (
            ldsi.Signature({**signature.input_fields}, "\n".join(instr))
            .append("trajectory", ldsi.InputField(), type_=str)
            .append("next_thought", ldsi.OutputField(), type_=str)
            .append("next_tool_name", ldsi.OutputField(), type_=Literal[tuple(tools.keys())])
            .append("next_tool_args", ldsi.OutputField(), type_=dict[str, Any])
        )

        fallback_signature = ldsi.Signature(
            {**signature.input_fields, **signature.output_fields}, signature.instructions
        ).append("trajectory", ldsi.InputField(), type_=str)

        self.tools = tools
        self.react = ldsi.Predict(react_signature)
        self.extract = ldsi.ChainOfThought(fallback_signature)

    def forward(self, **input_args):
        def format(trajectory: dict[str, Any], last_iteration: bool):
            adapter = ldsi.settings.adapter or ldsi.ChatAdapter()
            trajectory_signature = ldsi.Signature(f"{', '.join(trajectory.keys())} -> x")
        ...
        return ldsi.Prediction(trajectory=trajectory, **extract)


"""
Thoughts and Planned Improvements for ldsi.ReAct.
...
the formatter uses a general adapter.format_fields, the tracing of LDSI only sees the string, not the formatting logic.
...
One way to fix this is to support `format=fn` in the ldsi.InputField() for "trajectory" in the signatures. But this
