...
class ModuleGraph:
    def __init__(self, module_name, module):
        ...
        self.module = module
        self.inspect_settings(ldsi.settings)
    ...
    def add_module(self, module_name, module):
        ...
        if 'ldsi.predict' in str(module_type):
    ...
    def process_submodules(self, module_name, module):
        ...
        for sub_module_name, sub_module in module.__dict__.items():
            if isinstance(sub_module, ldsi.Predict):
            ...
            elif isinstance(sub_module, (ldsi.Module, ldsi.Retrieve)):
                self.add_module(f"{module_name}__{sub_module_name}", sub_module)
                if isinstance(sub_module, ldsi.Retrieve):
...
# from ldsi.experimental import ModuleGraph

# load_dotenv()

# # Configuration of ldsi models
# llm = ldsi.OpenAI(
#     model='gpt-3.5-turbo',
...
# colbertv2_wiki = ldsi.ColBERTv2(url='http://20.102.90.50:2017/wiki17_abstracts')

# ldsi.settings.configure(lm=llm, rm=colbertv2_wiki)

# class GenerateAnswer(ldsi.Signature):
#   "Answer with long and detailed answers"
#   context = ldsi.InputField(desc="may content relevant facts")
#   question = ldsi.InputField()
#   answer = ldsi.OutputField(desc="often between 10 and 50 words")

# class RAG(ldsi.Module):
#   def __init__(self, num_passages=3):
#     super().__init__()
#     self.retrieve = ldsi.Retrieve(k=num_passages)
#     self.generate_answer = ldsi.ChainOfThought(GenerateAnswer)

#   def forward(self, question):
#     context = self.retrieve(question).passages
#     prediction = self.generate_answer(context=context, question=question)
#     return ldsi.Prediction(context=context, answer=prediction.answer)
