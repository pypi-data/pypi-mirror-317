...
from ldsi.adapters.image_utils import Image  # noqa: F401
...
class SignatureMeta(type(BaseModel)):
    ...
    def _validate_fields(cls):
        for name, field in cls.model_fields.items():
            ...
            field_type = extra.get("__ldsi_field_type")
    ...
    def _get_fields_with_type(cls, field_type) -> dict[str, FieldInfo]:
        return {k: v for k, v in cls.model_fields.items() if v.json_schema_extra["__ldsi_field_type"] == field_type}
    ...
    def insert(cls, index: int, name: str, field, type_: Type = None) -> Type["Signature"]:
        ...
        lst = input_fields if field.json_schema_extra["__ldsi_field_type"] == "input" else output_fields
...
class Signature(BaseModel, metaclass=SignatureMeta):
    ...
    def replace(
        ...
        This is useful for updating the internal signatures of ldsi
...
def _parse_type_node(node, names=None) -> Any:
    ...
    def resolve_name(id_: str):
        ...
        # This allows handling of module-based annotations like `ldsi.Image`.
        try:
