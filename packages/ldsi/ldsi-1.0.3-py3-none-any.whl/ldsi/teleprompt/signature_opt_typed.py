...
import ldsi
from ldsi import BaseModel
from ldsi.functional.functional import TypedChainOfThought, TypedPredictor
...
def generate_with_avoidance(signatures_to_avoid: list[BaseModel]) -> type[Signature]:
    class GenerateSignature(ldsi.Signature, Generic[T]):
        __doc__ = textwrap.dedent(
            """\
...
def optimize_signature(
    ...
    prompt_model : ldsi.LanguageModel, optional
    ...
    prompt_model = prompt_model or ldsi.settings.lm
    ...
    for i in range(n_iterations):
        ...
        with ldsi.settings.context(lm=prompt_model):
            ...
            for name, _p in named_predictors:
                ...
                demos = [ldsi.Example(proposed_signature=info, score=sc) for info, sc in zip(candidates[name], scores)]
