...
* prompt_model: The model used for prompt generation. When unspecified, defaults to the model set in settings (ie. ldsi.settings.configure(lm=task_model)).
...
class GenerateInstructionGivenAttempts(ldsi.Signature):
    ...
    attempted_instructions = ldsi.InputField()
    proposed_instruction = ldsi.OutputField(desc="The improved instructions for the language model")
    proposed_prefix_for_output_field = ldsi.OutputField(
        desc="The string at the end of the prompt, which will help the model start solving the task",
    )
