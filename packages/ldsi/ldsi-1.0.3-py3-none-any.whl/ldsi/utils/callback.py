...
class BaseCallback:
    """A base class for defining callback handlers for LDSI components.
    ...
    Users can set the callback globally using `ldsi.settings.configure` or locally by passing it to the component
    constructor.


    Example 1: Set a global callback using `ldsi.settings.configure`.

    ```
    import ldsi
    from ldsi.utils.callback import BaseCallback
    ...
    ldsi.settings.configure(
        callbacks=[LoggingCallback()]
    )

    cot = ldsi.ChainOfThought("question -> answer")
    ...
    ```
    lm_1 = ldsi.LM("gpt-3.5-turbo", callbacks=[LoggingCallback()])
    lm_1(question="What is the meaning of life?")

    # > LM is called with inputs: {'question': 'What is the meaning of life?'}
    # > LM is finished with outputs: {'answer': '42'}

    lm_2 = ldsi.LM("gpt-3.5-turbo")
    ...
    def on_module_start(
        ...
        """A handler triggered when forward() method of a module (subclass of ldsi.Module) is called.
    ...
    def on_module_end(
        ...
        """A handler triggered after forward() method of a module (subclass of ldsi.Module) is executed.
    ...
    def on_lm_start(
        ...
        """A handler triggered when __call__ method of ldsi.LM instance is called.
    ...
    def on_lm_end(
        ...
        """A handler triggered after __call__ method of ldsi.LM instance is executed.
    ...
    def on_adapter_format_start(
        ...
        """A handler triggered when format() method of an adapter (subclass of ldsi.Adapter) is called.
    ...
    def on_adapter_format_end(
        ...
        """A handler triggered after format() method of an adapter (subclass of ldsi.Adapter) is called..
    ...
    def on_adapter_parse_start(
        ...
        """A handler triggered when parse() method of an adapter (subclass of ldsi.Adapter) is called.
    ...
    def on_adapter_parse_end(
        ...
        """A handler triggered after parse() method of an adapter (subclass of ldsi.Adapter) is called.
...
def with_callbacks(fn):
    ...
    def wrapper(instance, *args, **kwargs):
        ...
        callbacks = ldsi.settings.get("callbacks", []) + getattr(instance, "callbacks", [])
...
def _get_on_start_handler(callback: BaseCallback, instance: Any, fn: Callable) -> Callable:
    ...
    if isinstance(instance, ldsi.LM):
        return callback.on_lm_start

    if isinstance(instance, ldsi.Adapter):
    ...
    if isinstance(instance, ldsi.Tool):
