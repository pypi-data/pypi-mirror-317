...
def streamify(program: Module) -> Callable[[Any, Any], Awaitable[Any]]:
    """
    Wrap a LDSI program so that it streams its outputs incrementally, rather than returning them
    all at once.

    Args:
        program: The LDSI program to wrap with streaming functionality.
    Returns:
    ...
    Example:
        ...
        >>>     input_text: str = ldsi.InputField()
        >>>     output_text: str = ldsi.OutputField()
        >>>
        >>> # Create the program and wrap it with streaming functionality
        >>> program = ldsi.streamify(ldsi.Predict(TestSignature))
        >>>
    ...
    async def generator(args, kwargs, stream: MemoryObjectSendStream):
        with ldsi.settings.context(send_stream=stream):
...
async def streaming_response(streamer: AsyncGenerator) -> AsyncGenerator:
    """
    Convert a LDSI program output stream to an OpenAI-compatible output stream that can be
    used by a service as an API response to a streaming request.

    Args:
        streamer: An async generator that yields values from a LDSI program output stream.
    Returns:
        An async generator that yields OpenAI-compatible streaming response chunks.
    """
    async for value in streamer:
        if isinstance(value, Prediction):
            data = {"prediction": {k: v for k, v in value.items(include_ldsi=False)}}
