...
from google.ai.generativelanguage_v1 import Blob, Content, GenerateContentResponse, Part
from google.ai.generativelanguage_v1beta import FileData, FunctionCall, FunctionResponse
from google.generativeai.types import BlobDict, content_types, ContentDict, File, FileDataDict, PartDict
...
class GeminiSettings:
    ...
    Attributes:
        ...
        safety_settings (dict[genai_types.HarmCategory, genai_types.HarmBlockThreshold]): Safety settings for the model.
            Defaults to blocking no harmful content.
        generation_config (GenerationConfig): Configuration for text generation. Defaults to a
            conservative configuration.
        start_day (datetime.datetime): The start day for tracking usage limits. Defaults to the current day
            in the US/Eastern time zone.
        request_per_day_used (int): The number of requests used so far today. Defaults to 0.
        request_per_day_limit (int | None): The maximum number of requests allowed per day. Defaults to the limit
            specified in "data.GeminiLimits" for the chosen model.
        request_per_minute_limit (int | None): The maximum number of requests allowed per minute. Defaults to the limit
            specified in "data.GeminiLimits" for the chosen model.
        tokens_per_minute_limit (int | None): The maximum number of tokens allowed per minute. Defaults to the limit
            specified in "data.GeminiLimits" for the chosen model.
        raise_error_on_limit (bool): Whether to raise an error when a rate limit is exceeded. Defaults to True.
    ...
    def __init__(
        ...
        Args:
                ...
                safety_settings (dict[genai_types.HarmCategory, genai_types.HarmBlockThreshold]): Safety settings for the model.
                    Defaults to blocking no harmful content.
                generation_config (genai.GenerationConfig): Configuration for text generation. Defaults to a
                    conservative configuration.
                start_day (datetime.datetime): The start day for tracking usage limits. Defaults to the current day
...
class GeminiLimiter:
    ...
    def __init__(
        ...
        Args:
            ...
            raise_error_on_limit (bool): Whether to raise exceptions when hitting limits.
        """
    ...
    def check_limits(self, last_tokens: int):
        ...
        Raises:
            ...
            GeminiMinuteLimitException: If the per-minute request or token limit has been exceeded and
                                                raise_error_on_limit is True.
        """
        elapsed_time = time.time() - self.start_time

        if elapsed_time < 60:
            if self.request_per_day_used > self.request_per_day_limit:
                if datetime.datetime.now(tz=pytz.timezone("America/New_York")).day != self.start_day.day:
            ...
            elif datetime.datetime.now(tz=pytz.timezone("America/New_York")).day != self.start_day.day:
                self.request_per_day_used = 1

                current_date = datetime.datetime.now(tz=pytz.timezone("America/New_York"))
                self.start_day = datetime.datetime(
                    year=current_date.year, month=current_date.month, day=current_date.day, tzinfo=current_date.tzinfo
                )
            elif (
                self.request_per_minute_used > self.request_per_minute_limit
...
class Gemini:
    ...
    def send_message(
        self,
        message: content_types.ContentType,
        stream: bool = False,
    ...
    def generate_content(
        self,
        message: content_types.ContentsType,
        stream: bool = False,
        request_options: genai_types.RequestOptions = genai_types.RequestOptions(),
    ) -> GenerateContentResponse:
        """
...
class GeminiManager:
    ...
    def use_model(self, model_index: int | None = None, model_api_key: str | None = None) -> Gemini:
        ...
        elif model_index is None and model_api_key is None:
