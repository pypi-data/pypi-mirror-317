...
class CacheService(GenericService, ABC):
    ...
    def handle(self, event):
        if isinstance(event, CacheUpdateEvent):
            ...
            logging.debug(f"start executing CacheItemGetEvent: cache_key={cache_key}, item_key={item_key}")
            cache_data = self._caches.get(cache_key)
            rst, err = None, None
            if cache_data:
                rst = cache_data.get(item_key)
            else:
                err = f"cache item not found, cache_key={cache_key} item_key={item_key}"
            event.done(result=rst, error=err)
            logging.debug(f"done executing CacheItemGetEvent: cache_key={cache_key}, item_key={item_key}, result={result}")
        else:
            if event:
