...
class Dispatcher:
    ...
    def __init__(self, name="default"):
        ...
        self._mapping = dict()
        self._running = False
        self._thread = Thread(target=self._handle_in_one_thread, args=())
    ...
    def sync_dispatch(self, event, timeout=0):
        ...
        if not done:
            event.cancel(timeout=timeout, elapsed=time.time() - start_time)
        logging.debug(f"sync_dispatched event {get_event_class_path(event)}, done={done}")

    def _handle_in_one_thread(self):
    ...
    def _handle_event(self, event):
        asyncio.run(self._async_handle_event(event))
    ...
    async def _async_handle_event(self, event):
        ...
        if handlers is None:
            ...
            return
        logging.debug(f"[dispatcher {self.__name}] start processing event {event_name}, handlers: {list(handlers.keys())}")
        # set wait_done_num
        event.wait_done(len(handlers))
        tasks = []
            ...
            async def handle_event(handler, event):
                try:
                    ...
                    await asyncio.wait_for(
                        asyncio.to_thread(handler.handle, event), timeout=event.get_exec_timeout())
                    event.add_info(EventInfoKeyDoneTime, time.time())
                except asyncio.TimeoutError:
                    err_msg = (f"[dispatcher {self.__name}] handler {handler.get_name()} execute timed out for event "
                               f"{event_name}, exec_timeout={event.get_exec_timeout()}, "
                               f"exec_elapsed={time.time() - event.get_info().get(EventInfoKeyHandleTime):.2f}")
                    logging.error(err_msg)
                    event.done(error=RuntimeError(err_msg))
                except Exception as ex:
                    err_msg = f"[dispatcher {self.__name}] failed to handle event {event_name}, err={ex}"
                    logging.error(err_msg, exc_info=ex)
                    event.done(error=RuntimeError(err_msg))
            tasks.append(handle_event(handler, event))
        await asyncio.gather(*tasks)
        logging.debug(f"[dispatcher {self.__name}] processed event {event_name}, queue_size={self._queue.qsize()}, "
                      f"result={event.get_result()}, error={event.get_error()}")


class Event(ABC):

    def __init__(self, data=None, callback=None, event=None, exec_timeout=3):
        # init callback & data
        self._data = {}
        self._callback = None
        if event:
    ...
    def wait_done(self, wait_num=1):
        self.__wait_done_num = wait_num
        return self
    ...
    def cancel(self, timeout, elapsed):
        self._canceled = True
        self._error = f"canceled event {type(self).__name__}, timeout={timeout}, elapsed={elapsed:.2f}"
    ...
    def set_exec_timeout(self, exec_timeout):
        self._exec_timeout = exec_timeout
        return self
