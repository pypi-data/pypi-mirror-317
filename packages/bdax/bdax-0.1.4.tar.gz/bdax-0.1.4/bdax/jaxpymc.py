...
class Sampler(ABC):
    ...
    def sample(self, n_chains: int = 4, n_samples: int = 1_000, n_warmup: int = 1_000,
               target_accept: Optional[float] = None, path: Optional[os.PathLike] = None, verbose: bool = True) -> None:
        ...
        if verbose:
            print("Variable - R_hat")
            for var_name in self.free_rvs:
                r_hat = self.summary.loc[var_name, "r_hat"]
                print(var_name+": "+str(str(round(r_hat, 3))))

    def save_idata(self, idata: az.InferenceData, path: os.PathLike) -> None:
        az.to_netcdf(idata, path)

    def load_idata(self, path: os.PathLike, verbose: bool = True) -> None:
    ...
    def plot_posterior(self, path: os.PathLike, var_names: Optional[list] = None, ref_vals: Optional[dict] = None,
        ...
        for var_name in var_names:
            ...
            for i in range(n_dims):
                ...
                if plot_ref_val:
                    plt.axvline(ref_vals[var_name][i], c="k", linewidth=1, label="Reference value")
                if n_dims == 1:
