...
import webbrowser
import os, math
import scolor, jfinder, basic_data
...
class xy_map:
	...
	def _check_l2d(self, input_cxy_list):
		"""
		2차원 자료를 확인하는 것
		cxy의 자료는 2차눰자료를 기본으로 사용하며, 
		...
		if type(input_cxy_list[0]) != type([]):
			input_cxy_list = [input_cxy_list]
		return input_cxy_list
	...
	def change_date_data_to_time_line_style(self, input_l2d):
		"""
		plugin중 timeline을 만들때 사용하는 자료의 형탤호 바꿔주는 것
	...
	def make_main_map_object(self, input_cxy, zoom_no):
		...
		:return:
		"""
		input_cxy = self.self.vars["basic_cxy"](input_cxy)
		result = folium.Map(
			location=input_cxy,
			zoom_start=zoom_no,
			# width=750,
