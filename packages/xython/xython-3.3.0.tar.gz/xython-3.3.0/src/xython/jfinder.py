...
class jfinder:
    def __change_jfsql_to_resql(self, input_text=""):
        """
    ...
    def change_jf_sql_to_re_sql (self, jf_sql):
        ...
        :return:
        """
        result = self.__change_jfsql_to_resql(jf_sql)
        return result
    ...
    def delete_with_jf_sql (self, jf_sql, input_text):
        ...
        :return:
        """
        re_sql = self.__change_jfsql_to_resql(jf_sql)
        re.sub(re_sql, "", input_text)
        result = self.search_all_with_jf_sql(re_sql, input_text)
        return result

    def find_nth_match(self, re_sgl, text, n):
        # 정규표현식을 컴파일합니다
    ...
    def search_between_word1_word2_with_jf_sql(self, input_text, word_a, word_b):
        ...
        re_sql = self.__change_jfsql_to_resql(re_basic)
        result = self.search_all_with_jf_sql(re_sql, input_text)
        return result
    ...
    def check_or(self, input_text):
        ...
        result = []
        resql = "\[또는:(.*?)\]"
        temp = self.search_all_by_re_sql(resql, input_text)
