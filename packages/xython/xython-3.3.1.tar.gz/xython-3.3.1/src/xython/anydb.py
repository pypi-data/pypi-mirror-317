...
class anydb:
	...
	list_db_2d : series형태로된 리스트의 묶음, 일반 list_2d와는 가로세로가 반대로 되어있다
	"""
	...
	def add_new_line_in_list_db_2d(self, l2d_i, line_data=[]):
		temp = []
		new_no = l2d_i[-1][0] + 1

		if not line_data:
			for one in range(len(l2d_i[0])-1):
				temp.append(None)
			line_data = temp
		elif len(l2d_i[0])-1 > len(line_data):
			for index in range((len(l2d_i[0])-1) - len(line_data)):
				line_data.append("")

		line_data.insert(0, new_no)
		l2d_i.append(line_data)
		return l2d_i

	def append_df(self, df_obj_1, df_obj_2):
		"""
		dataframe의 끝에 dataframe로만든것을 맨끝에 추가하는것
		"""
	...
	def change_list_2d_to_list_db(self, list_2d):
		"""
		...
		:return:
		"""
		list_2d = self.util.check_data_types_for_list_2d(list_2d)  # 2차원이 아닐때 2차원으로 만들러 주는것
		list_db_2d = self.util.change_xylist_to_yxlist(list_2d)
		return list_db_2d

	def change_list_2d_to_list_db_n_title(self, list_2d, first_is_title = True):
		"""
	...
	def change_list_db_2d_n_title_list_to_dic(self, list_db_2d, col_list=""):
		"""
		...
		temp = []
		result = {}
		if type(list_db_2d) == type([]):
			list_db_2d = self.util.check_data_types_for_list_2d(list_db_2d) #2차원이 아닐때 2차원으로 만들러 주는것
			list_db_2d = self.util.change_list_2d_to_list_2d_as_same_len(list_db_2d) # 길이가 다를때 제일 긴것으로 똑같이 만들어 주는것
			#print("====> ", list_2d)
			# 별도로 column의 제목이 없다면, 1번부터 시작하는 번호를 넣어준다
			if col_list == "":
				for index, l1d in  enumerate(list_db_2d):
					temp.append(index+1)
				col_list = temp

			for index, l1d in enumerate(list_db_2d):
				result[col_list[index]] = list_db_2d[index]
		else:
			result = list_db_2d
		return result
	...
	def change_list_db_to_df(self, input_list_db):
		...
		:param list_db_2d: 2차원 값리스트형
		:return: dataframe로 바꾼것
		"""

		input_df = pandas.DataFrame(data=input_list_db[1], columns=input_list_db[0])
		return input_df

	def change_list_db_to_df_v2(self, list_db_2d, y_title_list="", x_title_list=""):
		"""
		...
		dic_l1d = self.change_list_db_2d_n_title_list_to_dic(list_db_2d, y_title_list)
		if x_title_list == "":
			df_obj = pandas.DataFrame(dic_l1d)
		else:
			df_obj = pandas.DataFrame(dic_l1d, index = x_title_list)
		return df_obj
	...
	def change_sqlite_table_to_list(self, db_name, table_name):
		"""
	...
	def change_value_in_list_db_2d_at_xy(self, l2d_i, x, y, value):
		l2d_i[x][y] = value
		return l2d_i
	...
	def check_index_x_in_df(self, input_df, input_index):
	...
	def check_index_x_in_df_old(self, input_df, input_index):
		"""
	...
	def check_index_y_in_df(self, input_df, input_index):
		"""
		index가 기본 index인 0부터 시작하는 것이 아닌 어떤 특정한 제목이 들어가 있는경우는
	...
	def delete_x_line_in_list_db_2d_by_no(self, l2d_i, input_no):
		del l2d_i[input_no]
		return l2d_i

	def delete_y_in_list_db_by_index_y_list(self, input_list_db, input_index_list=[1, 2, 3]):
		"""
	...
	def delete_y_in_list_db_by_title_list(self, input_list_db, input_name_list=["y_name_1, y_name_2"]):
		...
		# 맨뒤부터 삭제가 되어야 index가 유지 된다
		result = self.delete_y_in_list_db_by_index_y_list(input_list_db, input_index_list)
		return result
	...
	def delete_y_line_in_list_db_2d_by_no(self, l2d_i, input_no):
		for index in range(len(l2d_i)):
			del l2d_i[index][input_no]
		return l2d_i
	...
	def insert_y_line_in_list_db_2d_by_no(self, l2d_i, input_no, title_i="", type_i=""):
		if not type_i:
			type_i = "any"
		l2d_i[0].insert(input_no, [title_i, type_i])
		for index in range(1, len(l2d_i)):
			l2d_i[index].insert(input_no, None)
		return l2d_i
	...
	def make_df_by_basic_style(self, dic_list_1d,column_list, index_list):
		df_obj = pandas.DataFrame(dic_list_1d, columns=column_list, index=index_list)
		return df_obj
	...
	def make_list_db_2d_as_simple(self, line_len, y_len):
		"""
		단순하게 아래와 같은 형태로 만든다
	...
	def make_list_db_2d_with_data_type(self, line_len, y_len, title_list=[], type_list=[]):
		"""
		list_2d = [
	...
	def plus_seti_to_set2(self, set1, set2):
		set1.update(set2)
		return set1
	...
	def read_value_in_df_by_xx(self, input_df, x):
		...
		x_list = self.check_index_x_in_df(input_df, x)
	...
	def read_value_in_df_by_xyxy(self, input_df, xyxy):
		...
		x_list = self.check_index_x_in_df(input_df, x)
		y_list = self.check_index_y_in_df(input_df, y)
		#print(x_list, y_list)
	...
	def split_list_2d_as_data_x_title_y_title(self, list_2d, x_title_len, y_title_len):
		...
		total_y_len = len(list_2d[0])

		for _ in range(total_y_len - y_title_len):
			y_title_l2d.append([])

		for l1d in list_2d[y_title_len:]:
			data_l2d.append(l1d[x_title_len:])

		if x_title_len:
			for index, l1d in enumerate(list_2d):
				if index >= y_title_len:
					x_title_l2d.append(l1d[:x_title_len])

		if y_title_len:
			for index, l1d in enumerate(list_2d):
				if index < y_title_len:
					for index2, one in enumerate(l1d[x_title_len:]):
						y_title_l2d[index2].append(one)

		return [data_l2d, x_title_l2d, y_title_l2d]
	...
	def write_list_2d_to_new_sqlite_table_by_data_only(self, db_name, table_name, list_2d):
		title_list = []
		for index, one in enumerate(list_2d[0]):
	...
	def write_value_in_sqlite_with_data_only(self, db_name, table_name, list_2d_i):
		"""
		2차원 데이터만을 가지고 값쓰기
		"""
		self.check_db_for_sqlite(db_name)

		sql_values = "?," * len(list_2d_i[0])
		sql = f"insert into {table_name} values ({sql_values[:-1]});"
		print(sql)

		for l1d in list_2d_i:
			self.cursor.execute(sql, l1d)
