...
class xy_list(object):
	...
	def filter_by_nth_style(self, input_list, nth_style, nth_only=""):
		...
		if nth_style == "even":
			...
			for one in c_list:
				aaa = self.xyre.search_all_with_jf_sql("[+-:0~1][숫자:1~][~:1~1][+-:0~1][숫자:1~]", one)  # 3~5
				bbb = self.xyre.search_all_with_jf_sql("[+-:0~1][숫자:1~][+-:1~1][숫자:1~]", one)  # 3+5,-3-5
				ccc = self.xyre.search_all_with_jf_sql("[+-:0~1][숫자:1~]", one)  # 숫자만 분리
		...
		nth_jf = None
		nth_full = self.xyre.search_all_with_jf_sql("[+-:0~1][숫자:1~]n[+-:0~1][숫자:1~]", nth_style)
		if nth_full:
			nth_jf = nth_full[0][0]
	...
	def read_value_by_xx(self, x1, x2):
		"""
