...
class pynal():
	...
	def change_utftime_to_min_list(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		분 -----> ['07']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def change_utftime_to_month_list(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		월 -----> ['04', Apr, April]
		닞은숫자 -> 많은글자 순으로 정리
	...
	def change_utftime_to_sec_list(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		초 -----> ['48']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def change_utftime_to_week_list(self, input_data=""):
		"""
		입력값 : utf시간숫자, 1640995200.0 또는 ""
		주 -----> ['5', '13', 'Fri', 'Friday']
		닞은숫자 -> 많은글자 순으로 정리
	...
	def change_utftime_to_year_list(self, input_data=""):
		"""
		년 -----> ['22', '2022']
		닞은숫자 -> 많은글자 순으로 정리
		"""
	...
	def get_week_list_by_nth_week_later(self, n_week_later, base_date=""):
		...
		utc_time_stamp = self.change_dt_obj_to_utc(dt_obt)
		dic_time = self.get_all_infomation_for_dt_obj(utc_time_stamp)
		print(dic_time['year'], int(dic_time['yearweekno']) + n_week_later)
		week_list = self.get_7_day_list_for_yearweekno(dic_time['year'], int(dic_time['yearweekno']) + n_week_later)
		return week_list
	...
	def get_weekno_for_utc(self, input_data=""):
		"""
		시간이 들어온면
		입력값 : 년도, 위크번호
		한 주의 시작은 '월'요일 부터이다
	...
	def shift_month_for_dt_obj(self, input_dt_obj="", input_month_no=""):
		"""
		월을 이동
	...
	def shift_month_for_text_time(self, input_text_time, input_month_no):
		"""
		기준날짜에서 월을 이동시키는것
	...
	def shift_week_for_dt_obj(self, dt_obj, input_no):
		...
		"""
		if not dt_obj:
			dt_obj = self.get_dt_obj_for_now()
	...
	def time_change_sec(self, input_data=""):
		"""
			input_data = "14:06:23"
			"""
		re_compile = re.compile("\d+")
	...
	def time_day(self, time_char=time.localtime(time.time())):
		# 일 -----> '05'
	...
	def time_hour(self, time_char=time.localtime(time.time())):
		# 시 -----> '10'
		return time.strftime('%I', time_char)
	...
	def time_minute(self, time_char=time.localtime(time.time())):
		# 분 -----> ['07']
		return [time.strftime('%M', time_char)]
	...
	def time_month(self, time_char=time.localtime(time.time())):
		"""
		월 -----> '04'
		"""
	...
	def time_sec_change(self, input_data=""):
		"""
			input_data = 123456
			"""
		step_1 = divmod(int(input_data), 60)
	...
	def time_second(self, time_char=time.localtime(time.time())):
		# 초 -----> ['48']
		return [time.strftime('%S', time_char)]
	...
	def time_week(self, time_char=time.localtime(time.time())):
		# 주 -----> ['5', '13', 'Fri', 'Friday']
		return [time.strftime('%w', time_char), time.strftime('%W', time_char), time.strftime('%a', time_char),
				time.strftime('%A', time_char)]
	...
	def time_year(self, time_char=time.localtime(time.time())):
		# 년 -----> ['2002']
		return [time.strftime('%Y', time_char)]
