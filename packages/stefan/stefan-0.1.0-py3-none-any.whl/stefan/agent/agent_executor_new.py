...
from stefan.code_search.llm.llm_logger import global_llm_logger
...
class AgentExecutorNew:
    ...
    def _start_agent_internal(self, params_str: str, context: ExecutionContext) -> AgentResult:
        ...
        while True:
            ...
            success_result, error = LLMExecutor().generate(
                ...
                model=LLM_MODEL.CLAUDE_SONNET_3_5_NEW, 
                messages=messages,
            ...
            if error is not None:
                global_llm_logger.log_agent_finished_with_error(error, context)
            ...
            xml_answer = XMLAnswerParser.parse_answer_xml(response)
            xml_text = xml_answer.xml_text
            answer_dict = xml_answer.answer_dict

            print(f"Answer dict: {answer_dict}")

            # Extract the first key from the answer dict
            answer_key = self._get_first_answer_key(answer_dict)
            
            # Try to extract the agent name from the answer key
    ...
    def _on_new_agent_created(self, params_str: str, messages: List[Dict[str, Any]], context: ExecutionContext) -> None:
        ...
        global_llm_logger.log_agent_started(context)
    ...
    def _on_new_tool_created(self, tool_name: str, args: Dict[str, Any], context: ExecutionContext) -> None:
        ...
        global_llm_logger.log_tool_usage(
            agent_name=self.agent.name,
            tool_name=tool_name,
            execution_context=context,
        )
    ...
    def _on_agent_finished_with_success(self, success_result: LLMSuccessResult, output_action: AgentOutputAction, context: ExecutionContext) -> None:
        ...
        global_llm_logger.log_agent_finished(context)
    ...
    def _on_tool_finished_with_success(self, tool_name: str, result: str, context: ExecutionContext) -> None:
        ...
        global_llm_logger.log_tool_success(tool_name=tool_name, agent_name=self.agent.name, execution_context=context)
    ...
    def _on_tool_finished_with_error(self, tool_name: str, error: Exception, context: ExecutionContext) -> None:
        ...
        global_llm_logger.log_tool_error(tool_name=tool_name, agent_name=self.agent.name, error=error, execution_context=context)

    def _create_failure_tool_response(self, tool_name: str, error: Exception) -> str:
        stack_trace = traceback.format_exc()
        print(f"Tool '{tool_name}' failed with the following error: {str(error)}\n\nStack trace:\n{stack_trace}")
        return f"Tool '{tool_name}' failed with the following error: {str(error)}\n\nStack trace:\n{stack_trace}"
    
    def _create_failure_agent_response(self, agent_name: str, error: Exception) -> AgentResult:
        stack_trace = traceback.format_exc()
        print(f"Agent '{agent_name}' failed with the following error: {str(error)}\n\nStack trace:\n{stack_trace}")
