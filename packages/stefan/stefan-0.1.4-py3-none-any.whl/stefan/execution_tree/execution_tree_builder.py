...
from stefan.code_search.llm.llm_logger import get_global_llm_logger
...
class ExecutionTreeBuilder:
    ...
    def create_agent_node(
        ...
        node_id = uuid4()
        get_global_llm_logger().log_executor(f"Creating agent node {agent_name} - {node_id}")
    ...
    def create_tool_node(
        ...
        get_global_llm_logger().log_executor(f"Creating tool node {tool_name}")
    ...
    def update_last_agent_node_with_success(
        ...
        get_global_llm_logger().log_executor(f"Updating last agent node with success {output_action}")
    ...
    def update_last_agent_node_with_error(
        ...
        get_global_llm_logger().log_executor(f"Updating last agent node with error")
    ...
    def update_last_tool_node_with_success(
        ...
        get_global_llm_logger().log_executor(f"Updating last tool node with success")
    ...
    def update_last_tool_node_with_error(
        ...
        get_global_llm_logger().log_executor(f"Updating last tool node with error")
    ...
    def _save_tree_to_file(self) -> None:
        dir = self.project_context.execution_directory
        execution_id = self.project_context.execution_id

        os.makedirs(dir, exist_ok=True)
        with open(f'{dir}/execution_tree/execution_tree_{execution_id}.json', 'w') as f:
