...
def should_check_version():
    """判断是否需要检查版本"""
    if os.path.exists(conf.last_check_file):
        with open(conf.last_check_file, "r") as f:
            last_check_time = float(f.read().strip())
            logger.info(f"last check time: {last_check_time}")
        current_time = time.time()
        return (current_time - last_check_time) > conf.check_version_interval
    return True  # 文件不存在时，强制检查


def update_last_check_time():
    """更新最后检查时间"""
    with open(conf.last_check_file, "w") as f:
...
def check_version():
    ...
    if should_check_version():
        ...
        current_version = version(conf.app_name)
        latest_version = get_latest_version(conf.app_name)
        ...
        if latest_version and latest_version != current_version:
            ...
            prompt_for_update(conf.app_name, latest_version, current_version)
