...
def step1_data_validation(resource, db, plot_figures=True):
    ...
    for well_id in wls["well_id"].unique():
        for col in cols:
            finite_values = [v for v in wls.loc[wls.well_id == well_id, col].unique() if not pd.isnull(v)]
            if len(finite_values):
                wls.loc[wls.well_id == well_id, col] = wls.loc[wls.well_id == well_id, col].fillna(finite_values[0])
