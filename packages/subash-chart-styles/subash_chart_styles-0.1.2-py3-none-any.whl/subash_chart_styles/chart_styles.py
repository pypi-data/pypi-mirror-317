...
def apply_chart_styles(ax, dates, closing_prices, company_name, start_date, end_date):
    ...
    # Adjust annotations to stay within the grid
    x_offset = timedelta(days=int((dates[-1] - dates[0]).days * 0.05))  # 5% of date range
    y_offset = y_range * 0.1  # 10% of y-axis range as padding

    # Peak annotation (adjusted to not exceed top grid boundary)
    peak_y_position = min(max_value + y_offset, ylim_max - (y_range * 0.05))
    ax.annotate(
        f'$\mathbf{{Peak\ market\ cap:}}$\n{format_large_numbers(max_value, None)}',
        xy=(dates[max_idx], max_value),
        xytext=(dates[max_idx] - x_offset, peak_y_position),
        ha='right', va='bottom', fontsize=10, fontweight='bold', fontname='DejaVu Sans', zorder=6,
        bbox=dict(facecolor='#f5f5f5', edgecolor='#36454F', alpha=0.9, pad=6),
        arrowprops=dict(
            arrowstyle='-|>',
            facecolor='#36454F',
            edgecolor='#36454F',
    ...
    # Lowest annotation (adjusted to not overlap with bottom grid boundary)
    low_y_position = max(min_value - y_offset, ylim_min + (y_range * 0.05))
    ax.annotate(
        f'$\mathbf{{Lowest\ market\ cap:}}$\n{format_large_numbers(min_value, None)}',
        xy=(dates[min_idx], min_value),
        xytext=(dates[min_idx] + x_offset, low_y_position),
        ha='left', va='top', fontsize=10, fontweight='bold', fontname='DejaVu Sans', zorder=6,
        bbox=dict(facecolor='#f5f5f5', edgecolor='#36454F', alpha=0.9, pad=6),
        arrowprops=dict(
            arrowstyle='-|>',
            facecolor='#36454F',
