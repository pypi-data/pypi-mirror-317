...
def parse_cstring_to_graph(cstr: str) -> nx.Graph:
    ...
    Format: node_info:labels where labels are comma-separated
    Example: "4:9c43c661-703f-4fff-ae65-23e25cb7fcf0:255:HorizontalVector,IntersectionPoint,Point,Vector"
    """
    import re
    ...
    for node_entry in node_entries:
        ...
        print(f"Parsing node {node_entry}")
    ...
    if match_edges:
        ...
        edge_entries = edges_part.split("),")
        for e in edge_entries:
            e = e.strip().replace("(", "").replace(")", "")
            if "-" not in e:
                continue
            print(f"Parsing edge {e}")
            u, v = e.split("-")
