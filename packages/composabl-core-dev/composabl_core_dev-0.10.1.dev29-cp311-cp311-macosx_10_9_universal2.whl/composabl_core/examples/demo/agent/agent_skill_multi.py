...
agent_multidiscrete.add_selector_skill(
    selector_skill,
    children=[expert_skill_controller_multi_discrete, random_skill_controller_multi_discrete],
    fixed_order=True,
    repeat=False
)
...
with SkillSelector("target_skill_dictionary", TeacherSpaceDictionary) as ss1:
    with Skill("expert-controller", ControllerExpertDict) as expert_skill_controller_dict:
        ss1.add_child(expert_skill_controller_dict)
        agent_dictionary_api_context_manager.add_skill(expert_skill_controller_dict)

    with Skill("random-controller", ControllerRandomDict) as random_skill_controller_dict:
    ...
    agent_dictionary_api_context_manager.add_selector_skill(ss1, fixed_order=True, repeat=False)
