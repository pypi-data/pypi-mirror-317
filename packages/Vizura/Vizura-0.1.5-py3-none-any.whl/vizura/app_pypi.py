...
def run_dashboard(dfr: pd.DataFrame):
    """
    Launch the Streamlit dashboard with the provided DataFrame.

    Parameters:
        df (pd.DataFrame): The DataFrame to analyze.
    """
    # Save the DataFrame to a temporary file to share it with Streamlit
    temp_file = "data.csv"
    ...
    thread = threading.Thread(target=launch_streamlit, daemon=True)
    thread.start()

    print("\nDashboard is running... Open http://localhost:8501 in your browser.")
...
def main():
    ...
    if os.path.exists(data_file):
        ...
        st.error("No data found. Ensure the DataFrame is passed using the `run_dashboard` function.")
        return
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="Vizor Dashboard", layout="wide")
    main()