...
<body><div id="content">%(body_content)s</div></body>
        ...
        super(MarkdownMail, self).__init__(
            to_addr, from_addr, subject, text_body=content, html_body=html, cc_addr=cc_addr, bcc_addr=bcc_addr
        )

    def send(self, smtp_server, port=25, login=None, password=None, tls=False):
        if issubclass(smtp_server.__class__, NullServer):
            ...
            super(MarkdownMail, self).send(host=smtp_server, port=int(port), login=login, password=password, tls=tls)
