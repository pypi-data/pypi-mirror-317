...
class TestMarkdownMail(unittest.TestCase):
    ...
    def test_carbon_copy_and_blind_carbon_copy(self):
        mail = markdownmail.MarkdownMail(
            from_addr=(u"from@example.com", u"From Example"),
            to_addr=(u"to@example.com", u"To Example"),
            subject=u"SUBJECT",
            content=u"CONTENT",
            cc_addr=u"cc@example.com",
            bcc_addr=u"bcc@example.com",
        )

        self.assertEqual([u"cc@example.com"], mail.cc_addr)
        self.assertEqual([u"bcc@example.com"], mail.bcc_addr)
