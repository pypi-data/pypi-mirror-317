...
from dozersim.results.results import Load
from dozersim.simulation import constraints
...
def evaluate_ball_screw(paths: list[Path], ballscrew: BallScrewSettings, model: Model):
    ...
    path.transform(var_type='rotational', name="Ballscrew input")
    ...
    path.variables.add['flow'](speed_in, model, "GearboxSettings input")
    ...
    path.variables.add['effort'](values=np.array(friction_torque), parent=model, name="GearboxSettings friction")
    path.variables.add['effort'](values=torque_inertia, parent=model, name="Ball screw inertia")
