...
from dozersim.results import results
from scipy.optimize import NonlinearConstraint, minimize, Bounds


def get_non_iterable(parameters: dict[parameters.Parameter, Any]):
    iterable_pars = []
...
def get_iterable(parameters: dict[parameters.Parameter, Any]):
    iterable_pars = []
    for parameter in parameters.keys():
        try:
            iter(parameter)
...
class Simulation:
    ...
    def __init__(self, paths: list[Path] = None):
        ...
        self._analysis: results.Analysis = results.Analysis()
        self._parameters: dict[parameters.Parameter, Any] = {}
        self._objectives: list[objectives.Objective] = []
    ...
    def add_parameter(self, *parameters: parameters.Parameter):
        """ Add a parameter to the latest step """
        for parameter in parameters:
            self._parameters[parameter] = None

    def add_objective(self, *objectives: objectives.Objective):
    ...
    def reset(self):
        ...
        Parameters
        ----------
        load_cases: removes load_cases from simulation if true
        components: resets system from simulation if true
        parameters: resets parameters to default if true
        """
        #
        self._parameters = dict()
        self._objectives = list()
        self._analysis = results.Analysis()
        self._has_run = False

    @property
    def analysis(self) -> results.Analysis:
        return self._analysis
    ...
    def update_parameters(self, pars: tuple[parameters.Parameter] = (), values: tuple = ()):
        for parameter, value in zip(pars, values):
            self._parameters[parameter] = value

    def evaluate(self) -> results.Result:
        """
        ...
        for load_case in self.load_cases:
            ...
            par_list = [results.ParameterValue(value=val, parent=par) for par, val in self._parameters.items()]
...
class Optimizer:
    ...
    def __init__(self, eval_func: Callable, update_fun: Callable, pars: tuple[parameters.SettingsParameter]):
