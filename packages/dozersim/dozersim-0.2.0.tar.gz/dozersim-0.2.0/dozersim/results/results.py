...
from dozersim.simulation import parameters, constraints, objectives
...
class Result:
    ...
    def add_result_objects(self, load_case: str | None, variable_list: list[variables.Variables] = None, result_objs: list[ResultObject] = None):
    ...
    def get_variables(self, load_case: str = None, path: Path = None) -> list[variables.Variables]:
    ...
    def get_result_table(self, load_case: str = None, path: Path = None) -> (list[str], list[list[str]]):
...
def get_non_iterable(parameters: list[parameters.Parameter]):
    return tuple(filter(lambda parameter: not isinstance(parameter, Iterable), parameters))


def get_iterable(parameters: list[parameters.Parameter]):
...
class Analysis:
    ...
    def get_result_values(self, load_case: str,
                          target_obj: parameters.Parameter | objectives.Objective) -> list:
        values = []
    ...
    def get_fittest_set(self, load_case: str, objective: objectives.Objective) -> int:
...
class ConstraintValue(ResultObject):
    ...
    def __init__(self, path: Path, parent: constraints.Constraint, model: Model, value: float,
...
class ObjectiveValue(ResultObject):
    ...
    def __init__(self, path: Path, parent: objectives.Objective, unit: str, value: float):
        super().__init__(path=path)
        self.parent = parent
        self._value = value
        self.unit: str = ''
...
class ParameterValue(ResultObject):
    ...
    def __init__(self, parent: parameters.Parameter, value: float, path: Path = None):
        super().__init__(path=path)
        self.parent = parent
        self._value = value
        self.unit: str = ''
    ...
    @property
    def value(self):
        if type(self.parent) is parameters.DatabaseLink:
