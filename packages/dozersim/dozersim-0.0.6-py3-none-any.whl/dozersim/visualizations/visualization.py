...
class AnalysisWindows(QTabWidget):
    ...
    def __init__(self, analysis: Analysis, parent=None):
        ...
        self.addTab(MyTable(headers=headers, data=data), 'Results sets')

    def plot_variables(self, idx: int = -1):
        result_idx = self._analysis.results[idx]

        for load_case in result_idx.load_cases:
            tabs_level2 = QTabWidget()
            for path in result_idx.paths:
                if path is not None:
                    variables = result_idx.get_variables(load_case, path)
                    tabs_level3 = QTabWidget()
                    for var in variables:
                        tabs_level3.addTab(MyPlot(fig=get_variable_plot(var)), var.name)
                    tabs_level2.addTab(tabs_level3, path.name)
            headers, data = result_idx.get_result_table(load_case=load_case)
            tabs_level2.addTab(MyTable(headers=headers, data=data), 'Results')
            ...
            slider.orientation = Qt.Orientation.Horizontal
            bar = QWidget()
            label = QLabel()
            label.setText('set 1 of ...')
            bar_layout = QHBoxLayout()
            bar_layout.addWidget(label)
            bar_layout.addWidget(slider)
            bar.setLayout(bar_layout)
            layout.addWidget(bar)
            layout.addWidget(tabs_level2)

            self.addTab(var_tab, load_case)
