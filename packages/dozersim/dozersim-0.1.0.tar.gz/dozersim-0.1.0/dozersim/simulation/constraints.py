...
class Constraint(ABC):
    ...
    def get_bound_kws(self) -> dict:
        bounds = dict()
        for kw in {'ub', 'lb'}:
            if isinstance(self.__getattribute__(kw), str):
                try:
                    bound_val = self._settings.get_setting(self.__getattribute__(kw))
...
class PathConstraint(Constraint):
    ...
    def evaluate(self, model: Model, path: Path, value: float = None):
        ...
        kws = self.get_bound_kws()
        path.add_result_object(
            results.ConstraintValue(parent=self, model=model, value=value, path=path, **kws))
