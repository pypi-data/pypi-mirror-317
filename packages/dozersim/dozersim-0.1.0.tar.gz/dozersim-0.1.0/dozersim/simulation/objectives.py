...
from dozersim.simulation.utils import target_dict, statistic_dict
...
class SettingsObjective(Objective):
    ...
    def evaluate(self, paths):
        ...
        if isinstance(self._models, list):
            models = self._models
        elif isinstance(self._models, dozersim.modelling.Model):
            models = [self._models]
        else:
        ...
        paths[0].add_result_object(results.ObjectiveValue(parent=self, unit='', value=cost_value, path=paths[0]))
...
class ConstraintObjective(Objective):
    ...
    def evaluate(self, paths: tuple[Path] = ()):
        for path in paths:
            const_objs: list[results.ConstraintValue] = path.get_result_object(results.ConstraintValue)
            for const_obj in const_objs:
                if const_obj.model == self._model and const_obj.parent.name == self._target:
                    ...
                    path.add_result_object(
                        results.ObjectiveValue(parent=self, unit=const_obj.unit, value=cost_value, path=path))
...
class PathObjective(Objective):
    ...
    def evaluate(self, paths: tuple[Path]):
        ...
        path.add_result_object(
            results.ObjectiveValue(parent=self, unit=self.target.units(path), value=cost_value, path=path))
