...
class AlgoBaseModelScada(object):
    ...
    def __init__(self, data_date_range='D', alarm_date_range='D', is_to_db=False, db_info=None, dir_root='./', ftp_info=None, read_old=True,
    ...
    def get_args(self, data_date_range='D', alarm_date_range='D'):
        """
        手动指定或者自动获取运行日期参数
        ...
        if alarm_date_range.endswith('D'):
            match = re.match("(\d+)", alarm_date_range)
            num = match.group(0) if match else 1
            alarm_start_date_default = today - datetime.timedelta(days=int(num))
        elif alarm_date_range == 'W':
    ...
    def run(self):
        farm_codes, turbines_lis, start_date, end_date, alarm_start_date = self.get_args(data_date_range=self.data_date_range,
                                                                                         alarm_date_range=self.alarm_date_range)
