    ...
    def setBackground(self, *args):
        if self.weather != "":
            ...
            if self.checkTime():
                ...
                self.canvas.before.add(Rectangle(source="snow.jpg", size = self.size, pos = self.pos))
            else:
                self.canvas.before.add(Rectangle(source="default.jpg", size = self.size, pos = self.pos))
        else:
            self.canvas.before.add(Rectangle(source="default.jpg", size = self.size, pos = self.pos))
