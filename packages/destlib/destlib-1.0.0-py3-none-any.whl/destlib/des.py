...
class DES:
    ...
    def plot(
        ...
        if v_lines:
            plt.vlines(
                self.df["arrival_time"].values, ymin=0, ymax=state.max(), color=arrival_color, linestyle=':', label='Arrival'
                )
            plt.vlines(
                self.df["end_time"].values, ymin=0, ymax=state.max(), color=departure_color, linestyle=':', label='Departure'
                )
