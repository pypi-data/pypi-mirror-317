import copy
from typing import Type, Callable, List, Dict, Tuple, Any, Self, Optional, Union, Iterator
import functools
import inspect
import random
import re
...
class DST:
    ...
    def set_sim_class(self, sim_class: Type) -> Self:
        self._sim_class = sim_class
    ...
    def set_behaviors(self, behaviors: List[Callable]) -> Self:
        self._behaviors = behaviors
        return self

    def set_behaviors_calls(self, behaviors_calls: int) -> Self:
        self._behaviors_calls = behaviors_calls
        return self

    def set_weights(self, weights: List[int]) -> Self:
        self._weights = weights
        return self

    def set_args_ranges(self, **args_ranges: Dict[str, Tuple[int, int]]) -> Self:
        self._args_ranges = args_ranges
        return self

    def set_seed(self, random_seed: int) -> Self:
        self._random_seed = random_seed
        return self

    def set_instant_number(self, instant_number: int) -> Self:
