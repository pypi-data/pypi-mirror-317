...
def make_application(
    ...
    async def proxy(
        ...
        try:
            if alist_token and method == "GET" and path.startswith(("p/", "dav/")):
                path = path[path.find("/"):]
                storage = await storage_of(client, path)
                if storage and storage["driver"] == "WebDav":
                    ...
                    while True:
                        http_resp = await session.send(
                            request=session.build_request(method="GET", url=url), 
                        ...
                        if http_resp.status_code in (301, 302, 303, 307, 308):
                            ...
                            if not url.startswith(("http://", "https://")) or app.can_redirect(url):
