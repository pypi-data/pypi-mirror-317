...
# Kept for backwards compatibility with importmod
@deprecated(
    version="2.6", reason="Replaced by LocalRepo. Will be removed in portmod 2.8"
)
class Repo(LocalRepo): ...
...
class RemoteRepo(BaseRepo):
    def __init__(
        ...
        arch: AbstractSet[str] = frozenset(),
    ):
        super().__init__(name, sync_type, sync_uri, priority)
...
def get_repo(name: str) -> LocalRepo:
