...
module-update-help = Runs module updates.
cfg-update-help = Interactively applies pending updates to protected config files.

## cfg-update messages

cfg-update-nothing-to-do = No files need updating
# $index (int) - The index of the file being updated
# $total (int) - The total number of updates
cfg-update-file-index = Update { $index } of { $total }
# $file (String) - The file being modified
apply-above-change-qn = Would you like to apply the above change to { $file }?
update-file-prompt = Which file would you like to update?
# $file (String) File being updated
# $num (int) number of pending updates
pending-updates-for-file = { $file } ({ $num } pending updates)

# $total_updates (int) - The total number of file changes pending
# $total_files (int) - The total number of files with changes pending
# $command (String) - The command which can be used to interactively apply the updates
pending-cfg-updates = { $total_updates } pending updates to { $total_files } files. Updates can be applied using the '{ $command }' command"
...
# $num (Integer) number of packages which were merged
# $atom (String) Package which failed
...
query-invert-help = If set, returns packages which do not match the query
...
mergetool-info =
    Note: A custom mergetool can be configured using the { $var } config variable which will add an option below to manually merge the changes
...
remote-size-mismatch = Expected size is { $our_size }, but download server reports a size of { $remote_size }!"
remote-hash-mismatch = Local { $alg } hash is { $hash1 }, but remote hash is { $hash2 }!"
# $filename (String) - Name of the source file being checked
# $hash (String) - Algorithm used for hashing the file
# $hash1 (String) - Expected hash value
# $hash2 (String) - Actual hash value
local-hash-mismatch = { $filename } should have { $hash } of "{ $hash1 }", but instead it is "{ $hash2 }"
# $filename (String) - Name of the source file being checked
# $expected (String) - Expected file size
# $actual (String) - Actual file size
local-size-mismatch = { $filename } should have a size of { $expected } bytes, but instead its size is { $actual }.
possible-local-hash-mismatch = Filename "{ $filename }" matches source name "{ $name }" but it doesn't match the data recorded in the Manifest
...
profile-debug-help = Show debug information about the current profile
profile-available = Available profile symlink targets ({ $repo }):
profile-current-name = Current profile name ({ $path }):
...
# $path (String) The path of the pybuild file being run
# $phase (String) The phase function being run when the error was encountered
sandboxed-error-pybuild  = The { $phase } phase of the pybuild { $path } failed.
...
# $path (String) The path of the module file being run
# $function (String) The function being run
sandboxed-error-module = Executing the { $function } function of module { $path } failed.
    Do not report this error message by itself. There will be another error message from the sandboxed code for the actual problem, probably directly above this one.
...
mirror-repo-help = Repository with the files to mirror. Mirroring will be limited to this repository and its masters.
...
init-world-update =
    Installing the mandatory system packages by running a world update...
        portmod { $prefix } merge --update --deep @world
    This command is what you should also use to update all packages.
    '@world' refers to the world set, the set of all system and explicitly installed packages.
    See 'portmod { $prefix } merge --help' for details on the other arguments
...
destroy-exclude-prompt = Please select any you wish to exclude from deletion
...
acquiring-exclusive = Waiting for exclusive access to the { $prefix } prefix
