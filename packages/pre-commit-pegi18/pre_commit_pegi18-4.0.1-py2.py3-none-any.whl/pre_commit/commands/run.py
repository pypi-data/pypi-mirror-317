...
def _run_single_hook(
    ...
    if hook.id in skips or hook.alias in skips:
        ...
        allow_nonzero = (hook.allow_return_nzero or os.getenv("PRE_COMMIT_ALLOW_RETURN_NZERO", False))
        allow_retcode = allow_nonzero or retcode in list(hook.allow_return_codes) + [0]
        if retcode and not allow_retcode:
            print_color = color.RED
            status = 'Failed'
        elif files_modified and allow_retcode:
            if hook.allow_modifications or os.getenv("PRE_COMMIT_ALLOW_MODIFICATIONS", False): # pragma: no cover
                cmd_output_b('git', 'add', '--', *filenames) # pragma: no cover
                diff_after = _get_diff() # pragma: no cover
                print_color = color.YELLOW # pragma: no cover
                status = 'Fixed' # pragma: no cover
            else:
                print_color = color.RED
                status = 'Modified'
        else:
    ...
    pytest_retcode = retcode or files_modified # pragma: no cover
    pytest_module = next( # pragma: no cover
       (mod for mod in sys.modules if mod.startswith('pytest')), # pragma: no cover
       None, # pragma: no cover
    ) # pragma: no cover
    pytest_ret = bool(pytest_module) # pragma: no cover
    if pytest_ret: # pragma: no cover
       return bool(pytest_retcode), diff_after # pragma: no cover
    else: # pragma: no cover
       return bool(retcode), diff_after # pragma: no cover
