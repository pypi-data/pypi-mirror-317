...
class UmikService(NoiseSensorService):
    ...
    def __init__(self) -> None:
        ...
        self.buffer_pressures: Deque = deque(maxlen=11)  # Буфер для вычисления la_max
        self.current_time: float = 0.0
        self.old_time: Optional[float] = None
        self.filter_params = self.calculator.get_filter()
        self.correction_parameter = self.system.get_correction_param() # Получаем коректировочную константу с команды запуска скрипта
    ...
    def run(self):
        ...
        try:
            if not self.correction_parameter:
    ...
    def get_la_max(self, pressures: np.ndarray) -> float:
        ...
        """
        la_max = round(
            self.calculator.time_weighting(p_series=pressures) - self.correction_parameter + 94,
            1,
        )
        return la_max
