...
class MlflowTag(str, Enum):
    ...
    # Custom tags
    MLFACTORY_RUN_TYPE = "harbory.workflow.run_type"
    MLFACTORY_STEP_FINGERPRINT = "harbory.workflow.step.fingerprint"
    MLFACTORY_STEP_CACHE_STATUS = "harbory.workflow.step.cache_status"
...
def download_mlflow_artifacts(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for path in download_path.glob("*"):
            shutil.move(path, Path(directory) / path.name)
...
def get_mlflow_tags_from_run(run: MlflowRun) -> dict[MlflowTag, str]:
    return {MlflowTag(tag): value for tag, value in run.data.tags.items() if tag in MlflowTag.__members__}
...
def get_execution_state_from_run(run: MlflowRun) -> WorkflowExecutionState:
    ...
    status = MlflowRunStatus(tags[MlflowTag.MLFLOW_RUN_NAME]).to_execution_status()
