...
from gisdo import constants as const
from gisdo import xml_helpers
from gisdo.exceptions import NoDataInMo
from gisdo.index import ApplicationIndex
from gisdo.index import get_enrolled_age_filter
from gisdo.index import get_queue_index_collection
from gisdo.index.direct import ind_19_3
from gisdo.index.direct import DirectIndex
from gisdo.algorithm.direct import get_children_last_direct_ids
from gisdo.algorithm.direct import get_32_filter
from gisdo.algorithm.direct import get_321_filter
from gisdo.algorithm.direct import get_322_filter
from gisdo.algorithm.enrolled import get_22_index
from gisdo.algorithm.enrolled import get_22_8_x_index
from gisdo.algorithm.enrolled import get_22_x_index
from gisdo.algorithm.enrolled import get_26_index
from gisdo.algorithm.enrolled import get_27_index
from gisdo.algorithm.enrolled import get_28_index
from gisdo.algorithm.enrolled import get_30_index
from gisdo.algorithm.enrolled import get_30_1_index
from gisdo.algorithm.enrolled import get_30_2_index
from gisdo.alchemy_session import session_scope

from gisdo.index.direct import ind_29_1
from gisdo.index.direct import ind_29_2
from gisdo.index.direct import ind_29_3
from gisdo.index.capacities import childrens_in_age_category
from gisdo.index.enrolled import Index
from gisdo.index.enrolled import get_age_for_date_filter
from gisdo.models import ScheduleSettings
from gisdo.service import FilialGroupData
from gisdo.service import TemplateView
from gisdo.service import GroupData
from gisdo.service import ParentGroupData
from gisdo.xml_helpers import get_detailed_passport
from gisdo.utils import DateMixin
from gisdo.utils import get_ordered_dict_of_dicts
from gisdo.utils import set_empty_list_values
...
from .helpers import get_count_data
from ..algorithm.utils import HealthNeedIdsProvider

from ..collector import DataCollector
from .constants import *
from .helpers import prep_ind_19_3_queries
from .helpers import prep_ind_29_1_queries
from .helpers import prepare_direct_index_queries
from .helpers import prep_ind_29_2_query
from .helpers import prep_in_29_2_data
from .helpers import prep_ind_29_3_query
from .helpers import prep_directs_in_group_queries
from .helpers import prepare_enrolled_index_queries
from .helpers import prepare_31_x_index
from .helpers import prep_ind_enrolled_data
from .helpers import ReportQuery
from ..constants import AGE_CATEGORIES_EIGHT
from ..index import get_queue_index
from ..index.queue import get_age_filter
from ..index.queue import get_average_waiting_time
from ..index.queue import get_count
...
class ReportDataCollector(DataCollector):
    ...
    def collect(self, report_form=None):
        ...
        if not have_data:
            raise NoDataInMo(
                f'Нет садов в МО {self._mo} пригодных для отправки'
            )
        ...
        if self.has_selected_index(XML_TAGS):
            (xml_tags_data, xml_tags_data_by_group,
             xml_tags_data_by_group_only_values) = self._calculate_xml_tags(report_form)
    ...
    def _merge_dicts(first_dict, second_dict):
        ...
        for index, first_index_dict in first_dict[first_dou_id].items():
            ...
            age_categories = set(first_index_dict.keys()) | set(
                second_index_dict.keys())
            ...
            for age_cat in age_categories:
                ...
                if age_cat in first_index_dict:
                    if (
                            isinstance(first_index_dict[age_cat], dict)
    ...
    def get_enrolled_index_data(self, _unit, report_start_date):
        ...
        parent_group_data = ParentGroupData(
            _unit, report_start_date, filials,
            prep_enrollments_func=prep_ind_enrolled_data
        )

        # головная организация передается только если у нее есть группы
        if xml_helpers.has_fact_groups(_unit):
            for parent_group in Group.objects.filter(
                    status=GroupStatusEnum.FACT, unit=_unit).order_by('id'):
        ...
        for filial in (fd.filial for fd in filials.iterator()):
            ...
            if xml_helpers.has_fact_groups(filial):
                ...
                filial_group_data = FilialGroupData(
                    filial, parent_group_data, report_start_date,
                    prep_enrollments_func=prep_ind_enrolled_data
                )

                for filial_group in Group.objects.filter(
                        status=GroupStatusEnum.FACT, unit=_unit).order_by('id'):
    ...
    def _collect_enrolled_data(self, dou):
        ...
        def _inner(_dou, report_start_date, calc_filial=False):
            enrolled_indexes = (
                INDEX_ENROLLED, INDEX_7, INDEX_7_1, INDEX_7_2,
                INDEX_7_3, INDEX_7_4, INDEX_7_5, INDEX_7_6, INDEX_7_7,
                INDEX_8, INDEX_8_1, INDEX_8_2, INDEX_8_3,
                INDEX_9, INDEX_9_1, INDEX_10, INDEX_10_1, INDEX_11,
                INDEX_12, INDEX_12_1,
                INDEX_13, INDEX_13_1, INDEX_14, INDEX_15, INDEX_16,
                INDEX_17, INDEX_17_1,
                INDEX_18, INDEX_18_1, INDEX_18_2, INDEX_18_3,
                INDEX_18_4, INDEX_18_5,
                INDEX_19, INDEX_19_1, INDEX_19_2, INDEX_19_3,
                INDEX_20, INDEX_20_1, INDEX_20_2, INDEX_20_3, INDEX_20_4,
                INDEX_20_5, INDEX_20_6, INDEX_20_7, INDEX_20_8,
                INDEX_21, INDEX_22, INDEX_22_1, INDEX_22_1_1,
                INDEX_22_1_2, INDEX_22_2, INDEX_22_3, INDEX_22_3_1,
                INDEX_22_3_2, INDEX_22_4, INDEX_22_5, INDEX_22_5_1,
                INDEX_22_5_2, INDEX_22_6, INDEX_22_7,
                INDEX_22_8, INDEX_22_8_1, INDEX_22_8_2,
                INDEX_23, INDEX_24, INDEX_25, INDEX_26, INDEX_27,
                INDEX_28, INDEX_29, INDEX_29_1, INDEX_29_2, INDEX_29_3,
                INDEX_30, INDEX_30_1, INDEX_30_2,
                INDEX_31, INDEX_32, INDEX_32_1,
            )
            enrolled = {_dou.id: get_ordered_dict_of_dicts(enrolled_indexes)}
            dou_enrolled = enrolled[_dou.id]

            def chain_dou_enrolled(
                    _indexes_22, _on_date_enrolled_filter, *indexes):
                """Объединение различных показателей для dou_enrolled."""
                return list(
                    chain.from_iterable(
                        self.get_enrolled_data(
                            _indexes_22[_ind], _on_date_enrolled_filter)
                        for _ind in indexes
                    )
                )

            if INDEX_ENROLLED in self.indexes:
                ind_enrolled_data = self.get_enrolled_index_data(
            ...
            if INDEX_29_1 in self.indexes:
                index_29_1 = ind_29_1(_dou, self._report_start_date,
                                      prep_func=prep_ind_29_1_queries)
            ...
            for index in QUEUE_INDEXES:
                if index in self.indexes:
                    ...
                    queue_indexes[index] = get_queue_index_collection(
                        index_param, _dou, report=True, **params)

            if INDEX_10 in self.indexes or INDEX_9 in self.indexes or (
                    INDEX_9_1 in self.indexes):
            ...
            if INDEX_11 in self.indexes or INDEX_9 in self.indexes or (
                    INDEX_9_1 in self.indexes):
            ...
            if not calc_filial:
                ...
                if INDEX_21 in self.indexes:
                    index_21 = list(prepared_enrolled_index(
                        group_filter=lambda group: (
                                group.type and
                                group.type.code == GroupTypeEnumerate.FAMILY
                        )))

                # Формирование множества с показателям для расчёта (22.х).
                # Связано с тем, что могут быть выбраны не все показатели
                calc_indexes_22 = {
                    i for i in const.SIMPLE_INDEXES_22 if i in self.indexes
                }
                ...
                for ind in calc_indexes_22:
                    ...
                    if ind == INDEX_22_8_1:
                        indexes_22[ind] = get_22_8_x_index(
                            prepared_enrolled_index, hn)
                    else:
                        indexes_22[ind] = get_22_x_index(
                            prepared_enrolled_index, hn)

                if INDEX_23 in self.indexes:
                    index_23 = list(
                        prepared_enrolled_index(group_filter=lambda group: (
                            group.type and
                            group.type.code == GroupTypeEnumerate.COMP
                        ))
                    )

                if INDEX_24 in self.indexes:
                    index_24 = list(prepared_enrolled_index(
                        group_filter=lambda group: (
                            group.type and
                            group.type.code == GroupTypeEnumerate.HEALTH
                        )
                    ))
                ...
                if INDEX_25 in self.indexes:
                    index_25 = list(
                        prepared_enrolled_index(group_filter=lambda group: (
                            group.type and
                            (group.type.code == GroupTypeEnumerate.CARE or
                             group.type.code == GroupTypeEnumerate.YOUNG)
                        ))
                    )
            ...
            for age_cat in list(const.AGE_CATEGORIES_FULL.values()):
                ...
                current_year_enrolled_filter = get_enrolled_age_filter(
                    age_cat, DateMixin.get_current_calendar_year())
                on_date_enrolled_filter = get_enrolled_age_filter(
                    age_cat, datetime.date.today)
                next_calendar_year_enrolled = get_enrolled_age_filter(
                    age_cat, DateMixin.get_next_calendar_year)
                ...
                next_learn_year_first_day = get_age_filter(
                    age_cat,
                ...
                if not calc_filial:
                    if INDEX_19 in self.indexes:
                        ind_19_data = self.get_enrolled_data(
                            index_19, current_year_enrolled_filter)
                        if ind_19_data:
                            dou_enrolled[INDEX_19][age_cat] = ind_19_data

                    if INDEX_19_1 in self.indexes:
                        ind_19_1_data = self.get_enrolled_data(
                            index_19, on_date_enrolled_filter)
                        if ind_19_1_data:
                            dou_enrolled[INDEX_19_1][age_cat] = ind_19_1_data

                    if INDEX_19_2 in self.indexes:
                        ind_19_2_data = self.get_enrolled_data(
                            index_19, next_calendar_year_enrolled)
                        if ind_19_2_data:
                            dou_enrolled[INDEX_19_2][age_cat] = ind_19_2_data

                if INDEX_19_3 in self.indexes:
                    ind_19_3_children_ids = ind_19_3(
                        _dou, report_start_date, age_range=age_cat,
                        prep_direct_func=prep_ind_19_3_queries)
                    ind_19_3_data = ReportQuery.get_child_query(
                        Children.objects.filter(id__in=ind_19_3_children_ids))
                    if ind_19_3_data:
                ...
                if not calc_filial:
                    if INDEX_21 in self.indexes:
                        dou_enrolled[INDEX_21][age_cat] = self.get_enrolled_data(
                            index_21, on_date_enrolled_filter)

                    # Подсчёт показателей 22.х
                    for ind in const.SIMPLE_INDEXES_22:
                        if ind not in self.indexes:
                            continue
                        dou_enrolled[ind][age_cat] = self.get_enrolled_data(
                            indexes_22[ind], on_date_enrolled_filter)
                    ...
                    set_empty_list_values(
                        dou_enrolled,
                        (INDEX_21, *const.SIMPLE_INDEXES_22,
                         *list(const.SUM_INDEXES_22.keys())),
                        age_cat,
                    )
                ...
                if not calc_filial:
                    if INDEX_23 in self.indexes:
                        dou_enrolled[INDEX_23][age_cat] = self.get_enrolled_data(
                            index_23, on_date_enrolled_filter)
                    if INDEX_24 in self.indexes:
                        dou_enrolled[INDEX_24][age_cat] = self.get_enrolled_data(
                            index_24, on_date_enrolled_filter)
                    if INDEX_25 in self.indexes:
                        dou_enrolled[INDEX_25][age_cat] = self.get_enrolled_data(
                            index_25, on_date_enrolled_filter)

                    if INDEX_26 in self.indexes:
                        # Общая численность детей, зачисленных в
                        # группы круглосуточного пребывания детей
                        dou_enrolled[INDEX_26][age_cat] = self.get_enrolled_data(
                            index_26, on_date_enrolled_filter)

                    if INDEX_27 in self.indexes:
                        # Общая численность детей, зачисленных в группы
                        # кратковременного пребывания детей
                        dou_enrolled[INDEX_27][age_cat] = self.get_enrolled_data(
                            index_27, on_date_enrolled_filter)

                    # Общая численность детей, посещающих ДОО в режиме
                    # кратковременного пребывания
                    if INDEX_28 in self.indexes:
                        dou_enrolled[INDEX_28][age_cat] = self.get_enrolled_data(
                            index_28, on_date_enrolled_filter)
                else:
                    set_empty_list_values(
                        dou_enrolled,
                        (INDEX_23, INDEX_24, INDEX_25, INDEX_26,
                         INDEX_27, INDEX_28),
                        age_cat
                    )
                ...
                if not calc_filial:
                    if INDEX_30 in self.indexes:
                        dou_enrolled[INDEX_30][age_cat] = self.get_enrolled_data(
                            index_30, on_date_enrolled_filter)

                    if INDEX_30_1 in self.indexes:
                        dou_enrolled[INDEX_30_1][age_cat] = self.get_enrolled_data(
                            index_30_1, on_date_enrolled_filter)

                    if INDEX_30_2 in self.indexes:
                        dou_enrolled[INDEX_30_2][age_cat] = self.get_enrolled_data(
                            index_30_2, on_date_enrolled_filter)
                else:
                    set_empty_list_values(
                        dou_enrolled,
                        (INDEX_30, INDEX_30_1, INDEX_30_2),
                ...
                if INDEX_29_1 in self.indexes:
                    ind_29_1_data = self.get_enrolled_data(
                        index_29_1, on_date_enrolled_filter)
                    if ind_29_1_data:
                        dou_enrolled[INDEX_29_1][age_cat] = ind_29_1_data

                if INDEX_29_2 in self.indexes:
                    index_29_2_data = self.get_enrolled_data(
                        index_29_2, on_other_date_age_filter)
                    if index_29_2_data:
                        dou_enrolled[INDEX_29_2][age_cat] = prep_in_29_2_data(
                            index_29_2_data)

                if INDEX_29_3 in self.indexes:
                    index_29_3_data = self.get_enrolled_data(
                        index_29_3, next_calendar_year_enrolled)
                    if index_29_3_data:
                ...
                if INDEX_10_1 in self.indexes:
                    dou_enrolled[INDEX_10_1][age_cat] = get_count_data(
                        index_10_1, on_date_filter)
                ...
                if INDEX_9_1 in self.indexes:
                    ...
                    if index_9_1_value:
                        dou_enrolled[INDEX_9_1][age_cat] = [(
                            '', '', index_9_1_value, '', '')]
        ...
        for filial in self._unit_helper.get_filial(dou).iterator():
            filial_enrolled = _inner(
                filial, self._report_start_date, calc_filial=True)
    ...
    def _collect_8_queue(self, dou):
        ...
        for index in set(QUEUE_INDEXES_ONLY_VALUES) & set(self.indexes):
            queue_data = get_queue_index_collection(
                index, dou, report=True
            )
            for age_cat in AGE_CATEGORIES_EIGHT.values():
                queue_8[index][age_cat] = get_average_waiting_time(
                    queue_data, age_cat
                )
    ...
    def _collect_capacities(self, dou):
        ...
        def _inner(_dou, report_start_date, calc_filial=True):
            ...
            if calc_filial and any(set(INDEXES_31_x) & set(self.indexes)):
                common, short_stay, with_hn, compensating, health_group = (
                    self.get_free_spaces_by_group(
                        _dou, report_start_date, prep_function=(
                            prepare_31_x_index)
                    )
                )
                ...
                for age_cat in list(const.AGE_CATEGORIES_EIGHT.values()):
                    if calc_filial:
                        dou_capacities[INDEX_31][age_cat] = (
                            childrens_in_age_category(common, age_cat))
                    else:
                        dou_capacities[INDEX_31][age_cat] = []
                ...
                for age_cat in list(const.AGE_CATEGORIES_EIGHT.values()):
                    if calc_filial:
                        dou_capacities[INDEX_31_1][age_cat] = (
                            childrens_in_age_category(with_hn, age_cat))
                    else:
                        dou_capacities[INDEX_31_1][age_cat] = []
                ...
                for age_cat in list(const.AGE_CATEGORIES_EIGHT.values()):
                    if calc_filial:
                        dou_capacities[INDEX_31_2][age_cat] = (
                            childrens_in_age_category(compensating, age_cat))
                    else:
                        dou_capacities[INDEX_31_2][age_cat] = []
                ...
                for age_cat in list(const.AGE_CATEGORIES_EIGHT.values()):
                    if calc_filial:
                        dou_capacities[INDEX_31_3][age_cat] = (
                            childrens_in_age_category(health_group, age_cat))
                    else:
                        dou_capacities[INDEX_31_3][age_cat] = []
                ...
                for age_cat in list(const.AGE_CATEGORIES_EIGHT.values()):
                    if calc_filial:
                        dou_capacities[INDEX_31_4][age_cat] = (
                            childrens_in_age_category(short_stay, age_cat))
                    else:
                        dou_capacities[INDEX_31_4][age_cat] = []
            ...
            if INDEX_32 in self.indexes:
                index_32_data = prepare_direct_index_queries(
                    *direct_index.get_queries(*self.list_last_id_directs_32))
                if index_32_data:
        ...
        for filial in self._unit_helper.get_filial(dou).iterator():
            filial_capacities = _inner(
                filial, self._report_start_date, calc_filial=False)
    ...
    def _calculate_xml_tags(self, report_form=None):
        ...
        for mo in unit.get_descendants(
                include_self=True).filter(kind__id=UnitKind.MO):
            ...
            (mo_data, mo_data_by_groups,
             mo_data_by_groups_only_values) = (
                self._get_unit_data_from_template_view(document))
    ...
    def _get_unit_data_from_template_view(self, document):
        ...
        for dou_data in document.units:
            ...
            if INDEX_PASSPORT in self.indexes:
                result_dict[INDEX_PASSPORT][unit_id] = (
                    get_detailed_passport(ext_unit))
                ...
                for group_data in building['groups']:
                    for tag, param_name in XML_GROUP_TAGS_MAP:
                        if tag in self.indexes and param_name in group_data:
                            ...
                            result_dict_by_groups[tag][unit_id][age_cat].extend(
                                group_data[param_name]
                            )
        ...
        return (result_dict, result_dict_by_groups,
                result_dict_by_groups_only_values)
        ...
        def _inner(_dou):
            ...
            with session_scope() as session:
                ...
                for age_category in list(const.AGE_CATEGORIES_FULL.values()):
                    ...
                    on_date_filter = get_age_filter(
                        age_category, datetime.date.today)
                    ...
                    if INDEX_1_1 in self.indexes:
                        applications[INDEX_1_1][age_category] = (
                            index_2_value + index_3_value
                        )
                    if INDEX_4 in self.indexes:
                        applications[INDEX_4][age_category] = get_count(
                            index_4, on_date_filter)
                    if INDEX_4_1 in self.indexes:
                        applications[INDEX_4_1][age_category] = get_count(
                            index_4_1, on_date_filter)
                    if INDEX_4_2 in self.indexes:
                        applications[INDEX_4_2][age_category] = get_count(
                            index_4_2, on_date_filter)

                    if INDEX_5 in self.indexes:
                        applications[INDEX_5][age_category] = (
                            get_count(index_5, on_date_filter))
                ...
                if INDEX_1 in self.indexes:
                    applications[INDEX_1][const.ALL] = get_queue_index(
                        INDEX_1, _dou, distinct_children=True)
