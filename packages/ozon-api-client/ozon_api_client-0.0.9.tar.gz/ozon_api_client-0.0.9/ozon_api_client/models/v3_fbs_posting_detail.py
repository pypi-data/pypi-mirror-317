...
class V3FbsPostingDetail(object):
    ...
    def __init__(self, additional_data=None, addressee=None, analytics_data=None, barcodes=None, cancellation=None, courier=None, customer=None, delivering_date=None, delivery_method=None, delivery_price=None, financial_data=None, in_process_at=None, is_express=None, is_multibox=None, multi_box_qty=None, order_id=None, order_number=None, parent_posting_number=None, posting_number=None, product_exemplars=None, products=None, provider_status=None, prr_option=None, related_postings=None, requirements=None, shipment_date=None, status=None, substatus=None, tpl_integration_type=None, tracking_number=None):  # noqa: E501
    ...
    def status(self):
        ...
        Статус отправления: - `acceptance_in_progress` — идёт приёмка, - `arbitration` — арбитраж, - `awaiting_approve` — ожидает подтверждения, - `awaiting_deliver` — ожидает отгрузки, - `awaiting_packaging` — ожидает упаковки, - `awaiting_registration` — ожидает регистрации, - `awaiting_verification` — создано, - `cancelled` — отменено, - `cancelled_from_split_pending` — отменено, - `client_arbitration` — клиентский арбитраж доставки, - `delivered` — доставлено, - `delivering` — доставляется, - `driver_pickup` — у водителя, - `not_accepted` — не принят на сортировочном центре, - `sent_by_seller` – отправлено продавцом.   # noqa: E501
