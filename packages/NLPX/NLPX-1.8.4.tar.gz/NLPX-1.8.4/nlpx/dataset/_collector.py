...
from nlpx.tokenize import BaseTokenizer, TokenEmbedding, utils
...
class TextVecCollator:
	...
	def __call__(self, examples):
		...
		if isinstance(self.tokenize_vec, TokenizeVec):
			max_length = utils.get_texts_max_length(texts, cut_type='char') + 2
			...
			max_length = utils.get_texts_max_length(texts, cut_type=self.tokenize_vec.cut_type,
...
class TokenizeCollator:
	...
	def __call__(self, examples):
		...
		if isinstance(self.tokenizer, BaseTokenizer):
			max_length = utils.get_texts_max_length(texts, cut_type=self.tokenizer.cut_type, lang=self.tokenizer.lang,
