...
class Evrim(SyncAPIClient):
    ...
    def _validate_headers(self, headers: Headers, custom_headers: Headers) -> None:
        if self.session_id and headers.get("sessionid"):
            return
        if isinstance(custom_headers.get("sessionid"), Omit):
            return

        if self.jwt_access_token and headers.get("Authorization"):
            return
        if isinstance(custom_headers.get("Authorization"), Omit):
            return

        raise TypeError(
            '"Could not resolve authentication method. Expected either session_id or jwt_access_token to be set. Or for one of the `sessionid` or `Authorization` headers to be explicitly omitted"'
        )
