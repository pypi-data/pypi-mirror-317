...
class TestEvrim:
    client = Evrim(
        base_url=base_url, jwt_access_token=jwt_access_token, session_id=session_id, _strict_response_validation=True
    )
    ...
    def test_copy(self) -> None:
        ...
        copied = self.client.copy(session_id="another My Session ID")
        assert copied.session_id == "another My Session ID"
        assert self.client.session_id == "My Session ID"
    ...
    def test_base_url_env(self) -> None:
        with update_env(EVRIM_BASE_URL="http://localhost:5000/from/env"):
            client = Evrim(jwt_access_token=jwt_access_token, session_id=session_id, _strict_response_validation=True)
...
class TestAsyncEvrim:
    ...
    def test_base_url_env(self) -> None:
        with update_env(EVRIM_BASE_URL="http://localhost:5000/from/env"):
            client = AsyncEvrim(
                jwt_access_token=jwt_access_token, session_id=session_id, _strict_response_validation=True
            )
            assert client.base_url == "http://localhost:5000/from/env/"
