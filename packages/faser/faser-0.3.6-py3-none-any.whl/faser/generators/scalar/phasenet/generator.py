...
class PsfGenerator3D:
    ...
    def __init__(self, psf_shape, units, lam_detection, n, na_detection, n_threads=4):
        ...
        self.kcut = 1.0 * na_detection / self.lam_detection
        self.kmask3 = KR3 <= self.kcut

        H = np.sqrt(1.0 * self.n**2 - KR3**2 * lam_detection**2)

        self._H = H

        out_ind = np.isnan(H)
        self.kprop = np.exp(-2.0j * np.pi * self.KZ3 / lam_detection * H)
        self.kprop[out_ind] = 0.0
    ...
    def masked_phase_array(self, phi, normed=True):
        ...
        """
        return self.kmask2 * phi.phase(
            self.krho, self.kphi, normed=normed, outside=None
        )
    ...
    def coherent_psf(self, phi, normed=True):
        ...
        ku = self.kbase * np.exp(2.0j * np.pi * phi / self.lam_detection)
