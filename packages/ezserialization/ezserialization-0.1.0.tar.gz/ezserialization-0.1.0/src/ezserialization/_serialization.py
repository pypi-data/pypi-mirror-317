...
__all__ = ["TYPE_ATTR_NAME", "Serializable", "serializable", "deserialize", "isdeserializable", "set_typename_alias"]

TYPE_ATTR_NAME = "_type_"
"""
...
def isdeserializable(src: Mapping) -> bool:
    ...
    """
    return isinstance(src, dict) and TYPE_ATTR_NAME in src
        ...
        if cls_ not in _typenames_:
            ...
            def wrap_to_dict(method: Callable[..., Mapping]):
                ...
                def to_dict_wrapper(obj: Serializable) -> Mapping:
                    ...
                    if TYPE_ATTR_NAME in data:
                        raise KeyError(f"Key '{TYPE_ATTR_NAME}' already exist in the serialized data mapping!")
                    typename = _typenames_[type(obj)]
                    return {TYPE_ATTR_NAME: typename, **data}
...
def deserialize(src: Mapping) -> Serializable:
    if not isdeserializable(src):
        raise KeyError(f"Given data mapping does not contain key '{TYPE_ATTR_NAME}' required for deserialization.")

    typename = src[TYPE_ATTR_NAME]
