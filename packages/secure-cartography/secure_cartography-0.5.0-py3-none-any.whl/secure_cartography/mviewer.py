...
class TopologyViewer(QMainWindow):
    ...
    def generate_mermaid(self):
        ...
        # Map layout algorithms to Mermaid directives
        layout_mapping = {
            'TD': 'TD',
            'LR': 'LR',
            'circle': 'TB',  # We'll use special circle handling
            'kk': 'LR',  # Map other layouts to LR/TD but with different
            'rt': 'TD',  # node arrangements based on NetworkX layouts
            'circular': 'TB',
            'multipartite': 'LR'
        }
        ...
        # Apply NetworkX layout algorithms for node positioning hints
        if layout in ['kk', 'rt', 'circular', 'multipartite']:
            if layout == 'kk':
                ...
                # Create node partitions based on roles
                partition = {node: 0 if data['role'] == 'core' else 1 if data['role'] == 'gateway' else 2
                             for node, data in self.network_graph.nodes(data=True)}
                pos = nx.multipartite_layout(self.network_graph, subset_key=lambda x: partition[x])
