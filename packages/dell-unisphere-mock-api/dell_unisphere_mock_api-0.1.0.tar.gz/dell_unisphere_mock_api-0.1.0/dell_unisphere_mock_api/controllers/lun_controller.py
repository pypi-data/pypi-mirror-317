...
class LUNController:
    ...
    def create_lun(self, lun_create: LUNCreate) -> LUN:
        ...
        if pool.sizeFree < lun_create.size:
            raise HTTPException(
                status_code=400,
                detail="Pool does not have enough free space"
            )
        ...
        if existing_lun:
            raise HTTPException(
                status_code=409,
                detail="LUN with this name already exists"
            )
    ...
    def get_lun(self, lun_id: str) -> Optional[LUN]:
        ...
        if not lun:
            raise HTTPException(
                status_code=404,
                detail=f"LUN with ID '{lun_id}' not found"
            )
        return lun
    ...
    def get_lun_by_name(self, name: str) -> Optional[LUN]:
        ...
        if not lun:
            raise HTTPException(
                status_code=404,
                detail=f"LUN with name '{name}' not found"
            )
        return lun
