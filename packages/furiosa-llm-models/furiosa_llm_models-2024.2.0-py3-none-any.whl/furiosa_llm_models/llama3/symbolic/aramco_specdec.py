...
def get_decode_attn_weights(
    ...
    return attn_weights + causal_mask.to(
        attn_weights.dtype
    )  # for specdec with sp_len>1, decode also use causal_mask
...
class NewLlamaAttention(FastLlamaAttention):
    def forward(
        ...
        # specdec specific: for sp_len > 1, we only use causal_mask in both prefill and decode
        # specdec specific: but when sp_len is 1, we can still use attention_mask
        if is_prefill:
...
class NewLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # for sp_len > 1: attention mask is 3d. Therefore use make_causal_mask
