...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        if config is None:
            logger.warning_once(
                "`LlamaRotaryEmbedding` can now be fully parameterized by passing the model config through the "  # noqa: E501
                "`config` argument. All other arguments will be removed in v4.45"  # noqa: E501
            )
            self.rope_kwargs = {
                "rope_type": rope_type,
            ...
            if hasattr(config, "inv_freq_config") and config.inv_freq_config is not None:
                self.rope_type = config.inv_freq_config.get(
                    "rope_type", config.inv_freq_config.get("type")
                )
            else:
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        self._set_cache_from_freqs(freqs, dtype)

    def _set_cache_from_freqs(self, freqs, dtype):
        ...
        self.register_buffer(
            "sincos_cached", sincos.reshape(max_pos, dim, 2, 2).to(dtype), persistent=False
        )
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """LlamaRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""  # noqa: E501
    ...
    def __init__(
        ...
        super().__init__(dim, max_position_embeddings, base, device, config=config)
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """LlamaRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""  # noqa: E501
...
def apply_rotary_pos_emb(q, k, sincos, position_ids):
    def apply(tensor, sincos):
        ...
        activation = tensor.reshape(batch, num_head, seq_len, 2, rotary_dim // 2)
        einsum = torch.einsum(
            "bsfij,bhsjf->bhsif", sincos[:, sincos.shape[1] - seq_len :, :, :, :], activation
        )
        return einsum.reshape(batch, num_head, seq_len, -1)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self._skip_keys = ["past_key_values", "layer_past", "past_key_value"]
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self._init_rope(config=config)

    def _init_rope(self, config):
        if self.config.rope_scaling is None:
            self.rotary_emb = LlamaRotaryEmbedding(
                self.head_dim, max_position_embeddings=self.max_position_embeddings, config=config
            )
        else:
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    _skip_keys_device_placement = ["past_key_values", "layer_past", "past_key_value"]
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def from_huggingface(
        ...
        **kwargs,
        ...
        if pretrained_model_name_or_path != "LGAI-EXAONE/EXAONE-3.0-7.8B-Instruct":
        ...
        # If this IS LGAI-EXAONE, take separate path
        else:
            # 1. get checkpoint & llamify it. (cpu)
            exaone_infos = cls.get_exaone_infos(
                pretrained_model_name_or_path=pretrained_model_name_or_path,
                *model_args,
                config=config,
                cache_dir=cache_dir,
    ...
    # returns state_dict and conversion mapping for exaone
    @classmethod
    def get_exaone_infos(
        ...
        **kwargs,
        ...
        if pretrained_model_name_or_path != "LGAI-EXAONE/EXAONE-3.0-7.8B-Instruct":
            ...
            conversion_func = dict()
            _model = HFAutoModelForCausalLM.from_pretrained(
                pretrained_model_name_or_path=pretrained_model_name_or_path,
                *model_args,
                config=config,
                cache_dir=cache_dir,
            ...
            mapping_func = cls.get_exaone_mapping_func()
            ...
            return {
                "torch_dtype": _model.dtype,
                "conversion_func": conversion_func,
                "original_config": _model.config,
