...
class DreamUI:
    ...
    def download_and_save(file_url, folder_name, headers):
        ...
        try:
            ...
            file_path = DreamUI.sanitize_filename(parsed_url.path.lstrip("/"))
    ...
    def update_links(soup, base_url, folder_name, headers):
        ...
        for tag in soup.find_all(["link", "script", "img", "video"]):
            ...
            if tag.has_attr(attr):
                ...
                local_path = DreamUI.download_and_save(file_url, folder_name, headers)
                if local_path:
                    tag[attr] = local_path
    ...
    def download_webpage(url):
        ...
        try:
            folder_name = f"DreamUI_{urlparse(url).hostname.replace('www.', '')}"
            headers = {
            ...
            DreamUI.update_links(soup, url, folder_name, headers)
