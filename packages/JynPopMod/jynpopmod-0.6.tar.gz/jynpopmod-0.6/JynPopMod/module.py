...
def filter_profanity_in_text(text):return profanity.censor(text)
def speech_to_text_with_filter():
    recognizer = sr.Recognizer()
    with sr.Microphone() as source:
        recognizer.adjust_for_ambient_noise(source);print("Listening for speech...")
        try:audio = recognizer.listen(source);text = recognizer.recognize_google(audio);cleaned_text = filter_profanity_in_text(text);print(f"Filtered text: {cleaned_text}");return cleaned_text
