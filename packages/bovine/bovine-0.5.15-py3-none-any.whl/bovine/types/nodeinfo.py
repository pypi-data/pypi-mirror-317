...
class Services(BaseModel):
    ...
    inbound: List[InboundEnum] = Field(..., min_length=0)
    """
    The third party sites this server can retrieve messages from for combined display with regular traffic.
    """
    outbound: List[OutboundEnum] = Field(..., min_length=0)
    """
...
class Users(BaseModel):
    ...
    """
    activeHalfyear: conint(ge=0) = 0
    """
    The amount of users that signed in at least once in the last 180 days.
    """
    activeMonth: conint(ge=0) = 0
    """
...
class Usage(BaseModel):
    ...
    statistics about the users of this server.
    """
    localPosts: conint(ge=0) = 0
    """
    The amount of posts that were made by users that are registered on this server.
    """
    localComments: conint(ge=0) = 0
    """
...
class NodeInfo(BaseModel):
    ...
    Metadata about server software in use.
    """
    protocols: List[Protocol] = Field([Protocol.activitypub], min_length=1)
    """
    The protocols supported on this server.
    """
    services: Services = Services(inbound=[], outbound=[])
    """
    The third party sites this server can connect to via their application API.
    """
