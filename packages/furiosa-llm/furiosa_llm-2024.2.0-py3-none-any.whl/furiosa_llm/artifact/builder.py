...
from furiosa_llm.optimum.modeling import _load_quantized_model_meta, is_quantized_model_path
...
from .helper import build_pipelines, get_buckets, instantiate_and_save_model
...
class ArtifactBuilder:
    ...
    Args:
        ...
        devices: The devices to run the model. It can be a single device or a list of devices.
            ...
            If not given, build the artifact based on using just one device.
    ...
    qformat_path: Optional[Path]
    qparam_path: Optional[Path]
    quant_ckpt_file_path: Optional[Path]
    seed_for_random_weight: Optional[int]

    def __prestep_from_quantized_model(
        self,
        name: str,
        model_path: Path,
        seed_for_random_weight: Optional[int],
        calculate_logit_only_for_last_token: Optional[bool],
    ):
        (
            furiosa_config,
            optimization_config,
        ...
        self.model_id = furiosa_config['model_id']
        self.name = name if name else self.model_id
        model_metadata = ModelMetadata(
            pretrained_id=self.model_id,
            llm_config=LLMConfig(
                optimization_config=optimization_config,
                quantization_config=quantization_config,
            ),
        )
        self.model_metadata = model_metadata.with_hf_configs(hf_configs.to_diff_dict())
        assert self.model_metadata.is_generative_model
        self.qformat_path = qformat_path
        self.qparam_path = qparam_path
        self.quant_ckpt_file_path = quant_ckpt_file_path
    ...
    def __prestep_from_model_id(
        ...
        seed_for_random_weight: Optional[int],
        calculate_logit_only_for_last_token: Optional[bool],
        quantize_artifact_path: Optional[os.PathLike],
        prefill_chunk_size: Optional[int],
    ):

        self.name = name if name else model_id_or_path
        self.model_id = model_id_or_path
        self.model_metadata = (
            ModelMetadata.init_with_mlperf_optim_options(model_id_or_path)
            if is_mlperf_optimized_model(model_id_or_path)
            else ModelMetadata(model_id_or_path)
        )

        # Override model config, optimization config
        if num_hidden_layers:
            self.model_metadata = self.model_metadata.with_num_layers(num_hidden_layers)

        if calculate_logit_only_for_last_token:
            self.model_metadata = self.model_metadata.with_optimizations(
        ...
        if quantize_artifact_path:
            ...
            quant_ckpt_file_path = Path(f"{quantize_artifact_path}/exported_model.qckpt")
            ...
            self.quant_ckpt_file_path = (
                quant_ckpt_file_path if os.path.exists(quant_ckpt_file_path) else None
            )

            quantization_config = QuantizationConfig.from_qformat(qformat_path)
            self.model_metadata = self.model_metadata.with_quantization_config(quantization_config)
        else:
            self.qformat_path = None
            self.qparam_path = None
            self.quant_ckpt_file_path = None
    ...
    def __init__(
        ...
        *,
        ...
        calculate_logit_only_for_last_token: Optional[bool] = None,
        # Quantize Config
        ...
        **kwargs,
        ...
        if is_quantized_model_path(model_id_or_path):
            ...
            self.__prestep_from_quantized_model(
                name, model_path, seed_for_random_weight, calculate_logit_only_for_last_token
            )

        else:
            self.__prestep_from_model_id(
                model_id_or_path,
                name,
                num_hidden_layers,
                seed_for_random_weight,
                calculate_logit_only_for_last_token,
                quantize_artifact_path,
                default_scheduler_config.prefill_chunk_size,
            )
        ...
        if not prefill_buckets and not decode_buckets:
            self.prefill_buckets: Sequence[tuple[int, int]] = [(1, max_seq_len_to_capture)]
            self.decode_buckets: Sequence[tuple[int, int]] = (
    ...
    def build(
        ...
        **kwargs,
        ...
        prefill_buckets, decode_buckets, other_buckets = get_buckets(
            ManualBucketConfig(
                prefill_buckets=self.prefill_buckets,
                decode_buckets=self.decode_buckets,
            ),
        ...
        if not param_file_path:
            if cache_dir and model_creation_info.is_hashable():
                ...
                instantiate_and_save_model(
                    self.model_metadata,
                    model_creation_info.random_weight_model,
                    self.qformat_path,
                    self.qparam_path,
                    param_file_path,
                    quant_ckpt_file_path=self.quant_ckpt_file_path,
        ...
        pipelines = build_pipelines(
            ...
            self.model_rewriting_config.add_prefill_last_block_slice,
            self.model_metadata.is_generative_model,
            param_saved_format,
            **kwargs,
        )
