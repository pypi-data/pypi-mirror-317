...
```
usage: xray-uploader [-h] -r RESULT -f FORMAT -pi PROJECTID -s SUMMARY [-ci CLIENTID] [-cs CLIENTSECRET] [-tp TESTPLAN] [-se]
...
options:
  ...
  -se SETENV, --setEnv SETENV
                        Set azure pipeline environment "XRAY_EXECUTION_ID" with xray key.
```