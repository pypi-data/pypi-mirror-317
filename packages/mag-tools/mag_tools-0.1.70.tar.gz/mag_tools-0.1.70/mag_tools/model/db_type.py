...
class DbType(BaseEnum):
    ...
    @staticmethod
    def of(type_or_value):
        for db_type in DbType:
            if isinstance(type_or_value, int) and db_type.type == type_or_value:
                return db_type
            elif isinstance(type_or_value, str) and db_type.value == type_or_value:
                return db_type
        return None

if __name__ == '__main__':
    # 示例用法
    db_type = DbType.of("mysql")
    print(f"Code: {db_type.code}, Desc: {db_type.desc}, URL: {db_type.url}, Port: {db_type.port}, Driver: {db_type.driver_class_name}, Display Name: {db_type.display_name}")
