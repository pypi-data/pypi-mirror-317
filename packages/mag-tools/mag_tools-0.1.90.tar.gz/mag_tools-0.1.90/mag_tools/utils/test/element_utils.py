...
class ElementUtils:
    ...
    def set_table(driver, table:WebElement, data:List[List[str]]):
        ...
        for item_index, item in enumerate(table_items):
            cells = ElementUtils.find_elements_by_type(item, None, ControlType.EDIT)
            for cell_index, cell in enumerate(cells):
                if cell.is_enabled() and cell.is_displayed():  # 检查单元格是否可写
                    if item_index < len(data) and cell_index < len(data[item_index]):
                        actions = ActionChains(driver)
                        actions.move_to_element(cell).click().perform()
                        cell.clear()
                        cell.send_keys(data[item_index][cell_index])