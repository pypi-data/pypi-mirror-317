    ...
    example
{
    ...
    def send(self, data, http_result, proxies=None, xray_method=None):
        ...
        if xray_method is None:
            ...
            self._send(proxies=proxies, deal_data=data, http_result=http_result)
            return
        if xray_method == "simple":
            for tmp_data in self._xray_params(data):
                ...
                self._send(proxies=proxies, deal_data=tmp_data, http_result=http_result)  # 同步
        if xray_method == "high":
            # todo 高级模式
            pass
    ...
    def _send(self, deal_data, http_result, proxies=None):
        ...
        if self.body_mode == "file":
            self.ice.http_request.send(ice_url, self.req_data.method, params, FileUtils.chunk_read(files[0]),
        ...
        if self.body_mode == "form-data":
            self.ice.http_request.send(ice_url, self.req_data.method, params, data, header,
