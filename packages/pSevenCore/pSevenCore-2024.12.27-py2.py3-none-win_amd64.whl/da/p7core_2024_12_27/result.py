...
class Result(object):
  ...
  def _reconstruct_linear_responses(self, x, resp, active_x):
    # _refill_linear_responses helper method
    if not len(x):
      return None
    ...
    if x_recons.shape[0] <= (x_recons.shape[1] + 1):
      weights_recons, _ = _utils._linear_rsm_fit(x_recons, resp[:, _numpy.newaxis])
    else:
      weights_recons, rrms = _utils._linear_rsm_stepwise_fit(x_recons, resp[:, _numpy.newaxis])
      if rrms >= 1.:
        return None
  ...
  def _solutions_filter(self, filter_type):
    filter_type = _shared._normalize_string_list(filter_type, name="filters list")
    ...
    undefined_filters = [_ for _ in filter_type if _ not in known_filters]
  ...
  def _raw_solutions(self, fields=None, filter_type=None):
    fields = _shared._normalize_string_list(fields, name="fields")
    if fields is not None:
      invalid_fields = [field for field in fields if field not in self._fields and field not in self._extended_fields]
      if invalid_fields:
        raise _exceptions.WrongUsageError("Unexpected fields %s. Available fields are %s." % (invalid_fields, list(self._fields) + list(self._extended_fields)))
    ...
    if fields is None:
      ...
      return designs_data[:, 0:0][points_filter], [] # right shape, empty data
  ...
  def _raw_designs(self, fields=None, sample_type=None):
    ...
    if fields is not None:
      ...
      invalid_fields = [field for field in fields if field not in designs_fields]
      if invalid_fields:
    ...
    sample_type = _shared._normalize_string_list(sample_type, name="samples list")
    points_mask = self._designs_filter(sample_type)

    if not fields:
      return self._designs_data[:, 0:0][points_mask], [] # right shape, empty data
  ...
  def pprint(self, fields=None, filter_type=None, file=_sys.stdout, precision=8, designs=False, limit=None):
    ...
    fields = _shared._normalize_string_list(fields, name="fields")
    filter_type = _shared._normalize_string_list(filter_type, name="filters")
