...
def _safe_pickle_dump(source, fout):
  class NumpyPickler(object):
    ...
    def __call__(self, obj):
      if isinstance(obj, _numpy.ndarray):
        obj_id = "numpy.array_%04d" % self.next_id
        self.next_id += 1
        self.data[obj_id] = {"shape": obj.shape, "dtype": str(obj.dtype), "data": obj.tostring()}
        return obj_id
      else:
        return None
  ...
  try:
    ...
    pickle_stream = pickle.Pickler(data_stream, protocol=2, fix_imports=True)
  except (ImportError, TypeError):
    # Python 2 mode
    from StringIO import StringIO as _BytesIO
    data_stream = _BytesIO()
    pickle_stream = pickle.Pickler(data_stream, protocol=2)

  pickle_stream.persistent_id = NumpyPickler()
  pickle_stream.dump(source)

  try:
    # Python 3 mode
    pickle.dump(pickle_stream.persistent_id.data, fout, protocol=2, fix_imports=True)
  except TypeError:
    # Python 2 mode
    pickle.dump(pickle_stream.persistent_id.data, fout, protocol=2)
...
def _safe_pickle_load(fin):
  class NumpyLoader(object):
    ...
    def __call__(self, id):
      if id in self.data:
        array_data = self.data.pop(id)
        try:
          return _numpy.fromstring(array_data["data"].encode("latin1"), dtype=array_data["dtype"]).reshape(array_data["shape"])
        except (AttributeError, UnicodeDecodeError):
          return _numpy.fromstring(array_data["data"], dtype=array_data["dtype"]).reshape(array_data["shape"])
      else:
        raise pickle.UnpicklingError('Invalid persistent id')

  try:
    # Python 3 mode
    numpy_stream = NumpyLoader(pickle.load(fin, fix_imports=True, encoding='latin1'))
    pickle_stream = pickle.Unpickler(fin, fix_imports=True, encoding='latin1')
  except TypeError:
    # Python 2 mode
    numpy_stream = NumpyLoader(pickle.load(fin))
    pickle_stream = pickle.Unpickler(fin)

  pickle_stream.persistent_load = numpy_stream
  return pickle_stream.load()
...
def _normalize_string_list(string_list, default_value=None, name=None):
  if string_list is None:
    return default_value

  try:
    if isinstance(string_list, _six.string_types):
      string_list = [_.strip() for _ in string_list.lower().split(",")]
    else:
      string_list = [_.strip().lower() for _ in string_list]
