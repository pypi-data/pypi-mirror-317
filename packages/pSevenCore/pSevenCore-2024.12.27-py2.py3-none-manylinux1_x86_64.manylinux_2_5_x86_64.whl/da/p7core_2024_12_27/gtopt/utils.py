...
def _penalize_small_weights(W, std_x, std_y, threshold):
  # W.shape() == (len(var_x) + 1), len(var_y)
  noise = _numpy.vstack((_numpy.tile(std_x, (len(std_y), 1)).T, std_y)) * threshold
  W[_numpy.fabs(W) < noise] = 0.

  # round weights a little to compensate over-accuracy
  W_mantissa, W_exponent = _numpy.frexp(W)
  return _numpy.ldexp(_numpy.round(W_mantissa, 14), W_exponent)

def _linear_rsm_stepwise_fit(x, y):
  try:
  ...
  return _linear_rsm_fit(x, y, active_terms=active_terms)

def _linear_rsm_fit(x, y, rcond_threshold=None, active_terms=None):
  """
  ...
  if not known_y.all():
    if not (known_y.any(axis=1) == known_y.all(axis=1)).all():
      ...
      weights, rrmse = zip(*tuple([_linear_rsm_fit(x, y_i.reshape(-1, 1)) for y_i in y.T]))
  ...
  std_x = _numpy.std(x, axis=0) if npoints > 1 else _numpy.zeros(x.shape[1])
  ...
  noise_threshold = rcond_threshold or 2.*_numpy.finfo(float).eps

  for i in range(npoints):
    W = _numpy.linalg.lstsq(X1[:-1], Y1[:-1], rcond=rcond_threshold)[0]
    W = _penalize_small_weights(W, std_x[x_columns], std_y, noise_threshold)
