...
if not NODE_IP:
    raise ValueError("NODE_IP environment variable not set")
...
async def lifespan(app: FastAPI):
    """
    Lifespan context manager to initialize and clean up resources when the app starts and stops.
    Manages the lifecycle of a StorageNode instance with comprehensive logging of all major state transitions.
    """
    try:
        logger.info(f"Beginning StorageNode initialization process for {NODE_IP}:{NODE_PORT}")
        
        # Initialize the node as a StorageNode
        NODE_TCP_PORT = int(os.getenv("NODE_TCP_PORT", NODE_PORT + 500))
        ...
        logger.info(
            f"StorageNode instance created with configuration:"
            f"\n\tIP: {NODE_IP}"
            f"\n\tPort: {NODE_PORT}"
            f"\n\tStorage Directory: {app.state.node.storage_service.storage_dir}"
            f"\n\tCleanup Interval: {app.state.node.storage_service.cleanup_interval}s"
            f"\n\tRepublish Interval: {app.state.node.storage_service.republish_interval}s"
        )

        # # Determine bootstrap nodes
        # bootstrap_nodes = [("node1", 8000), ("node2", 8000), ("node3", 8000)]
        # logger.info(f"Node {NODE_IP} configured - using bootstrap nodes: {bootstrap_nodes}")

        # Exclude self from bootstrap nodes
        all_nodes = [("node1", 8000), ("node2", 8000), ("node3", 8000)]
        bootstrap_nodes = [(ip, port) for ip, port in all_nodes if ip != NODE_IP]
        logger.info(f"Node {NODE_IP} configured - using bootstrap nodes: {bootstrap_nodes}")


        # Start the node and bootstrap
        logger.info("Initiating StorageNode startup sequence...")
        await app.state.node.start(bootstrap_nodes=bootstrap_nodes)

        # Verify node initialization
        if app.state.node is None:
            logger.error("Critical failure: StorageNode initialization resulted in None")
            raise RuntimeError("StorageNode failed to initialize")

        # Log successful startup with comprehensive details
        logger.info(
            f"StorageNode startup completed successfully:"
            f"\n\tNode ID: {app.state.node.node_id}"
            f"\n\tStorage Service: {'Active' if app.state.node.storage_service is not None else 'Inactive'}"
            f"\n\tBootstrap Nodes: {bootstrap_nodes if bootstrap_nodes else 'None (Primary Node)'}"
        )
        ...
        logger.error(
            f"Critical error during StorageNode lifecycle:"
        ...
        if app.state.node is not None:
            logger.info(
                f"Beginning StorageNode shutdown sequence for node ID: {app.state.node.node_id}"
            )
            try:
                await app.state.node.stop()
                logger.info(
                    f"StorageNode shutdown completed successfully:"
                    f"\n\tNode ID: {app.state.node.node_id}"
                ...
                logger.error(
                    f"Error during StorageNode shutdown:"
                    f"\n\tNode ID: {app.state.node.node_id}"
                    f"\n\tError: {str(e)}",
                    exc_info=True
                )
...
async def create_product(
    ...
    try:
        ...
        logger.debug(
            f"Starting product creation process for user {current_user.username}"
        )
        ...
        if not await deduct_credits_from_user(current_user.username, required_credits):
            raise HTTPException(
                ...
                detail="Insufficient credits to create a product.",
            )

        product_id = hashlib.sha1(product_in.core.name.encode("utf-8")).hexdigest()
        logger.debug(f"Generated product_id: {product_id}")

        # Perform find_nodes to populate routing table with nodes close to product_id
        ...
        product = Product(
            ...
            image_refs=product_in.image_refs,
        )

        # Store the product
        logger.debug(f"Attempting to store product {product_id} in network")
        await node.store_product(product, ENCRYPTION_PASSWORD)
        logger.debug(f"Product {product_id} stored successfully")
        ...
        if not verification_product:
            ...
            await refund_credits_to_user(current_user.username, required_credits)
            raise HTTPException(
        ...
        try:
            # Index the product with retries
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    logger.debug(f"Calling index_product for product_id {product_id} (attempt {attempt + 1}/{max_retries})")
                    await node.index_product(product)
                    logger.debug(f"Product {product_id} indexed successfully on attempt {attempt + 1}")
                    break
                except Exception as index_error:
                    if attempt == max_retries - 1:  # Last attempt
                        logger.error(f"Failed to index product after {max_retries} attempts: {str(index_error)}")
                        raise
                    logger.warning(f"Indexing attempt {attempt + 1} failed with error: {str(index_error)}, retrying...")
        ...
        # Ensure credits are refunded in case of unexpected errors
        try:
            logger.debug(f"Attempting to refund credits for user {current_user.username}")
            await refund_credits_to_user(current_user.username, required_credits)
            logger.debug(f"Successfully refunded {required_credits} credits to user {current_user.username}")
...
async def retry_find_product(node, product_id, encryption_password, max_retries=3, delay=1):
    ...
    """
    # Log routing table state once before starting retries
    nodes = node.routing_table.get_all_nodes()
    logger.debug(f"Current routing table: {[{'node_id': n.node_id, 'ip': n.ip, 'port': n.port} for n in nodes]}")
...
async def search_products(
    ...
    At least one of 'category', 'shop_name', or 'seller location' must be provided.
    """
    ...
    if latitude is not None and longitude is not None:
        # Calculate bins to search based on the radius
...
def generate_bins(center_coord, radius_km, bin_size):
    ...
    coord_radius = radius_km / 111.0  # Convert km to degrees
...
async def process_payment_background(checkout_request_id: str):
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            ...
            # Check if 'ResultCode' is in the response
            if 'ResultCode' in status_result:
                ...
                if result_code == "0":
                    ...
                    # Other ResultCodes can be handled here
                ...
                if error_code == "500.001.1001" and error_message == "The transaction is being processed":
                    # Transaction is still being processed; continue polling
...
async def get_purchase_status(
    ...
    Args:
        checkout_request_id (str): The CheckoutRequestID of the payment transaction.

    Returns:
        dict: The status and details of the payment transaction.
    """
    transaction = transactions.get(checkout_request_id)
...
async def suggest_category(
    ...
    Args:
        ...
        suggestion (CategorySuggestion): The category suggestion from the request body.
        current_user (DBUser): The current authenticated user.

    Returns:
        dict: A message indicating the result of the suggestion.

    Raises:
        HTTPException: If the node is not initialized or an error occurs.
    """
    ...
    logger.info(
        f"User '{current_user.username}' is suggesting category '{category_name}'"
    )
    ...
    try:
        ...
        if success:
            logger.info(
                f"Category '{category_name}' was approved and added to the categories list."
            )
            return {
                "message": f"Category '{category_name}' has been approved and added."
            }
        else:
        ...
        logger.error(
            f"Error suggesting category '{category_name}': {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="An error occurred while suggesting the category."
        )
...
async def debug_network(request: Request):
    ...
    return {
        ...
        # "is_unresponsive": node.is_unresponsive,  # If applicable
    }
...
async def node_status(request: Request):
    ...
    try:
        return {
            ...
            "is_running": hasattr(node, "_running") and node._running
            if node
            else False,
        }
    except Exception as e:
...
async def upload_product_image(
    ...
    try:
        ...
        # Check if user has enough credits (e.g., 3 credits per image upload)
        ...
        await node.storage_service.store_file(
            file_hash, image_data, ttl=86400
        )  # 24-hour TTL
        ...
        # Update the product's image_refs with the ImageManifest
        ...
        image_manifest = ImageManifest(
            ...
            chunk_hashes=[],  # Empty list since you might not have chunk hashes
        )
        existing_product.image_refs.append(image_manifest)

        # Store the updated product
        await node.store_product(existing_product, ENCRYPTION_PASSWORD)
...
async def get_product_image(request: Request):
    product_id = request.path_params.get("product_id")
    node = request.app.state.node
    # Use node.find_product to retrieve the product
    product = await node.find_product(product_id, ENCRYPTION_PASSWORD)
    if not product:
    ...
    # Assume we're using the first image in image_refs
    image_manifest = product.image_refs[0]  # Get the first ImageManifest
    file_hash = image_manifest.file_hash

    try:
        # Retrieve the image data using the file hash
