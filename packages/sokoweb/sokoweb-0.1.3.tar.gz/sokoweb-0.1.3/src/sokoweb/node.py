...
class Node:
    ...
    async def handle_store(self, message, addr):
        ...
        if key and value:
            expiration_time = time.time() + ttl if ttl else time.time() + 3600
    ...
    async def store_value(self, key, value, ttl=3600):
        try:
    ...
    async def store_product(self, product, encryption_password):
        ...
        if len(product_json.encode("utf-8")) > MAX_UDP_PAYLOAD_SIZE - 512:
            ...
            await self.storage_service.store_file(
                product_hash, product_json.encode("utf-8")
            )
            product_ref = {"type": "storage_ref", "hash": product_hash}
            product_key = hashlib.sha1(product.product_id.encode("utf-8")).hexdigest()
            await self.store_value(product_key, json.dumps(product_ref))
        else:
            product_key = hashlib.sha1(product.product_id.encode("utf-8")).hexdigest()
            await self.store_value(product_key, product_json)
    ...
    async def store_sub_chunks(self, chunk_hash, sub_chunks, ttl=86400):
        self.logger.debug(
