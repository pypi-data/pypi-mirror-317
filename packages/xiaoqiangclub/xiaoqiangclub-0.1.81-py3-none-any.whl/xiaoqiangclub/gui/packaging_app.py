...
def packaging_app(script_path: str,
                  ...
                  dependencies: str | list = "requirements.txt",
                  extra_args: str = ""):
    """
    ...
    :param dependencies: 项目依赖文件路径或依赖列表，默认为 requirements.txt
    :param extra_args: 传递给 PyInstaller 的额外参数，默认为空字符串
    """
    input("如果已安装 PyInstaller 按任意键继续...")

    # 处理依赖文件或列表
    hidden_imports = []
    if isinstance(dependencies, str) and os.path.exists(dependencies):
        with open(dependencies, "r", encoding="utf-8") as f:
            hidden_imports = [line.strip() for line in f if line.strip()]
    elif isinstance(dependencies, list):
        hidden_imports = dependencies
