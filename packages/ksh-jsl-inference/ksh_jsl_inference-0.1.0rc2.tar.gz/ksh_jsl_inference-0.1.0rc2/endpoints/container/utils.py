...
def _generate_healthcare_nlp_docker_files(
    ...
    if legacy:
        ...
        entrypoint_command = f"from inference_model import {inference_class_name};from jsl_inference.container.serve import serve;serve(sagemaker={sagemaker}, snowflake={snowflake}, inference_model={inference_class_name}())"
        copy_and_replace(
            ...
            f"{output_dir}/Dockerfile",
            {
                ...
                "{{MODEL_TO_LOAD}}": model_to_serve,
                "{{LANGUAGE}}": language,
                "{{ENTRYPOINT}}": f'ENTRYPOINT ["python" , "-c", "{entrypoint_command}"]',
            },
        )
        copy_and_replace(
