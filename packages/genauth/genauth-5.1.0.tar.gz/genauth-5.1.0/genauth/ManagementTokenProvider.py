...
from genauth.GenAuthException import GenAuthException
...
class ManagementTokenProvider:
    ...
    def __get_access_token(self):
        ...
        if statusCode != 200:
            raise GenAuthException(statusCode, message, apiCode)
        access_token, expires_in = data.get("access_token"), data.get("expires_in")
        if not access_token:
            raise GenAuthException(500, "get access token failed")
