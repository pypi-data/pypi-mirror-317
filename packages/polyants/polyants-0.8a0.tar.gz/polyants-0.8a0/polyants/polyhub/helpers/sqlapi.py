...
class Querier(metaclass=ABCMeta):
    ...
    def _adapt(self, condition):
        ...
        if operation and column and value:
    ...
    def _get_equality_condition(self, left_alias, right_alias):
            ...
            if insert := self.raw.get('insert'):
                ...
                if (begin_column := self._get_column_by_role(BEGIN_ROLE, strict=False)) and (
                    ...
                    raw = {
                        "with": {
                            "dataset": {
                                "columns": [с.name for с in writables],
                                "select": [{"eval": "t.*"}],
                            ...
                            "upsert": {
                                ...
                                "where": self._get_equality_condition('b', 'd'),
                                "returning": ["<=b=>.*"],
                            },
                        },
                        "insert": {
    ...
    def delete(self, data, columns=None):
        ...
        if data:
            ...
            if delete := self.raw.get('delete'):
                ...
                if (self._get_column_by_role(BEGIN_ROLE, strict=False)) and (
                    end_column := self._get_column_by_role(END_ROLE, strict=False)
                ):
    ...
    def create_table(self, name, columns, temporary=False):
        ...
        if name and columns:
            ...
            sql += ','.join(attrs)
            sql += ')'
            log.debug(f'Структура SQL таблицы:\n{sql}')
            self.execute(sql)
