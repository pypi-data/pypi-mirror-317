...
from clickzetta.zettapark._internal.utils import (
    ...
    unwrap_volume_path_single_quote,
...
"""
    ...
    def custom_package_usage_config(self) -> Dict:
        ...
        a remote volume path as ``cache_path`` where unsupported pure Python packages will be persisted. To use a specific
        ...
        Configurations:
            ...
            - **cache_path** (*str*): Cache custom Python packages on a volume directory. This parameter greatly reduces latency of custom package import.
        ...
        Example::
            ...
            >>> session.custom_package_usage_config = {"enabled": True, "cache_path": "@my_permanent_volume/folder"} # doctest: +SKIP
        ...
        Note:
            ...
            - These configurations also allow custom package addition for all UDFs created later in the current session. If you only want to add custom packages for a specific UDF, you can use ``packages`` argument in :func:`functions.udf` or :meth:`session.udf.register() <clickzetta.zettapark.udf.UDFRegistration.register>`.
        """
    ...
    def add_import(
        ...
        """
        Registers a remote file in volume or a local file as an import of a user-defined function
        ...
        Args:
            path: The path of a local file or a remote file in the volume. In each case:

                * if the path points to a local file, this file will be uploaded to the
                  volume where the UDF is registered and ClickZetta will import the file when
                  executing that UDF.

                * if the path points to a local directory, the directory will be compressed
                  as a zip file and will be uploaded to the volume where the UDF is registered
                  and ClickZetta will import the file when executing that UDF.

                * if the path points to a file in a volume, the file will be included in the
            ...
            import_path: The relative Python import path for a UDF.
                ...
                a no-op if the path  points to a volume file or a non-Python local file.
        ...
        Note:
            1. In favor of the lazy execution, the file will not be uploaded to the volume
            ...
            file in the volume. If there is an existing file or directory, the Zettapark
            library will compare their checksums to determine whether it should be re-uploaded.
            Therefore, after uploading a local file to the volume, if the user makes
            some changes to this file and intends to upload it again, just call this
            function with the file path again, the existing file in the volume will be
    ...
    def remove_import(self, path: str) -> None:
        """
        Removes a file in volume or local file from the imports of a user-defined function (UDF).

        Args:
            path: a path pointing to a local file or a remote file in the volume
        ...
        abs_path = (
            ...
            if not trimmed_path.startswith(VOLUME_PREFIX)
            else trimmed_path
        )
    ...
    def clear_imports(self) -> None:
        """
        Clears all files in a volume or local files from the imports of a user-defined function (UDF).
        """
        ...
        if not trimmed_path.startswith(VOLUME_PREFIX):
            ...
            if not os.path.isfile(trimmed_path) and not os.path.isdir(
                ...
                raise ValueError(
                    ...
                    f"or a file in a volume, but got {trimmed_path}"
                )
            ...
            # will change and the file in the volume will be overwritten.
            return (
                abs_path,
                calculate_checksum(
                    abs_path,
    ...
    def _resolve_imports(
        self,
        import_only_volume: str,
        upload_and_import_volume: str,
        ...
        *,
        ...
        """Resolve the imports and upload local files (if any) to the volume."""
        resolved_volume_files = []
        volume_file_list = self._list_files_in_volume(
            import_only_volume, statement_params=statement_params
        )

        normalized_import_only_location = unwrap_volume_path_single_quote(
            import_only_volume
        )
        normalized_upload_and_import_location = unwrap_volume_path_single_quote(
            upload_and_import_volume
        )
        ...
        for path, (prefix, leading_path) in import_paths.items():
            ...
            if path.startswith(VOLUME_PREFIX):
                resolved_volume_files.append(path)
            else:
                filename = (
                ...
                if filename_with_prefix in volume_file_list:
                    _logger.debug(
                        f"{filename} exists on {normalized_import_only_location}, skipped"
                    )
                    resolved_volume_files.append(
                        normalize_remote_file_or_dir(
                    ...
                    if os.path.isdir(path) or path.endswith(".py"):
                        with zip_file_or_directory_to_stream(
                            ...
                            self._conn.upload_stream(
                                ...
                                volume_path=normalized_upload_and_import_location,
        ...
        if len(unsupported_packages) != 0:
            ...
            if self._custom_package_usage_config.get(
                ...
                try:
                    ...
                    dependency_packages = self._load_unsupported_packages_from_volume(
    ...
    def _upload_unsupported_packages(
        ...
        """
        Uploads a list of Pypi packages, which are unavailable in ClickZetta, to session volume.
        ...
        try:
            ...
            # Add packages to volume
            volume_name = self.get_session_volume()

            if self._custom_package_usage_config.get("cache_path", False):
                # Switch the volume used for storing zip file.
                volume_name = self._custom_package_usage_config["cache_path"]
                ...
                self._conn.upload_file(
                    ...
                    volume_path=normalize_remote_file_or_dir(volume_name),
                    compress_data=False,
                    overwrite=True,
                )
            ...
            volume_zip_path = f"{volume_name}/{zip_file}"
            self.add_import(
                volume_zip_path
                if volume_zip_path.startswith(VOLUME_PREFIX)
                else f"{VOLUME_PREFIX}{volume_zip_path}"
            )
        except Exception as e:
            raise RuntimeError(
    ...
    def _load_unsupported_packages_from_volume(
        self, environment_signature: str
    ) -> List[pkg_resources.Requirement]:
        """
        Uses specified volume path to auto-import a group of unsupported packages, along with its dependencies. This
        saves time spent on pip install, native package detection and zip upload to volume.

        A cached environment on a volume consists of two files:
        ...
        if metadata_file not in files:
            _logger.info(
                f"Metadata file named {metadata_file} not found at volume path {cache_path}."
            )
        ...
        if required_file not in files:
            _logger.info(
                f"Matching environment file not found at volume path {cache_path}."
            )
        ...
        self.add_import(
            import_path
            if import_path.startswith(VOLUME_PREFIX)
            else f"{VOLUME_PREFIX}{import_path}"
        )
        return dependency_packages
    ...
    def get_session_volume(self) -> str:
        """
        Returns the name of the temporary volume created by the Zettapark library
        ...
        """
        volume_name = self.get_fully_qualified_name_if_possible(self._session_volume)
        if not self._volume_created:
            self._run_query(
                f"create {get_temp_type_for_object(self._use_scoped_temp_objects, True)} \
                volume if not exists {volume_name}",
                is_ddl_on_temp_object=True,
            )
            self._volume_created = True
        return f"{VOLUME_PREFIX}{volume_name}"
