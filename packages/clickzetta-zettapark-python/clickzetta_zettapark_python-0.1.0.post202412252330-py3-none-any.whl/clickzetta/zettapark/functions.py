...
def udf(
    ...
    *,
    ...
    Note:
        ...
        and all UDF related files will be uploaded to a temporary session volume
        (:func:`session.get_session_volume() <clickzetta.zettapark.Session.get_session_volume>`).
        For a permanent UDF, these files will be uploaded to the volume that you provide.
...
def udtf(
    ...
    *,
    ...
    Note:
        ...
        and all UDTF related files will be uploaded to a temporary session volume
        (:func:`session.get_session_volume() <clickzetta.zettapark.Session.get_session_volume>`).
        For a permanent UDTF, these files will be uploaded to the volume that you specify.
    ...
    *,
    ...
    Args:
        ...
        is_permanent: Whether to create a permanent UDAF. The default is ``False``.
            If it is ``True``, a valid ``volume_path`` must be provided.
        volume_path: The volume location where the Python file for the UDAF
            and its dependencies should be uploaded. The volume location must be specified
            when ``is_permanent`` is ``True``, and it will be ignored when
            ``is_permanent`` is ``False``. It can be any volume other than temporary
            volumes and external volumes.
    ...
    Note:
        ...
        and all UDAF related files will be uploaded to a temporary session volume
        (:func:`session.get_session_volume() <clickzetta.zettapark.Session.get_session_volume>`).
        For a permanent UDAF, these files will be uploaded to the volume that you provide.
...
def pandas_udtf(
    ...
    *,
    ...
    either the decorator `@vectorized(input=pandas.DataFrame)` or setting `<class>.end_partition._vectorized_input = pandas.DataFrame`
