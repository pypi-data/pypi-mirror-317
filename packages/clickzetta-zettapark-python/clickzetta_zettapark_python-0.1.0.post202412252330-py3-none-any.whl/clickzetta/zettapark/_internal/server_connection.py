...
def _build_target_path(volume_path: str, dest_prefix: str = "") -> str:
    qualified_volume_name = unwrap_volume_path_single_quote(volume_path)
    dest_prefix_name = (
        dest_prefix
        if not dest_prefix or dest_prefix.startswith("/")
        else f"/{dest_prefix}"
    )
    return f"{qualified_volume_name}{dest_prefix_name if dest_prefix_name else ''}"
...
def _build_put_statement(
    ...
    target_path = normalize_remote_file_or_dir(
        _build_target_path(volume_path, dest_prefix)
    )
