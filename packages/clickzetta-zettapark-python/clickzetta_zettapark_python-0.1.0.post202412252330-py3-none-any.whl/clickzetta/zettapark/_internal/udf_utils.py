...
# Max code size to inline generated closure. Beyond this threshold, the closure will be uploaded to a volume for imports.
...
def is_local_python_file(file_path: str) -> bool:
    return not file_path.startswith(VOLUME_PREFIX) and file_path.endswith(".py")
        ...
        if len(column_type_hints) > 1 and column_type_hints[1] == Ellipsis:
            return StructType(
                [
                    StructField(
                        name,
                        type_utils.python_type_to_data_type(column_type_hints[0])[0],
                    )
            ...
            return StructType(
                [
                    StructField(
                        name,
                        type_utils.python_type_to_data_type(column_type)[0],
                    )
...
def get_types_from_type_hints(
    ...
    if isinstance(func, Callable):
        ...
        elif object_type in (TempObjectType.FUNCTION,):
            python_types_dict = retrieve_func_type_hints_from_source(
                filename, func_name
            )
        else:
            raise ValueError(
                f"Expecting FUNCTION, TABLE_FUNCTION, or AGGREGATE_FUNCTION as object_type, got {object_type}"
            )
    ...
    if object_type == TempObjectType.TABLE_FUNCTION:
        ...
        return_type = (
            python_type_to_data_type(python_types_dict["return"])[0]
            if "return" in python_types_dict
            else None
        )
    ...
    for key, python_type in python_types_dict.items():
        if key != "return":
            input_types.append(python_type_to_data_type(python_type)[0])
        index += 1
...
def get_error_message_abbr(object_type: TempObjectType) -> str:
    ...
    raise ValueError(f"Expect FUNCTION, but get {object_type}")
...
def check_register_args(
    ...
    if is_permanent:
        ...
        if not volume_path:
            raise ValueError(
                f"volume_path must be specified for permanent {get_error_message_abbr(object_type)}"
            )
...
def process_file_path(file_path: str) -> str:
    ...
    if not file_path.startswith(VOLUME_PREFIX) and not os.path.exists(file_path):
...
def extract_return_input_types(
    ...
    if (
        ...
        if num_args != len(input_types_from_type_hints):
            raise TypeError(
                f'{"" if object_type == TempObjectType.FUNCTION else "Excluding session argument, "}'
...
def cleanup_failed_permanent_registration(
    session: "clickzetta.zettapark.Session",
    upload_file_volume_path: str,
    volume_path: str,
) -> None:
    if volume_path and upload_file_volume_path:
        try:
            logger.debug(
                "Removing Zettapark uploaded file: %s",
                upload_file_volume_path,
            )
            session._run_query(f"REMOVE {upload_file_volume_path}")
            logger.info(
...
def generate_python_code(
    ...
    if object_type == TempObjectType.TABLE_FUNCTION:
        ...
        raise ValueError(
            f"Expecting FUNCTION, TABLE_FUNCTION or AGGREGATE_FUNCTION for object_type, got {object_type}"
        )

    # clear the annotations because when the user annotates Variant,
...
{_DEFAULT_HANDLER_NAME}{vectorized_sub_component}._vectorized_input = pandas.DataFrame
""".rstrip()
        ...
        if max_batch_size:
            ...
            pandas_code = f"""
{pandas_code}
{_DEFAULT_HANDLER_NAME}{max_batch_size_sub_component}._max_batch_size = {int(max_batch_size)}
""".rstrip()
        func_code = f"""
{func_code}
{pandas_code}
...
def resolve_imports_and_packages(
    ...
    *,
    ...
    import_only_volume = (
        unwrap_volume_path_single_quote(volume_path)
        if volume_path
        else session.get_session_volume()
    )

    upload_and_import_volume = (
        import_only_volume if is_permanent else session.get_session_volume()
    )
    ...
    # Upload closure to volume if it is beyond inline closure size limit
    if isinstance(func, Callable):
        ...
        if not force_inline_code and len(code) > _MAX_INLINE_CLOSURE_SIZE_BYTES:
            dest_prefix = get_udf_upload_prefix(udf_name)
            upload_file_volume_path = normalize_remote_file_or_dir(
                f"{upload_and_import_volume}/{dest_prefix}/{udf_file_name}"
            )
            ...
            with io.BytesIO() as input_stream:
                ...
                session._conn.upload_stream(
                    ...
                    volume_path=upload_and_import_volume,
            ...
            all_urls.append(upload_file_volume_path)
            inline_code = None
            handler = f"{udf_file_name_base}.{_DEFAULT_HANDLER_NAME}"
        else:
            inline_code = code
            upload_file_volume_path = None
        ...
        if func[0].startswith(VOLUME_PREFIX):
            upload_file_volume_path = None
...
def create_python_udf(
    ...
    if isinstance(return_type, StructType):
        return_sql = f'RETURNS TABLE ({",".join(f"{field.name} {convert_data_type_to_name(field.datatype)}" for field in return_type.fields)})'
    elif installed_pandas and isinstance(return_type, PandasDataFrameType):
        return_sql = f'RETURNS TABLE ({",".join(f"{name} {convert_data_type_to_name(datatype)}" for name, datatype in zip(return_type.col_names, return_type.col_types))})'
    else:
        return_sql = f"RETURNS {convert_data_type_to_name(return_type)}"
    input_sql_types = [convert_data_type_to_name(arg.datatype) for arg in input_args]
    sql_func_args = ",".join(
    ...
    # Since this function is called for UDFs we need to
...
"""
    ...
    api_call_source = api_call_source or "_internal.create_python_udf"
