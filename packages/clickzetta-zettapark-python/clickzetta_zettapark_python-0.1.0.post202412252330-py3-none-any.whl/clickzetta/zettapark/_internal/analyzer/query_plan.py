...
class QueryPlanBuilder:
    ...
    def file_operation_plan(
        self, command: str, file_name: str, volume_path: str, options: Dict[str, str]
    ) -> QueryPlan:
        return self.query(
            file_operation_statement(command, file_name, volume_path, options),
            None,
        )
    ...
    def read_file(
        ...
        select_statement = select_from_path_with_format_statement(
            path,
            file_format,
            pattern,
            schema,
        ...
        queries: List[Query] = [Query(select_statement)]
        if schema:
            schema_query = schema_value_statement(schema)
        else:
            schema_query = select_statement + " LIMIT 0 "
        return QueryPlan(queries, schema_query, session=self.session)
