...
def to_sql(value: Any, datatype: DataType, from_values_statement: bool = False) -> str:
    ...
    if isinstance(value, (list, bytes, bytearray)) and isinstance(datatype, BinaryType):
        if isinstance(value, (list, bytearray)):
            return f"CAST('{binascii.hexlify(bytes(value)).decode()}' AS {convert_data_type_to_name(datatype)})"
        else:
            return f"CAST('{bytes(value).decode('utf-8')}' AS {convert_data_type_to_name(datatype)})"
    ...
    if isinstance(value, dict) and isinstance(datatype, StructType):
        params = [value.get(unquote_name(n), None) for n in datatype.names]
        return (
