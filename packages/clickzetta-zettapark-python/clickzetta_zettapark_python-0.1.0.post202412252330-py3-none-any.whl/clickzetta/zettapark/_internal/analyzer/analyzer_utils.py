...
def file_operation_statement(
    command: str, file_name: str, volume_path: str, options: Dict[str, str]
) -> str:
    if is_volume_uri(volume_path):
        return volume_file_operation_statement(command, file_name, volume_path, options)
    if command.lower() == "put":
        return f"{PUT}{file_name}{SPACE}{volume_path}{SPACE}{get_options_statement(options)}"
    if command.lower() == "get":
        return f"{GET}{volume_path}{SPACE}{file_name}{SPACE}{get_options_statement(options)}"
...
def select_from_path_with_format_statement(
    ...
    schema_string = (
        f"({attribute_to_schema_string(schema)}) " if schema else EMPTY_STRING
    )
    return (
        f"SELECT * "
...
def copy_into_location(
    ...
    **copy_options: Any,
    ...
    """
    COPY INTO { internalVolume | externalVolume | externalLocation }
