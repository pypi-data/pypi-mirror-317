...
class DataType:
    ...
    def accept(self, visitor):
        type_name = self.__class__.__name__[:-4].lstrip("_").lower()
        method = getattr(visitor, f"visit_{type_name}", None)
        if method is None:
            raise NotImplementedError(f"{self.__class__.__name__}.accept()")
        return method(self)
...
class TimestampType(_AtomicType):
    ...
    def __init__(self, timezone: TimestampTimeZone = TimestampTimeZone.LTZ) -> None:
...
class ColumnIdentifier:
    ...
    def __init__(self, normalized_name: str) -> None:
        ...
        self.normalized_name = clickzetta.zettapark._internal.utils.quote_name(
            normalized_name
        )
...
# Add a type hint that can be used to annotate Vector data. Python does not
...
class DataTypeVisitor(Generic[_T]):
    def visit_any(self, t: DataType) -> _T:
        raise NotImplementedError(f"Unsupported type: {t.__class__.__name__}")

    def visit_null(self, t: NullType) -> _T:
        return self.visit_any(t)

    def visit_atomic(self, t: _AtomicType) -> _T:
        return self.visit_any(t)

    def visit_binary(self, t: BinaryType) -> _T:
        return self.visit_atomic(t)

    def visit_boolean(self, t: BooleanType) -> _T:
        return self.visit_atomic(t)

    def visit_date(self, t: DateType) -> _T:
        return self.visit_atomic(t)

    def visit_string(self, t: StringType) -> _T:
        return self.visit_atomic(t)

    def visit_timestamp(self, t: TimestampType) -> _T:
        return self.visit_atomic(t)

    def visit_numeric(self, t: _NumericType) -> _T:
        return self.visit_atomic(t)

    def visit_integral(self, t: _IntegralType) -> _T:
        return self.visit_numeric(t)

    def visit_fractional(self, t: _FractionalType) -> _T:
        return self.visit_numeric(t)

    def visit_byte(self, t: ByteType) -> _T:
        return self.visit_integral(t)

    def visit_short(self, t: ShortType) -> _T:
        return self.visit_integral(t)

    def visit_integer(self, t: IntegerType) -> _T:
        return self.visit_integral(t)

    def visit_long(self, t: LongType) -> _T:
        return self.visit_integral(t)

    def visit_float(self, t: FloatType) -> _T:
        return self.visit_fractional(t)

    def visit_double(self, t: DoubleType) -> _T:
        return self.visit_fractional(t)

    def visit_decimal(self, t: DecimalType) -> _T:
        return self.visit_fractional(t)

    def visit_array(self, t: ArrayType) -> _T:
        return self.visit_any(t)

    def visit_map(self, t: MapType) -> _T:
        return self.visit_any(t)

    def visit_struct(self, t: StructType) -> _T:
        return self.visit_any(t)

    def visit_variant(self, t: VariantType) -> _T:
        return self.visit_any(t)

    def visit_vector(self, t: VectorType) -> _T:
        return self.visit_any(t)

    def visit_pandas(self, t: _PandasType) -> _T:
        return self.visit_any(t)

    def visit_pandas_series(self, t: PandasSeriesType) -> _T:
        return self.visit_pandas(t)

    def visit_pandas_dataframe(self, t: PandasDataFrameType) -> _T:
        return self.visit_pandas(t)
