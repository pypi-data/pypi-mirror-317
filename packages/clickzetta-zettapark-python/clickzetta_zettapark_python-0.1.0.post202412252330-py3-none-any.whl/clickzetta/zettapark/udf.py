...
class UDFRegistration:
    ...
    There are two ways to register a UDF with Zettapark:
        ...
        - Use :func:`~clickzetta.zettapark.functions.udf` or :meth:`register`. By pointing to a
          ...
          Python function will be serialized. If the size of the serialized bytecode is over 8K bytes, it will be uploaded to a volume location as a Python file.
          ...
          the function from elsewhere. However, the limitations of this approach are:
            ...
            * All code inside the function will be executed on every row, so you are not able to
              ...
              to read a file from a volume in a UDF, this file will be read on every row. However,
        ...
        - Use :meth:`register_from_file`. By pointing to a `Python file` or a `zip file containing
          Python source code` and the target function name, Zettapark uploads this file to a volume
    ...
    Example 7
        Creating a UDF from a Python file on an internal volume::

            >>> from clickzetta.zettapark.types import IntegerType
            >>> _ = session.sql("create external volume my_vol ...").collect()
            >>> _ = session.file.put("tests/resources/test_udf_dir/test_udf_file.py", "volume://my_vol", auto_compress=False)
            >>> mod5_udf = session.udf.register_from_file(
            ...     file_path="volume://my_vol/test_udf_file.py",
            ...     func_name="mod5",
    ...
    Example 8
        Use cache to read a file once from a volume in a UDF::
    ...
    def describe(
        ...
        """
        func_args = [convert_data_type_to_name(t) for t in udf_obj._input_types]
        ...
        **kwargs,
        ...
        Args:
            ...
            is_permanent: Whether to create a permanent UDF. The default is ``False``.
                If it is ``True``, a valid ``volume_path`` must be provided.
            volume_path: The volume location where the Python file for the UDF
                and its dependencies should be uploaded. The volume location must be specified
                when ``is_permanent`` is ``True``, and it will be ignored when
                ``is_permanent`` is ``False``. It can be any volume other than temporary
        ...
        check_register_args(
            TempObjectType.FUNCTION, name, is_permanent, volume_path, parallel
        )
    ...
    def register_from_file(
        ...
        *,
        ...
        Args:
            file_path: The path of a local file or a remote file in the volume. See
            ...
            source_code_display: Display the source code of the UDF `func` as comments in the generated script.
                ...
                `func` is originally defined. The default is ``True``.
                If it is ``False``, source code will not be generated or displayed.
            skip_upload_on_content_match: When set to ``True`` and a version of source file already exists on volume, the given source
                file will be uploaded to volume only if the contents of the current file differ from the remote file on volume. Defaults
                to ``False``.
            external_access_integrations: The names of one or more external access integrations. Each
                integration you specify allows access to the external network locations and secrets
                the integration specifies.
    ...
    def _do_register_udf(
        ...
        *,
        ...
        # (e.g., a dependency might not be found on the volume),
        ...
        # python file and raise the exception
            ...
            if raised:
                cleanup_failed_permanent_registration(
                    self._session, upload_file_volume_path, volume_path
                )
