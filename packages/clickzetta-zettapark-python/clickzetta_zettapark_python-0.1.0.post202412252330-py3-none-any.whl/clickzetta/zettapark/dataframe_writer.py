...
class DataFrameWriter:
    ...
    def copy_into_location(
        ...
        **copy_options: Optional[str],
        ...
        """Executes a `COPY INTO <location> <https://doc.clickzetta.com/>`__ to unload data from a ``DataFrame`` into one or more files in a volume or external volume.

        Args:
            location: The destination volume path.
        ...
        Example::
            ...
            >>> # save this dataframe to a parquet file on the session volume
            >>> df = session.create_dataframe([["John", "Berry"], ["Rick", "Berry"], ["Anthony", "Davis"]], schema = ["FIRST_NAME", "LAST_NAME"])
            >>> remote_file_path = f"{session.get_session_volume()}/names.parquet"
        ...
        """
        volume_path = normalize_remote_file_or_dir(location)
