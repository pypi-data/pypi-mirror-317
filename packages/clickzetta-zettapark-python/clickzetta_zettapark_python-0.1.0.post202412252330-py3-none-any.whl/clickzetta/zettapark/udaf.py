...
class UDAFRegistration:
    ...
    Example 3
        Create a permanent UDAF with a name and call it in SQL:
            ...
            >>> @udaf(is_permanent=True, name="sum_udaf", replace=True, volume_path="volume://my_vol")
            ... class PythonSumUDAF:
            ...     def __init__(self) -> None:
            ...         self._sum = 0
            ...
    ...
    Example 7
        Creating a UDAF from a Python file on an internal volume:

            >>> from clickzetta.zettapark.functions import udaf
            >>> from clickzetta.zettapark.types import IntegerType
            >>> _ = session.sql("create external volume my_vol ...").collect()
            >>> _ = session.file.put("tests/resources/test_udaf_dir/test_udaf_file.py", "volume://my_vol", auto_compress=False)
            >>> sum_udaf = session.udaf.register_from_file(
            ...     file_path="volume://my_vol/test_udaf_file.py",
    ...
    def describe(
        ...
        """
        func_args = [convert_data_type_to_name(t) for t in udaf_obj._input_types]
