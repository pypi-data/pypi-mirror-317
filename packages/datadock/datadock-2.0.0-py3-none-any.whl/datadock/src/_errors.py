...
Module: _errors.py
========================
DataDock Error handling to display status codes and error messages
"""

from typing import Optional, Dict, Union, cast


class DataDockError(Exception):
    """
    Base exception class for Edgar SEC API errors.

    Attributes:
    message: Optional error message.
    headers: Optional HTTP headers from the response.
    http_body: Optional HTTP body from the response.
    status_code: Optional HTTP status code from the response.
    error_code: Optional error code.
    request_id: Optional request ID from the headers.
    """
    ...
    def __init__(
        ...
        """
        Initialize a new instance of DataDockError.

        If http_body is provided and can be decoded, it will be stored as a string in self.http_body.
        Otherwise, it will be set to "Unable to decode body as utf-8. Please try again".
        ...
        super(DataDockError, self).__init__(self._format_message())
...
class APIConnectionError(DataDockError):
    """
    Connection Error
    """
...
class IdentityError(DataDockError):
    """
    Identity Error
    """

    def __init__(
        self, message: str, status_code: int = 403, error_code: str = "identity_error"
    ) -> None:
        super().__init__(
            message=message, status_code=status_code, error_code=error_code
        )
...
class TypeValueError(DataDockError):
    """
    Type Value Error
    """
...
class InvalidRequestMethodError(DataDockError):
    """
    Request Time Error
    """
...
class DataDockServerError(DataDockError):
    """
    Server Error
    """
    ...
    def __init__(
        ...
        message: str,
        status_code: int,
        error_code: str = "server_error. See Admin",
    ) -> None:
        # Check if status code is a server error
        if status_code and 500 <= status_code < 600:
            error_code = "server_error. See Admin"
        super().__init__(
            message=message, status_code=status_code, error_code=error_code
        )
