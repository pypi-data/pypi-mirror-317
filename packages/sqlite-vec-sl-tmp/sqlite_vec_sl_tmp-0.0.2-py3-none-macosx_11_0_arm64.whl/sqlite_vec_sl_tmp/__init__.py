...
def loadable_path():
  """ Returns the full path to the sqlite-vec-sl-tmp loadable SQLite extension bundled with this package """

  loadable_path = path.join(path.dirname(__file__), "vec0")
  return path.normpath(loadable_path)

def load(conn: sqlite3.Connection)  -> None:
  """ Load the sqlite-vec-sl-tmp SQLite extension into the given database connection. """
