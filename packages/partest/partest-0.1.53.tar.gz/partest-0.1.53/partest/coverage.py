    ...
    async def wrapper(*args, **kwargs):
        ...
        if defining_url:
            ...
            for path in paths_info:
                # Убедитесь, что path - это объект Path
                if hasattr(path, 'parameters'):
                    for param in path.parameters:  # Изменено на атрибут
                        if param.type == 'path':
        ...
        if method is not None and final_endpoint is not None:
            found_match = False
            for path in paths_info:
                if path.method == method and path.path == final_endpoint:  # Изменено на атрибуты
            ...
            for path in paths_info:
                key = (path.method, path.path, path.description)  # Изменено на атрибуты
