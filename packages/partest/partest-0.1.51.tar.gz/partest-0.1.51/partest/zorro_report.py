...
def zorro():
    ...
    for (method, endpoint, description), count in call_count.items():
        ...
        swagger_title = next((path.title for path in paths_info if path.path == endpoint), "Unknown API")
    ...
    # Добавляем общую оценку покрытия в отдельную вкладку
    allure.attach(summary, name='Общая оценка покрытия', attachment_type=allure.attachment_type.TEXT)

    # Добавляем отчеты для каждого Swagger API в отдельные вкладки
    for swagger_title, lines in swagger_reports.items():
        allure.attach("\n".join(lines), name=f'Отчет по {swagger_title}', attachment_type=allure.attachment_type.TEXT)
