...
class OpenAPIParser:
    ...
    def resolve_ref(self, ref):
        ...
        # Check if the reference is to an external file
        if ref.startswith('./') or ref.startswith('../'):
            ...
            # Resolve the internal reference within the external file
            internal_ref = ref.split('#')[1]  # Get the part after #
    ...
    def resolve_internal_ref(self, internal_ref, swagger_dict):
        """Resolves an internal reference within a given swagger dictionary."""
        if not internal_ref:
            raise ValueError("Internal reference cannot be empty.")

        parts = internal_ref.split('/')
        print(f"Resolving internal reference: {internal_ref}")  # Debugging line
        resolved = swagger_dict
        for part in parts:
            print(f"Current part: '{part}'")  # Debugging line
            if part:  # Check if part is not empty
                resolved = resolved.get(part)
                if resolved is None:
                    raise KeyError(
                        f"Reference '{internal_ref}' could not be resolved in the provided swagger dictionary.")
            else:
                raise ValueError("Invalid reference part encountered: empty string.")
    ...
    def resolve_param(self, param):
        ...
        if '$ref' in param:
            ...
            resolved_param = self.resolve_ref(param['$ref'])
            return Parameter(
