...
class RequestBody:
    ...
    @staticmethod
    def resolve_schema(schema_ref, swagger_dict, base_path=None):
        return OpenAPIParser.resolve_ref(schema_ref, swagger_dict, base_path)
...
class OpenAPIParser:
    ...
    @staticmethod
    def resolve_ref(ref, swagger_dict, base_path=None):
        """Resolves a reference to a component in the OpenAPI specification."""
        if ref.startswith('#/'):  # Локальная ссылка
            parts = ref.split('/')
            resolved = swagger_dict
            for part in parts[1:]:
                resolved = resolved.get(part, {})
            return resolved
        elif ref.startswith('./'):  # Внешняя ссылка
            if base_path is None:
                raise ValueError("Base path is required to resolve external references.")
            external_file, ref_path = ref.split('#', 1)
            external_path = os.path.join(base_path, external_file)
            with open(external_path, 'r', encoding='utf-8') as f:
                external_dict = yaml.safe_load(f)
            # Разрешаем ссылку внутри внешнего файла
            return OpenAPIParser.resolve_ref(f'#{ref_path}', external_dict)
        else:
    ...
    def extract_paths_info(self, base_path=None):
        ...
        for path, methods in paths.items():
            for method, details in methods.items():
                ...
                request_body = self.extract_request_body(details, base_path=base_path)  # Передаем base_path
                responses = self.extract_responses(details, base_path=base_path)  # Передаем base_path
    ...
    def extract_request_body(self, details, base_path=None):
        ...
        if 'requestBody' in details:
            ...
            if 'application/json' in content:
                if 'schema' in content['application/json'] and '$ref' in content['application/json']['schema']:
                    ...
                    resolved_schema = RequestBody.resolve_schema(schema_ref, self.swagger_dict, base_path=base_path)
                    content['application/json']['schema'] = resolved_schema
                return RequestBody(content['application/json'])
        return None

    def extract_responses(self, details, base_path=None):
            ...
            for code, response in details['responses'].items():
                if 'content' in response and 'application/json' in response['content']:
                    ...
                    if 'schema' in content and '$ref' in content['schema']:
                        ...
                        resolved_schema = Response.resolve_schema(schema_ref, self.swagger_dict, base_path=base_path)
...
class SwaggerSettings:
    ...
    def load_swagger(self):
        ...
        for source_type, path in self.swaggers:
            parser, base_path = OpenAPIParser.load_swagger_yaml(source_type, path)  # Теперь возвращает tuple
            extracted_data = parser.extract_paths_info(base_path=base_path)  # Передаем base_path
