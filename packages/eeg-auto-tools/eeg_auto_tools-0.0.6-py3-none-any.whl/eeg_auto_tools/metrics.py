...
def calculate_SN_ratio(epochs, option='neighbours', mode='log'):
    ...
    elif option == 'mean_channels':
        clean_signal = np.mean(epochs_data, axis=1, keepdims=True).repeat(n_channels, axis=1)
    elif option == 'median_channels':
        clean_signal = np.median(epochs_data, axis=1, keepdims=True).repeat(n_channels, axis=1)
...
def mae(y_true, y_pred):
    # Отклоненик в среднем в тех же единицах измерения
...
def cos_distance(y_true, y_pred):
    # Сравнивает направления векторов
...
def compute_psd(signal, sfreq):
    ...
    spectrum = np.abs(np.fft.rfft(signal))**2  # Плотность мощности
    # Нормировка PSD на длину сигнала и частоту дискретизации
...
def calculate_statistics(signals, z_score=1.96):
    ...
    ci_95 = z_score * se  # 95% доверительный интервал
    return mean, ci_95
...
def rythm_metric(epochs, picks):
    ...
    # Функция для фильтрации данных epochs по заданному диапазону частот
    def filter_band(epochs, band):
        return epochs.copy().filter(band[0], band[1])

    # Функция для получения сигналов, усреднения и вычисления статистики
    ...
    # Добавление легенд и улучшение оформления
    for ax in axs.flat:
        ax.legend()
        ax.grid(True)
