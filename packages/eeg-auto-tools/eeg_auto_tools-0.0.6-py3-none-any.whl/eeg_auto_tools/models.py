...
class Encoder(nn.Module):
    def __init__(self, input_dim, latent_dim):
        ...
        self.fc_mu = nn.Linear(64, latent_dim)  # Среднее значения
        self.fc_logvar = nn.Linear(64, latent_dim)  # Логарифм дисперсии
...
class Feature_Extractor():
    def __init__(self, epochs, latent_dim=5, lr=1e-3, alpha=1.0):
        # Гиперпараметры
        self.alpha=alpha  # Коэффициент для infomax
        self.lr = lr
        input_dim = epochs.get_data().shape[1] * epochs.get_data().shape[2]  # Учитываем все каналы и временные точки
        ...
        self.epochs_data = epochs_data.view(epochs_data.shape[0], -1)  # Векторизуем входные данные
    ...
    def train(self,):
        ...
        for epoch in range(n_epochs):
            ...
            self.optimizer.step()
            if (epoch + 1) % 10 == 0:
                print(f"Эпоха [{epoch+1}/{n_epochs}], Потери: {loss.item():.4f}")
...
def latency_and_peaks(epochs, component):
    ...
    if component not in peak_windows:
        raise ValueError(f"Неизвестный компонент: {component}. Доступные компоненты: {list(peak_windows.keys())}")
    ...
    # Добавляем значения в список для каждого канала
