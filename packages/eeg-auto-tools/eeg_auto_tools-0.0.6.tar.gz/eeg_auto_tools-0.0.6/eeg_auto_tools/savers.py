...
def plot_clusters(clusters, clusters_avg, clusters_max, clusters_min, ch_names, ch_pos, correlation_values):
    figs = []
    # Создаем массив данных, где каждому каналу назначается значение кластера
    data = np.full(len(ch_names), np.nan)
    # Создаем маску для электродов в кластерах
    mask = np.zeros(len(ch_names), dtype=bool)
    # Создаем отображение имени канала в индекс
    ch_name_to_idx = {ch: idx for idx, ch in enumerate(ch_names)}

    for cluster_idx, cluster in enumerate(clusters):
        cluster_value = cluster_idx + 1  # Значения кластеров начинаются с 1
        for ch_name in cluster:
            if ch_name in ch_name_to_idx:
                idx = ch_name_to_idx[ch_name]
                data[idx] = cluster_value
                mask[idx] = True  # Помечаем электрод как принадлежащий кластеру
    ...
    # Создаем пользовательскую легенду
    legend_elements = []
    ...
    figs.append(fig)
    plt.close(fig)
    # Строим гистограмму распределения корреляций
    fig = plt.figure(figsize=(8, 6))
    plt.hist(np.abs(correlation_values), bins=100, edgecolor='black')
    plt.title('Гистограмма корреляций пар электродов')
    plt.xlabel('Коэффициент корреляции')
    plt.ylabel('Количество пар электродов')
    figs.append(fig)
    plt.close(fig)
    return figs
...
def ASSR_erp_plot(ERP_c, ERP_n, noised_c_idx, noised_n_idx, roi, comp, baseline=0.2, save=None, vis=False):
    ...
    # Среднее по усреднённым электродам ERP
    ...
    # Среднее по эпохам и электродам
...
def compared_snr(snr_matrices, names):
    ...
    for snr_matrix, name, color in zip(snr_matrices, names, colors):
        ...
        color = kde.collections[-1].get_facecolor()[0]  # Цвет в формате RGBA
...
def plot_noise_data(raw, noisy_channel_flags, n_channels, threshold_noise, integral_curve):
    # Создание фигуры и GridSpec для размещения подграфиков
    fig = plt.figure(figsize=(18, 8))
    gs = GridSpec(1, 3, width_ratios=[5, 1, 3], figure=fig)

    # Визуализация матрицы шумных каналов
    ax0 = fig.add_subplot(gs[0, 0])
    sns.heatmap(noisy_channel_flags, vmin=0, vmax=1, cmap='viridis', 
                cbar_kws={'label': 'Среднее значение PSD'}, ax=ax0)
    ax0.set_xlabel('Окна')
    ax0.set_ylabel('Каналы')
    ax0.set_title('Матрица средних значений PSD по каналам и окнам')
    ax0.set_yticks(np.arange(n_channels) + 0.5)
    ax0.set_yticklabels(raw.ch_names, rotation=0)
    
    # Визуализация интегральной кривой
    ax1 = fig.add_subplot(gs[0, 1], sharey=ax0)  # Используем sharey для общего Y между ax0 и ax1
    ax1.plot(integral_curve, np.arange(n_channels)+0.5, color='blue', label='Интегральная кривая вероятностей')
    ax1.axvline(threshold_noise, color='red', linestyle='--', label=f'Порог - {threshold_noise}')
    ax1.set_xlim((0, 1))
    ax1.set_xlabel('Распределение вероятностей')
    ax1.set_title('Интегральная кривая')
    ax1.legend(loc='right', fontsize='small')
    ax1.grid(True)

    # Визуализация топомапы
    ax2 = fig.add_subplot(gs[0, 2])  # Отдельный подграфик для топомапы
    pos = np.array([raw.info['chs'][i]['loc'][:2] for i in range(raw.info['nchan'])])  # X и Y координаты
    mne.viz.plot_topomap(
        integral_curve, pos, axes=ax2, show=False, cmap='RdBu_r', vlim=(0,1),
        names=raw.ch_names
    )
    ax2.set_title('Топомапа интегральной кривой')
    
    plt.tight_layout()
    plt.show()

def plot_topomap(coherence, band_name, info, title):
    # Усреднение когерентности по всем парам каналов
    coherence_mean = coherence.mean(axis=1)
    # Создание маски для каналов
    ...
    # Визуализация топографического распределения
...
def char_plot(spec_con, epochs, method, mode, cwt_freqs):
    ...
    plt.colorbar(label=f'Когерентность {method} {mode}')
    plt.xlabel('Time (s)')
    plt.ylabel('Frequency (Hz)')
    plt.title(f'Когерентный анализ {method} с использованием CWT Morlet')
    plt.show()
    plt.savefig(f'{method} {mode}.png')

def plot_circular_connectivity(con, epochs, threshold=0.5, title='Circular Connectivity Diagram'):
    con = np.mean(np.abs(con), axis=2)  # Усреднение по времени
    con[con < threshold] = 0  # Установить порог для визуализации
