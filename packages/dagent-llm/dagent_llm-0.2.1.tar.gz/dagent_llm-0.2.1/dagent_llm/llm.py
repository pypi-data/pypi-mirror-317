...
class LLM:
    ...
    def chat(self, message, role="human", add_to_history=False):
        if role == "human":
    ...
    def choose(self, options, prompt, option_type=None, need_reason=False, multiple=False, add_to_history=False, max_try=3, examples=[], notes=[]):
        """
    ...
    def choose_with_args(self, options, prompt, option_type, need_reason=False, multiple=False, add_to_history=False, examples=[], notes=[]):
        """
        ...
        if not need_reason:
            ...
            return response.choice, response.args, response.reason
    ...
    def function_choose(self, functions_info, prompt, need_reason=False, multiple=False, add_to_history=False, max_try=3, examples=[], notes=[]):
        ...
        """
        if need_reason:
            raw_add_to_history = add_to_history
            add_to_history = True
        ...
        while max_try > 0:
            ...
            if set(needed_args) == set(parsed_args.keys()):
                if not need_reason:
                    if not raw_add_to_history:
                        self.history.pop() # remove the last message
        ...
        while max_try > 0:
            try:
                ...
                class JSONList(BaseModel):
                    ...
                    # 说明
                    description: str = Field(..., description="The list of items")
                # 生成结构化的语言模型
    ...
    def get_code(self, text, code_type=["python","py",""]):
