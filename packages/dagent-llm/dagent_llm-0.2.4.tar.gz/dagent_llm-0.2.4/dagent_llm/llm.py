...
class LLM:
    def __init__(self, llm_server="ollama", redundancy=-1, max_retries=2, temperature=0, api_key=None, base_url=None, model_name=None, history=[], sql_setup=None):
        ...
        if self.redundancy > 0:
            ...
            console.rule(f"DAgent LLM Version: {INFO.__version__} | Update: {INFO.__update__}")
        ...
        for message in history:
            if isinstance(message, (HumanMessage, SystemMessage, AIMessage)):
                ...
                if isinstance(message, dict):
                    ...
                    elif message["role"]=="ai" or message["role"]=="bot" or \
                             message["role"]=="agent" or message["role"]=="model" or  \
                             message["role"]=="assistant" or message["role"]=="llm":
                        self.history.append(AIMessage(content=message["content"]))
                    elif message["role"]=="system" or message["role"]=="sys" or message["role"]=="admin":
    ...
    def chat(self, message: str, role: str="human", add_to_history: bool=True):
        """
    ...
    def choose(self, options, prompt, option_type=None, need_reason=False,
                multiple=False, add_to_history=False, max_try=3, examples=[], notes=[]):
        """
        ...
        with console.status("[bold green] LLM is choosing..."):
            ...
            if not option_type:
                try:
                    ...
                    if self.redundancy > 0:
                        cosole.print(f"Option Type is not provided. Using LLM to generate it.")
            ...
            for attempt in range(max_try):
                ...
                if choices:
                    ...
                    if self.redundancy > 0:
                        console.print(f"[red]Invalid choices: {response.choice}. Attempt {attempt + 1}/{max_try}.")
            if not result:
                if self.redundancy > 0:
                    console.print(f"[red]Max retries reached. No valid options chosen.")
    ...
    def choose_with_args(self, options, prompt, option_type, 
                            need_reason=False, multiple=False, 
                            add_to_history=False, max_try=3, 
                            examples=[], notes=[]):
        """
        ...
        if need_reason:
            class Choice(BaseModel):
                choice: str = Field(description="name: The chosen option" + 
                ("(s), please separate multiple options with commas" if multiple else ""))
                reason: str = Field(description="The reason for the choice")
                args: str = Field(description="The arguments for the chosen option in the format \
    ...
    def function_choose(self, functions_info, prompt, need_reason=False, multiple=False, add_to_history=False, max_try=3, examples=[], notes=[]):
        """
        ...
        while max_try > 0:
            ...
            if set(needed_args) == set(parsed_args.keys()):  
                return {"function_name": function_name, "args": parsed_args, "reason": reason}
            else:
            ...
            if self.redundancy > 0:
                console.print(f"[red]Missing or incorrect arguments for function '{function_name}'. Needed: {set(needed_args)}, Provided: {set(parsed_args.keys())}")
                console.print(f"[red]Attempt {max_try}/{max_try}")

    def generate_jsons(self, prompt: str, fields: List[Dict[str, Any]], add_to_history: bool = False, max_try: int = 3):
        """
        ...
        while max_try > 0:
            try:
                ...
                for _field in fields:
                    ...
                    # 动态设置字段的注解和默认值
                    annotations[_field["name"]] = (str, Field(..., description=_field["description"] + "\nFor example: " + _field["example"]))
                ...
                if self.redundancy > 0:
                    ...
                    console.print(f"[green]Generating JSON...")
    ...
    def generate_json(self, prompt: str, fields: List[Dict[str, Any]], add_to_history: bool = False, max_try: int = 3):
        """
        动态创建 Pydantic BaseModel 子类，并生成结构化 JSON 输出。
    ...
    def get_code(self, text, code_type=["python","py",""]) -> List[str]:
        ...
        for code_type_item in code_type:
            ...
            for code in r:
                ...
                # 利用re将已提取的内容替换为空
                text = re.sub(rf"```{code_type_item}(.*?)```", "", text, flags=re.DOTALL)
        return code_list
    ...
    def choose_multi_process(self,
                ...
                notes_list,
                num_processes=4):
        list_lengths = [len(options_list), len(prompt_list), len(option_type_list), len(need_reason_list),
                        len(multiple_list), len(max_try_list), len(examples_list), len(notes_list)]
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_processes) as executor:
            future_to_index = {
                executor.submit(
                    ...
                    self.api_key,
                    self.base_url
                ): i for i in range(len(options_list))
            }
    ...
    # Multi-process
    def choose_with_args_multi_process(self,
                options_list,
                prompt_list, 
                option_type_list, 
                need_reason_list, 
                ...
                num_processes=4):
        list_lengths = [len(options_list), len(prompt_list), len(option_type_list), len(need_reason_list),
                        len(multiple_list),  len(examples_list), len(notes_list)]
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_processes) as executor:
            future_to_index = {
                executor.submit(
                    ...
                    self.api_key,
                    self.base_url
                ): i for i in range(len(message_list))
            }
            for future in concurrent.futures.as_completed(future_to_index):
                idx = future_to_index[future]
                try:
    ...
    def analyze_content(self,content, dimension, standard, options,
                            multiple=False,
                            add_to_history=False, notes=[], examples=[]):
        ...
        r = self.choose(options, prompt, f"Does the content meet the requirements for the {dimension} dimension?",
                        need_reason=True,
                        multiple=multiple,
                        add_to_history=add_to_history,
                        notes=notes,
...
def chat_task(message, role, model, temperature, max_retries, api_key, base_url, init_history=[]):
    model = ChatOpenAI(
        model=model,
...
def choose_with_args_task(options, prompt, option_type, need_reason, multiple, examples, notes, model, temperature, max_retries, api_key, base_url):
    model = ChatOpenAI(
        model=model,
        temperature=float(temperature),
        max_retries=int(max_retries),
...
def choose_task(options, prompt, option_type, need_reason, multiple, max_try, examples, notes, model, temperature, max_retries, api_key, base_url):
    model = ChatOpenAI(
        model=model,
        temperature=float(temperature),
        max_retries=int(max_retries),
