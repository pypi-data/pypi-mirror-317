...
class GB(list):
  ...
  def algorithm1(self, F, output=False):
    assert Monom.cmp != Monom.POTdeglex and Monom.cmp != Monom.TOPdeglex # Change
    ...
    while self.F or self.B:
      ...
      while i < len(self.F):
        ...
        Poly(self.F[i]).NFhead(self) # Change
        if output: print('->', self.F[i].lm() if self.F[i] else '0')
        if not self.F[i]:
          del self.F[i]
        else:
          Poly(self.F[i]).pp() # Change
          if not p or Poly(self.F[i]).lm().cmp(p.lm()) < 0: # Change
            p = self.F[i]
          i += 1
      if not p:
        self.B.sort()
