        ...
        while cnt >= 0:
            try:
                if PROJECT_TYPE == 'bot':
                    ...
                    sql = "SELECT UB_USERNAME, UB_FISRTNAME, UB_BOTUSERNAME, UB_CHANNELLINK FROM UB WHERE UB_TID=?"
                    data_bot = await db_select(sql, (ENT_TID,), BASE_D)
                    UB_USERNAME, UB_FISRTNAME, UB_BOTUSERNAME, UB_CHANNELLINK = data_bot[0]

                    if UB_USERNAME:
                        ENT_USERNAME = f"@{UB_USERNAME}"
                        ENT_LINK = f"https://t.me/{UB_USERNAME}"
                    else:
                        ENT_USERNAME = UB_FISRTNAME
