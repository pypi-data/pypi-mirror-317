...
# region app
async def podcast_start_app(UB_TID, lz, ENT_TID, POST_TEXT, POST_MEDIA):
    try:
        for item in POST_MEDIA:
            ...
            try:
                ...
                is_video = POST_TYPE in ['document'] and POST_FILENAME.lower().endswith(('.mp4', '.mov'))
                ...
                if POST_TYPE in ['audio', 'voice'] or is_audio:
                    ...
                    try:
                        ...
                        logger.info(log_ % str(e))
                        await asyncio.sleep(round(random.uniform(0, 1), 2))
                elif POST_TYPE in ['video', 'video_note'] or is_video:
                    file = await extra_bot.get_file(POSTB_FID)
                    await extra_bot.download_file(file.file_path, POST_FILENAME)
                ...
                logger.info(log_ % f"py_tg_calls_fun start: {ENT_TID=}, {POST_FILENAME=}, {duration=}")
                await tgcalls_app(app, lz, ENT_TID, stream)
            except Exception as e:
                logger.info(log_ % str(e))
                await asyncio.sleep(round(random.uniform(0, 1), 2))
            finally:
                if POST_FILENAME and os.path.exists(POST_FILENAME): os.remove
                await extra_bot.session.close()
    except TelegramRetryAfter as e:
        logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
        await asyncio.sleep(e.retry_after + 1)
...
async def tgcalls_app(app, lz, ENT_TID, stream, duration=7200):
