...
class ThreadABC(metaclass=abc.ABCMeta):
    ...
    async def send_message(
        ...
        Example:
            ...
            >>> message_id = thread.send_message("A message", mentions=[mention])

            Reply to the message.

            >>> thread.send_message("A reply", reply_to_id=message_id)
    ...
    async def send_files(self, files: Iterable[Tuple[str, str]]):
        ...
        """
        return await self.send_message(text=None, files=files)
...
class Thread(ThreadABC):
    ...
    #: The session to use when making requests.
    session: _session.Session #type: ignore
    #: The unique identifier of the thread.
    id: str = field(converter=str) #type: ignore
