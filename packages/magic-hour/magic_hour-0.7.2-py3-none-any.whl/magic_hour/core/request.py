...
def to_encodable(*, item: Any, dump_with: Type) -> Any:
    ...
    """
    filtered_item = _filter_not_given(item)
...
def _filter_not_given(value: Any) -> Any:
    ...
    if isinstance(value, NotGiven):
        ...
        return {
            k: _filter_not_given(v)
            for k, v in value.items()
            if not isinstance(v, NotGiven)
        }
    elif isinstance(value, (list, tuple)):
        return type(value)(
            _filter_not_given(item) for item in value if not isinstance(item, NotGiven)
        )
    return value
