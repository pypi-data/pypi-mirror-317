...
def make_application(
    ...
    async def redirect(
        ...
        if url is None:
            if redirect_first and request.method.upper() in ("GET", "HEAD"):
                response = redirect(base_url + str(request.url))
                remote_hostname = urlsplit("http://" + request.host).hostname or "localhost"
                if base_url_is_localhost:
                    if is_private(remote_hostname) if resolve_localhost else is_localhost(remote_hostname):
                        return response
                elif base_url_is_private:
                    if is_private(remote_hostname):
                        return response
                else:
                    return response
