...
class FileUpdater:
    ...
    async def get_signed_url(self, hashname):
        """通过API获取带有签名的URL"""
        up:UpInfo = UpInfo.getMaster()
        up.set_par(hashname)
        response = await up.send_back("apisimple/mes/mes_message/generateSignedUrl", up.to_url_encode())
        return response
...
def update_files(local_directory, remote_base_url):
    updater = FileUpdater(local_directory, remote_base_url)
