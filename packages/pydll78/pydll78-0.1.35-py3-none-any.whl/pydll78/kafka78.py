...
class Kafka78:
    ...
    async def setup(self):
        ...
        if self.producer is None or not self.producer._running:
    ...
    async def get_remaining_messages(self, topic: str, group_id: str="testgroup") -> int:
        ...
        while not self.consumer.assignment() and (asyncio.get_event_loop().time() - start_time) < 5:
    ...
    async def setup_consumer(self, topic,groupid="testgroup",client_id="",max_poll_records=10):
        ...
        self.consumer = AIOKafkaConsumer(
            ...
            max_poll_records=max_poll_records,  # 设置每次 poll 拉取的最大消息数量
            session_timeout_ms=300000,    # 5分钟
            rebalance_timeout_ms=600000   # 10分钟
        )
    ...
    async def send(self, data, topic):
        ...
        if data:
            # 发送数据到指定的 topic
            return await self.producer.send_and_wait(topic, value=data)
            # await future
...
async def main():
    ...
    bootstrap_servers='192.168.31.181:30008'
    # 使用上下文管理器初始化生产者并发送数据
    async with Kafka78(bootstrap_servers) as kafka78:
        # 发送 3 条数据到 `test-topic`
        data1 = {"key": "value1", "message": "Hello from message 4"}
        data2 = {"key": "value2", "message": "Hello from message 5"}
        data3 = {"key": "value3", "message": "Hello from message 6"}
 
        tmp=await kafka78.send(data1, 'test-topic')
        print (tmp)
