...
try:
    from collections.abc import Iterable, MutableMapping, \
        MutableSequence  # pylint: disable=no-name-in-module
except ImportError:
...
class JsonVersionConverter(JsonDataTypeConverter):
    ...
    def __init__(self, version_spec=None):
        self._version_spec = semantic_version.SimpleSpec(
            version_spec) if isinstance(version_spec, string) else version_spec
...
class JsonVersionSpecConverter(JsonDataTypeConverter):
    ...
    def convert_from(self, data_type, value):
        ...
        assert isinstance(data_type, JsonString) and isinstance(value, string)  # pylint: disable=unidiomatic-typecheck
        try:
            value = semantic_version.SimpleSpec(value)
        except ValueError:
    ...
    any_version = semantic_version.SimpleSpec('*')
