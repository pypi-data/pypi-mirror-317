...
try:
    from collections.abc import Iterable, Mapping  # pylint: disable=no-name-in-module
except ImportError:
    from collections import Iterable, Mapping  # pylint: disable=no-name-in-module

from collections import OrderedDict, deque  # pylint: disable=no-name-in-module
...
class AppInstallation(object):
    ...
    # Ensure `==` is included for exact matches
    def format_version(self, version):
        # Check if the version starts with any of the allowed prefixes
        valid_prefixes = ("==", "~", ">=", "<=", ">", "<")
        if not version.startswith(valid_prefixes):
            return "=={}".format(version)   # Add `==` if missing
        return version
    ...
    def reset_version_range(self):
        ...
        if not version_range:
            self._version_range = semantic_version.SimpleSpec('*')
        else:
            version_range = [self.format_version(version.strip()) for version in version_range if version.strip()]
            version_range = ",".join(version_range)
            self._version_range = semantic_version.SimpleSpec(version_range)
        ...
        for name in self._dependencies:
            try:
                ...
                if name in self.optional_dependencies:
                    ...
                    raise KeyError('Cannot resolve dependency for app ', encode_string(self.qualified_id), ': ', name)
            else:
        ...
        def visit(app_source, app_dependencies, app_dependents):
            ...
            if app_source is root:
                ...
                try:
                    ...
                    version = semantic_version.SimpleSpec(*(string(dependency.version)
