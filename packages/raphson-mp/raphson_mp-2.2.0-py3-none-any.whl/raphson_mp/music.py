...
async def get_cover(
    ...
    # If multiple requests for an album cover come in at the same time, only one should continue
    # to download a cover and put it in the cache. Then, the other requests can just read the
    # cover from cache.
    # The retrieve_or_store function also solves this issue, but we can't use it here.
    async with COVER_LOCK:
        ...
        log.info("Cover thumbnail not cached, need to download album cover image: %s - %s meme:%s", artist, album, meme)
        ...
        async for cover_bytes in get_possible_covers(artist, album, meme):
            with tempfile.TemporaryDirectory(prefix="music-cover") as temp_dir:
                ...
                image_types = list(itertools.product(ImageFormat, ImageQuality))

                async def generate_thumbnail(img_format2: ImageFormat, img_quality2: ImageQuality):
                    output_path = Path(temp_dir, "output" + img_quality2.value + img_format2.value)
                ...
                try:
                    images = await asyncio.gather(*[generate_thumbnail(*img_type) for img_type in image_types])
                except Exception:
                    log.warning(
                        "Failed to generate thumbnail, image is probably corrupt. Trying another image.", exc_info=True
                    )
                    continue

                return_data: bytes | None = None
                for image_data, (img_format2, img_quality2) in zip(images, image_types):
                    await cache.store(cache_key + img_quality2.value + img_format2.value, image_data, cache.HALFYEAR)
                    if img_format2 == img_format and img_quality2 == img_quality:
...
class Track:
    ...
    async def _transcoded_audio(self, audio_format: AudioFormat) -> cache.CacheData:
        ...
        return cache.CacheData(audio_data, cache.HALFYEAR)
    ...
    async def transcoded_audio(self, audio_format: AudioFormat) -> bytes:
        ...
        return await cache.retrieve_or_store(
            f"audio{audio_format.value}{self.relpath}", self._transcoded_audio, audio_format
        )
...
class Playlist:
    ...
    def tracks(self) -> list[Track]:
        tracks: list[Track] = []
        for relpath, mtime, duration in self.conn.execute(
            "SELECT path, mtime, duration FROM track WHERE playlist = ?", (self.name,)
        ):
