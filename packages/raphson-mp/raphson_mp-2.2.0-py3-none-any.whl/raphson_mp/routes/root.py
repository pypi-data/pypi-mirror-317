...
async def route_csp_reports(request: web.Request):
    ...
    if len(data) > 1000:
        log.warning("Received large Content-Security-Policy: %s bytes", len(data))
    else:
...
async def route_error_report(request: web.Request):
    data = cast(bytes, await request.json())
    if len(data) > 1000:
        log.warning("Received large error report: %s bytes", len(data))
    else:
