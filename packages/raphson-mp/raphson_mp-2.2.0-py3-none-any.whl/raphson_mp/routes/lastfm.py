...
from raphson_mp import auth, db, response
from raphson_mp.decorators import route, simple_route


@simple_route("/callback")
...
async def connect(request: web.Request, _conn: Connection, user: auth.User):
    ...
    with db.connect() as writable_conn:
        cast(auth.StandardUser, user).conn = writable_conn
...
async def disconnect(_request: web.Request, _conn: Connection, user: auth.User):
    with db.connect() as writable_conn:
        writable_conn.execute("DELETE FROM user_lastfm WHERE user=?", (user.user_id,))
