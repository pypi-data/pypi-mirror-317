document.addEventListener('DOMContentLoaded', () => {
    ...
    function registerListener() {
        ...
        if (document.visibilityState == 'visible'
            ...
            updateSyncedLyrics(); // also trigger immediate update, especially necessary when audio is paused
        } else {
    ...
    document.getElementById('album-cover-box').addEventListener('click', () => {
        ...
        lyricsSetting.dispatchEvent(new Event('change'));
    });

    // Handle lyrics setting being changed
    lyricsSetting.addEventListener('change', () => {
        replaceLyrics();
