...
async def get_random_playlist(client: RaphsonMusicClient) -> Playlist:
...
async def get_random_track(client: RaphsonMusicClient) -> Track:
    playlist = await get_random_playlist(client)
...
async def test_share(client: RaphsonMusicClient):
    ...
    assert share.code
    assert await share.audio()
    assert await share.cover()
