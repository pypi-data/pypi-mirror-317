...
def _serialize_value(value: Any) -> str:
    ...
    - If the value is serializable (e.g., can be converted to JSON), serialize it.
    - If JSON serialization fails, attempt to use str().
    - If all serialization attempts fail, return a default placeholder.
    """
...
def _extract_span_attributes(data: Any, **kwargs: Any) -> Dict[str, str]:
    ...
    if kwargs:
        span_attributes["kwargs"] = _serialize_value(kwargs)
...
class MyScaleCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        self._log.info(
            "on chain start run_id: %s parent_run_id: %s, inputs: %s, tags: %s, metadata: %s, kwargs: %s", run_id,
            parent_run_id, inputs, tags, metadata, kwargs
        )
        try:
    ...
    def on_chain_end(
        ...
        self._log.info(
            "on chain end run_id: %s parent_run_id: %s, outputs: %s, kwargs: %s", run_id, parent_run_id, outputs, kwargs
        )
        try:
            span_attributes = {}
    ...
    async def on_agent_action(
        ...
        self._log.info(
            "on agent action end run_id: %s, parent_run_id: %s, action:%s, kwargs:%s", run_id, parent_run_id, action,
            kwargs
        )
    ...
    async def on_agent_finish(
        ...
        self._log.info(
            "on agent finish end run_id: %s, parent_run_id: %s, finish:%s, kwargs:%s", run_id, parent_run_id, finish,
            kwargs
        )
    ...
    async def on_custom_event(
        ...
        self._log.info(
            "on custom event run_id: %s, name: %s, data: %s, tags: %s, metadata: %s, kwargs: %s", run_id, name, data,
            tags, metadata, kwargs
        )
