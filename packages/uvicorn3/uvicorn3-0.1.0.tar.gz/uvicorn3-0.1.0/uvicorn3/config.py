...
from uvicorn3._types import ASGIApplication
from uvicorn3.importer import ImportFromStringError, import_from_string
from uvicorn3.logging import TRACE_LOG_LEVEL
from uvicorn3.middleware.asgi2 import ASGI2Middleware
from uvicorn3.middleware.message_logger import MessageLoggerMiddleware
from uvicorn3.middleware.proxy_headers import ProxyHeadersMiddleware
from uvicorn3.middleware.wsgi import WSGIMiddleware
...
HTTP_PROTOCOLS: dict[HTTPProtocolType, str] = {
    "auto": "uvicorn3.protocols.http.auto:AutoHTTPProtocol",
    "h11": "uvicorn3.protocols.http.h11_impl:H11Protocol",
    "httptools": "uvicorn3.protocols.http.httptools_impl:HttpToolsProtocol",
}
WS_PROTOCOLS: dict[WSProtocolType, str | None] = {
    "auto": "uvicorn3.protocols.websockets.auto:AutoWebSocketsProtocol",
    "none": None,
    "websockets": "uvicorn3.protocols.websockets.websockets_impl:WebSocketProtocol",
    "wsproto": "uvicorn3.protocols.websockets.wsproto_impl:WSProtocol",
}
LIFESPAN: dict[LifespanType, str] = {
    "auto": "uvicorn3.lifespan.on:LifespanOn",
    "on": "uvicorn3.lifespan.on:LifespanOn",
    "off": "uvicorn3.lifespan.off:LifespanOff",
}
LOOP_SETUPS: dict[LoopSetupType, str | None] = {
    "none": None,
    "auto": "uvicorn3.loops.auto:auto_loop_setup",
    "asyncio": "uvicorn3.loops.asyncio:asyncio_setup",
    "uvloop": "uvicorn3.loops.uvloop:uvloop_setup",
}
...
LOGGING_CONFIG: dict[str, Any] = {
    ...
    "formatters": {
        "default": {
            "()": "uvicorn3.logging.DefaultFormatter",
            "fmt": "%(levelprefix)s %(message)s",
            "use_colors": None,
        },
        "access": {
            "()": "uvicorn3.logging.AccessFormatter",
    ...
    "loggers": {
        "uvicorn3": {"handlers": ["default"], "level": "INFO", "propagate": False},
        "uvicorn3.error": {"level": "INFO"},
        "uvicorn3.access": {"handlers": ["access"], "level": "INFO", "propagate": False},
    },
}

logger = logging.getLogger("uvicorn3.error")
...
class Config:
    ...
    def configure_logging(self) -> None:
        ...
        if self.log_config is not None:
            if isinstance(self.log_config, dict):
                ...
                # Install the PyYAML package or the uvicorn3[standard] optional
        ...
        if self.log_level is not None:
            ...
            logging.getLogger("uvicorn3.error").setLevel(log_level)
            logging.getLogger("uvicorn3.access").setLevel(log_level)
            logging.getLogger("uvicorn3.asgi").setLevel(log_level)
        if self.access_log is False:
            logging.getLogger("uvicorn3.access").handlers = []
            logging.getLogger("uvicorn3.access").propagate = False
    ...
    def load(self) -> None:
        ...
        self.encoded_headers = (
            [(b"server", b"uvicorn3")] + encoded_headers
