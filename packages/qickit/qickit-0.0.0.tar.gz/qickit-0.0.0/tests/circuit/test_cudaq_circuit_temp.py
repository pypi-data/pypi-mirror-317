...
from qickit.circuit.cudaqcircuit_temp import CUDAQCircuit
...
@pytest.mark.skip(reason="CUDAQCircuit is not implemented yet.")
class TestCUDAQCircuit(Template):
    ...
    def test_Identity(
        ...
        # Define the `qickit.circuit.CUDAQCircuit` instance
        circuit = CUDAQCircuit(num_qubits)
    ...
    def test_single_measurement(self) -> None:
        ...
        # Define the equivalent `cudaq.kernel` instance, and
        # ensure they are equivalent
        cudaq_circuit = cudaq.make_kernel()
        qr = cudaq_circuit.qalloc(1) # type: ignore
        cudaq_circuit.mz(qr[0]) # type: ignore

        assert circuit.circuit == cudaq_circuit
    ...
    def test_multiple_measurement(self) -> None:
        ...
        # ensure they are equivalent
        cudaq_circuit = cudaq.make_kernel()
        qr = cudaq_circuit.qalloc(2) # type: ignore
        cudaq_circuit.mz(qr[0]) # type: ignore
        cudaq_circuit.mz(qr[1]) # type: ignore
    ...
    def test_horizontal_reverse(self) -> None:
        ...
        # Define the equivalent `qickit.circuit.CUDAQCircuit` instance, and
        # ensure they are equivalent
        updated_circuit = CUDAQCircuit(4)
