...
DownloadText = namedtuple("DownloadText", ["url", "name", "dist", "text"])
DownloadRequest = namedtuple("DownloadRequest", ["url", "name", "dist", "expire"])
...
class Downloader:
    ...
    def down_one(self, info: Tuple[str, str, str, datetime.timedelta]):
        url, tag, dist, expire = info
        dist_path = Path(dist)
        ...
        if dist_path.is_file():
            ...
            if lifetime <= expire:
                with open(dist_path, "r") as f:
                    return DownloadText(url, tag, dist_path, f.read())
        LOGGER.debug(url)
            ...
            try:
                ...
                if CONFIG.proxy:
                    try:
                        ...
                        return DownloadText(url, tag, dist_path, None)
                else:
                    LOGGER.error("使用代理下载失败 %s", url, exc_info=True)
                    return DownloadText(url, tag, dist_path, None)
        ...
        return DownloadText(url, tag, dist_path, text_res)
