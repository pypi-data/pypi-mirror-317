...
class WorkerOptions:
    ...
    max_retry: int = 16
    """Maximum number of times to retry connecting to Wirtual."""
    ws_url: str = "ws://localhost:7880"
    """URL to connect to the Wirtual server.

    By default it uses ``LIVEKIT_URL`` from environment"""
    api_key: str | None = None
    """API key to authenticate with Wirtual.

    By default it uses ``LIVEKIT_API_KEY`` from environment"""
    api_secret: str | None = None
    """API secret to authenticate with Wirtual.
        ...
        while not self._closed:
            try:
                ...
                if retry_count >= self._opts.max_retry:
                    raise RuntimeError(
                        f"failed to connect to wirtual after {retry_count} attempts",
                    )
                ...
                logger.warning(
                    f"failed to connect to wirtual, retrying in {retry_delay}s: {e}"
                )
