...
class SageMakerScheduler(BaseScheduler):
    ...
    async def _get_system_dependencies_version(
        ...
        self.log.info("[SageMakerScheduler] Successfully called S3 GetObject for system dependencies")
        latest_file = json.loads(response_content)
        self.log.info(f"[SageMakerScheduler] S3 response for {s3_bucket_name} with {latest_file}")
