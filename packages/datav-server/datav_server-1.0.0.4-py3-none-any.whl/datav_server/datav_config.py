...
def get_datav_config_rest():
    ...
    user_info = header.get("user_info")
    if not user_info:
        DataVWebResInfo.error("","用户信息为空")
    user_info = json.loads(user_info)

    result = get_datav_config(user_info.get("user_id"),user_info.get("mof_div_code"),user_info.get("year"))
...
def check_exists_dic_info_and_table_by_table_name():
    ...
    if not user_info:
        DataVWebResInfo.error("","用户信息为空")
    dic_info,dic_cols,table = check_exists_dic_info_and_table(args.get("tableName"),user_info.get("year"),user_info.get("mof_div_code"))
    result = {
        "dic_cols":dic_cols,
        "dic_info":dic_info,
        "table":table
