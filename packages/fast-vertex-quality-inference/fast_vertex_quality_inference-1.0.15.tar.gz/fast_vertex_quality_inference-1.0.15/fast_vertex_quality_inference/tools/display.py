            ...
            if particle == "->":
                ...
                if len(particle) > 0:
                    ...
                    if model_scheme[int(ordering_i)] != "PHSP":
                        label_i += f", {model_scheme[int(ordering_i)]}"
...
def print_file_info(file_path, time=None):
    ...
    if time:
        myGlobals.console.print(
            f"[grey39][{datetime.now().strftime('%H:%M:%S')}][grey39] [sea_green1] [dim] \tentries/min: {file.num_entries/(time/60.):.1f} [sea_green1]"
        )
...
def timings_table(only_rapidsim=False, only_vertexing=False):
    ...
    if not only_vertexing:
        table.add_row(
            "RapidSim",
            f"{myGlobals.stopwatches.read('RapidSim - config'):.2f}s ({myGlobals.stopwatches.read('RapidSim - config')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('RapidSim - generation'):.2f}s ({myGlobals.stopwatches.read('RapidSim - generation')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('RapidSim - processing'):.2f}s ({myGlobals.stopwatches.read('RapidSim - processing')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('RapidSim - config')+myGlobals.stopwatches.read('RapidSim - generation')+myGlobals.stopwatches.read('RapidSim - processing'):.2f}s ({(myGlobals.stopwatches.read('RapidSim - config')+myGlobals.stopwatches.read('RapidSim - generation')+myGlobals.stopwatches.read('RapidSim - processing'))/total_time*100.:.2f} %)",
        )
    if not only_rapidsim:
        table.add_row(
            "Vertexing",
            f"{myGlobals.stopwatches.read('Networks - config'):.2f}s ({myGlobals.stopwatches.read('Networks - config')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('Networks - generation'):.2f}s ({myGlobals.stopwatches.read('Networks - generation')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('Networks - processing'):.2f}s ({myGlobals.stopwatches.read('Networks - processing')/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('Networks - config')+myGlobals.stopwatches.read('Networks - generation')+myGlobals.stopwatches.read('Networks - processing'):.2f}s ({(myGlobals.stopwatches.read('Networks - config')+myGlobals.stopwatches.read('Networks - generation')+myGlobals.stopwatches.read('Networks - processing'))/total_time*100.:.2f} %)",
        )
    if not only_vertexing and not only_rapidsim:
        table.add_row("", "", "")  # This will act as a horizontal line
        table.add_row(
            "Total",
            f"{myGlobals.stopwatches.read('RapidSim - config')+myGlobals.stopwatches.read('Networks - config'):.2f}s ({(myGlobals.stopwatches.read('RapidSim - config')+myGlobals.stopwatches.read('Networks - config'))/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('RapidSim - generation')+myGlobals.stopwatches.read('Networks - generation'):.2f}s ({(myGlobals.stopwatches.read('RapidSim - generation')+myGlobals.stopwatches.read('Networks - generation'))/total_time*100.:.2f} %)",
            f"{myGlobals.stopwatches.read('RapidSim - processing')+myGlobals.stopwatches.read('Networks - processing'):.2f}s ({(myGlobals.stopwatches.read('RapidSim - processing')+myGlobals.stopwatches.read('Networks - processing'))/total_time*100.:.2f} %)",
            f"{total_time:.2f}s",
        )
