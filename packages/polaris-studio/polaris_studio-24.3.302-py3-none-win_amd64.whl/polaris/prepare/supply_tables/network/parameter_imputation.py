...
def impute_missing_attributes(aeq_project: Project, imputation_parameters: dict, path_to_file: Path) -> None:
    ...
    if list_imputed_cols:
        ...
        aeq_project.conn.executemany(f"UPDATE links SET {qry} WHERE link_id=?;", attr_zip)

        aeq_project.conn.commit()

    add_column_unless_exists(aeq_project.conn, "links", "free_flow_time", "REAL")
    add_column_unless_exists(aeq_project.conn, "links", "capacity_ab", "REAL")
    add_column_unless_exists(aeq_project.conn, "links", "capacity_ba", "REAL")
    aeq_project.conn.commit()

    # Makes sure we have enough lanes in each valid direction
    aeq_project.conn.execute("UPDATE links SET lanes_ab = 1 WHERE lanes_ab < 1 and direction>=0")
    aeq_project.conn.execute("UPDATE links SET lanes_ba = 1 WHERE lanes_ba < 1 and direction<=0")
    aeq_project.conn.execute("UPDATE links SET lanes_ab = 0 WHERE direction<0")
    aeq_project.conn.execute("UPDATE links SET lanes_ba = 0 WHERE direction>0")

    # Makes speeds 5km/h minimum
    aeq_project.conn.execute("UPDATE links SET speed_ab = 5 WHERE speed_ab < 5")
    aeq_project.conn.execute("UPDATE links SET speed_ba = 5 WHERE speed_ba < 5")

    # Sets free-flow travel times
    aeq_project.conn.execute("UPDATE links SET free_flow_time = 60 * (distance/1000) / speed_ab;")
    aeq_project.conn.execute(
        "UPDATE links SET free_flow_time = coalesce(free_flow_time, 60 * (distance/1000) / speed_ba);"
    )
    # Computes capacities
    aeq_project.conn.execute("UPDATE links SET capacity_ab = lanes_ab * (1800+10*(speed_ab-50)) where lanes_ab>0;")
    aeq_project.conn.execute("UPDATE links SET capacity_ba = lanes_ba * (1800+10*(speed_ba-50)) where lanes_ba>0;")
    aeq_project.conn.commit()
...
def make_speeds_numeric(aeq_project, list_fixed_speeds):
    ...
    attr_zip = list(zip(*list_zip_arr))
    curr = aeq_project.conn.cursor()
    curr.executemany("UPDATE links SET speed_ab=?, speed_ba=? WHERE link_id=?;", attr_zip)
    aeq_project.conn.commit()
