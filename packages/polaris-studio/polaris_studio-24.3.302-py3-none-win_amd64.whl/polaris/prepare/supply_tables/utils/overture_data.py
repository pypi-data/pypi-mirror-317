    ...
    if os.path.exists(tempfile_name):
        ...
        qrys = {
            "buildings": f"""COPY (
                        ...
                        FROM read_parquet('s3://overturemaps-us-west-2/release/2024-07-22.0/theme=buildings/type=*/*', filename=true, hive_partitioning=1, union_by_name = true)
            ...
            "places": f"""COPY (
                        ...
                        FROM read_parquet('s3://overturemaps-us-west-2/release/2024-07-22.0/theme=places/type=*/*')
                        WHERE bbox.xmin BETWEEN {minx} AND {maxx} AND bbox.ymin BETWEEN {miny} AND {maxy}
                        ) TO '{tempfile_name}'
                    WITH (FORMAT 'parquet', COMPRESSION 'ZSTD');""",
        }
