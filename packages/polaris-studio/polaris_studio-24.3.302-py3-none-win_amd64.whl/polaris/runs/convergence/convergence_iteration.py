...
class ConvergenceIteration:
    ...
    def is_standard(self):
        return not any([self.is_pop_synth, self.is_abm_init, self.is_skim, self.is_dta, self.is_cristal])
    ...
    def of_type(cls, iteration_type, iteration_number):
        ...
        elif iteration_type == "cristal":
            return ConvergenceIteration(is_cristal=True, iteration_number=iteration_number)
    ...
    def _get_base_str(self):
        ...
        elif self.is_cristal:
            return "03_cristal_iteration"
        elif self.is_dta:
    ...
    def parse_dirname(dirname):
        ...
        if "iteration_" in dirname:
            it_num = int(dirname.split("_iteration_")[-1])
        else:
            it_num = None
