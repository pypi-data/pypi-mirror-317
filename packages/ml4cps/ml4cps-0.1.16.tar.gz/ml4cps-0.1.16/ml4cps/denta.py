...
from ml4cps import learn as automata_learn
...
class DENTA(nn.Module, automata.Automaton):
    def __init__(self, num_signals, num_hidden_units, first_hidden_size=None, sigma=1., sigma_learnable=False,
                 sparsity_weight=0.01, persistence=True, num_hidden_layers=1, split_on_quantile=1,
    ...
    def free_energy(self, x, level=1):
        bx = self._decoder[-level].bias #[f"linear_h2v{level}"]
    ...
    def sample_x(self, x):
        return torch.normal(x, std=torch.atleast_3d(torch.exp(self.log_sigma_x)))
    ...
    def learn_denta_network(self, train_data, valid_data, max_epoch=10, min_epoch=0, estimate_switches_every=5,
        ...
        if self.log_mlflow:
            ...
            mlflow.log_param('estimate_switches_every', estimate_switches_every)
    ...
    def learn_latent_automaton(self, train_data, valid_data):
        ...
        if self.log_mlflow:
            ...
            mlflow.log_figure(vis.plot_cps_plotly(self), f'figures/denta_automaton_{mlflow.active_run().info.run_id}.html')
...
if __name__ == '__main__':
    ...
    ###
    mdl = DENTA(num_signals=2, num_hidden_layers=1, num_hidden_units=8, sigma=0.1, log_mlflow=True)
