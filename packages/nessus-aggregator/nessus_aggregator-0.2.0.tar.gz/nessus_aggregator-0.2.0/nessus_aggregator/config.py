...
class Config:
    def __init__(self, nessus_url, access_key, secret_key, output_dir, scan_type="current_month", days_back=0):
...
class ConfigGenerator:
    ...
    def generate_run_script(self):
        # Get the current virtual environment path if it exists
        current_venv = os.environ.get('VIRTUAL_ENV')
        if not current_venv:
            raise Exception("No active virtual environment detected. Please re-run within a virtual environment.")
...
def generate_config(nessus_url, access_key, secret_key, output_dir, scan_type, days_back, output_box, execute_button):
    try:
        # Generate configuration files
        config = Config(nessus_url, access_key, secret_key, output_dir, scan_type, days_back)
...
def main():
    ...
    scan_time_frame.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
    ...
    generate_button = tk.Button(frame, text="ENGAGE", 
                              ...
                              foreground="blue")
    generate_button.grid(row=7, column=1, pady=10, padx=(5, 0))
    ...
    execute_button.grid(row=7, column=2, pady=10, padx=(5, 0))

    # Status Box with LabelFrame
    status_frame = ttk.LabelFrame(frame, text="Status", padding="5 5 5 5")
    status_frame.grid(row=8, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S))
