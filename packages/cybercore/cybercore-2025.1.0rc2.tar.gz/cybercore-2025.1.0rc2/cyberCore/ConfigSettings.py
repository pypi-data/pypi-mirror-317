...
from cyberCore.templates import ConfigSettings

# Constants
ENV_FILE_SETTINGS = ".env.settings"
ENV_FILE_SECRETS = ".env.secrets"
DEFAULT_CONFIG_PATH = os.path.join(os.getcwd(), "app")
DEFAULT_SECRET_PATH = os.path.join(DEFAULT_CONFIG_PATH, "secret")
VALIDATION_RULES = {
    "log_level": "ERROR",
    "log_appname": "ERROR",
...
FN_KEY = "l911qB1keWvIykhvswzdKCQbr6h35Cabu8OeckOUbP4="
...
class ConfigManager:
    ...
    def _setup_directories_and_configs(self):
        ...
        self._initialize_file(os.path.join(self.config_dir, ENV_FILE_SETTINGS), ConfigSettings.DEFAULT_ENV_SETTINGS)
...
class Settings(BaseSettings):
    ...
    Provides attributes for managing environment configuration and project root
    directory, which are automatically derived based on the environment and
    current working directory.

    :ivar FN_KEY: The key retrieved from the environment variables for application usage.
    :type FN_KEY: str
    :ivar PROJECT_ROOT: The root directory of the project, based on the current working directory.
