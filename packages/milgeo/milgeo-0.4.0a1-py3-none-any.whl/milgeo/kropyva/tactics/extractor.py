...
class TacticsExtractor(GeometryExtractor):
    ...
    """
    def __init__(self, embedded_kml_extractor: GeometryExtractor | None = None, parser: high_level.KMLParser | None = None, url_mapping: UrlToSidcMapping | None = None, color_finder: ColorFinder | None = None):
        self.embedded_kml_extractor = embedded_kml_extractor
        self.parser = parser or high_level.KMLParser()
    ...
    def _process_geometry(self, placemark: high_level.AbstractPlacemark):
        ...
        try:
            return self._create_geometry(
                ...
                metadata={},
                observation_datetime=None, # TODO: get from placemark
                sidc=sidc,
    ...
    def _get_coordinates(self, placemark: high_level.AbstractPlacemark) -> list[float] | list[list[float]] | list[list[list[float]]]:
        match placemark:
            ...
            case high_level.LineString():
                return [list(point.xy) for point in placemark.get_points()]
            case high_level.Polygon():
    ...
    def _create_geometry(self, geometry_type: Type[Geometry], name: str, sidc: str, coordinates: list, description: str, metadata,
                         outline_color: str | None = None, fill_color: str | None = None, fill_opacity: float | None = None, observation_datetime=None):
    ...
    def _determine_sidc(self, placemark: high_level.AbstractPlacemark) -> str:
        ...
        identity_from_color = self._get_identity_from_color(placemark_color)
        type_based_sidc = placemark_type and self._kropyva_url_to_sidc(placemark_type, identity_from_color=identity_from_color)
        ...
        if stroke_pattern:
            stroke_pattern = self._unwrap_kropyva_url(stroke_pattern)
    ...
    @staticmethod
    def _unwrap_kropyva_url(url: str) -> str:
    ...
    def _get_identity_from_color(self, color: str | None) -> str:
    ...
    def _kropyva_url_to_sidc(self, url: str, identity_from_color: str | None = None) -> str | None:
        '''
        Convert a Kropyva URL to a SIDC and staff comments (if any)
        '''
        url = self._unwrap_kropyva_url(url)
        sidc = self.url_to_sidc_mapping.map_url(url, identity_from_color)
        return sidc
