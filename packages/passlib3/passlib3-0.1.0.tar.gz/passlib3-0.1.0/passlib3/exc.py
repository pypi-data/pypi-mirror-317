"""passlib3.exc -- exceptions & warnings raised by passlib3"""
#=============================================================================
# exceptions
#=============================================================================
...
class PasswordTruncateError(PasswordSizeError):
    ...
    Hashers such as :class:`~passlib3.hash.bcrypt` can be configured to raises
...
class TokenError(ValueError):
    """
    Base error raised by v:mod:`passlib3.totp` when
...
class MalformedTokenError(TokenError):
    """
    Error raised by :mod:`passlib3.totp` when a token isn't formatted correctly
...
class InvalidTokenError(TokenError):
    """
    Error raised by :mod:`passlib3.totp` when a token is formatted correctly,
...
class UsedTokenError(TokenError):
    """
    Error raised by :mod:`passlib3.totp` if a token is reused.
...
class UnknownHashError(ValueError):
    """
    Error raised by :class:`~passlib3.crypto.lookup_hash` if hash name is not recognized.
    ...
    As of version 1.7.4, this may be raised if a :class:`~passlib3.context.CryptContext`
...
class PasslibConfigWarning(PasslibWarning):
    ...
    of a :class:`~passlib3.context.CryptContext` instance.
...
class PasslibHashWarning(PasslibWarning):
    ...
    or hash string passed to a passlib3 hash class.
