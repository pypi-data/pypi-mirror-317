"""passlib3.ext.django.utils - helper functions used by this plugin"""
#=============================================================================
# imports
#=============================================================================
# core
...
try:
    ...
    DJANGO_VERSION = ()
# pkg
from passlib3 import exc, registry
from passlib3.context import CryptContext
from passlib3.exc import PasslibRuntimeWarning
from passlib3.utils.compat import get_method_function, iteritems, OrderedDict, unicode
from passlib3.utils.decor import memoized_property
# local
__all__ = [
    "DJANGO_VERSION",
    "MIN_DJANGO_VERSION",
...
#: minimum version supported by passlib3.ext.django
...
# map preset names -> passlib3.app attrs
_preset_map = {
...
def get_preset_config(name):
    ...
    * ``"passlib3-default"`` - default config used by this release of passlib3.
    ...
    * ``"django-1.6"`` - config used by stock Django 1.6 installs
    ...
    if name == "django-default":
        ...
        name = "django-1.6"
    if name == "passlib3-default":
        return PASSLIB_DEFAULT
    try:
        attr = _preset_map[name]
    except KeyError:
        raise ValueError("unknown preset config name: %r" % name)
    import passlib3.apps
    return getattr(passlib3.apps, attr).to_string()

# default context used by passlib3 1.6
PASSLIB_DEFAULT = """
[passlib3]
...
#: prefix used to shoehorn passlib3's handler names into django hasher namespace
PASSLIB_WRAPPER_PREFIX = "passlib_"

#: prefix used by all the django-specific hash formats in passlib3;
...
class DjangoTranslator(object):
    """
    Object which helps translate passlib3 hasher objects / names
    ...
    #: internal cache of passlib3 hasher -> django hasher instance.
    #: key stores weakref to passlib3 hasher.
    _django_hasher_cache = None

    #: special case -- unsalted_sha1
    _django_unsalted_sha1 = None

    #: internal cache of django name -> passlib3 hasher
    #: value stores weakrefs to passlib3 hasher.
    ...
    def _get_passlib_hasher(self, passlib_name):
        """
        resolve passlib3 hasher by name, using context if available.
        """
    ...
    #=============================================================================
    # resolve passlib3 hasher -> django hasher
    #=============================================================================

    def passlib_to_django_name(self, passlib_name):
        """
        Convert passlib3 hasher / name to Django hasher name.
        """
    ...
    def passlib_to_django(self, passlib_hasher, cached=True):
        """
        Convert passlib3 hasher / name to Django hasher.

        :param passlib_hasher:
            passlib3 hasher / name
    ...
    def _create_django_hasher(self, django_name):
        ...
        module = sys.modules.get("passlib3.ext.django.models")
    ...
    def django_to_passlib(self, django_name, cached=True):
        ...
        # but passlib3 uses "django_salted_sha1" for both of these.
        ...
        raise ValueError("can't translate django name to passlib3 name: %r" %
                         (django_name,))
...
class DjangoContextAdapter(DjangoTranslator):
    ...
    When installed in django, :mod:`!passlib3.ext.django` will create
    ...
    def __init__(self, context=None, get_user_category=None, **kwds):
        ...
        if make_password.__module__.startswith("passlib3."):
    ...
    def identify_hasher(self, encoded):
        ...
        if handler.name == "django_salted_sha1" and encoded.startswith("sha1$$"):
            ...
            # passlib3 identifies it as belonging to "sha1$salt$digest" handler.
    ...
    def make_password(self, password, salt=None, hasher="default"):
        ...
        if "salt" not in passlib_hasher.setting_kwds:
            ...
            # but passlib3 just reuses it's "sha1" handler ("sha1$salt$digest"). To make
    ...
    def check_password(self, password, encoded, setter=None, preferred="default"):
        ...
        if preferred == "default":
            ...
            # passlib3 hasher, and call it's original needs_update() method.
    ...
    def install_patch(self):
        ...
        if DJANGO_VERSION < MIN_DJANGO_VERSION:
            raise RuntimeError("passlib3.ext.django requires django >= %s" %
                               (MIN_DJANGO_VERSION,))
    ...
    def load_model(self):
        ...
        if self.enabled:
            ...
            self.remove_patch()
        log.debug("passlib3.ext.django loaded")
    ...
    def _load_settings(self):
        ...
        # TODO: wrap and import any custom hashers as passlib3 handlers,
        #       so they could be used in the passlib3 config.
...
#=============================================================================
# wrapping passlib3 handlers as django hashers
#=============================================================================
...
class _PasslibHasherWrapper(object):
    """
    adapter which which wraps a :cls:`passlib3.ifc.PasswordHash` class,
    and provides an interface compatible with the Django hasher API.

    :param passlib_handler:
        passlib3 hash handler (e.g. :cls:`passlib3.hash.sha256_crypt`.
    """
    #=====================================================================
    # instance attrs
    #=====================================================================

    #: passlib3 handler that we're adapting.
    ...
    def _translate_kwds(self):
        ...
        used to translate passlib3 hash options -> django keywords
        """
    ...
    #=====================================================================
    def salt(self):
        # NOTE: passlib3's handler.hash() should generate new salt each time,
    ...
    def safe_summary(self, encoded):
        ...
        handler = self.passlib_handler
        items = [
            # since this is user-facing, we're reporting passlib3's name,
    ...
    def must_update(self, encoded):
        ...
        #       for now (as of passlib3 1.6.6), replicating django policy that this returns True
...
#=============================================================================
# adapting django hashers -> passlib3 handlers
#=============================================================================
# TODO: this code probably halfway works, mainly just needs
#       a routine to read HASHERS and PREFERRED_HASHER.

##from passlib3.registry import register_crypt_handler
##from passlib3.utils import classproperty, to_native_str, to_unicode
##from passlib3.utils.compat import unicode
##
##
##class _HasherHandler(object):
##    "helper for wrapping Hasher instances as passlib3 handlers"
