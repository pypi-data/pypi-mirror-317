"""passlib3.crypto.digest -- crytographic helpers used by the password hashes in passlib3
...
from passlib3.utils import join_bytes, to_native_str, join_byte_values, to_bytes, \
                          SequenceMixin, as_bool
from passlib3.utils.compat import irange, int_types, unicode_or_bytes_types, PY3, error_from
...
def _get_hash_aliases(name):
    ...
    if name.startswith("scram-"): # helper for SCRAM protocol (see passlib3.handlers.scram)
        name = name[6:]
...
def _get_hash_const(name):
    ...
    try:
        ...
        const.__doc__ = ("wrapper for hashlib.new(%r),\n"
                         "generated by passlib3.crypto.digest.lookup_hash()") % name
        return const

    # use builtin md4 as fallback when not supported by hashlib
    if name == "md4":
        from passlib3.crypto._md4 import md4
        return md4
...
def lookup_hash(digest,  # *,
    ...
    :param required:
        By default (True), this function will throw an :exc:`~passlib3.exc.UnknownHashError` if no hash constructor
    ...
    :param return_unknown:
        ...
        .. deprecated:: 1.7.3
            ...
            deprecated, and will be removed in passlib3 2.0.
    ...
    if isinstance(digest, unicode_or_bytes_types):
        ...
        if const and mock_fips_mode and name not in _fips_algorithms:
            def const(source=b""):
                raise ValueError("%r disabled for fips by passlib3 set_mock_fips_mode()" % name)
...
def pbkdf2_hmac(digest, secret, salt, rounds, keylen=None):
    ...
    .. versionchanged:: 1.7
        ...
        * builtin pure-python backend
        ...
        See :data:`passlib3.crypto.digest.PBKDF2_BACKENDS` to determine
...
if PY3 and _force_backend in ["any", "from-bytes"]:
    ...
    from struct import Struct
    from passlib3.utils import sys_bits
    ...
    def _get_pbkdf2_looper(digest_size):
        ...
        # compile helper
        #
        code = compile(source, "<generated by passlib3.crypto.digest._get_pbkdf2_looper()>", "exec")
    ...
    from passlib3.utils import int_to_bytes
