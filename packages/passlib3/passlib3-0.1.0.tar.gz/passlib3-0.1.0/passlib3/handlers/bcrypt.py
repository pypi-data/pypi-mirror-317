"""passlib3.bcrypt -- implementation of OpenBSD's BCrypt algorithm.
...
from passlib3.crypto.digest import compile_hmac
from passlib3.exc import PasslibHashWarning, PasslibSecurityWarning, PasslibSecurityError
from passlib3.utils import safe_crypt, repeat_string, to_bytes, parse_version, \
                          rng, getrandstr, test_crypt, to_unicode, \
                          utf8_truncate, utf8_repeat_string, crypt_accepts_bytes
from passlib3.utils.binary import bcrypt64
from passlib3.utils.compat import get_unbound_method_function
from passlib3.utils.compat import u, uascii_to_str, unicode, str_to_uascii, PY3, error_from
import passlib3.utils.handlers as uh
...
class _BcryptCommon(uh.SubclassBackendMixin, uh.TruncateMixin, uh.HasManyIdents,
    ...
    # HasRounds
    #--------------------
    default_rounds = 12 # current passlib3 default
    ...
    def needs_update(cls, hash, **kwds):
        ...
        # check for incorrect padding bits (passlib3 issue 25)
    ...
    #===================================================================
    # specialized salt generation - fixes passlib3 issue 25
    #===================================================================
    ...
    def _finalize_backend_mixin(mixin_cls, backend, dryrun):
        ...
        def assert_lacks_8bit_bug(ident):
            ...
            even though it's not known to be present in any of passlib3's backends.
            ...
            """
            # NOTE: passlib3 1.7.2 and earlier used the commented-out LATIN-1 test vector to detect
            #       this bug; but python3's crypt.crypt() only supports unicode inputs (and
            #       always encodes them as UTF8 before passing to crypt); so passlib3 1.7.3
            ...
            if verify(secret, bug_hash):
                ...
                raise PasslibSecurityError(
                    "passlib3.hash.bcrypt: Your installation of the %r backend is vulnerable to "
        ...
        if result is NotImplemented:
            ...
            if detect_wrap_bug(IDENT_2A):
                if backend == "os_crypt":
                    ...
                    warn("passlib3.hash.bcrypt: Your installation of the %r backend is vulnerable to "
    ...
    def _norm_digest_args(cls, secret, ident, new=False):
        ...
        if ident == IDENT_2A:
            ...
            raise RuntimeError("$2x$ hashes not currently supported by passlib3")
...
class _PyBcryptBackend(_BcryptCommon):
    ...
    def _calc_checksum_threadsafe(self, secret):
        ...
        # used through passlib3, this should be safe.
...
class _OsCryptBackend(_BcryptCommon):
    ...
    def _calc_checksum(self, secret):
        ...
        #
        # XXX: maybe just let safe_crypt() throw UnicodeDecodeError under passlib3 2.0,
        ...
        if PY3 and isinstance(secret, bytes):
            try:
                ...
                raise error_from(uh.exc.PasswordValueError(
                    ...
                    "passlib3 recommends running `pip install bcrypt` for general bcrypt support.",
                    ), None)

        #
        # else crypt() call failed for unknown reason.
        #
        # NOTE: getting here should be considered a bug in passlib3 --
...
class _BuiltinBackend(_BcryptCommon):
    """
    backend which uses passlib3's pure-python implementation
    """
    @classmethod
    def _load_backend_mixin(mixin_cls, name, dryrun):
        from passlib3.utils import as_bool
        ...
        from passlib3.crypto._blowfish import raw_bcrypt as _builtin_bcrypt
        return mixin_cls._finalize_backend_mixin(name, dryrun)
...
class bcrypt(_NoBackend, _BcryptCommon):
    ...
    The :meth:`~passlib3.ifc.PasswordHash.using` method accepts the following optional keywords:
    ...
    :param bool truncate_error:
        ...
        Setting ``truncate_error=True`` will cause :meth:`~passlib3.ifc.PasswordHash.hash`
        to raise a :exc:`~passlib3.exc.PasswordTruncateError` instead.
    ...
    :param relaxed:
        ...
        and the error can be corrected, a :exc:`~passlib3.exc.PasslibHashWarning`
...
class bcrypt_sha256(_wrapped_bcrypt):
    ...
    The :meth:`~passlib3.ifc.PasswordHash.hash` and :meth:`~passlib3.ifc.PasswordHash.genconfig` methods accept
    ...
    #: wrapper version.
    #: v1 -- used prior to passlib3 1.7.3; performs ``bcrypt(sha256(secret), salt, cost)``
    #: v2 -- new in passlib3 1.7.3; performs `bcrypt(sha256_hmac(salt, secret), salt, cost)``
    version = 2
