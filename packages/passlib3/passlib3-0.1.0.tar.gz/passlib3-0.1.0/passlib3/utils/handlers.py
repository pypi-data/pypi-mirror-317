"""passlib3.handler - code for implementing handlers, and global registry for handlers"""
#=============================================================================
# imports
#=============================================================================
...
# site
# pkg
import passlib3.exc as exc, passlib3.ifc as ifc
from passlib3.exc import MissingBackendError, PasslibConfigWarning, \
                        PasslibHashWarning
from passlib3.ifc import PasswordHash
from passlib3.registry import get_crypt_handler
...
from passlib3.utils.binary import (
    BASE64_CHARS, HASH64_CHARS, PADDED_BASE64_CHARS,
    HEX_CHARS, UPPER_HEX_CHARS, LOWER_HEX_CHARS,
    ALL_BYTE_VALUES,
)
from passlib3.utils.compat import join_byte_values, irange, u, native_string_types, \
                                 uascii_to_str, join_unicode, unicode, str_to_uascii, \
                                 join_unicode, unicode_or_bytes_types, PY2, int_types
from passlib3.utils.decor import classproperty, deprecated_method
# local
__all__ = [
...
# deprecated aliases - will be removed after passlib3 1.8
...
def guess_app_stacklevel(start=1):
    ...
    looks for first frame not part of passlib3.
    """
    ...
    try:
        while frame:
            ...
            if name.startswith("passlib3.tests.") or not name.startswith("passlib3."):
                return max(1, count)
            count += 1
            frame = frame.f_back
        return start
...
def mask_value(value, show=4, pct=0.125, char=u"*"):
    ...
    if not isinstance(value, unicode):
        if isinstance(value, bytes):
            from passlib3.utils.binary import ab64_encode
...
#    relaxed = False # when _norm_xxx() funcs should be strict about inputs
    ...
    # NOTE: this is only used by genconfig(), and will be removed in passlib3 2.0
    @property
    ...
    def hash(cls, secret, **kwds):
        if kwds:
            # Deprecating passing any settings keywords via .hash() as of passlib3 1.7; everything
...
class HasManyIdents(GenericHandler):
    ...
    def using(cls,  # keyword only...
        ...
        """
        This mixin adds support for the following :meth:`~passlib3.ifc.PasswordHash.using` keywords:
...
class HasRounds(GenericHandler):
    ...
    # hack to pass info to _CryptRecord (will be removed in passlib3 2.0)
...
class BackendMixin(PasswordHash):
    ...
    .. classmethod:: _load_backend_{name}(dryrun=False)
        ...
        :raises passlib3.exc.PasslibSecurityError:
        ...
        .. warning::
            ...
            Due to the way passlib3's internals are arranged,
    ...
    def get_backend(cls):
        ...
        :raises passlib3.exc.MissingBackendError:
...
class HasManyBackends(BackendMixin, GenericHandler):
    ...
    def _get_backend_loader(cls, name):
        ...
        (will be removed in passlib3 2.0)
        """
...
class PrefixWrapper(object):
    ...
    this is used mainly by the :doc:`ldap crypt <passlib3.hash.ldap_crypt>` handlers;
    such as :class:`~passlib3.hash.ldap_md5_crypt` which wraps :class:`~passlib3.hash.md5_crypt` and adds a ``{CRYPT}`` prefix.
