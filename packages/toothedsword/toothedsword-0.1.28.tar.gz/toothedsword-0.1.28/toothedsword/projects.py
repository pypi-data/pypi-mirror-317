...
def gen_process_tab(ns, nsl, type='low'):
    ...
    if type == 'low':
        stmp = '['+\
				...
				']'
    stmp += ' '+str(int(ns/nsl*100))+'% ('+str(ns)+'/'+str(nsl)+')'
...
def print_status(progress):
    ...
    sys.stdout.write(f"\033[{term_height};0HProgress: {progress}%\033[K")  # 定位到最后一行
    sys.stdout.flush()

def print_status_best(progress):
    """在终端底部打印进度状态"""
    _, term_height = shutil.get_terminal_size()
    sys.stdout.write(f"\033[{term_height};0H{progress}\033[K")  # 定位到最后一行
    sys.stdout.flush()
...
def print_output(line):
    ...
    _, term_height = shutil.get_terminal_size()
    while len(displayed_lines) > term_height+40:
...
class PROJECT():
    ...
    def __init__(self, inputjson, steps='初始化|输出', pname='', prodir='',processtab='low'):
        """TODO: to be defined. """
    ...
    def flow(self, sname):
        ...
        for s in self.steps:
            ...
            if s == sname:
                break
        stmp = gen_process_tab(ns, len(self.steps), type=self.processtab)
    ...
    def result(self, outpath='', info={}):
        ...
        self.flow('处理')
        product = json2dict(self.prodir+'/'+self.pname+'/product.json')
