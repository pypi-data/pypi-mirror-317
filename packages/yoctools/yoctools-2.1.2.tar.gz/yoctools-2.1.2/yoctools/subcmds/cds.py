        ...
        def _build_cds_proj(cpu, platform, kernel, solname=''):
            ...
            if kernel:
                project_content = project_template.format(
                    name = "%s_%s_%s_%s" % (solution.solution_component.name, cpu, platform, kernel),
...
if [ -f "$BOARD_PATH/script/aft_build.sh" ];then
    cd $SOLUTION_PATH
    sh $BOARD_PATH/script/aft_build.sh $SOLUTION_PATH $BOARD_PATH $CHIP_PATH $PLATFORM $CPU
fi
'''