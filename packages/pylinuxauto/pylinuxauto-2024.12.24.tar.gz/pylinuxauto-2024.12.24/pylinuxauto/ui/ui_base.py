...
class ButtonCenter:
    ...
    def window_left_top_position(self) -> tuple:
        ...
        try:
            ...
            raise exceptions.GetWindowInformation(f"获取窗口左上角坐标错误 {exc}") from exc
    ...
    def btn_center(
        ...
        if btn_x and btn_y:
            ...
            return btn_x, btn_y
        raise exceptions.NoSetReferencePoint(
    ...
    def get_windows_id(self, name: str) -> list:
        ...
        if config.IS_X11:
            ...
            raise exceptions.ApplicationStartError(app_id)
        else:
            info = self.wwininfo.window_info().get(self.appname)
            if isinstance(info, dict):
    ...
    def get_lastest_window_id(self, app_name: str) -> int:
        ...
        if config.IS_X11:
            try:
                ...
                raise exceptions.ApplicationStartError(f"{app_name, exc}") from exc
        else:
            info = WaylandWindowInfo().window_info().get(self.appname)
            if isinstance(info, dict):
                return info.get("window_id")
