...
class File:
    ...
    def create_files(path: str, file_name: str = "", file_type: str = "dir"):
        ...
        """
        parent_dir = f"/home/{config.USERNAME}/{path}"
    ...
    def delete_files(path: str, ignores: tuple = (), includes: tuple = ()):
        ...
        """
        abs_file_path = f"/home/{config.USERNAME}/{path}"
        if os.path.isdir(abs_file_path):
            ...
            if file_list:
                ...
                if ignores and not includes:
                    ...
                    os.system(
                        f"echo '{config.PASSWORD}' | " f"sudo -S rm -rf {abs_file_path}/*"
                    )
                else:
    ...
    def move_files(path: str, file_name: str, new_path: str = None, new_file_name: str = None):
        ...
        """
        parent_dir = f"/home/{config.USERNAME}/{path}"
        new_parent_dir = f"/home/{config.USERNAME}/{new_path}"
    ...
    def file_exists(*files, abspath=False, _try=True):
        ...
        """
        if not abspath:
            abspath = f"/home/{config.USERNAME}"
        for file in files:
