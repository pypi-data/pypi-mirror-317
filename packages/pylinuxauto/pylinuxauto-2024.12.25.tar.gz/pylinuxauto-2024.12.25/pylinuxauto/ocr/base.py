...
class OCRBase:
    ...
    def _pdocr_client(
        ...
        if config.OCR_CMD:
            import tempfile
            import json
            json_path = f"{tempfile.gettempdir()}/ocr.json"
            Cmd.run(f"ocr-cmd -i {screen_path} -o {json_path}", timeout=120)
            results = None
            for _ in range(20):
            ...
            for i in results:
                for index, j in enumerate(i[0]):
                    i[0][index] = [int(d.strip()) for d in j.split(",")]
                i[-1].append(1)
                i[-1] = tuple(i[-1])

            return [results]
        elif GET_OCR_FORM_RPC:
