...
from typing import Optional, Any, Callable, Union, Literal
import jsonpickle
...
def openai_stream_handler(fn, run_id, name, type, *args, **kwargs):
    try:
...
async def async_openai_stream_handler(fn, run_id, name, type, *args, **kwargs):
...
def anthropic_stream_handler(fn, run_id, name, *args, **kwargs):
    try:
        stream = fn(*args, **kwargs)
        completion_text = ""
        tokens = 0
        ...
        output = {"role": "assistant", "content": completion_text}
        track_event("llm", "end", run_id, name=name, output=output, token_usage={"completion": tokens, "prompt": None})

async def async_anthropic_stream_handler(fn, run_id, name, *args, **kwargs):
    try:
        stream = await fn(*args, **kwargs)
        completion_text = ""
        tokens = 0
    ...
    def sync_wrapper(*args, **kwargs):
        ...
        try:
            ...
            if kwargs.get("stream") == True:
                return stream_handler(fn, run.id, name or parsed_input["name"], type, *args, **kwargs)
...
# TODO: type hint, but the user doesn't necessarily have the openai or anthropic package installed
def monitor(client):
    try:
        provider = getattr(client, "__name__", getattr(type(client), "__name__", None)) 

        # provider == "openai"?
        if provider in ["OpenAI", "AsyncOpenAI", "AzureOpenAi", "AsyncAzureOpenAI"]:
            if parse_version(version("openai")) < parse_version("1.0.0"):
                ## todo raise error package version
                pass

            if provider in ["OpenAI", "AzureOpenAI"] :
                client.chat.completions.create = wrap(
                    client.chat.completions.create,
                    stream_handler=openai_stream_handler, 
                    type="llm",
                    input_parser=OpenAIUtils.parse_input,
                    output_parser=OpenAIUtils.parse_output
                )
                return
                ## TODO: error if openai api key not set before monitor? (like in previous version)
            if provider in ["AsyncOpenAI", "AsyncAzureOpenAI"]:
                client.chat.completions.create = async_wrap(
                    client.chat.completions.create,
                    stream_handler=async_openai_stream_handler,
                    type="llm",
                    input_parser=OpenAIUtils.parse_input,
                    output_parser=OpenAIUtils.parse_output,
                )
        ...
        if provider in ["Anthropic", "AsyncAnthropic"]:
            if parse_version(version("anthropic")) < parse_version("0.40.0"):
                # todo: raise error package version
                pass

            if provider in ["Anthropic", "AnthropicBedrock"]:
            ...
            if provider in ["AsyncAnthropic", "AsyncAnthropicBedrock"]:
                client.messages.create = async_wrap(
                    client.messages.create,
        ...
        logging.warning("The `openai` or `anthropic` package is not installed")
