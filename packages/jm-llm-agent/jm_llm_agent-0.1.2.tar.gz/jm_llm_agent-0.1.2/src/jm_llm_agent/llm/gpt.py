...
class GPT(BaseLLM):
    ...
    async def gen_image(
        self, prompt: str, size: str = "1024x1024", n: int = 1, timeout: int = 60
    ) -> Dict[str, Any]:
    ...
    async def chat_stream(
        self, messages: List[Dict[str, str]], timeout: int = 0, **kwargs
    ) -> AsyncIterator[str]:
        data = {
            "messages": messages,
            "model": self.model,
    ...
    async def chat(
        self, messages: List[Dict[str, str]], detail: bool = False, **kwargs
    ) -> str:
