...
class ComboBoxStyleSheet(BaseStyleSheet):
    ...
    def __init__(self, combo_box_style: ComboBoxStyle | list[ComboBoxStyle] | None = None):
        """
        Initializes a ComboBoxStyleSheet object.

        Args:
            combo_box_style (ComboBoxStyle | list[ComboBoxStyle] | None): A ComboBoxStyle object or list of ComboBoxStyle objects representing the styles to be applied to the QComboBox objects.
        """
        super().__init__()

        if combo_box_style is not None:
            if isinstance(combo_box_style, ComboBoxStyle):
