...
class RadialGradient:
    ...
    def __init__(self, center_point: AxisPoint, radius: float, focal_point: AxisPoint, stops: list[GradientStop]):
        ...
        self.gradient_string = "qradialgradient(%s, %s)" % (
            ...
            % (center_point.x, center_point.y, radius, focal_point.x, focal_point.y),
...
class LinearGradient:
    ...
    def __init__(self, points: list[AxisPoint], stops: list[GradientStop]):
        ...
        self.gradient_string = "qlineargradient(%s, %s)" % (
            ", ".join(
                ["x%d:%g, y%d:%g" % (i + 1, point.x, i + 1, point.y) for point, i in zip(points, range(len(points)))]
            ),
            ", ".join(["stop:%g %s" % (stop.stop, stop.color_on_stop) for stop in stops]),
        )
...
class Brush:
    ...
    def __init__(self, color: Color | Gradient, palette_role: PaletteRole | None = None):
        self.brush = ""
        self.set_brush(color, palette_role)

    def set_brush(self, color: Color | Gradient, palette_role: PaletteRole | None = None):
...
class BoxColors:
    ...
    def set_color(self, brushes: Brush | list[Brush]):
        self.color = " ".join([brush.brush for brush in brushes]) if isinstance(brushes, list) else brushes.brush
        return self
