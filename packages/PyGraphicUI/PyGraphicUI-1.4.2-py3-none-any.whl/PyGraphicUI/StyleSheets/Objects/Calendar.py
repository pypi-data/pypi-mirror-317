            ...
            class YearEdit(ChainLineEditStyle):
                ...
                def __init__(self, **kwargs):
                    ...
                    parent_objects, kwargs = get_objects_of_style(
                        ("QCalendarWidget", Selector(SelectorFlag.Type)), **kwargs
                    )
                    super().__init__(
...
class CalendarStyleSheet(BaseStyleSheet):
    ...
    def __init__(
        self,
        calendar_style: CalendarStyle | list[CalendarStyle] | None = None,
    ):
        """
        Initializes a CalendarStyleSheet object.

        Args:
            calendar_style (CalendarStyle | list[CalendarStyle] | None): A CalendarStyle object or list of CalendarStyle objects representing the styles to be applied to the QCalendarWidget objects.
        """
        super().__init__()

        if calendar_style is not None:
            if isinstance(calendar_style, CalendarStyle):
