...
class ChainAbstractButtonStyle(BaseStyle):
    ...
    def __init__(
        ...
        icon: IconProperty | None = None,
        text: TextProperty | None = None,
        **kwargs
    ):
        """
        ...
        Args:
            parent_css_object (ObjectOfStyle | list[ObjectOfStyle]): The style sheet object or list of objects that the style is applied to, from which the button will inherit styles.
            ...
            icon (IconProperty | None): An IconProperty object representing the icon to be used for the button.
            text (TextProperty | None): A TextProperty object representing the text to be displayed on the button.
            **kwargs: Additional keyword arguments passed to the BaseStyle constructor.
        """
        new_parent_objects = get_new_parent_objects(
            parent_css_object, widget_selector, (button_type, Selector(SelectorFlag.Descendant))
        )
...
class AbstractButtonStyleSheet(BaseStyleSheet):
    ...
    def __init__(self, button_style: AbstractButtonStyle | list[AbstractButtonStyle] | None = None):
        """
        Initializes an AbstractButtonStyleSheet object.

        Args:
            button_style (AbstractButtonStyle | list[AbstractButtonStyle] | None): An AbstractButtonStyle object or list of AbstractButtonStyle objects representing the styles to be applied to the buttons.
        """
        super().__init__()
