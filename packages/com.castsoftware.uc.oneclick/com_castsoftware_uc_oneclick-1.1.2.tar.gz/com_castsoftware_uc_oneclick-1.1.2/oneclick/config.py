...
class Config():
    ...
    def command_line(self) -> ArgumentParser:
        ...
        parser.add_argument('--highlightCLI',required=False,help='Add Highlight CLI location and exit')
        parser.add_argument('--profilerCLI',required=False,help='Add profiler CLI location and exit')
...
#    def __init__(self,parser:ArgumentParser,default_args={},log_level: int=INFO):
    def __init__(self):
        ...
        if exists(base_config_name):
            ...
            if args.global_config or \
               ...
               not self.check_list(self.highlight,['cli']) or \
               (len(self.console)>0 and not self.check_list(self.console, ['cli','URL', 'user', 'token'])) :
    ...
    def setup_default_config(self,first=True):
        ...
        if self.highlightCLI:
            self.hl_cli = self.highlightCLI
            print(f'Setting Highlight CLI to {self.hl_cli}')
            exit(0)
        if self.profilerCLI:
            self.profiler = self.profilerCLI
            print(f'Setting Profiler CLI to {self.profiler}')
            exit(0)
        ...
        if first:
            ...
            msg = 'Detected some missing configuration information, running setup now.'
        print (
"""
...
OneClick consists of two main sections: the base and work locations. The base section holds all the
project configuration details, logs, and runtime code. The work section stores the results of the 
application code analysis and the generated reports. While these sections can be placed together, 
it is advisable to separate them when operating in a network setting.
...
""".format(msg=msg)
        ...
        if self.update_global or not self.quiet:
            self.work = folder_input('What is the working folder location',folder=self.work,create=True) 
...
========================================================================================================================
    ...
    @property
    def highlightCLI(self):
        return Config._args.highlightCLI

    @property
    def profilerCLI(self):
        return Config._args.profilerCLI
    ...
    def highlight_active(self):
        self.check_list(self.highlight,['hl_cli','URL','hl_user','hl_instance'])
    ...
    @property
    def mri(self):
        return self.check_list(self.console, ['cli','URL', 'user', 'token'])
