...
class RunAnalysis(Analysis):
                            ...
                            try:    
                                ...
                                while True:
                                    ...
                                    if line and len(line) > 0:
                                        ...
                                        break
                                app_log = '\t'.join(out_ary)
                                self.log.info(f'Analysis completed for {config.project_name}\{app_name}')
                                self.log.info(app_log)
    ...
    def can_run(self, appl):
        ...
        if app_status in [Status.DISCOVERY_REPORT_END, Status.ANALYSIS_START, Status.ANALYSIS_QUEUE] and app_name not in self.process:
            return True
        return False
