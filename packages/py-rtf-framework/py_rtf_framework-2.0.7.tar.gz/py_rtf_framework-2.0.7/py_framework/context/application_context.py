...
class ApplicationContext:
    ...
    def get_config_dict_by_prefix(self, config_prefix: str, required: bool = True) -> dict[str, Any]:
        """从应用配置中，根据配置前缀获取配置属性dict。如果是必填，配置数据为空，则抛出异常。"""
        config_props = self.app_environment.get_config(config_prefix)
    ...
    def get_config_value_by_key(self, config_key: str, default_value: Any | None = None) -> Any:
        """从应用配置中，根据配置Key获取配置属性值"""
        config_prop = self.app_environment.get_config(config_key)

        return config_prop or default_value

    def is_dev_environment(self) -> bool:
        """是否为开发环境"""
        app_bootstrap: AppBootstrap = self.app_environment.app_bootstrap
