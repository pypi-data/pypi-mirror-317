...
from mewtools.mydatetime import timezone
from mewtools.mydatetime import month_pair
from mewtools.mydatetime import extract_prep
...
timestamp_reg = re.compile(
    ...
    r"^(\d{13,})$|"  # 13位时间戳
    r"^(\d{10,}\.\d{1,7})$"
)  # 这种形式只匹配最近的时间戳，由1开始，列如 1654280040
...
def replace_comma(text):
    ...
    text = re.sub(
        r"日\D", r"日", text
    )  # 去掉 日后面的非数字干扰，列如：2022年7月12日T20:50:00
...
def _interchange_month_day(text):
    ...
    """
    interchanged_text = re.sub(
        r"(0?[1-9]|[12][0-9]|3[01])[/.](0?[1-9]|1[012])[/.](\d{4})", r"\2/\1/\3", text
    )
...
def str2datetime(text, custom_timezones, day_first):
    ...
    try:
        with warnings.catch_warnings():
            ...
            if maya_tmp.epoch < 0:
                ...
                maya_datetime = datetime(1970, 1, 1) - timedelta(
                    seconds=abs(maya_tmp.epoch)
                )
            else:
    ...
    try:
        ...
        dateutil_datetime = parser.parse(
            text, tzinfos=timezones_dic, dayfirst=day_first
        )
    except:
        pass
    ...
    if dateutil_datetime and maya_datetime and dateutil_datetime.tzinfo:
        ...
        matched = re.search(
            r"\D(\d{2}:\d{2}(\.\d{3,6})?([+-])\d{2}:?\d{2})($|\D)", text
        )
        if matched:
            # 获取相对于 UTC 的时间差
