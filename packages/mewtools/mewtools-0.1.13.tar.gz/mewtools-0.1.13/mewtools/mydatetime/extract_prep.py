...
from mewtools.mydatetime import month_pair
...
def format2year_month_day(text, day_first):
    ...
    def _to_ymd(matched):
        ...
        ymd_str = (
            f"{non_ymd_list[3]}-{non_ymd_list[0]}-{non_ymd_list[2]} {non_ymd_list[-1]}"
        )
        return ymd_str

    text = re.sub(
        r"(\d{1,2})([ /.·-]+)(\d{1,2})\2(\d{4})[^\d](\d{1,2}:\d{2})", _to_ymd, text
    )
    """↑ mm/dd/yy | dd/mm/yy 格式处理结束 ↑"""

    """↓ yy-mm-dd 格式处理开始 ↓"""
    text = re.sub(
        r"\D([12]\d{3})([/.·-]+)(\d)\2(\d{2})\D", r" \1-0\3-\4 ", text
    )  # 修改 单位数月份为双位数
    text = re.sub(
        r"\D([12]\d{3})([/.·-]+)(\d{2})\2(\d)\D", r" \1-\3-0\4 ", text
    )  # 修改 单位数日期为双位数

    text = re.sub(
        r"([12]\d{3})([ /.·]+)(\d{2})\2(\d{2})\D", r"\1-\3-\4 ", text
    )  # 修改其他日期分隔符为-

    text = _filter_middle_part(text)  # 过滤日期和时间中间的干扰

    time_reg = re.compile(
        r"(\d{4}-\d{2}-\d{2}) {1,2}([0-2]?\d(:[0-6]\d){1,2})( ?am| ?pm| ?a.m.| ?p.m.|[^:\d])",
        re.I,
    )  # 第一组为日期，第二组为时间，第三组为 am 或 pm
    text = re.sub(time_reg, _final_time_callback, text)  # 修改 单位数小时为双位，补全秒

    text = re.sub(
        r"(\D)(\d{4}-\d{2}-\d{2} [0-2]\d:[0-6]\d)", r";\2", text
    )  # 日期前面有数字会影响提取，此处用分隔符隔开，如 0.105 2021-04-01 to 0.105;2021-04-01
    """↑ yy-mm-dd 格式处理结束 ↑"""
...
def _replace_named_month(text):
    ...
    reg1 = re.compile(
        r"(?<=[^a-zA-Z])(January|February|March|April|May|June|July|August|September|October|November|December)(?=[/.,\d -]+)",
        re.I,
    )  # 匹配月份全称
    step1_text = re.sub(reg1, _month_callback, text)  # 月份转换后用 '龘' 作为识别符

    reg1 = re.compile(
        r"(?<=[^a-zA-Z])(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\.?(?=[/.,\d -]+)",
        re.I,
    )  # 匹配月份简写，可以匹配 Jan. 这样的简写。 https://regexr.com/6n41e
    step1_text = re.sub(
        reg1, _month_callback, step1_text
    )  # 月份转换后用 '龘' 作为识别符
    ...
    # 月份在中间的情况
    reg2 = re.compile(
        r"(\d{4}|\d{1,2})([ /.·-]+)(\d{2}龘)\2(\d{4}|\d{1,2})[\D]"
    )  # 英文名月份很可能由空格分割
    step2_text = re.sub(reg2, _sort_ymd_by_mark_callback, step1_text)

    # 月份在前的情况
    reg3 = re.compile(
        r"(\d{2}龘)([ /.·-]+)(\d{4}|\d{1,2})\2(\d{4}|\d{1,2})[^:\d]"
    )  # 英文名月份很可能由空格分割
...
def timez_str_convert(text):
    """text 中时间字符串转换成偏移格式"""
    text = re.sub(
        r"(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2}) GMT([+-])([:\d]+)",
        _gmt2offset,
        text,
    )
    return text
