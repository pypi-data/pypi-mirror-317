...
def get_media(cls, media_id):
    """
    Get media info from WP
    :param cls: wordpress object
    :param media_id: media id
    :return: Json/None
    """
    api_url = f"{cls.api_url}/media/{media_id}"
    r = Carehttp().get(url=api_url, auth=(cls.wp_user, cls.api_key), verify=cls.verify)
    if r.status_code != 200:
        logger.error(f"Get media {media_id} failed: {r.status_code}")
        return
...
def post_file(cls, target_file, title):
    ...
    try:
        new_file = {
            ...
            "file_name": f"{title}.{kind.extension}",  # use title as file name
            "mine": kind.mime,
...
def upload2wp(cls, file_obj):
    ...
    r = Carehttp(file_obj["file_name"]).post(
        url=api_url,
        data=multipart_data,
