...
def generate_avro_and_mapping(sql_file_path, avro_output_path, json_output_path, sql_output_path, date_format):
    ...
    # Extraer la sección de definición de columnas dentro de los paréntesis
    columns_section_match = re.search(r'CREATE TABLE\s+"[^"]+"\."[^"]+"\s*\((.*?)\)\s*', sql_content, re.IGNORECASE | re.DOTALL)
    if not columns_section_match:
        raise ValueError("No se pudo encontrar la sección de definición de columnas en el archivo SQL.")
    
    columns_section = columns_section_match.group(1)

    # Definir tipos de datos permitidos para evitar capturar restricciones y configuraciones
    allowed_data_types = [
        "VARCHAR2", "NUMBER", "DATE", "CHAR", "CLOB", "NCLOB", "RAW", "BLOB"
    ]
    # Crear una regex que solo capture líneas que definen columnas con tipos de datos permitidos
    type_pattern = '|'.join(allowed_data_types)
    column_regex = re.compile(
        rf'^\s*"([^"]+)"\s+({type_pattern})(?:\s*\([^\)]+\))?(?:\s+NOT NULL ENABLE)?\s*(?:,|$)',
        re.IGNORECASE | re.MULTILINE
    )

    # Extraer columnas usando la regex mejorada
    columns = column_regex.findall(columns_section)

    if not columns:
        raise ValueError("No se encontraron columnas válidas en la definición de la tabla.")
...
def parse_oracle_schema(schema_file):
    ...
    También detecta las claves primarias definidas en la tabla, excluyendo constraints y configuraciones adicionales.
    """
    with open(schema_file, 'r') as file:
        content = file.read()

    # Filtrar solo las líneas de definición de columnas válidas
    column_lines = re.findall(
        r'^\s*"([A-Za-z0-9_]+)"\s+([A-Z0-9\(\),\s]+)(?:\s+NOT NULL ENABLE)?(?:,|$)',
        content, re.MULTILINE
    )

    # Excluir constraints, tablespaces, y otros elementos adicionales
    excluded_keywords = {"CONSTRAINT", "TABLESPACE", "SUPPLEMENTAL", "LOG", "STORAGE", "PCTFREE", "INITRANS", "ENABLE"}
    filtered_lines = [
        line for line in column_lines
        if not any(keyword in line[0].upper() for keyword in excluded_keywords)
    ]
    ...
    fields = []
    for column_name, column_type in filtered_lines:
        ...
        is_not_null = "NOT NULL" if "NOT NULL" in column_type else ""
