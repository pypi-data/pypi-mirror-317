...
class ArtifexController(ISphereAbstract):
    ...
    def get_power_measure(self) -> float:
        ...
        # Extract the numeric part and unit (e.g., 'I0,36uA', 'I0,36mA', 'I0,36nA')
        match = re.search(
            r"I(\d+),(\d+)([a-zA-Z]+)", measuredvalue
        )  # Match 'I<whole>,<fraction><unit>'

        if match:
            # Combine the extracted parts into a float (e.g., '0,36' -> 0.36)
            measured_value = float(f"{match.group(1)}.{match.group(2)}")
    ...
    def set_power_gain(self, gain: int = 3) -> bool:
        """
