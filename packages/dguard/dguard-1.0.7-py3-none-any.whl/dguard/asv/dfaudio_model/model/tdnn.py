...
from dguard.asv.dfaudio_model.model.pooling import (
    AttentiveStatsPool,
    MlutiheadAttentiveStatsPool,
    MlutiheadAttentiveStatsPool3,
)
...
class SCBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = nn.Conv1d(group_width * 2, planes, kernel_size=1, bias=False)
...
class TDNN(nn.Module):
    ...
    def __init__(self, channel, feature_dim, context=True, **kwargs):
        ...
        self.conv1 = nn.Conv1d(
            feature_dim, channel, kernel_size=5, stride=1, padding=2
        ).cuda()
        self.relu = nn.ReLU().cuda()
        self.bn1 = nn.BatchNorm1d(channel).cuda()

        self.layer1 = Bottle2neck(
            channel, channel, kernel_size=3, dilation=2, scale=8
        ).cuda()
        self.layer2 = Bottle2neck(
            channel, channel, kernel_size=3, dilation=3, scale=8
        ).cuda()
        self.layer3 = Bottle2neck(
            channel, channel, kernel_size=3, dilation=4, scale=8
        ).cuda()

        self.layer5 = nn.Conv1d(3 * channel, channel * 3, kernel_size=1).cuda()

        cat_channel = channel * 3

        self.pooling = AttentiveStatsPool(cat_channel, 128, context=True).cuda()
        self.bn5 = nn.BatchNorm1d(cat_channel * 2).cuda()
        self.fc6 = nn.Linear(cat_channel * 2, 256).cuda()
...
if __name__ == "__main__":
    ...
    os.environ["CUDA_VISABLE_DEVICE"] = "3,5,6,7"
    # print(
    #     summary(
    #         TDNN(channel=512, feature_dim=60, pooling_way="ASP", context=True),
    #         torch.randn((32, 60, 400)),
    #         show_input=False,
    #     )
    # )
