...
class Tradehull:    
	...
	def get_live_pnl(self):
		...
		try:
			...
			for pos_ in pos_book_dict:
				...
				underlying1 = instrument_df[((instrument_df['SEM_SMST_SECURITY_ID']==security_id))].iloc[-1]['SEM_TRADING_SYMBOL']
				underlying2 = instrument_df[((instrument_df['SEM_SMST_SECURITY_ID']==security_id))].iloc[-1]['SEM_CUSTOM_SYMBOL']
				ltp_list.append(underlying1.upper())
				ltp_list.append(underlying2.upper())

			ltp_data = self.get_pnl_ltp_data(ltp_list)

			for pos_ in pos_book_dict:
				security_id = int(pos_['securityId'])
				underlying1 = instrument_df[((instrument_df['SEM_SMST_SECURITY_ID']==security_id))].iloc[-1]['SEM_TRADING_SYMBOL']
				underlying2 = instrument_df[((instrument_df['SEM_SMST_SECURITY_ID']==security_id))].iloc[-1]['SEM_CUSTOM_SYMBOL']
				if underlying1.upper() in ltp_data:
					closePrice = ltp_data[underlying1]
				elif underlying2.upper() in ltp_data:
					closePrice = ltp_data[underlying2]
				else:
					raise Exception(f"Underlying {underlying1} and {underlying2} not found in ltp data")
	...
	def get_pnl_ltp_data(self,names, debug="NO"):
		try:
			instrument_df = self.instrument_df.copy()
			instruments = {'NSE_EQ':[],'IDX_I':[],'NSE_FNO':[],'NSE_CURRENCY':[],'BSE_EQ':[],'BSE_FNO':[],'BSE_CURRENCY':[],'MCX_COMM':[]}
			instrument_names = {}
