...
class HPLCController:
    ...
    def _check_data_status(self):
        ...
        self._set_status()
        ready_with_data = isinstance(self._most_recent_hplc_status,
                                     HPLCAvailStatus) and old_status != self._most_recent_hplc_status
    ...
    def _send(self, cmd: Command, cmd_no: int, num_attempts=5):
        """
        ...
        for _ in range(num_attempts):
            ...
            try:
                with open(self.cmd_file, "w", encoding="utf8") as cmd_file:
                    cmd_file.write(f"{cmd_no} {cmd.value}")
            except IOError as e:
                err = e
                self.logger.warning("Failed to send command; trying again.")
                continue
            else:
                self.logger.info("Sent command #%d: %s.", cmd_no, cmd.value)
                return
        else:
            raise IOError(f"Failed to send command #{cmd_no}: {cmd.value}.") from err
    ...
    def switch_method(self, method_name: str, method_dir=DEFAULT_METHOD_DIR):
        ...
        """
        self.send(
            Command.SWITCH_METHOD_CMD.value.format(method_dir=method_dir, method_name=method_name)
        )
    ...
    def load_method_details(self, method_name: str, method_dir=DEFAULT_METHOD_DIR) -> HPLCMethodParams:
        """
        TODO: this method is not complete yet, and just returns dummy object
        Args:
            method_name:
            method_dir:
        ...
        return HPLCMethodParams(
            organic_modifier=Param(val=None, chemstation_key=RegisterFlag.SOLVENT_B_COMPOSITION),
            flow=Param(val=None, chemstation_key=RegisterFlag.FLOW),
            maximum_run_time=Param(val=None, chemstation_key=RegisterFlag.MAX_TIME),
            temperature=Param(val=None,
                              chemstation_key=[RegisterFlag.COLUMN_OVEN_TEMP1, RegisterFlag.COLUMN_OVEN_TEMP2]),
            inj_vol=Param(val=None, chemstation_key=None),
            equ_time=Param(val=None, chemstation_key=None)
        )
