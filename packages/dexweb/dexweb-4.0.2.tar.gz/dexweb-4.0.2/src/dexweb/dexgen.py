...
class Dexgen:
    def __init__(self):
        ...
        self.dummy_json_folder = os.path.join(root, 'dummy_json')
      
        updates=None
        if os.path.exists(self.update_path) and len(os.listdir(self.update_path))!=0:
            if len(os.listdir(self.update_path))!=1 or os.listdir(self.update_path)[0]!='clear':
    ...
    def create_indexpage(self,pages):
        ...
        <head>
            ...
            <title>{self.dexname} Dex</title>
        </head>
        <body>
            <br><h1>{self.dexname.upper()} DEX</h1><br>
            {k}
            <br><br><br><br><br><br><br>
            <h3><a href="index.html">{self.dexname.upper()} DEX</a></h3>
        </body>
        </html>
        """
        return template
    ...
    def create_webpage(self, title, body):
        ...
        template = f"""<html lang="en">
            <head>
                ...
                <link rel="shortcut icon" type="image/x-icon" href="assets/favicon.ico">
                <link rel="stylesheet" href="styles.css">
                <title>{title} - {self.dexname} Dex</title>
            </head>
            <body>
            <br>
            <h1>{title.upper()}</h1>
