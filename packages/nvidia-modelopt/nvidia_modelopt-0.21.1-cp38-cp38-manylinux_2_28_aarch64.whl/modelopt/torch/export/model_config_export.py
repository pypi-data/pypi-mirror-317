...
from .quantization_utils import process_layer_quant_config
...
def torch_to_tensorrt_llm_checkpoint(
    ...
    if has_mcore and isinstance(model, MegatronModule):
        ...
        architecture = (
            model.config.architectures[0] if model.config.architectures is not None else None
        )
    ...
    for model in models:
        ...
        for layer in config.layers:
            if layer.quantization != QUANTIZATION_NONE:
                config.quantization = layer.quantization
                break
        ...
        for model_config in model_configs:
            ...
            tensorrt_llm_config = convert_to_tensorrt_llm_config(model_config, weights.keys())
...
def export_tensorrt_llm_checkpoint(
    ...
    try:
        ...
        for (
            ...
            exclude_modules.update(tensorrt_llm_config["quantization"].get("exclude_modules", {}))
            # For T5 model
