...
class QKVConfig:
    ...
    def weights_scaling_factor(self):
        ...
        if self.q.weights_scaling_factor.numel() != 1:
            ...
            if self.q.weights_scaling_factor.dtype == torch.float8_e4m3fn:
                ...
                qkv_weights_scaling_factor = NVFP4QTensor.get_weights_scaling_factor(
    ...
    def quantization(self):
        ...
        assert (
            self.q.quantization == self.k.quantization == self.v.quantization
        ), "Quantization format of QKV should be the same."
        return self.q.quantization
...
class MLPConfig:
    ...
    def quantization(self):
        ...
        assert self.merged_fc1_gate, "Gate and fc layers are not merged"
        assert self.fc == self.gate, "Quantization format of fc and gate should be the same."
        return self.fc.quantization
