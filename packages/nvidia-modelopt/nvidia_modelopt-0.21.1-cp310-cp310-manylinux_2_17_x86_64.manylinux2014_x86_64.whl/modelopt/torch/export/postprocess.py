                        ...
                        if config.awq_block_size != 0:
                            ...
                            if w_scaling_factors[0].ndim == 2:
                                ...
                                if scaling_factors_total_size != config.weight.numel():
                                    ...
                                    config.weights_scaling_factor, _ = (
                                        get_weights_scaling_factor_and_amax(
...
def update_lm_head_quantization(
    ...
    # Disable lm_head quantization for TRTLLM
    if get_quantization_format(lm_head) != QUANTIZATION_NONE:
        disable_lm_head_quantization = True

    assert config.layers is not None, "Expect layers in the model config."
    # TRT-LLM currently hardcodes llm_head dtype of FP8 quantized model as the default dtype (fp16/bf16)
    for layer in config.layers:
        if layer.quantization == QUANTIZATION_FP8:
            disable_lm_head_quantization = True
        ...
        if isinstance(field_value, torch.Tensor):
            ...
            for i, v in enumerate(field_value):
                if isinstance(v, torch.Tensor):
                    ...
                    postprocess_tensors(v)  # type: ignore[arg-type]
        elif is_dataclass(field_value):
            postprocess_tensors(field_value)  # type: ignore[arg-type]
