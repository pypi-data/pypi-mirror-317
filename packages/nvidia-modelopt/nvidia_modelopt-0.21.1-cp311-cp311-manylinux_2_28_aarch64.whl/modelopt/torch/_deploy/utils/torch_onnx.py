...
from .onnx_utils import _get_onnx_external_data_tensors
...
def get_onnx_bytes_and_metadata(
    ...
    with torch.inference_mode():
        if not dynamo_export:
            ...
            if Version(torch.__version__) < Version("2.1"):
                raise ValueError("Dynamo export is only supported for torch>=2.1")
    ...
    if len(onnx_opt_graph.SerializeToString()) > TWO_GB:
