...
class KernelVirtualDriver:
    ...
    def __init__(
        ...
        if conn:
            ...
            user, password = auth
            if password:
                auth = [[libvirt.VIR_CRED_AUTHNAME, libvirt.VIR_CRED_PASSPHRASE], request_cred(user, password), None]
                self.conn = libvirt.openAuth(
                    f"qemu+libssh2://{user}@{host}/system?sshauth=password&known_hosts_verify=auto", auth, 0
                )
            elif ssh_key:
...
class KernelVirtualMachineDriver(KernelVirtualDriver):
    ...
    def __init__(self, host: str, uuid: str | None = None, **kwargs) -> None:
        KernelVirtualDriver.__init__(self, host=host, **kwargs)
            ...
            ret.append(
                {
                    ...
                    "cpu": cpu,
                    "ram": ram,
                    "driver": KernelVirtualSnapshotDriver(vm_id=self.uuid, snap_id=snap.getName(), conn=self.conn),
                }
            )
...
class KernelVirtualHostDriver(KernelVirtualDriver):
    ...
    def __init__(self, address: str, auth: tuple[str]) -> None:
        KernelVirtualDriver.__init__(self, host=address, auth=auth)
        self.address = address

    def virtual_machine_drivers(self) -> list[KernelVirtualMachineDriver]:
        """Get list of virtual machines on the hypervisor"""
        return [
            KernelVirtualMachineDriver(host=self.host, uuid=domain.UUIDString(), conn=self.conn)
            for domain in self.conn.listAllDomains()
        ]

    def import_vm(self, source: str, storage: str, name: str) -> KernelVirtualMachineDriver:
  ...
  <target>
    ...
    <permissions>
      ...
      <label>virt_image_t</label>
        ...
        return KernelVirtualMachineDriver(host=self.host, uuid=result_domain.UUIDString(), conn=self.conn)
...
class KernelVirtualSnapshotDriver:
    ...
    def apply(self) -> None:
        """Apply the snapshot"""
        snap = self.domain.snapshotLookupByName(self.identifier)
