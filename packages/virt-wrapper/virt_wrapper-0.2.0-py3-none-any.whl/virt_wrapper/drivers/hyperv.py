...
HYPERV_USER = os.environ.get("SSH_USER", getpass.getuser())
...
class HyperVirtualDriver:
    ...
    def __init__(
        self,
        host: str,
        auth: tuple[str] = (HYPERV_USER, ""),
        winrm_session: winrm.Session = None,
        ...
        if conn:
            ...
            auth = [[libvirt.VIR_CRED_AUTHNAME, libvirt.VIR_CRED_PASSPHRASE], request_cred(auth[0], auth[1]), None]
            self.conn = libvirt.openAuth(f"hyperv://{host}/?transport=https", auth, 0)
...
class HyperVirtualMachineDriver(HyperVirtualDriver):
    ...
    def __init__(
        self, host: str, uuid: str, winrm_session: winrm.Session = None, conn: libvirt.virConnect = None, **kwargs
    ) -> None:
