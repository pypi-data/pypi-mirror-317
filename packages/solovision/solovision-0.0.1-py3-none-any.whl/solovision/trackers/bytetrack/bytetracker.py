...
from solovision.trackers.bytetrack.basetrack import BaseTrack, TrackState
from solovision.utils.matching import (embedding_distance, fuse_score,
                                   iou_distance, linear_assignment)
from solovision.trackers.basetracker import BaseTracker
    ...
    def _create_detections(self, dets_first, features_high):
        if len(dets_first) > 0:
            if self.with_reid:
                ...
                detections = [STrack(det) for det in dets_first]
        else:
            detections = []
        return detections
