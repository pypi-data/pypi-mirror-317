...
class WorkingUniversal:
    def __init__(self, data):
        ...
        for vol, oi in zip(self.volume, self.open_interest):
            ...
            # Calculate Time to Expiry (DTE) in days
        self.dte = [
        ...
        # Intrinsic Value: Handles None for S, K, and ct
        ...
        # Extrinsic Value: Handles None for midpoint and intrinsic
        ...
        # DTE (Days to Expiry): Handles None for expiration_date
        self.dte = [
            max((datetime.datetime.fromisoformat(exp_date) - datetime.datetime.now()).days, 0)
        ...
        # Time to Maturity (T): Converts DTE to years, handles None
        self.time_to_maturity = [
            d / 365.0 if d is not None else None
            for d in self.dte
        ]

        # Spread: Handles None for ask and bid
        self.spread = [
            (ask - bid) if ask is not None and bid is not None else None
            for ask, bid in zip(self.ask, self.bid)
        ]

        # Spread Percent: Handles None for ask, bid, and midpoint
        self.spread_percent = [
        ...
        # Premium Percent: Handles None for midpoint and underlying_price
        ...
        if valid_iv_strike_pairs:
            # Sort by implied_volatility ascending
        ...
        if self.lowest_iv_strike_price is not None and self.underlying_price:
            # For safety, pick the first non-None underlying price 
            # (or do an average if you have multiple?).
