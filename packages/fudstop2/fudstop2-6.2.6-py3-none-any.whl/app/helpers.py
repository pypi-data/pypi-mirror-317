...
def parse_xml_from_url(url):
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to fetch XML: HTTP {response.status_code}")

    # Parse XML content
    root = ET.fromstring(response.content)

    # Assuming you want to parse specific elements from your XML
    # Modify this as per the structure of your XML
    data = []
    for child in root:
        element_data = {}
        for element in child:
            element_data[element.tag] = element.text
...
def parse_image_type(url):
    """
    Parse the URL to extract the image type.
    :param url: The image URL
    :return: The extracted image type or None if not found
    """
    match = re.search(r'/([^/]+)\.png', url)
    return match.group(1) if match else None
def clean_image_type(image_type):
    """
    Clean the image type by removing any part after the last underscore.
    :param image_type: The extracted image type
    :return: The cleaned image type
    """
    parts = image_type.split('_')
    # If there is only one part, there is no underscore, return as is
    if len(parts) == 1:
        return image_type
...
async def save_screenshot_to_db(url, ticker, filing_type):
    # Start the BrowserMob Proxy server

    server = Server(path="browsermob_ok/bin/browsermob-proxy.bat")
    server.start()
    ...
    try:
        ...
        await conn.execute('''
            INSERT INTO screenshots (image_data, url, ticker, filing_type) VALUES ($1, $2, $3, $4)
        ''', screenshot_bytes, url, ticker, filing_type)
    finally:
...
def parse_delimited_text(text_content, delimiter='|'):
    data = []
    f = io.StringIO(text_content)
    reader = csv.DictReader(f, delimiter=delimiter)
    
    for row in reader:
        # Clean up empty keys if any (due to extra delimiters)
...
# Fetch all URLs
def convert_ms_timestamp(timestamp_ms):
...
def get_cik_by_ticker(df, ticker):
    row = df[df['ticker'] == ticker]
    if not row.empty:
        # Convert the CIK number to an integer to remove the decimal part
        cik = int(row.iloc[0]['cik'])
        return str(cik)  # Convert it back to string if necessary
    else:
        return None