import re
import questionary
from pyravel.utils.helper import create_file

def create_model(folder_name: str, class_name: str):
    model_data = f"""
...
"""
    ...
    create_file(filepath, model_data)


def schema_create(folder_name: str, class_name: str):
    model_data = f"""
...
def schema_update(folder_name: str, class_name: str):
    model_data = f"""
...
def schema_list(folder_name: str, class_name: str):
    model_data = f"""
...
def schema_one(folder_name: str, class_name: str):
    model_data = f"""
from sqlmodel import SQLModel
...
def schema_remote(folder_name: str, class_name: str):
    model_data = f"""
from sqlmodel import SQLModel
from typing import List
...
def schema_delete(folder_name: str, class_name: str):
    model_data = f"""
from sqlmodel import SQLModel
...
def create_service(folder_name: str, class_name: str):
    model_data = f"""
...
def create_repository(folder_name: str, class_name: str):
    model_data = f"""
...
def create_controller(folder_name: str, class_name: str):
    model_data = f"""
...
@handle_exceptions("get_one_record")
...
@handle_exceptions("get_list_record")
...
@handle_exceptions("get_remote_record")
...
def modify_file(file_path: str, import_stm: str, router_stm: str):
    ...
    if not lines:  # Check if the file is empty
        ...
        with open(file_path, 'w') as file:
            file.write(import_stm)
            file.write(router_stm)
    else:
...
def register_api(folder_name: str, class_name: str):
    ...
    modify_file(filepath, import_stm, router_stm)


def register_model(folder_name: str, class_name: str):
    create_file("app/models/register_models.py", "")
...
def start_create():
    ...
    create_model(folder_name, class_name)
    schema_create(folder_name, class_name)
    schema_list(folder_name, class_name)
    schema_remote(folder_name, class_name)
    schema_one(folder_name, class_name)
    schema_update(folder_name, class_name)
    schema_delete(folder_name, class_name)
    create_service(folder_name, class_name)
    create_repository(folder_name, class_name)
    create_controller(folder_name, class_name)
    register_api(folder_name, class_name)
    register_model(folder_name, class_name)