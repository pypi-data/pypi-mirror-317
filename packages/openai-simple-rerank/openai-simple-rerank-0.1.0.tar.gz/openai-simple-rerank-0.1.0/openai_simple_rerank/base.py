...
def get_rerank_scores(
    ...
    if len(query) > max_size:
        _logger.warning(
            "get_rerank_scores: the query string exceeds the limit, max_size=%s, query_size=%s",
            max_size,
            len(query),
        )
    ...
    for document_index in range(len(documents)):
        ...
        if len(document) > max_size:
            _logger.warning(
                "get_rerank_scores: the document string exceeds the limit, max_size=%s, document_size=%s, document_index=%s",
                max_size,
                len(document),
                document_index,
            )
