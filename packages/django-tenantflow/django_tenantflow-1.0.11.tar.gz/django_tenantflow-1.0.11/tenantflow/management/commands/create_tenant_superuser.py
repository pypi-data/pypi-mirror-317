...
class Command(BaseCommand):
    help = "Create a superuser in a specific tenant"
    ...
    def handle(self, *args, **kwargs):
        ...
        # Cambiar al esquema del tenant y crear el superusuario
        self.stdout.write(f"[TenantFlow] Switching to schema '{schema_name}'...")
        with connection.cursor() as cursor:
            cursor.execute(f"SET search_path TO {schema_name}")

        User = get_user_model()
        if User.objects.filter(username=username).exists():
            self.stderr.write(self.style.ERROR(f"User '{username}' already exists in schema '{schema_name}'."))
            return

        # Crear el superusuario
        self.stdout.write(f"[TenantFlow] Creating superuser '{username}'...")
        User.objects.create_superuser(username=username, email=email, password=password)
        self.stdout.write(self.style.SUCCESS(f"[TenantFlow] Superuser '{username}' created successfully in schema '{schema_name}'."))
