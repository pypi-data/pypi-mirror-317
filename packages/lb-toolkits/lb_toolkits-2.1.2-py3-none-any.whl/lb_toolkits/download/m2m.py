...
class M2M(object):
    ...
    def createToken(self, token_description):
        ...
        return token_description, token

    def deleteToken(self, token_description):

        token_id  = self.searchToken(token_description)
        ...
        if rsp.json().get('success') :
            logger.info('成功删除【%s】' % token_description)
        else:
            logger.error('失败删除【%s】' % token_description)

    def searchToken(self, token_id):

        rsp = self.session.get(USGS_ROOT_URL)
