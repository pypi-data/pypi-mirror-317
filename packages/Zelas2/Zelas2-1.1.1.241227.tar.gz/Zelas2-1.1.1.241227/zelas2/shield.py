...
def find_CS_FixedLength(txti_in,num_cpu=mp.cpu_count()):
    '环缝提取（固定长度版）'
    # 点云强度值离散化
    ...
    c_reduce = np.unique(id_c_ry)  # 精简环缝下标
    c_reduce = c_reduce.astype(int)
    # np.savetxt('E:\\2023硕士研究上\\0：盾构隧道论文\\c_id_25421.txt', c_reduce)
    print('环缝id', c_reduce)
    c_name_reduce = c_un[c_reduce]  # 细挑后的环缝名
    ...
    # np.savetxt('E:\\2023硕士研究上\\0：盾构隧道论文\\FIG2\\25421\\25421.RB_S.txt', self.RB_S, fmt='%.05f')
    # np.savetxt('E:\\2023硕士研究上\\0：盾构隧道论文\\FIG2\\25421\\25421.RB_E.txt', self.RB_E, fmt='%.05f')
    # np.savetxt('E:\\2023硕士研究上\\0：盾构隧道论文\\FIG2\\25421\\搜索区间.txt', c_name_ins)
    '找到对应y值'
