...
from lmdeployv1.api import BatchChatCompletionRequest, BatchChatCompletionResponse, LimiterConfig
...
class MessageBuilder:
    ...
    def build_chat_messages(
        ...
        Build chat messages.
        Args:
            kv: Dictionary with format {"image_url": "prompt"}
        Returns:
    ...
    def build_batch_messages(
        ...
        """
        Build batch chat messages from list of key-value pairs.
        Args:
            kv_list: List of dictionaries with format [{"image_url": "prompt"}, ...]
        Returns:
            List of lists of ChatCompletionMessageParam
        """
        return [MessageBuilder.build_chat_messages(kv) for kv in kv_list]
...
class LMDeployClient:
    ...
    def _setup_endpoint(self) -> None:
        ...
        if self.base_url == "":
            self.base_url = "v1"
        self.endpoint = (
            f"{self.endpoint.rstrip('/')}/{self.base_url.strip('/')}/"
        )
    ...
    def _setup_limiter(self) -> None:
        ...
        if isinstance(self.limiter_config, LimiterConfig):
            self._limiter = Limiter(
                RequestRate(
                    self.limiter_config.limit,
                    self.limiter_config.interval
                )
            )
    ...
    def _build_headers(self) -> Dict[bytes, bytes]:
        ...
        if self.context:
            headers.update({
                b"x-impersonate-target-org-id": self.context.get("OrgID", "").encode("utf-8"),
                b"x-impersonate-target-user-id": self.context.get("UserID", "").encode("utf-8"),
                b"x-impersonate-target-project-id": self.context.get("ProjectID", "").encode("utf-8"),
            })
        return headers
    ...
    def _create_completion_params(self, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            "n": kwargs.get("n", 1),
            "max_completion_tokens": kwargs.get("max_completion_tokens", self.DEFAULT_MAX_TOKENS),
