        ...
        async with self._event_broadcaster._context_manager_lock:
            if self._listen:
                ...
                if self._event_broadcaster._listen_count == 1:
                    ...
                    logger.info(
                        "Vixhal Listening for incoming events from broadcast channel (first listener started)"
                    )
            ...
            if self._share:
                ...
                if self._event_broadcaster._share_count == 1:
                    ...
                    logger.debug(
                        "Vixhal Subscribing to ALL_TOPICS, and sharing messages with broadcast channel"
                    )
