                ...
                if callable(attr) and hasattr(attr, "__annotations__"):
                    ...
                    if attr_name in HTTP_METHODES:
                        ...
                        if params.get("response_class") == HTMLResponse:
                            # Configurer le dossier contenant les templates
                            template_folder = "components"  # Use relative path for better portability
                            env = Environment(loader=FileSystemLoader(template_folder))
                            template = env.get_template("index.html")
                            # Rendre le template avec des données
                            render = template.render( asyncio.run(attr))
                            
                            appModule.add_api_route(
                                path=pathname,
                                endpoint= lambda: render ,
                                methods=[method],
                                **params 
                            )
                        else:
