def 邮件IMAP_按日期和主题搜索邮件(连接对象, 年, 月, 日, 主题, 条件="SINCE"):
    """
    按日期和主题搜索邮件。
    ...
    示例:
        邮件ID列表 = 邮件IMAP_按日期和主题搜索邮件(连接对象, 2024, 5, 4, "[Important]")
        邮件ID列表 = 邮件IMAP_按日期和主题搜索邮件(连接对象, 2024, 5, 4, "[Important]", 'BEFORE')
    """
    try:
        月份名称 = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
        日期 = f'{日}-{月份名称[月 - 1]}-{年}'
        搜索条件 = f'({条件} "{日期}") (SUBJECT "{主题}")'
        # print(搜索条件)
