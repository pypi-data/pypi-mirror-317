...
def 文件_封装Py代码成EXE_普通编译(脚本路径):
    ...
    try:
        ...
        # 如果找不到 pyinstaller，抛出异常
        if not os.path.exists(pyinstaller_path):
            raise FileNotFoundError(f"未找到 pyinstaller，请确保它已正确安装。当前路径：{pyinstaller_path}")

        # 创建一个临时目录
        with tempfile.TemporaryDirectory() as 临时目录:
            # 使用 subprocess.run 调用 pyinstaller 命令，将脚本打包成 EXE 文件，并指定输出目录为桌面
            subprocess.run([pyinstaller_path, '--distpath', 桌面路径, '--workpath', 临时目录, '--specpath', 临时目录, 脚本路径], check=True)
        ...
        # 如果找不到 pyinstaller，输出错误信息
        print(str(e))


# 调用函数进行打包
文件_封装Py代码成EXE_普通编译(r"F:\Python源码\微软号绑定验证秘钥\20241229A.py")
