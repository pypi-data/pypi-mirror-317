...
from typing import (
    Tuple, TextIO, Literal, TypeAlias, List, Union
)

import importlib.resources

import os, webbrowser, http.server, pathlib
...
def splitUpPath(path: str) -> List[str]:
    """
    Splits up a path into individual directory/file parts, removing all
...
def fileURL(path: Union[pathlib.Path, str]) -> str:
    """
    ...
    tailParts = splitUpPath(tail)
    return 'file://' + drive + '/' + '/'.join(tailParts)
