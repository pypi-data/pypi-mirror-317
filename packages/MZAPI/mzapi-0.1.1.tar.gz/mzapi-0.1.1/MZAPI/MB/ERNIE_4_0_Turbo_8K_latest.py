...
class ERNIE_4_0_Turbo_8K_latest:
    """
    ERNIE_4_0_Turbo_8K_latest 类用于与百度ERNIE-4.0-Turbo-8K-latest模型进行交互。
    初始化参数:
    ...
    主要方法:
    - get_response: 发送请求到ERNIE-4.0-Turbo-8K-latest模型并获取响应
    """
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            host_name="ERNIE-4.0-Turbo-8K-latest",
            token="kCrxvCIYEzhZfAHETXEB",
            peer_service="ERNIE-4.0-Turbo-8K-latest",
            peer_instance=" 180.97.107.95:443",
            peer_address=" 180.97.107.95",
            peer_ipv6="240e:ff:e020:934:0:ff:b0dc:3636",
            http_host="https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-4.0-turbo-8k-latest",
            server_name="MZAPI",
        )
    ...
    def get_response(
        ...
        """
        发送请求到ERNIE-4.0-Turbo-8K-latest模型并获取响应。
        ...
        with self.tracer.start_as_current_span("ERNIE-4.0-Turbo-8K-latest") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-4.0-Turbo-8K-latest", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
