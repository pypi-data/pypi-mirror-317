...
class ERNIE_4_0_8K_latest:
    """
    ERNIE_4_0_8K_preview 类用于与百度ERNIE 4.0 8K-Latest模型进行交互。
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-4.0-8K- Latest",
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-4.0-8K-Latest") as span:
            ...
            self.log.start_process_log(response.json(), "ERNIE-4.0-8K-Latest", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
