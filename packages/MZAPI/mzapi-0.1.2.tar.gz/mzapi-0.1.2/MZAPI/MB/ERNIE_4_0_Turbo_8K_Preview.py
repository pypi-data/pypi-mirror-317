...
class ERNIE_4_0_Turbo_8K_Preview:
    """
    ERNIE_4_0_Turbo_8K_Preview 类用于与百度ERNIE-4.0-Turbo-8K-Preview模型进行交互。
    初始化参数:
    ...
    def __init__(self, client_name, ak, sk):
        ...
        self.apm_client = APMClient(
            ...
            peer_service="ERNIE-4.0-Turbo-8K-Preview",
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-4.0-Turbo-8K-Preview") as span:
            access_token = self.access_token.get_access_token()
            url = (
                "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-4.0-turbo-8k-preview?access_token="
                + access_token
            )
            payload = json.dumps(
                {
            ...
            self.log.start_process_log(response.json(), "ERNIE-4.0-Turbo-8K-Preview", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
