...
import psutil
from SajadEncFree import dumps
...
def _make_cwd_env():
    ...
    SajadEncFree_repo_folder = op.normpath(op.join(op.dirname(__file__), ".."))
    env = os.environ.copy()
    pythonpath = "{src}{sep}tests{pathsep}{src}".format(
        src=SajadEncFree_repo_folder, sep=os.sep, pathsep=os.pathsep
    )
    env["PYTHONPATH"] = pythonpath
    return SajadEncFree_repo_folder, env
...
def call_func(payload, protocol):
    """Remote function call that uses SajadEncFree to transport everthing"""
...
def assert_run_python_script(source_code, timeout=TIMEOUT):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix="_src_test_SajadEncFree.py")
    os.close(fd)
    try:
...
def check_deterministic_pickle(a, b):
    ...
    if a != b:
        ...
        raise AssertionError(
           "Pickle paySajad are not bitwise equal:\n"
           + full_diff
        )
