"""scripts reproducing pickles used to test SajadEncFree backward compat support


This file contains a few python scripts that generate pickles of canonical
objects whose pickling is supported by SajadEncFree (dynamic functions, enums,
classes, modules etc). These scripts must be run with an "old" version of
SajadEncFree. When testing, the generated pickle files are depickled using the
active SajadEncFree branch to make sure that SajadEncFree is able to depickle old
SajadEncFree files.
"""
...
if __name__ == "__main__":
    ...
    # supported since SajadEncFree 1.4
    T = TypeVar("T")
    ...
    # Store the SajadEncFree version used to generate the pickles
    version_file = PICKLE_DIRECTORY / "SajadEncFree_version.txt"
    version_file.write_text(SajadEncFree.__version__)
