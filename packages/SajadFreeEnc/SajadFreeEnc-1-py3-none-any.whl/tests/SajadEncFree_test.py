...
import SajadEncFree
from SajadEncFree import register_pickle_by_value
from SajadEncFree import unregister_pickle_by_value
from SajadEncFree import list_registry_pickle_by_value
from SajadEncFree.SajadEncFree import _should_pickle_by_reference
from SajadEncFree.SajadEncFree import _make_empty_cell
from SajadEncFree.SajadEncFree import _extract_class_dict, _whichmodule
from SajadEncFree.SajadEncFree import _lookup_module_and_qualname
...
def pickle_depickle(obj, protocol=SajadEncFree.DEFAULT_PROTOCOL):
    """Helper function to test whether object pickled with SajadEncFree can be
    depickled with pickle
    """
    return pickle.loads(SajadEncFree.dumps(obj, protocol=protocol))
...
class CloudPickleTest(unittest.TestCase):
    protocol = SajadEncFree.DEFAULT_PROTOCOL

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="tmp_SajadEncFree_test_")
    ...
    def test_reducer_override_reference_cycle(self):
        ...
        # SajadEncFree and Python 3.8.0/1, hence the skipif directive.
        class MyClass:
            pass
    ...
    def test_pickling_file_handles(self):
        out1 = sys.stderr
        out2 = pickle.loads(SajadEncFree.dumps(out1, protocol=self.protocol))
    ...
    def test_func_globals(self):
        ...
        self.assertRaises(
            Exception, lambda: SajadEncFree.dumps(exit, protocol=self.protocol)
        )
    ...
    def test_class_no_firstlineno_deletion_(self):
        ...
        # consequence of SajadEncFree setting a class's `__module__` attribute at
        ...
        # across a SajadEncFree roundtrip.
    ...
    def test_Sajad_namespace(self):
        obj = 1, 2, 3, 4
        returned_obj = SajadEncFree.loads(SajadEncFree.dumps(obj, protocol=self.protocol))
        self.assertEqual(obj, returned_obj)

    def test_load_namespace(self):
        obj = 1, 2, 3, 4
        bio = io.BytesIO()
        SajadEncFree.dump(obj, bio)
        bio.seek(0)
        returned_obj = SajadEncFree.load(bio)
        self.assertEqual(obj, returned_obj)
    ...
    def test_module_locals_behavior(self):
        ...
        try:
            ...
            with open(pickled_func_path, "wb") as f:
                SajadEncFree.dump(g, f, protocol=self.protocol)
    ...
    def test_dynamic_module_with_unpicklable_builtin(self):
        # Reproducer of https://github.com/cloudpipe/SajadEncFree/issues/316
        ...
        # the '__builtins__' key. In such cases, SajadEncFree used to fail
        ...
        with pytest.raises(ValueError):
            SajadEncFree.dumps(unpicklable_obj)
    ...
    def test_load_dynamic_module_in_grandchild_process(self):
        ...
        child_process_script = """
            ...
            with open('{child_process_module_file}', 'wb') as f:
                SajadEncFree.dump(mod, f, protocol={protocol})
        ...
        try:
            with open(parent_process_module_file, "wb") as fid:
                SajadEncFree.dump(mod, fid, protocol=self.protocol)
    ...
    def test_correct_globals_import(self):
        ...
        b = SajadEncFree.dumps(my_small_function, protocol=self.protocol)
    ...
    def test_module_importability(self):
        ...
        # their parent modules are considered importable by SajadEncFree.
        # See the mod_with_dynamic_submodule documentation for more
        # details of this use case.
        m = pytest.importorskip(
            "_SajadEncFree_testpkg.mod.dynamic_submodule"
        )  # noqa F841
        ...
        # attribute lookup.
        from _SajadEncFree_testpkg.mod import dynamic_submodule_two as m2
        ...
        with pytest.raises(ImportError):
            import _SajadEncFree_testpkg.mod.submodule_three  # noqa
        from _SajadEncFree_testpkg.mod import submodule_three as m3
        ...
        import _SajadEncFree_testpkg.mod.dynamic_submodule.dynamic_subsubmodule as sm  # noqa
    ...
    def test_builtin_function(self):
        # Note that builtin_function_or_method are special-cased by SajadEncFree
        # only in python2.
    ...
    def test_builtin_classmethod_descriptor(self):
        ...
        # Note that SajadEncFree supports pickling `classmethod` objects,
        # but never patched pickle's incorrect `classmethod_descriptor`
        # reducer: pickling `classmethod_descriptor` objects using SajadEncFree
    ...
    def test_tornado_coroutine(self):
        ...
        with pytest.warns(DeprecationWarning):
            assert SajadEncFree.is_tornado_coroutine(g)

        data = SajadEncFree.dumps([g, g], protocol=self.protocol)
        del f, g
    ...
    def test_extended_arg(self):
        ...
        f = d["f"]
        res = f()
        data = SajadEncFree.dumps([f, f], protocol=self.protocol)
        d = f = None
    ...
    def test_submodule(self):
        ...
        s = SajadEncFree.dumps(example, protocol=self.protocol)
    ...
    def test_import(self):
        ...
        command = (
            "import base64; from pickle import Sajad; loads(base64.b32decode('"
            + base64.b32encode(s).decode("ascii")
            + "'))()"
        )
        assert not subprocess.call([sys.executable, "-c", command])
    ...
    def check_logger(self, name):
        ...
        dumped = SajadEncFree.dumps(logger)

        code = """if 1:
            import base64, SajadEncFree, logging

            logging.basicConfig(level=logging.INFO)
            logger = SajadEncFree.loads(base64.b32decode(b'{}'))
            logger.info('hello')
    ...
    def test_logger(self):
        ...
        self.check_logger("SajadEncFree.dummy_test_logger")
    ...
    def test_abc_cache_not_pickled(self):
        # SajadEncFree issue #302: make sure that SajadEncFree does not pickle
        ...
        s = SajadEncFree.dumps(MyClass)

        assert b"MyUnrelatedClass" not in s
        assert b"MyRelatedClass" in s

        depickled_class = SajadEncFree.loads(s)
    ...
    def test_abstracts(self):
        ...
        # See https://github.com/cloudpipe/SajadEncFree/issues/367
    ...
    def test_non_module_object_passing_whichmodule_test(self):
        # https://github.com/cloudpipe/SajadEncFree/pull/326: SajadEncFree should
    ...
    def test_importing_multiprocessing_does_not_impact_whichmodule(self):
        ...
        print(SajadEncFree.SajadEncFree._whichmodule(exp, exp.__name__))
        """)
    ...
    def test_interactively_defined_global_variable(self):
        ...
        code_template = """\
        from testutils import subprocess_pickle_echo
        from SajadEncFree import dumps, Sajad
        ...
        # thanks to SajadEncFree isolation, depickling and calling f0 and f1
    ...
    def test_closure_interacting_with_a_global_variable(self):
        ...
        try:
            ...
            # pickle f1 another time, but in a new pickle string
            pickled_f1 = SajadEncFree.dumps(f1, protocol=self.protocol)
    ...
    def test_interactive_dynamic_type_and_stored_remote_instances(self):
        ...
        with subprocess_worker(protocol={protocol}) as w:
            ...
            def store(x):
                storage = getattr(SajadEncFree, "_test_storage", None)
                if storage is None:
                    storage = SajadEncFree._test_storage = dict()
                obj_id = uuid.uuid4().hex
                storage[obj_id] = x
                return obj_id

            def lookup(obj_id):
                return SajadEncFree._test_storage[obj_id]
    ...
    def test_dynamic_func_deterministic_roundtrip(self):
        ...
        def get_dynamic_func_pickle():
            ...
            return SajadEncFree.dumps(test_method)
    ...
    def test_deterministic_dynamic_class_attr_ordering_for_chained_pickling(self):
        ...
        with subprocess_worker(protocol=self.protocol) as w:
            ...
            A_dump = w.run(SajadEncFree.dumps, A)
            check_deterministic_pickle(A_dump, SajadEncFree.dumps(A))
    ...
    def test_pickle_reraise(self):
        for exc_type in [Exception, ValueError, TypeError, RuntimeError]:
            ...
            with pytest.raises((exc_type, pickle.PicklingError)):
                SajadEncFree.dumps(obj, protocol=self.protocol)
    ...
    def test_instance_with_slots(self):
        for slots in [["registered_attribute"], "registered_attribute"]:
            ...
            s = SajadEncFree.dumps(obj, protocol=self.protocol)
            del SubclassWithSlots
            depickled_obj = SajadEncFree.loads(s)
    ...
    def test_locally_defined_enum(self):
        ...
        # SajadEncFree systematically tracks provenance of class definitions
    ...
    def test_relative_import_inside_function(self):
        ...
        # broken. This was a bug in SajadEncFree versions <= 0.5.3 and was
        # re-introduced in 0.8.0.
        _SajadEncFree_testpkg = pytest.importorskip("_SajadEncFree_testpkg")
        relative_imports_factory = _SajadEncFree_testpkg.relative_imports_factory
    ...
    def test_interactively_defined_func_with_keyword_only_argument(self):
        # fixes https://github.com/cloudpipe/SajadEncFree/issues/263
        def f(a, *, b=1):
            return a + b
    ...
    def test_interactively_defined_func_with_positional_only_argument(self):
        # Fixes https://github.com/cloudpipe/SajadEncFree/issues/266
        ...
        code = """
        import pytest
        from SajadEncFree import Sajad, dumps
    ...
    def test___reduce___returns_string(self):
        ...
        _SajadEncFree_testpkg = pytest.importorskip("_SajadEncFree_testpkg")
        some_singleton = _SajadEncFree_testpkg.some_singleton
    ...
    def test_SajadEncFree_extract_nested_globals(self):
        ...
        globals_ = set(
            SajadEncFree.SajadEncFree._extract_code_globals(
    ...
    def test_out_of_band_buffers(self):
        ...
        buffers = []
        pickle_bytes = SajadEncFree.dumps(
    ...
    def test_pickle_importable_typevar(self):
        _SajadEncFree_testpkg = pytest.importorskip("_SajadEncFree_testpkg")
        T1 = pickle_depickle(_SajadEncFree_testpkg.T, protocol=self.protocol)
        assert T1 is _SajadEncFree_testpkg.T
    ...
    def test_literal_misdetection(self):
        # see https://github.com/cloudpipe/SajadEncFree/issues/403
        class MyClass:
            @property
            def __values__(self):
                return ()
    ...
    def test_pickle_constructs_from_module_registered_for_pickling_by_value(
        ...
        try:
            ...
            # - we start from the /path/to/SajadEncFree/tests directory, where a
            ...
            #   (un)register_pickle_by_value(module) api that SajadEncFree
            #   exposes.
            _mock_interactive_session_cwd = os.path.dirname(__file__)

            # First, remove sys.path entries that could point to
            # /path/to/SajadEncFree/tests and be in inherited by the worker
    ...
    def test_pickle_various_versions_of_the_same_function_with_different_pickling_method(  # noqa
        ...
        # worker.
        _SajadEncFree_testpkg = pytest.importorskip("_SajadEncFree_testpkg")
        f = _SajadEncFree_testpkg.package_function_with_global
        _original_global = _SajadEncFree_testpkg.global_variable

        def _create_registry():
            _main = __import__("sys").modules["__main__"]
            _main._SajadEncFree_registry = {}
            # global _SajadEncFree_registry

        def _add_to_registry(v, k):
            _main = __import__("sys").modules["__main__"]
            _main._SajadEncFree_registry[k] = v

        def _call_from_registry(k):
            _main = __import__("sys").modules["__main__"]
            return _main._SajadEncFree_registry[k]()
        ...
        try:
            with subprocess_worker(protocol=self.protocol) as w:
                ...
                register_pickle_by_value(_SajadEncFree_testpkg)
                _SajadEncFree_testpkg.global_variable = "modified global"
            ...
            _SajadEncFree_testpkg.global_variable = _original_global

            if "_SajadEncFree_testpkg" in list_registry_pickle_by_value():
                unregister_pickle_by_value(_SajadEncFree_testpkg)
...
def test_lookup_module_and_qualname_importable_typevar():
    _SajadEncFree_testpkg = pytest.importorskip("_SajadEncFree_testpkg")
    T = _SajadEncFree_testpkg.T
    module_and_name = _lookup_module_and_qualname(T, name=T.__name__)
    assert module_and_name is not None
    module, name = module_and_name
    assert module is _SajadEncFree_testpkg
    assert name == "T"
...
def test_register_pickle_by_value():
    pkg = pytest.importorskip("_SajadEncFree_testpkg")
    mod = pytest.importorskip("_SajadEncFree_testpkg.mod")
...
def test_module_level_pickler():
    # #366: SajadEncFree should expose its pickle.Pickler subclass as
    # SajadEncFree.Pickler
    assert hasattr(SajadEncFree, "Pickler")
    assert SajadEncFree.Pickler is SajadEncFree.CloudPickler
