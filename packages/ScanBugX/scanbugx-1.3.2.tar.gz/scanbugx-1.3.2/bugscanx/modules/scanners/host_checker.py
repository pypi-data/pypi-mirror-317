...
def get_user_input():
	filename = get_input("\n Enter the filename")
	mode = get_input(" Enter the mode (direct, proxy, ssl)","direct")
	method_list = ""
	port_list = get_input(" Enter the port list (comma-separated)","80")
	proxy = ""
	output = get_input(" Enter the output file name")
	threads = int(get_input(" Enter the number of threads", "50"))

	if mode == 'direct':
		method_list = get_input(" Enter the method list (comma-separated)","HEAD")
	elif mode == 'proxy':
		proxy = get_input(" Enter the proxy (host:port)")
