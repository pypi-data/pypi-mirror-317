...
class OpenAiReasoner(Reasoner):
    def __init__(self):
        self.__openai = OpenAITool()
        self.__initialised = False

    def selectSkill(self, skills: list[SkillDescription], queries: list[QueryDescription], utterance: str) -> ActionParameters:
        if not self.__initialised:
           instructionAccepted = self.__openai.instruct(self.__getInitialInstruction(skills, queries))
           self.__initialised = True
           if not instructionAccepted:
               raise KeyError('Its wrong error and openai doesnt understand me (reasoner)')
        
        response = self.__openai.completion(utterance)
