...
class TopK(CupyKernel):
    def forward(self, x, k):

        # 使用 argpartition 找到前 k 个最大的元素的未排序索引
        partition_indices = cp.argpartition(x, -k)[-k:]

        # 对这 k 个元素进行排序，以获得从大到小的顺序
        sorted_topk_indices = partition_indices[cp.argsort(x[partition_indices])[::-1]]
...
class Multinomial(CupyKernel):
    def forward(self, input_probs, num_samples):
        if cp.any(input_probs < 0):
        ...
        # 归一化权重为概率
        probabilities = input_probs / cp.sum(input_probs)

        # 使用 cp.random.choice 进行抽样
        # replace=False 表示不放回抽样，类似于 torch.multinomial 的默认行为
        sampled_indices = cp.random.choice(
            a=cp.arange(len(input_probs)),
            size=num_samples,
            replace=True, #cp不支持
            p=probabilities
        )
