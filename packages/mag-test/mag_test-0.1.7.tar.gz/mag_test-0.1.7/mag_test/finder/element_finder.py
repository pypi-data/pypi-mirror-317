...
class ElementFinder:
    ...
    def find(driver:AppDriver, element_info:ElementInfo):
        ...
        # # Pane
        # elif element_info.element_type == ControlType.PANE:
        #     element = self.__find_pane(driver, element_info)
        # 工具栏
        elif element_info.control_type == ControlType.TOOL:
            element = ElementFinder.__find_toolbar(driver, element_info, parent)
        # 日期时间
    ...
    @staticmethod
    def __find_button(driver, element_info, parent=None):
        """
        查找按钮控件（包括：普通按钮和拆分按钮）
        参数：
        name 按钮名
    ...
    @staticmethod
    def __find_menu(driver, element_info, parent=None):
        """
        查找菜单控件
        参数：
    ...
    @staticmethod
    def __find_combox_box(driver, element_info, parent=None):
        """
            查找组合框或列表项
            参数：
    ...
    @staticmethod
    def __find_list_box(driver, element_info, parent=None):
        """
        查找列表框
        参数：
    ...
    @staticmethod
    def __find_list_view(driver, element_info, parent=None):
        """
            查找列表视图
            参数：
    ...
    @staticmethod
    def __find_tree_view(driver, element_info, parent=None):
        """
            查找树视图
            参数：
    ...
    @staticmethod
    def __find_table(driver, element_info, parent=None):
        """
            查找表格
            参数：
    ...
    @staticmethod
    def __find_datetime(driver, element_info, parent=None):
        """
        查找日期时间控件
        参数：
        name 日期时间控件名
    ...
    @staticmethod
    def __find_toolbar(driver, element_info, parent=None):
        """
            查找工具栏
            参数：
            name 工具栏名
            parent_name 父控件名，格式：全为数字表示它是一个automationId
        """
        return ElementFinder.__find_main_element(driver, element_info, parent)

    @staticmethod
    def __find_window(driver, element_info):
        """
        查找窗口
        参数：
        name 窗口名（关键词）
    ...
    @staticmethod
    def __find_context_menu(driver, element, menu_item_name):
        if menu_item_name:
            actions = ActionChains(driver)
    ...
    @staticmethod
    def __find_main_element(driver, element_info, parent=None):
        try:
            ...
            if element_info.control_type.is_composite():
                actions = ActionChains(driver)
                offset_x = element_info.get_parent_offset(element.size['width'])
                offset_y = element_info.get_parent_offset_y(element.size['height'])
                actions.move_to_element_with_offset(element, offset_x, offset_y).perform()
                sleep(1)
        except (NoSuchElementException, InvalidSelectorException, WebDriverException, TimeoutException) as e:
            element = None
    ...
    @staticmethod
    def __find_parent(driver, element_info):
        parent = None
