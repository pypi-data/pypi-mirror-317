...
def show_points_from_files(file_paths: list, color=None):
    """
    从多个文件显示点
    :param file_paths: 文件路径列表
    :param colors: 颜色列表，如果为None则使用默认颜色
    :param handle_type: 文件类型，"txt"或"cpt"
    """
    init_jupyter()
    plotter = pv.Plotter()
    ...
    if isinstance(file_paths, list):
        for i, file_path in enumerate(file_paths):
            points_data = handle_txt(file_path)
            # 如果color是列表，则使用对应索引的颜色
            # 如果color是单个颜色值，则所有点都使用该颜色
            # 如果color为None，则使用默认颜色
            current_color = color[i] if isinstance(color, list) else color
            plotter.add_points(points=points_data, color=current_color)
    elif isinstance(file_paths, str):
        points_data = handle_txt(file_paths)
        plotter.add_points(points=points_data, color=color)
    plotter.show()
...
# show_points_from_files(file_paths, colors)
