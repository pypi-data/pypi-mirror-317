...
import ppl
from ppl.compiler.code_generator import kernel_suffix
from ppl.compiler.make_launcher import ty_to_cpp

desc = """
Ppl ahead-of-time compiler:
...
will compile ppl.JITFunction of name `kernel` inside the file `/path/to/kernel.py`.
...
if __name__ == "__main__":
    ...
    config = ppl.compiler.instance_descriptor(divisible_by_16=divisible_by_16, equal_to_1=equal_to_1)
    for i in equal_to_1:
        constexprs.update({i: 1})
    ccinfo = ppl.compile(kernel, signature=signature, constants=constexprs, configs=[config], num_warps=args.num_warps)
    arg_names = []
    arg_types = []
    ...
    ppl_kernel_name = '_'.join([args.kernel_name, suffix])
    hex_ = str(binascii.hexlify(ccinfo.asm["cubin"]))[2:-1]
    params = {
        "kernel_name": func_name,
        "ppl_kernel_name": ppl_kernel_name,
        "bin_size": len(hex_),
