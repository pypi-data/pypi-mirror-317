...
def build_code(work_path, file_path, func_name, chip, build_debug=True):
    ...
    _os_system([
        "cmake .. -DDEBUG={} -DCHIP={} -DOUT_NAME={}".format(
            build_debug, chip_name, file_name_without_ext)
    ])
    if build_debug:
...
class pl_extension:
    ...
    @staticmethod
    def load(file_path, func_name, chip, build_debug=True):
        ppl_root = os.environ["PPL_PROJECT_ROOT"]
        file_name = os.path.basename(file_path)
        file_name_without_ext = os.path.splitext(file_name)[0]
        # open mem check func
        so_path = pl_extension.cache.get_cached_file("lib/lib" + file_name_without_ext + ".so")
