...
def num_programs(axis: int, builder: ir.builder) -> pl.tensor:
    return pl.tensor(builder.create_get_num_programs(axis), pl.int32)
...
def minus(out,
    ...
    if input.type.is_block():
        ...
        return sub(out, _0, input, 0, pl.RM_HALF_TO_EVEN, False, builder)
...
def view(input: pl.tensor,
    ...
    return pl.tensor(builder.create_view(input.handle, shape, get_scalar_dtype(input.type).is_int_unsigned()), ret_ty)
...
def _convert_elem_to_ir_value(builder, elem, require_i64):
    ...
    if isinstance(elem, pl.constexpr):
        ...
        return elem.handle
    elif isinstance(elem, pl.ctensor):
        return elem.handle
...
def flog(dst,
    ...
    ret_ty = pl.block_type(get_scalar_dtype(src.type), [1])
    if dst is None:
        return pl.tensor(builder.create_flog(pl._to_tensor(None, builder).handle, src.handle,
                                             work0.handle, coeff.handle), ret_ty)
    else:
        return pl.tensor(builder.create_flog(dst.handle, src.handle, work0.handle, coeff.handle), pl.void)
