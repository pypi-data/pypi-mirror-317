...
def codegen(path, chip, function_name, build_debug, mode, only_emit_kernel):
    ...
    if only_emit_kernel:
        ...
        library_dirs = [os.path.join(ppl_root, "runtime/{chip}/tpuv7-runtime-emulator_0.1.0/lib".format(chip=chip))
                        if chip == "bm1690" else
    ...
    if mode == "cmodel":
        if arch == "bm1684x" or arch == "bm1690" or arch == "bm1688":
            if arch == "bm1684x":
                ...
                runtime_lib = "tpuv7-runtime-emulator_0.1.0"
...
def optimize_ttir(mod, arch, path, function_name, build_debug, mode, only_emit_kernel, grid_0):
    mod = inline_ppl_ir(mod)
    pm = ir.pass_manager(mod.context)
    pm.enable_debug()
    #pm.add_inline_pass()
    pm.add_core_num_assign_pass(grid_0)
    pm.add_cse_pass()
...
def compile(fn, **kwargs):
    ...
    key = f"{fn.__name__}-{''.join(signature.values())}-{constants}-{debug}-{mode}-{arch}-{grid_0}-{grid_1}-{grid_2}"
    ...
    stages["ttir"] = (
        ...
        lambda src: optimize_ttir(
            ...
            arch, fn_cache_manager.get_cache_dir(), fn.__name__, debug, mode, only_emit_kernel, grid_0))
    ...
    if metadata_path is not None:
        ...
        return CompiledKernel(fn, so_path, metadata, dict(), target_path if enable_dump_ir else fn_cache_manager.get_cache_dir(),
                              arch, fn.__name__, mode, only_emit_kernel, constexpr_index, grid_0, grid_1, grid_2)
...
class CompiledKernel:
    ...
    def __init__(self, fn, so_path, metadata, asm, path, arch, function_name, mode,
                 only_emit_kernel, constexpr_index, grid_0, grid_1, grid_2, desc=False):
        # initialize launcher
