...
PPL_VERSION = "2.1.0"
data_path = os.path.join(os.getenv("PPL_TMP", Path.home()), ".ppl", "tmp")
...
class JITFunction(KernelInterface[T]):
    ...
    def run(self, *args_, **kwargs):
        ...
        for index, arg in enumerate(args):
            if arg.param.is_constexpr:
                constexpr_key_list.append(arg.value)
                constexpr_index.append(index)
        ...
        if os.path.exists(path):
            shutil.rmtree(path)
        only_emit_kernel = True
        scalar_value = {}
        for index, (tensor, name) in enumerate(zip(non_constexpr_arg_values, non_constexpr_arg_names)):
            if isinstance(tensor, torch.Tensor):
                if tensor.is_cpu:
                    dic = {}
                    only_emit_kernel = False
                    if not os.path.exists(path):
                        os.makedirs(path)
                    #numpy don't support bf16
                    if tensor.dtype == torch.bfloat16:
                        dic[str(index)] = tensor.float().numpy()
                        file = os.path.join(path, str(index) + ".ini")
                        with open(file, 'w') as f:
                            f.write("bf16")
                    else:
                        dic[str(index)] = tensor.numpy()
                    np.savez(os.path.join(path, str(index) + ".npz"), **dic)
            else:
        ...
        if not only_emit_kernel and not kernel.errrCode:
            kernel.run(kernel.path, kernel.arch, kernel.function_name, kernel.mode, kernel.only_emit_kernel, kernel.desc)
            new_data = np.load(os.environ["PPL_DATA_PATH"]+"/" + self.fn.__name__+"_tar.npz")
            #copy result data to output
            for index, (tensor, name) in enumerate(zip(non_constexpr_arg_values, non_constexpr_arg_names)):
                if isinstance(tensor, torch.Tensor) and tensor.is_cpu:
                    args_[index].data =  torch.from_numpy(new_data[str(index)]).to(tensor.dtype).data.view(args_[index].data.shape)
        self.kernel = kernel
        return kernel
    ...
    def __init__(self, fn, version=None, do_not_specialize=None, debug=None, noinline=None):
        ...
        self.kernel = None
        self.debug = True if os.environ.get("DEBUG", "0") == "1" else False
...
class AutoTiling(KernelInterface):
    ...
    def run(self, *args, **kwargs):
        while True:
            ...
            if not ret is None and ret.errrCode == 0:
                return ret
            else:
