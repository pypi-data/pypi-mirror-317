...
def generate_launcher(constants, signature):
    ...
    {{
      if (code != HIP_SUCCESS)
      {{
         const char* prefix = "Ppl Error [HIP]: ";
      ...
      if (ptr) {{
        ...
        if (status == hipErrorInvalidValue) {{
            PyErr_Format(PyExc_ValueError,
                         "Pointer argument (at %d) cannot be accessed from Ppl (cpu tensor?)", idx);
            ptr_info.valid = false;
        }}
    ...
    PyMODINIT_FUNC PyInit___ppl_launcher(void) {{
...
{{
   ...
   {{
      const char* prefix = "Ppl Error [CUDA]: ";
      const char* str;
