...
'''
@builtin
def flog(*args, work0, coeff, _builder=None):
    if all(isinstance(t, (tensor, constexpr)) for t in args):
        if len(args) == 1:
            return semantic.flog(None, _to_tensor(args[0], _builder),
    ...
    使用示例:
        ...
        .. code-block:: python
            ...
            def conv2d_asym_kzp_tensor_kernel(
                ...
                ws_w = 32
                ws_h = pl.tiu.cast((ic + ws_w - 1) // ws_w, pl.int32) * kh * kw
    ...
    使用示例:
        ...
        .. code-block:: python
            ...
            def mm_kernel(
                ...
                if ltrans:
                    x_global = pl.gtensor([K, pl.tiu.cast(M // l_cols_perchannel, pl.int32), 1, l_cols_perchannel], pl.GLOBAL, x_ptr)
                else:
                    x_global = pl.gtensor([M, pl.tiu.cast(K // l_cols_perchannel, pl.int32), 1, l_cols_perchannel], pl.GLOBAL, x_ptr)

                y_global = pl.gtensor([K, pl.tiu.cast(N // r_cols_perchannel, pl.int32), 1, r_cols_perchannel], pl.GLOBAL, y_ptr)
                o_global = pl.gtensor([M, pl.tiu.cast(N // r_cols_perchannel, pl.int32), 1, r_cols_perchannel], pl.GLOBAL, output_ptr)
                output = pl.make_tensor([M, pl.tiu.cast(N // r_cols_perchannel, pl.int32), 1, r_cols_perchannel], output_ptr.dtype)
