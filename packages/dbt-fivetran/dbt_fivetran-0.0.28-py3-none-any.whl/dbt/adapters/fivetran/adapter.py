...
class FivetranAdapter(DuckDBAdapter):
    ...
    def __init__(self, config, mp_context: SpawnContext) -> None:
        ...
        self.secrets = config.credentials.get_secrets()
        if self.secrets.type.lower() != 's3':
        ...
        self.catalog = load_catalog("polaris_rest",
            **{
                ...
                "oauth2-server-uri": f"{config.credentials.polaris_uri}/api/catalog/v1/oauth/tokens"
            }
        )
        super().__init__(config, mp_context)
    ...
    def create_table_as(self, temporary: bool, relation: BaseRelation, sql: str):
        ...
        location = f"s3://{relation.database}/{relation.schema}/{relation.identifier}"
        create_table_args = {"identifier": table_identifier,
                "schema": schema,
                "location": location,
                "properties": {
        ...
        try:
            ...
            if "because it conflicts with existing table or namespace at location" in str(e):
                ...
                create_table_args['location'] = f"s3://{relation.database}/{relation.schema}/{relation.identifier}_{TBL_NAME_CONF_RESLV}"
                table = self.catalog.create_table(**create_table_args)
    ...
    def _create_db_view_from_iceberg(self, identifier: str, table: pyiceberg.table.Table):
        ...
        secret_sql = f'''CREATE OR REPLACE PERSISTENT SECRET {secret_name} (
            ...
            SCOPE '{scope}',
            REGION '{self.secrets.secret_kwargs['region']}'
        )'''
    ...
    def _drop_table(self, relation: BaseRelation, table_exists: bool = False):
        ...
        if table_exists or self.catalog.table_exists(identifier):
            ...
            self._delete_iceberg_table(relation, table.metadata_location)
    ...
    def _delete_iceberg_table(self, relation: BaseRelation, metadata_location: str):
        ...
        s3_session = boto3.Session(
            aws_access_key_id=self.secrets.secret_kwargs['key_id'],
            aws_secret_access_key=self.secrets.secret_kwargs['secret'],
            region_name=self.secrets.secret_kwargs['region'])
