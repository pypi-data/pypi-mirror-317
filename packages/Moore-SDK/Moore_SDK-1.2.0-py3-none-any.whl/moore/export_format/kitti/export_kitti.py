...
class ExportKitti(ExportData):
    def __init__(self, source_data, out_path=None, mapping=None):
        """
        Converting moore data format to kitti data format
        :param source_data: source moore data
        :param out_path:
        :param mapping: label-color mapping
        """
        super(ExportKitti, self).__init__(source_data, out_path, mapping)

    def moore_json2odkitti(self):
        """
        Converting MOORE raw data to kitti format
        :return: txt file in lidar-based kitti format
        """
        for i in tqdm(self.source_data.data):
            moore_url = i.info.info.url
            for j in range(len(moore_url)):
                image_name = Path(moore_url[j]).parts[-1].split('.')[0]
                image_name = image_name + '.txt'
                start = find_nth(moore_url[j], '/', 4)
                end = find_last(moore_url[j], '/')
                if self.out_path:
                    file_path = 'labels'
                    if start != end:
                        root = moore_url[j][start + 1: end]
    ...
    def moore_json2segkitti(self):
        """
        Converting MOORE raw data to labelme format
