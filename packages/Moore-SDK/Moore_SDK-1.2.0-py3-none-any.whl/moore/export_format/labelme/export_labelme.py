...
class ExportLabelme(ExportData):
    def __init__(self, source_data, out_path=None, mapping=None):
        """
        Converting moore data format to labelme data format
    ...
    def moore_json2labelme(self):
        ...
        for data in tqdm(self.label_obj_list):
            moore_url = data['info']
            image_name = Path(moore_url).parts[-1]
            LABELME.shapes = []
            labels = data['labels']
            if len(labels) == 0:
                continue
            labels = sorted(labels,
                            key=lambda x: x['data']['zIndex'] if 'zIndex' in self.moore2dict(x['data']) else x['data'][
                                'id'])
            for l in labels:
            ...
            imageData = image2base64(moore_url)
            ...
            if '/'.join(Path(moore_url).parts[4:-1]) is not None:
                root = '/'.join(Path(moore_url).parts[4:-1])
            else:
                root = ''
            root = urllib.parse.unquote(root)

            file_name = image_name.replace(image_name.split('.')[-1], 'json')
            self.save_labels(self.moore2dict(LABELME), file_name, root)
