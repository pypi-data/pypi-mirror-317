...
class ExportYolo(ExportData):
    def __init__(self, source_data, out_path=None, mapping=None):
        """
        Converting moore data format to yolo data format
    ...
    def moore_json2yolo(self):
        """
        Converting MOORE raw data to yolo format
        ...
        for data in tqdm(self.label_obj_list):
            moore_url = data['info']
            label_name = Path(moore_url).parts[-1].split('.')[0]
