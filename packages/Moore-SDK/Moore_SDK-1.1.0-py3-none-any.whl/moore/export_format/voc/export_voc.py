...
class ExportVoc(ExportData):
    def __init__(self, source_data, out_path=None, mapping=None):
        """
        Converting moore data format to voc data format
    ...
    def moore_json2voc(self):
        """
        Converting MOORE raw data to voc format
        ...
        for data in tqdm(self.label_obj_list):
            ...
            anno_tree = VOC_MAKER.annotation(
                ...
                VOC_MAKER.path('/'.join(Path(moore_url).parts[4:])),
                VOC_MAKER.source(
