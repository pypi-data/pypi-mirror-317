...
def check_block_neigh(ham,sp_lat,group_x,group_y,group_z):
    shape = sp_lat.shape
    nx,ny,nz,nat,idx = get_dynamics_sites_indices(shape)
...
def run_parallel_monte_carlo(ham,sp_lat,
    ...
    if group_comm != MPI.COMM_NULL:
        ...
        shape = sp_lat.shape
        nx,ny,nz,nat,idx = get_dynamics_sites_indices(shape,fix_boundary_spin,pinned_idx)
        ...
        Etot = ham.calc_total_E(sp_lat,average_on_sites=True,parallel=True)
            ...
            for ix0,iy0,iz0,iat in np.ndindex(group_x,group_y,group_z,nat):
                ...
                if group_rank==rank_root:
                    ...
                    for iblock in range(nblock):
                        ...
                        izz = iz0 + zstarts[iblock]
                        if len(shape)==4: site_idx = (ixx,iyy,iat)
                        if len(shape)==5: site_idx = (ixx,iyy,izz,iat)
...
class MC_controller(object):
    ...
    def estimate_nblock(self,sp_lat):
        nx,ny,nz,nat,idx = get_dynamics_sites_indices(sp_lat.shape)
    ...
    def run_parallel_monte_carlo(self,ham,sp_lat):
        return run_parallel_monte_carlo(ham, sp_lat,
        self._temperature,
        self._mcs,
        self._verbosity,
...
class PTMC_controller(MC_controller):
    ...
    def run_one_epoch_v1(self,iepoch,ham,sp_lat,rank_groups,rank):
        ...
        sp_lat = load_conf_from_ovf(sp_lat,'{}/Epoch_{}_itemp_{}_final_confs.ovf'.format(self._outdir,iepoch,itemp))
        return sp_lat
    
 

    def run_one_epoch_v2(self,iepoch,ham,sp_lat,rank_groups,rank):
        #self._MC_controller._log_handle.set_outdir('{}/Ep_{}'.format(self._MC_controller._outdir, iepoch))
        #print (self._MC_controller._log_handle._outdir)
