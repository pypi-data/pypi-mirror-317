class SIConverter:
    ...
    def convert_from_si(self, value, unit_type, to_unit):
        ...
        if to_unit not in self.unit_conversions[unit_type]:
            raise ValueError(f"Единица '{to_unit}' не поддерживается для типа '{unit_type}'. Используйте: {list(self.unit_conversions[unit_type].keys())}")
        conversion_factor = self.unit_conversions[unit_type][to_unit]
