...
class Trainer():
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, G]):
            latents = torch.randn(batch_size, latent_dim).to(which_device)
            image_batch = next(self.loader).to(which_device)
    ...
    def evaluate(self, num = 0, num_image_tiles = 4):
        ...
        latents = det_randn((num_rows ** 2, latent_dim)).to(which_device)
    ...
    def generate(self, num=0, num_image_tiles=4, checkpoint=None, types=['default', 'ema']):
        ...
        if 'default' in types:
            for i in tqdm(range(num_image_tiles), desc='Saving generated default images'):
                latents = torch.randn((1, latent_dim)).to(which_device)
    ...
    def show_progress(self, num_images=4, types=['default', 'ema']):
        ...
        for checkpoint in tqdm(checkpoints, desc='Generating progress images'):
            ...
            if checkpoint == 0:
                latents = torch.randn((num_images, self.GAN.latent_dim)).to(which_device)
    ...
    def calculate_fid(self, num_batches):
        ...
        for batch_num in tqdm(range(num_batches), desc='calculating FID - saving generated'):
            # latents and noise
            latents = torch.randn(self.batch_size, latent_dim).to(which_device)
    ...
    def generate_interpolation(self, num = 0, num_image_tiles = 8, num_steps = 100, save_frames = False):
        ...
        latents_low = torch.randn(num_rows ** 2, latent_dim).to(which_device)
        latents_high = torch.randn(num_rows ** 2, latent_dim).to(which_device)
