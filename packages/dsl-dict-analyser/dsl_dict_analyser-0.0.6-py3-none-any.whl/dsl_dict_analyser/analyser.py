    ...
    for line_index, line in enumerate(data):
        ...
        if starts_with == "#":
            if '\t' in line:
                ...
                metadata[key.strip()] = format_line(value)
            else:
                meta = re.search(r"#[\S]+", line).group(0)
                metadata[meta[1:]]=line.split(meta)[-1].strip().strip('"')
            continue
        ...
        if starts_with in STARTING_CHARACTERS:
            ...
            if ',' in definition:
                ...
                definition = [d.strip() for d in definition]
            elif ';' in definition:
                definition = definition.split(';')
                definition = [d.strip() for d in definition]
            else:
                ...
                definition = [d.strip() for d in definition]
            words[index - 1].definitions.extend(definition)
