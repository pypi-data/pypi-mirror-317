...
import aiohttp
from containers import AuthorizationInfo, SnooData, SnooDevice, SnooStates
...
class Snoo:
    ...
    #     data = {
    #         "AuthParameters": {
    #             "REFRESH_TOKEN": self.tokens.aws_refresh
    #         },
    ...
    async def subscribe(self, device: SnooDevice, function):
        ...
        if device_id not in self.pubnub_instances:
            self.pubnub_instances[device_id] = SnooPubNub(pubnub, device_id)
    ...
    def set_sticky_white_noise(self, device: SnooDevice, on: bool):
        self.send_command("set_sticky_white_noise", device, **{"state": "on" if on else "off", "timeout_min": 15})
