...
def read_table_in_chunks(gql, table_name, cols, cutoff=None, chunk_size=1000, show=False):
    """
    ...
    """
    # Create the fields string from cols list
    fields = "\n            ".join(cols)
    # Define the where clause based on cutoff
    where_clause = 'where: { updated_at: {_gte: $date} },' if cutoff else ''
    query_template = f"""
    query Get{table_name.title()}($limit: Int!, $offset: Int!{', $date: timestamptz!' if cutoff else ''}) {{
        {table_name}(
            {where_clause}
            limit: $limit,
            offset: $offset,
    ...
    if cutoff:
        variables["date"] = cutoff.isoformat()
    response = gql.exec(query_template, variables)
    nrows = response[f'{table_name}_aggregate']['aggregate']['count']
    if show: print(f'Reading {table_name} (nrows: {nrows})')
    # Create progress bar
    pbar = tqdm(total=nrows, desc=f'Reading {table_name}', unit=' rows') if show else None
    chunk = response[table_name]
