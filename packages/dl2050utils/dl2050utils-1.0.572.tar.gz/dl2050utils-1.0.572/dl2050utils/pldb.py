...
def get_tbl(df, cols=None, filters=None, sfilters=None, sort=None, ascending=True, offset=None, limit=None,
            one=False, excel=False, join=None, lookups=None):
    """
    ...
    df = df.select(cols)
    # if excel: return df_save_tmp(d['path'], 'excel', df)
    nrows = len(df)
