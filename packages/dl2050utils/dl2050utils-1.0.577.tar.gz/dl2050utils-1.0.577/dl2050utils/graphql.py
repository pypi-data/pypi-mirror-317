...
def build_query_global(cols):
    q_cols = " ".join(cols)
    Q = f"""
        query GetBrands($offset: Int!, $limit: Int!) {{
            brand(
                offset: $offset,
                limit: $limit
            )
...
def build_query_cutoff(cols):
    q_cols = " ".join(cols)
    Q = f"""
    query GetBrands($offset: Int!, $limit: Int!, $cutoff: timestamptz!) {{
        brand(
            offset: $offset,
            limit: $limit,
...
def read_table_in_chunks(gql, table_name, cols, cutoff=None, chunk_size=1000, show=False):
    ...
    query = build_query_global(cols) if cutoff is None else build_query_cutoff(cols)
