...
def SistemaEstructural(LoadName:str, Direccion:int, StoryName:str, resultado:str, Tabla:pd.DataFrame):
    ...
    try:
        ...
        for i in range(0, NumberResults):
            if PierName[i] == "Muros":
                if Direccion == 1:
                    V_muro = V2[i] * math.cos(math.pi * AxisAngle_Wall[0] / 180) + V3[i] * math.sin(math.pi * AxisAngle_Wall[0] / 180)
                else:
                     V_muro = V3[i] * math.cos(math.pi * AxisAngle_Wall[0] / 180) - V2[i] * math.sin(math.pi * AxisAngle_Wall[0] / 180)
            elif PierName[i] == "Columnas":
                if Direccion == 1:
                    V_column = V2[i] * math.cos(math.pi * AxisAngle_Col[0] / 180) + V3[i] * math.sin(math.pi * AxisAngle_Col[0] / 180)
                else:
                     V_column = V3[i] * math.cos(math.pi * AxisAngle_Col[0] / 180) - V2[i] * math.sin(math.pi * AxisAngle_Col[0] / 180)
