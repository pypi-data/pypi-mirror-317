...
class DataEntryForm(tk.Frame):
    ...
    def __init__(self, master, window_width, window_height, x, y):
        ...
        self.city = tk.Entry(cityframe, highlightthickness=1, fg='gray', font=("Arial", 10, "bold"),
                             width=33, relief=FLAT, textvariable=self.city_str, state='readonly')
        ...
        emailframe.pack()
        emaillabel = tk.Label(emailframe, text='Institute E-mail: ', bg='white', fg='black',
                              font=("Arial", 10, "bold"), relief=FLAT)
        ...
        if len(self.ids) > 0:
            ...
            s = ttk.Scrollbar(f)
            self.t = ttk.Treeview(f, columns=('c1', 'c2', 'c3', 'c4', 'c5', 'c5'), show="headings",
            ...
            for i in range(len(self.ids)):
                self.t.insert('', i + 1,
                              values=[i + 1, self.samples[i], self.speciess[i], self.ps[i], self.sample_dates[i],self.submission_dates])
                        ...
                        for i in range(len(self.ids)):
                            self.t.insert('', i + 1,
                                          values=[i + 1, self.samples[i], self.speciess[i], self.ps[i],
                                                  self.sample_dates[i], self.submission_dates])
                        self.t.bind("<<TreeviewSelect>>", self.on_item_selected)
        else:
    ...
    def upload_file(self, file_path_list, lon, lat, city, sample_institute, sample_date, email,
                    submission_date):
        ...
        if len(self.file_path_list) == 0:
            ...
            if self.ste.get() == '':
                ...
                if self.department.get() == '':
                    messagebox.showwarning("Warning", f"Sampling institute can not be empty !!!",
                                           default="ok",
                                           icon="warning")
                else:
                    ...
                    self.upload_file(self.file_path_list, lon, lat, city, sample_institute, sample_date, email,
                                     submission_date)
