...
class RPowerPlan(Juham):
    ...
    def on_powerplan(self, ts_utc_now: float) -> None:
        ...
        if elapsed < 60:
            self.debug(f"Relay started {elapsed} s ago, bail out")
            return
        ...
        self.info(f"Heating state {self.name} published with relay state {relay}", "")
    ...
    def consider_heating(self, ts: float) -> int:
        ...
        if self.current_temperature < self.minimum_boiler_temperature:
            self.info(
                f"Low temp, force heating because {self.current_temperature}C is less than {self.minimum_boiler_temperature}C"
            )
            return 1

        if self.current_temperature > self.maximum_boiler_temperature:
            self.debug(
                f"Temperature beyond maximum already {self.current_temperature}C"
            )
            return 0
    ...
    def create_heating_plan(self) -> list[dict[str, Any]]:
        ...
        for hp in self.power_plan:
            ...
            if (
                ...
                state = 0
            self.debug(f"{self.name} power {state} count {count}, spot {spot}")
            ...
            self.debug(
                f"Heating {self.name} {timestampstr(ts)} with spot {spot} and state {state}"
            )
