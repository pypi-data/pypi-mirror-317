...
class RPowerPlan(Juham):
    ...
    def sort_by_power(
        ...
        for h in sh:
            ...
            if utc_ts >= ts_utc:
                ranked_hours.append(h)
        self.info(f"Forecast sorted for the next {str(len(ranked_hours))} hours")
        return ranked_hours
    ...
    def on_message(self, client: object, userdata: Any, msg: MqttMsg) -> None:
        ...
        if msg.topic == self.topic_spot:
            ...
            return
        self.on_powerplan(ts_utc_quantized)
    ...
    def on_powerplan(self, ts_utc_now: float) -> None:
        ...
        if self.current_relay_state != relay:
            ...
            self.info(f"Relay state {self.name} changed to {relay}", "")
