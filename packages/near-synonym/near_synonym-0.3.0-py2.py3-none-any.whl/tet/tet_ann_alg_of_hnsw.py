#!/usr/bin/env python
# -*- coding: utf-8 -*-
# codefrom  : https://github.com/ekzhu/datasketch, date=20240101
...
def any2str(text, encoding="utf8", errors="strict"):
    """   强制转string   """
...
if __name__ == '__main__':
    ...
    # # Batch insert data into the indexes.
    # index1.update({i: d for i, d in enumerate(data1)})
    # sample = np.random.random_sample((1, 10))
    # res = index1.query(sample[0], k=16)
    # print(res)
    # print(len(res))
    ...
    vectors, index2word, word2index = load_word2vec_from_format(path, limit=None, binary=True)
    index_hnsw = HNSW(distance_func=lambda x, y: np.linalg.norm(x - y))
    index_hnsw.update({idx: vec for idx, vec in enumerate(vectors)})


    for idx, vec_i in enumerate(vectors[:16]):
        docs = index_hnsw.query(vec_i, k=16)
        print("#" * 168)
        for vdx, distance in docs:
            print(idx, vdx)
            print(index2word.get(vdx), distance)
    ...
    while True:
        try:
            ...
            if word.strip():
                ...
                vec_idx = vectors[word_idx]
                docs = index_hnsw.query(vec_idx, k=16)
                print("#" * 128)
                for vdx, distance in docs:
                    print(idx, vdx, index2word.get(vdx), distance)
