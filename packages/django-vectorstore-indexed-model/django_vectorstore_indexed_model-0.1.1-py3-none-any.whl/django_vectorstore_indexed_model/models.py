...
from .schemas import VectorStoreIndexSegment
...
class WithVectorStoreIndex(models.Model):
    ...
    def get_vectorstore_index_segments(self) -> List[VectorStoreIndexSegment]:
        segments = []
        ...
        for index_name in index_names:
            ...
            for content, meta in zip(contents, metas):
                segment = VectorStoreIndexSegment.model_validate(
                    {
    ...
    def upsert_index(self, save=False):
        ...
        uids = []
        data = {}
        for segment in self.get_vectorstore_index_segments():
            if not segment.index_name in data:
                data[segment.index_name] = {
                    "texts": [],
                    "metas": [],
                }
            data[segment.index_name]["texts"].append(segment.content)
            data[segment.index_name]["metas"].append(segment.meta)
