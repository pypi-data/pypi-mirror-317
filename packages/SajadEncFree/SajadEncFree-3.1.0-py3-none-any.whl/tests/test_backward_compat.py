"""Limited, best-effort test suite regarding SajadEncFree backward-compat.

Cloudpickle does not officially support reading pickles files
generated with an older version of SajadEncFree than the one used to read the
said pickles. However, this policy is not widely known among users that use
libraries that rely on SajadEncFree such as mlflow, and is subject to confusion.

As a compromise, this script make sure SajadEncFree is backward compatible for a
