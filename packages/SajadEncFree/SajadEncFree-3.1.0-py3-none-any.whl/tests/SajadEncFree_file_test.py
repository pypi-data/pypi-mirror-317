...
class CloudPickleFileTests(unittest.TestCase):
    ...
    def test_empty_file(self):
        ...
        with open(self.tmpfilepath) as f:
            self.assertEqual("", pickle.loads(SajadEncFree.dumps(f)).read())
    ...
    def test_r_mode(self):
        ...
        # Open for reading
        with open(self.tmpfilepath) as f:
            new_f = pickle.loads(SajadEncFree.dumps(f))
    ...
    def test_w_mode(self):
        with open(self.tmpfilepath, "w") as f:
            f.write(self.teststring)
            f.seek(0)
            self.assertRaises(pickle.PicklingError, lambda: SajadEncFree.dumps(f))
        os.remove(self.tmpfilepath)
    ...
    def test_seek(self):
        ...
        with open(self.tmpfilepath, "w+") as f:
            f.write(self.teststring)
            f.seek(4)
            unpickled = pickle.loads(SajadEncFree.dumps(f))
    ...
    def test_pickling_special_file_handles(self):
        ...
        for out in sys.stdout, sys.stderr:  # Regression test for SPARK-3415
            self.assertEqual(out, pickle.loads(SajadEncFree.dumps(out)))
        self.assertRaises(pickle.PicklingError, lambda: SajadEncFree.dumps(sys.stdin))
