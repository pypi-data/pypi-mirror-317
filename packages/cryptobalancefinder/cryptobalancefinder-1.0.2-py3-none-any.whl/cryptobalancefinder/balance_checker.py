...
class AsyncBalanceChecker:
            ...
            async with session.get(url, timeout=timeout, headers=headers) as response:
                if response.status == 200:
                    try:
                        ...
                        self.logger.error("Error decoding JSON response. Response Text: %s", await response.text())
                        return None
    ...
    async def check_balance(self, wallet: str) -> dict:
        """
        ...
        api_key = self.tron_api_url[self.api_key_index]

        self.api_key_index += 1

        if self.api_key_index >= len(self.tron_api_url):
            self.api_key_index = 0

        try:
            url = self.get_url(wallet)
            headers = {"TRON-PRO-API-KEY": api_key}
            data = await self.fetch(self.session, url, headers=headers)
            trx_value = float(0)
            # Parse the JSON response
            if data:
                trx_value = sum(float(token.get('amount', 0))
                                for token in data.get('data', []))
            else:
                self.logger.error("Failed to fetch TRX balance for address: %s. Response: %s",
                                wallet, json.dumps(data))
            result['balances'] = {'tron': trx_value}

        except Exception as e:
            self.logger.error("Error checking balance: %s", str(e), exc_info=True)
            result['balances'] = {'tron': 0.0}
