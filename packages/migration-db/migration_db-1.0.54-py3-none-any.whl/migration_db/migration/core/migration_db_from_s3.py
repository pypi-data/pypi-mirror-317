...
def restore_dbs_from_s3(config: dict, s3_sql_locations: list, prefix: str, bucket: str, app_env: str,
    ...
    for idx, file_dto in enumerate(file_dtos, start=1):
        # 从S3下载文件到本地
        ...
        progress_info = f"[{idx}/{len(file_dtos)}]"
        try:
            Logger().info(f"{progress_info} File: {file_dto.name} is start.")
            ...
            Logger().info(f"{progress_info} Filename:{file_dto.name}-{database} is end.")
        except Exception as e:
            traceback.print_exc()
            Logger().error(f"{progress_info} Filename:{file_dto.name}-{database} is error: {e}.")
            raise Exception(e)
        finally:
