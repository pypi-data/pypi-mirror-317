...
def populate_render_tab(
    ...
    fov_degrees = server.gui.add_slider(
        ...
        step=0.01,
        hint=(
            "Field-of-view for rendering, which can also be overridden on a"
            " per-keyframe basis."
        ),
    )
    ...
    with playback_folder:
        ...
        transition_sec_number = server.gui.add_number(
            ...
            initial_value=2.0,
            hint=(
                "Time in seconds between each keyframe, which can also be overridden on"
    ...
    reset_up_button = server.gui.add_button(
        ...
        color="gray",
        hint=(
            "Set the up direction of the camera orbit controls to the camera's current"
            " up direction."
        ),
    )
