...
_config_file = os.getenv("CONFIG_FILE", default='settings.local.ini')
...
def _config_path(cur_path):
    filename = os.path.join(cur_path, _config_file)
...
def _init_config_path():
    ...
    while root_deep > 0:
        ...
        if state:
            ...
            print(f"load config file: {SETTINGS_PATH}")
            return None
        else:
            current_path = os.path.dirname(current_path)
            root_deep -= 1
    print(f'not find settings config file: {_config_file}')
