...
from wirtual.agents import llm, utils
from wirtual.agents.llm import ToolChoice
from wirtual.agents.types import DEFAULT_API_CONNECT_OPTIONS, APIConnectOptions
    ...
    def __init__(
        ...
        *,
        ...
        if not hasattr(test_ctx, "_metadata"):
            raise Exception(
                "This beta feature of 'wirtual-plugins-openai' requires a newer version of 'wirtual-agents'"
            )
...
class AssistantLLMStream(llm.LLMStream):
    ...
    async def _run(self) -> None:
        ...
        #    so we create a random uuid (we call it the Wirtual message id) and set that in the metdata.
        ...
        # 7. Resolve the OpenAI message id with all messages that have a Wirtual message id.
        try:
            ...
            for msg in self._chat_ctx.messages:
                ...
                if lk_msg_id and lk_msg_id in lk_to_oai_lookup:
                    ...
                    # We don't need the Wirtual message id anymore
