...
class HushAuth(object):
    ...
    def _do_call(self, ep, method, params=None, body=None):
        ...
        msg = "httpie-hush: [%s] url=%s" % (self.eorg, url)
        if params:
            msg += ", params=%s" % params
        if body:
            msg += ", body=%s" % body
        ...
        ret = response.json()
        self._vprint("httpie-hush: [%s] response=%s" % (self.eorg, ret))
        return ret
    ...
    def _get_token_path(self, name):
        ...
        return os.path.join(tmpdir, "httpie-hush.%s.%s" % (self.org, name))
