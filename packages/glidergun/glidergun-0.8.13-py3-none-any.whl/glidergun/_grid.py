...
class Grid(GridCore, Interpolation, Prediction, Focal, Zonal):
    ...
    def from_points(
        self, points: Iterable[Tuple[float, float, float]], all_touched: bool = False
    ):
        return self.rasterize(((Point(p[:2]), p[2]) for p in points), all_touched)
    ...
    def from_polygons(
        ...
        return self.rasterize(polygons, all_touched)
    ...
    def rasterize(
        self,
        items: Iterable[Tuple[BaseGeometry, float]],
        all_touched: bool = False,
    ):
        array = features.rasterize(
            shapes=[(item[0], float(item[1])) for item in items],
    ...
    def to_stack(self):
        ...
        arrays = plt.get_cmap(self.display)(grid2.data).transpose(2, 0, 1)[:3]  # type: ignore
        mask = self.is_nan()
...
def grid(
    ...
    match data:
        ...
        case _:
            ...
            points = cast(List[Tuple[float, float, float]], list(data))
            g = create(extent, get_crs(crs or 4326), cell_size, np.nan)  # type: ignore
            return g.from_points(points)
