...
from datarobot._experimental.models.enums import ModerationGuardModelTargetType
...
class GuardModelInfo(APIObject):
    ...
    * None  (all others)
    ...
    _converter = t.Dict(
        {
            t.Key("input_column_name"): String(),
            t.Key("output_column_name"): String(),
            t.Key("target_type"): t.Enum(*[e.value for e in ModerationGuardModelTargetType]),
            t.Key("replacement_text_column_name", optional=True): String(allow_blank=True),
            t.Key("class_names", optional=True): t.List(String()),
            t.Key("model_id", optional=True): String(allow_blank=True),
            t.Key("model_name", optional=True): String(allow_blank=True),
        }
    ).ignore_extra("*")
    ...
    def _set_values(
        ...
        replacement_text_column_name : str, optional
