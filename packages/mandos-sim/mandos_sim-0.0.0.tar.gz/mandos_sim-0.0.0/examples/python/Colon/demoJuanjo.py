...
insertion_speed = 500 /division_Coef
total_mass = 1
vUnit = np.zeros(3)
m_point = []
...
def create_model():
    ...
    Ks = [500000.0 for _ in range(n_points - 1)]
    stiffness_tensor = [200000 * np.ones(3) for i in range(n_points - 2)]
    cosserat_stiffness = [10000.0 for _ in range(n_points - 1)]
    ...
    collision_radius = 0.01 /division_Coef
    ...
    colon_sdf = colon.add_sdf(colon_mesh, 0.1*np.max((np.max(colon_mesh.x, axis=0) - np.min(colon_mesh.x, axis=0))/512) , 1024)
    ...
    model.add_collision_pair(colon_sdf, rod_collider, stiffness=10000, name="collision_points", threshold=0.1)
    model.compute_dag()
...
def simulate_callback(model):
    ...
    step_parameters.grad_norm = 1e-4
    ...
    if doStep:
        ...
        indices = np.array([(i, i+1) for i in range(n_points -1)])
        global frame
        meshio.Mesh(endoscope.x[:, 0:3], cells=[("line", indices)]).write(f"endoscope_{frame}.ply")
        frame += 1
