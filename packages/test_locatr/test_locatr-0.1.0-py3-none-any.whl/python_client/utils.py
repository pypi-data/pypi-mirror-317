...
from python_client.constants import SOCKET_RETRY_DELAY, SOCKET_SEND_DATA_MAX_RETRIES
from python_client.exceptions import (
    LocatrBinaryNotFound,
    LocatrExecutionError,
    LocatrSocketError,
)
from python_client.constants import SocketFilePath
import struct
import atexit
import socket
import time
...
def read_data_over_socket(sock: socket.socket) -> bytes:
    try:
        ...
        raise LocatrSocketError(f"Unexpected error occured while receving data: {e}")
