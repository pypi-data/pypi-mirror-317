...
class PtpTrackerJobs(TrackerJobsBase):
    ...
    def autodetect_type(self, _):
        # There is no way to tell if a VIDEO_TS or BDMV release is a miniseries or a movie or a
        # short film. A playlist may contain episodes of a miniseries or seamlessly concatenated
        # parts of a movie. .VOB files often lie about their duration. For now, we do not autodetect
        # anything for discs.
        if utils.disc.is_disc(self.content_path):
            return None
        ...
        main_videos = utils.fs.find_main_videos(self.content_path)
        if utils.mediainfo.get_duration(main_videos[0]) <= 45 * 60:
            return 'Short Film'
    ...
    def screenshots_job(self):
        # screenshots_job uses screenshots_count, which uses on type_job to determine if we're
        # dealing with a miniseries (`screenshots_from_episode`) or something else
