...
class MPVideo:
    ...
    def frames_indexes(self):
        ...
        """
        return get_frame_indexes(self.duration, self.fps)
    ...
    def frame_index_to_frame_time(self, frame_index: int):
        """
        Get the frame time of the given frame index 
        'frame_index'.
        """
        return self.frames_time_moments[frame_index]
    
    def frame_time_to_frame_index(self, t: float):
        """
        Get the frame index of the given frame time 't'.
        """
        # This will only return a value if the 't' is
        # exactly stored
        return self.frames_indexes[
            self.frames_time_moments.index(t)
        ]

        # This below is able to look for the nearest 
        # (smaller or greater) value in the array
        return self.frames_indexes[
            self.frames_time_moments.index(min(self.frames_time_moments, key = lambda ft: abs(ft - t)))
        ]

        # This below is able to look for the nearest (but
        # greater) value in the array
        return self.frames_indexes[
            self.frames_time_moments.index(
                min([ft for ft in self.frames_time_moments if ft >= t], key = lambda ft: abs(ft - t))
            )]
    ...
    def wrap_video_with_transparent_background(video: Clip):
        ...
        if video.size != MOVIEPY_SCENE_DEFAULT_SIZE:
            ...
            video = CompositeVideoClip([
                ...
                original_center_positions[VideoHandler.t_to_frame_index(t, 1 / video.fps)][0] - MOVIEPY_SCENE_DEFAULT_SIZE[0] / 2,
                original_center_positions[VideoHandler.t_to_frame_index(t, 1 / video.fps)][1] - MOVIEPY_SCENE_DEFAULT_SIZE[1] / 2
            ))
    ...
    def frame_time_to_frame_index(t: float, fps: float):
        ...
        """
        #
        return int(t * fps + SMALL_AMOUNT_TO_FIX)
...
def get_frame_indexes(duration, fps):
    ...
    """
    return [int(frame_index) for frame_index in np.linspace(0, int(duration * fps), int(duration * fps) + 1)[:-1]]
...
def get_frame_time_moments(duration, fps):
    ...
    """
    return np.linspace(0, duration, int(duration * fps) + 1)[:-1]
...
def calculate_real_video_duration(video: Clip):
    ...
    return ((last_frame + 1 / video.fps + SMALL_AMOUNT_TO_FIX) // (1 / video.fps)) * (1 / video.fps)
