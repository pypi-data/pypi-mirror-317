...
def parse_raw_df(df_hour_raw, time_zone):
    columns_names = ["LOG_TIME", "STOP_TIMESTAMP", "START_TIMESTAMP", "APP_PACKAGE_NAME", "USAGE_DURATION_MIN"]
    if df_hour_raw.shape[0] > 0:
        ...
        df_hour_raw['START_TIME'] = convert_timestamp_int_list_to_readable_time(
            df_hour_raw['START_TIMESTAMP'], time_zone)
        df_hour_raw['STOP_TIME'] = convert_timestamp_int_list_to_readable_time(
            df_hour_raw['STOP_TIMESTAMP'], time_zone)
        df_hour_raw = df_hour_raw[
            ['LOG_TIME', 'START_TIMESTAMP', 'START_TIME', 'STOP_TIMESTAMP', 'STOP_TIME',
