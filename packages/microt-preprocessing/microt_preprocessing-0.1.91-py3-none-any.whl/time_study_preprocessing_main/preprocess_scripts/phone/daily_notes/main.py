        ...
        if path.exists(date_folder_path):
            ...
            if not path.exists(day_file_save_path_dnd) and not path.exists(
                ...
                if hours_with_target_file != 0:
                    ...
                    if df_pid_sleep_wake.shape[0] != 0:
                        df_pid_sleep_wake["LOG_TIMESTAMP"] = [convert_readable_time_to_timestamp(x) for x in
                        ...
                        df_pid_sleep_wake = df_pid_sleep_wake[
                            ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP",
                             'SLEEP_OR_WAKE', 'SLEEP_OR_WAKE_TIME']]
                    if df_pid_burst_status.shape[0] != 0:
                        df_pid_burst_status["LOG_TIMESTAMP"] = [convert_readable_time_to_timestamp(x) for x in
                                                            df_pid_burst_status["LOG_TIME"]]
                        df_pid_burst_status = df_pid_burst_status[
                            ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP",
                             'IS_BURST_DAY']]
                    if df_pid_dnd_status.shape[0] != 0:
                        df_pid_dnd_status["LOG_TIMESTAMP"] = [convert_readable_time_to_timestamp(x) for x in
