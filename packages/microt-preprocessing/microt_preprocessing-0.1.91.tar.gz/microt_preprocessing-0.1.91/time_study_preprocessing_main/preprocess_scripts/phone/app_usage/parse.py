...
def get_foreground_matrix_minute(df_foreground, app_categorization_dict_path, date):
    ...
    # create dataframe
    rows = []
    start_time_of_last_row = df_foreground.loc[df_foreground.index[-1], "EVENT_START_TIME"]
    year = int(start_time_of_last_row.split(" ")[0].split("-")[0])
    month = int(start_time_of_last_row.split(" ")[0].split("-")[1])
    day = int(start_time_of_last_row.split(" ")[0].split("-")[2])
