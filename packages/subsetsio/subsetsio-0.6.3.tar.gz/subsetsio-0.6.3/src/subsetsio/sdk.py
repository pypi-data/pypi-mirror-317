...
class SubsetsClient:
    ...
    def create(self, charts: List[Dict]) -> List[str]:
        """Create new charts without sync_id tracking"""
        response = requests.post(f"{self.api_url}/chart", headers=self.headers, json=charts)
        response.raise_for_status()
        return response.json()["chart_ids"]

    def update_data(self, chart_data: Dict[str, Dict[str, List[List[Any]]]]) -> None:
        """Update chart data with create/update/delete operations"""
        if chart_data:
    ...
    def update_metadata(self, chart_id: str, metadata: Dict[str, Any]) -> None:
        """Update chart metadata without sync_id tracking"""
    ...
    def delete(self, chart_ids: List[str]) -> None:
        """Delete charts by chart_id without sync_id tracking"""
        if chart_ids:
    ...
    def sync(self, charts_to_sync: List[Dict[str, Any]], remove_missing: bool = False) -> None:
        """Sync operation that maintains sync_id tracking"""
