...
def geom_extract(geometry, indicator, stats_out=('mean', 'std', 'min', 'max', 'sum', 'counts'), afi=None,
    ...
    try:
        read_bounds, read_shape = raster.get_common_bounds_and_shape([geometry], rasters_list)
    ...
    indicator_arr = raster.read_masked(ds=indicator_ds, mask=[geometry], window=indicator_ds.window(*read_bounds),
    ...
    # fetch mask array
    if afi:
        afi_arr = raster.read_masked(ds=afi_ds, mask=[geometry], indexes=None,
    ...
    if any(val in ('min', 'max', 'mean', 'sum' 'std') for val in stats_out):
        ...
        output['stats'] = raster.arr_stats(indicator_arr, afi_arr if afi else None, stats_out)
    ...
    if classification:
        ...
        class_res = raster.arr_classes_count(indicator_arr, cls_def=cls_def, weights=afi_arr if afi else None,
