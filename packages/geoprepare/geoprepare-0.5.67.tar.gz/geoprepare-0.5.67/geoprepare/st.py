...
with plt.style.context("science"):
    data = {
        'State': ['Illinois', 'Indiana', 'Iowa', 'Kansas', 'Michigan', 'Minnesota', 'Missouri',
                  'Nebraska', 'North Dakota', 'Ohio', 'South Dakota', 'Wisconsin'],
        'May': [62.3, 61.45, 60.41, 43.15, 52.25, 52.91, 50.94, 60.8, 40.52, 58.9, 47.95, 53.84],
        'Jun': [58.13, 57.07, 58.66, 43.31, 49.85, 49.75, 51.4, 59.8, 37.97, 56.12, 43.66, 52.84],
        'Jul': [60.88, 60.97, 58.5, 42.34, 53.42, 48.5, 52.46, 60.88, 39.04, 55.78, 46.48, 52.15],
        'Aug': [62.57, 60.72, 59.09, 43.24, 53.43, 49.45, 50.57, 60.88, 41.33, 57.78, 49.28, 52.08],
        'Sep': [63.12, 60.98, 60.16, 43.24, 53.6, 50.51, 50.77, 61.83, 42.48, 55.26, 50.05, 52.27],
    ...
    n_cols = 6
    n_rows = (len(df) + n_cols - 1) // n_cols  # This ensures the correct number of rows based on number of states

    # Plotting the time-series data with 3 columns
    fig, axes = plt.subplots(nrows=n_rows, ncols=n_cols, figsize=(15, 8), sharex=True, sharey=True)
    ...
    for i, state in enumerate(df['State']):
        ...
        axes[i].plot(['Aug', 'Sep'], [df.loc[i, 'USDA_August'], df.loc[i, 'USDA_September']], color='black', marker='o', label='USDA')
