...
np.random.seed(42)
n = 200
temperature = np.random.uniform(-10, 45, n)  # Wider temperature range from -10°C to 45°C
...
temp_range = np.linspace(-10, 45, 500).reshape(-1, 1)  # 500 points between -10°C and 45°C
...
with plt.style.context("science"):
    ...
    plt.plot(temp_range, yield_pred_linear, color='green', linestyle='--', label='Linear Model', linewidth=1.5)

    # Plot predictions from Polynomial Regression (degree=2)
    plt.plot(temp_range, yield_pred_poly2, color='orange', linestyle='-.', label='Polynomial Model (Degree 2)', alpha=0.5, linewidth=1.5)

    # Plot predictions from Polynomial Regression (degree=3)
    plt.plot(temp_range, yield_pred_poly3, color='purple', linestyle='-.', label='Polynomial Model (Degree 3)', alpha=0.5, linewidth=1.5)

    # Plot predictions from Polynomial Regression (degree=4)
    plt.plot(temp_range, yield_pred_poly4, color='brown', linestyle='-.', label='Polynomial Model (Degree 4)', alpha=0.5, linewidth=1.5)
