...
def process_JRC(val):
    ...
    params, country, crop, scale, var, year, _ = val

    df_cmask = gp.GeoDataFrame.from_file(params.dir_regions_shp / params.parser.get(country, "shp_boundary"), engine="pyogrio")
    ...
    for idx, row in df_country.iterrows():
        breakpoint()
        afi_file = r"D:\Users\ritvik\projects\GEOGLAM\Input\Global_Datasets\cropland_v9.tif"
        aa = r"D:\Users\ritvik\projects\GEOGLAM\Input\intermed\ndvi\mod09.ndvi.global_0.05_degree.2017.305.c6.v1.tif"
        ac = geom_extract(row["geometry"], aa, stats_out=['mean'], afi=afi_file, afi_thresh=0, thresh_type='Fixed')
                        ...
                        ac.extend(
                            list(
                                itertools.product(
                                    ...
                                    [var],
                                    years,
                                    crop_masks if params.method == "GEOGLAM" else [crop_masks[0]],
                                )
                            )
                        )
