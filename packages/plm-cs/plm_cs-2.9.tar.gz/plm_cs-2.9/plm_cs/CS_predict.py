...
def load_ckpt():
    ckpt_path = pkg_resources.resource_filename('ca_ckpt', f'reg_ca/reg_ca.pth')
    with open(ckpt_path, 'rb') as f:
        ca_ckpt = f.read()
    ckpt_path = pkg_resources.resource_filename('cb_ckpt', f'reg_cb/reg_cb.pth')
    with open(ckpt_path, 'rb') as f:
        cb_ckpt = f.read()
    ckpt_path = pkg_resources.resource_filename('c_ckpt', f'reg_c/reg_c.pth')
    with open(ckpt_path, 'rb') as f:
        c_ckpt = f.read()
    ckpt_path = pkg_resources.resource_filename('h_ckpt', f'reg_h/reg_h.pth')
    with open(ckpt_path, 'rb') as f:
        h_ckpt = f.read()
    ckpt_path = pkg_resources.resource_filename('ha_ckpt', f'reg_ha/reg_ha.pth')
    with open(ckpt_path, 'rb') as f:
        ha_ckpt = f.read()
    ckpt_path = pkg_resources.resource_filename('n_ckpt', f'reg_n/reg_n.pth')
    with open(ckpt_path, 'rb') as f:
        n_ckpt = f.read()
...
def predict_from_seq(protein_sequence, result_file):
        ...
        for atom in ['ha', 'h', 'n', 'ca', 'cb', 'c']:
            model.load_state_dict(
                torch.load(f'{atom}_ckpt', map_location=torch.device('cpu'), weights_only=True))
            # load the model
            model.eval()
