...
class MainClass:
    ...
    def _build_where_clause(self):
        if self.conditions:
            return f"WHERE {' '.join(self.conditions)}"  # Combine conditions with space and add WHERE
        return ""

    # Query execution methods
    def select(self, table, columns="*"):
        self.query = f"SELECT {columns} FROM {table} {self._build_where_clause()};"
    ...
    def distinct(self, table, *args):
        ...
        self.query = f"SELECT DISTINCT {', '.join(args)} FROM {table} {self._build_where_clause()};"
        return self  # Return self to allow chaining

    def update(self, table, updates):
        update_clause = ", ".join([f"{column} = %s" for column in updates.keys()])
        self.query = f"UPDATE {table} SET {update_clause} {self._build_where_clause()};"
        cursor = self.connection.cursor()
        try:
            cursor.execute(self.query, tuple(updates.values()))
            self.connection.commit()
    ...
    def delete(self, table):
        self.query = f"DELETE FROM {table} {self._build_where_clause()};"
        cursor = self.connection.cursor()
        try:
