...
from softwareai.CoreApp.SoftwareAI.Instructions.{self.AgentCategory_AgentCreate}.{self.NameAgent_AgentCreate} import *
        ...
        with open(os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), f'../../../CoreApp')), f"_init_agents_.py"), 'r+', encoding='utf-8') as file:
            ...
            if f"{self.AgentCategory_AgentCreate}.{self.NameAgent_AgentCreate}" not in content:
                ...
                file.write(F'''
from softwareai.CoreApp.Agents.{self.AgentCategory_AgentCreate}.{self.NameAgent_AgentCreate} import {NameAgent_AgentCreate}_{KeyInFirebase_AgentCreate}
                ''')
                file.close()
        ...
        new_import = f"from softwareai.CoreApp.Agents.{self.AgentCategory_AgentCreate}.{self.NameAgent_AgentCreate} import {NameAgent_AgentCreate}_{KeyInFirebase_AgentCreate}\n"
        agent_initializer_entry = (
            f"        cls._agents['{self.NameAgent_AgentCreate}'] = {NameAgent_AgentCreate}_{KeyInFirebase_AgentCreate}()\n"
        )
