...
from duowen_agent.llm.entity import MessagesSet, UserMessage, AssistantMessage, SystemMessage, Message
...
def prompt_now_day(lang: str = 'zh') -> str:
    ...
    # 中文星期几
    weekday_zh = {"Monday": "星期一", "Tuesday": "星期二", "Wednesday": "星期三", "Thursday": "星期四",
        "Friday": "星期五", "Saturday": "星期六", "Sunday": "星期日"}

    if lang == "zh":
        # 格式化日期为中文格式
        formatted_date = current_date.strftime("今天是%Y年%m月%d日，") + weekday_zh[weekday] + '。'
    elif lang == "en":
        # 格式化日期为英文格式
...
class GeneralPromptBuilder:
    ...
    def __init__(self, instruction: Union[StringTemplate, str], step: Optional[Union[StringTemplate, str]] = None,
                 background: Optional[Union[StringTemplate, str]] = None,
                 output_format: Optional[Union[Type[BaseModel], StringTemplate, dict, str]] = None,
        ...
        if user_input:
            if "user_input" in user_input.variables:
                ...
                raise ValueError("GeneralPromptBuilder user_input Template must be {user_input} variables")
        else:
    ...
    def _build(self, temp_vars: dict) -> str:
        ...
        if self.output_format:
            ...
            if isinstance(self.output_format, StringTemplate):
                ...
                raise ValueError(f"GeneralPromptBuilder output_format type error {str(self.output_format)}")
    ...
    def get_instruction(self, user_input: Union[MessagesSet, str], temp_vars: dict[str, Any] = None) -> MessagesSet:
        ...
        if isinstance(user_input, str):
            ...
            raise ValueError(f"GeneralPromptBuilder get_instruction user_input error {str(user_input)}")

        _history = []
        if type(user_input) is MessagesSet and len(user_input) == 2 and user_input[0].role == 'system':
            _history = []
        ...
        if not _history:
            ...
            if self.user_input:
                _prompt.add_user(self.user_input.format(**dict(_temp_vars, **{"user_input": _user_input})))
            else:
    ...
    def _extract(self):
        ...
        if isinstance(self.instruction, StringTemplate):
            _dct["instruction"] = {"format": self.instruction.template_format, "value": self.instruction.template}
        else:
            _dct["instruction"] = {"format": "str", "value": self.instruction}

        if self.step:
            if isinstance(self.step, StringTemplate):
                _dct["step"] = {"format": self.step.template_format, "value": self.step.template}
            else:
                _dct["step"] = {"format": "str", "value": self.step}

        if self.background:
            if isinstance(self.step, StringTemplate):
                _dct["background"] = {"format": self.background.template_format, "value": self.background.template}
            else:
                _dct["background"] = {"format": "str", "value": self.background}

        if self.output_format:
            if isinstance(self.output_format, StringTemplate):
                _dct["output_format"] = {"format": self.output_format.template_format,
                ...
                _dct["output_format"] = {"format": "dict", "value": self.output_format.model_json_schema()}
            else:
                raise ValueError(f"GeneralPromptBuilder output_format type error {str(self.output_format)}")

        if self.sample:
            if isinstance(self.sample, StringTemplate):
                _dct["sample"] = {"format": self.sample.template_format, "value": self.sample.template}
            elif isinstance(self.sample, dict):
                _dct["sample"] = {"format": 'str', "value": json.dumps(self.sample, ensure_ascii=False, indent=4)}
            else:
                _dct["sample"] = {"format": 'str', "value": self.sample}

        if self.note:
            if isinstance(self.note, StringTemplate):
                _dct["note"] = {"format": self.note.template_format, "value": self.note.template}
            else:
                _dct["note"] = {"format": "str", "value": self.note}

        if self.conversation:
            _dct["conversation"] = {"format": "list", "value": [i.to_dict() for i in self.conversation]}

        if self.user_input:
            if isinstance(self.user_input, StringTemplate):
                _dct["user_input"] = {"format": self.user_input.template_format, "value": self.user_input.template}
            else:
    ...
    def export_yaml(self, path: str = None) -> str:
        _yaml = '\n'.join([yaml.dump(i, allow_unicode=True, default_flow_style=False) for i in
                           [{k: v} for k, v in self._extract().items()]])
    ...
    def load_dict(cls, data: dict) -> 'GeneralPromptBuilder':
        ...
        for k, v in data.items():
            ...
            if v['format'] == "str":
                ...
                _params[k] = StringTemplate(template=v['value'], template_format=v['format'])
            elif v['format'] == "dict":
                if k != 'output_format':
                    raise ValueError(f"GeneralPromptBuilder load_yaml type error {str(k)}")
    ...
    def load(cls, name: str = None) -> 'GeneralPromptBuilder':
        ...
        if os.path.exists(f"{practice_dir}{name}.yaml"):
            ...
            raise FileNotFoundError(f"GeneralPromptBuilder load Built-in [{name}] not found")
    ...
    def load_yaml(cls, path_name: str) -> 'GeneralPromptBuilder':
        ...
        if os.path.exists(path_name):
            ...
            raise FileNotFoundError(f"GeneralPromptBuilder load [{path_name}] not found")
