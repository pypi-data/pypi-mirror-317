...
class GPDExtremeValueAnalysis(DataHandler):
    ...
    def qq_plot(self):
        """
        Generate QQ plots for exceedances compared to theoretical distributions:
        normal, Laplace, Student's t, and the fitted GPD.
        Distributions are fitted using all returns, but only exceedances are plotted.
        """
        ...
        shape, loc_gpd, scale_gpd = self.shape, 0, self.scale  # Parameters from prior GPD fitting

        # Extract exceedances values
        exceedances = -self.exceedances.values  # Negate exceedances to "flip" the data

        # Create a 2x2 grid of QQ plots with black background
        fig, axes = plt.subplots(2, 2, figsize=(12, 12), sharey=True, facecolor='black')
        fig.suptitle("QQ Plots for Exceedances (Flipped)", fontsize=16, color='white')

        # Normal distribution QQ plot
        flipped_normal = (-mu, sigma)  # Flip theoretical parameters
        probplot(exceedances, dist=norm, sparams=flipped_normal, plot=axes[0, 0])
        axes[0, 0].set_title("Normal Distribution (Flipped)", color='white')
        axes[0, 0].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)

        # Laplace distribution QQ plot
        flipped_laplace = (-loc_lap, scale_lap)  # Flip theoretical parameters
        probplot(exceedances, dist=laplace, sparams=flipped_laplace, plot=axes[0, 1])
        axes[0, 1].set_title("Laplace Distribution (Flipped)", color='white')
        axes[0, 1].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)

        # Student's t distribution QQ plot
        flipped_t = (df_t, -loc_t, scale_t)  # Flip theoretical parameters
        probplot(exceedances, dist=t, sparams=flipped_t, plot=axes[1, 0])
        axes[1, 0].set_title("Student's t Distribution (Flipped)", color='white')
        axes[1, 0].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)

        # GPD distribution QQ plot (highlighted)
        flipped_gpd = (shape, -loc_gpd, scale_gpd)  # Flip theoretical parameters
        probplot(exceedances, dist=genpareto, sparams=flipped_gpd, plot=axes[1, 1])
        axes[1, 1].set_title("GPD Distribution (Flipped)", color='yellow')
        axes[1, 1].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)
        axes[1, 1].set_facecolor('black')  # Highlight GPD plot background

        # Add a yellow border to emphasize the GPD plot
        ...
        for ax_row in axes:
            for ax in ax_row:
                ...
                ax.set_ylabel("Sample Quantiles (Flipped)", color='white')
