...
class GPDExtremeValueAnalysis(DataHandler):
    ...
    def qq_plot(self):
        """
        Generate flipped QQ plots for exceedances compared to theoretical distributions:
        ...
        fig.suptitle("QQ Plots for Exceedances", fontsize=16, color='white')

        # Helper function to flip sample quantiles
        def flip_sample_quantiles(data, dist, params):
            (theoretical_quantiles, sample_quantiles), _ = probplot(data, dist=dist, sparams=params)
            return theoretical_quantiles, -sample_quantiles  # Flip the sample quantiles

        # Normal distribution QQ plot
        tq, sq = flip_sample_quantiles(exceedances, norm, (mu, sigma))
        axes[0, 0].plot(tq, sq, 'o')
        axes[0, 0].plot(tq, tq, 'r-', lw=2)  # Identity line
        ...
        tq, sq = flip_sample_quantiles(exceedances, laplace, (loc_lap, scale_lap))
        axes[0, 1].plot(tq, sq, 'o')
        axes[0, 1].plot(tq, tq, 'r-', lw=2)
        ...
        tq, sq = flip_sample_quantiles(exceedances, t, (df_t, loc_t, scale_t))
        axes[1, 0].plot(tq, sq, 'o')
        axes[1, 0].plot(tq, tq, 'r-', lw=2)
        axes[1, 0].set_title("Student's t Distribution", color='white')
        axes[1, 0].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)

        # GPD distribution QQ plot
        tq, sq = flip_sample_quantiles(exceedances, genpareto, (shape, loc_gpd, scale_gpd))
        axes[1, 1].plot(tq, sq, 'o', color='yellow')
        axes[1, 1].plot(tq, tq, 'r-', lw=2)
        axes[1, 1].set_title("GPD Distribution", color='yellow')
        axes[1, 1].grid(color='gray', linestyle='--', linewidth=0.7, alpha=0.5)
        axes[1, 1].set_facecolor('black')

        # Highlight the GPD plot
        ...
        for ax_row in axes:
            for ax in ax_row:
                ...
                ax.set_ylabel("Flipped Sample Quantiles", color='white')
