...
if async_:
    ...
    return None"""
            ...
            if argspec:
                method_body = f"arg = [ParseDict(arg, clouddrive.pb2.{argtype}()) if isinstance(a, dict) else a for a in arg]"
            else:
                method_body = f"if isinstance(arg, dict): arg = ParseDict(arg, clouddrive.pb2.{argtype}())"
            method_body += f"""
if async_:
