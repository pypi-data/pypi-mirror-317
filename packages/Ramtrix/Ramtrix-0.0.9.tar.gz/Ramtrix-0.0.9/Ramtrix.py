...
def row_reduce(matrix1):
    ...
    for rownum in range(inputMatrixLen):
        pivot = 0
        pivot_colnum = "WAIT"
        inputMatrixRowNumLen = len(inputMatrix[rownum])
        for colnum in range(inputMatrixRowNumLen):
            if pivot == 0:
        ...
        for nest_rownum in range (inputMatrixLen):
            ...
            if inputMatrix[nest_rownum][pivot_colnum] != 0 and nest_rownum != rownum:
                ...
                inputMatrixNestRowNumLen = len(inputMatrix[nest_rownum])
                for nest_colnum in range(inputMatrixNestRowNumLen):
...
def brute_inverse(matrix1, multiplyDet=True):
    ...
    if not check_matrix(matrix1, check_Square=True):
        print("Please input a valid matrix")
        return
    determ = laplace_determinant(matrix1)
    if determ == 0:
    ...
    if len(matrix1)>2:
        for row in range(matrix1Len):
            for col in range(matrix1RowLen):
                ...
                result[row][col] = laplace_determinant(relevant_matrix)
    elif len(matrix1)==2:
