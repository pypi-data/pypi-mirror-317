...
def generate_stub_file(db: 'GroovinDB', stubs_dir: str = None) -> None:
...
async def run_introspection():
    ...
    # Preguntar por la ubicación de los stubs
    stubs_location = questionary.text(
        "¿Dónde quieres guardar los stubs?",
        default="stubs",
        validate=lambda text: len(text.strip()) > 0
    ).ask()
    
    generate_stub_file(db, stubs_location)
