...
class QueryBuilder:
    ...
    def build_select_query(self, schema: str, table: str, **kwargs) -> str:
        """Construir una query SELECT."""
        table_name = f'"{schema}"."{table}"' if schema else f'"{table}"'
        fields = "*" if not self._select else ", ".join(f'"{col}"' for col in self._select)
        query = f"SELECT {fields} FROM {table_name}"

        # Construir WHERE
        where_conditions = []
        for key, value in self._where.items():
            where_conditions.append(f'"{key}" = ${len(where_conditions) + 1}')
        
        if where_conditions:
            query += f" WHERE {' AND '.join(where_conditions)}"

        # Agregar ORDER BY, LIMIT, OFFSET
        if self._order_by:
            order_clause = ", ".join(f'"{col}" {direction}' for col, direction in self._order_by)
