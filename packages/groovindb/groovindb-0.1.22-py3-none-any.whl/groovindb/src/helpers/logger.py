...
def load_config():
    """Cargar configuración desde el directorio actual del proyecto"""
    try:
        config_path = Path(os.getcwd()) / 'groovindb.json'
        with open(config_path, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        return {}  # Devuelve un diccionario vacío si el archivo no existe
    except json.JSONDecodeError:
        return {}  # Devuelve un diccionario vacío si hay error al parsear
